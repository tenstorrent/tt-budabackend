...
void tt_runtime_workload::check_program(string prog_name) {
    ...
    if (qptrs_wrap.pending_queue_settings.size()) {
        ...
        log_info(LogRuntime, "Pending queue settings queue is non-empty with {} entries! Queues that span multiple programs may not end up in the correct state...", num_pending);
        for (auto &it : qptrs_wrap.pending_queue_settings) {
            log_debug(LogRuntime, " {} - {} ", it.first, it.second);
        }
    }
}
...
tt_dram_io_desc tt_runtime_workload::get_io_desc_for_queue(string queue_name) {
    ...
    log_debug(LogRuntime, "Get IO desc for {}, queue buffer epoch_tiles={}, {}", queue_name, get_entry_size_in_tiles(queue_info), queue_info);
...
void tt_runtime_workload::deallocate_memory(void* ptr) {
    if (tt::mem::host_shared_memory.find(ptr) != tt::mem::host_shared_memory.end()) {
        log_debug(tt::LogRuntime, "Deallocating {} -- shared_ptr use_count={}",
        ...
        log_debug(LogRuntime, "Skip deallocating {}, either it's already freed or was never allocated", reinterpret_cast<intptr_t>(ptr));
    }
}
...
void tt_runtime_workload::print_cross_chip_e2e_queues() {
    ...
    for (const auto &e2e_queue : e2e_queues) {
        ...
        if (is_cross_chip_e2e_queue(q_info.name)) {
            ...
            log_info(LogRuntime, "Cross chip {}->{} e2e queue: {:<40}, entry per core = {:>4} KB, microbatch = {}, per core = {:>6} KB, total = {:.2f} MB", 
    ...
    for (const auto &kv : cross_chip_total_bytes) {
        log_info(LogRuntime, "Cross chip e2e queue total bytes: src_device = {}, dst_device = {}, io per input = {:.2f} MB, total_bytes = {:.2f} MB", kv.first.first, kv.first.second, (float)kv.second.first/(1024*1024), (float)kv.second.second/(1024*1024));
    }
}
