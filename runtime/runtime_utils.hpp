...
void run_pipegen(const string &build_dir_path, const std::string &graph_name, int temporal_epoch,
                 const std::vector<chip_id_t> &chip_ids, const perf::PerfDesc &perf_desc, const string &desc_name,
                 const std::unordered_map<chip_id_t, buda_soc_description>& sdesc_per_chip);
void run_pipegen2(const string &desc_name, const string &pipegen_yaml_path, const int temporal_epoch,
                  const string &blob_yaml_path, const uint32_t perf_dump_info);
