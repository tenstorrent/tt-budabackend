...
namespace {
    ...
    std::vector<tt::ARCH> get_arch_vec(const std::string &user_desc) {
        ...
        if (user_desc != "") {
            ...
            arch_vec = {tt::ARCH::GRAYSKULL, tt::ARCH::WORMHOLE, tt::ARCH::WORMHOLE_B0};
        }
        return arch_vec;
    }
    ...
    uint64_t get_dram_bw(const tt::ARCH &arch) {
        static const std::unordered_map<tt::ARCH, uint64_t> dram_bw_map = {
            ...
            {tt::ARCH::WORMHOLE,    (uint64_t)(32.0f * (1024*1024*1024))},
            {tt::ARCH::WORMHOLE_B0, (uint64_t)(32.0f * (1024*1024*1024))},
        };
    ...
    bool store)
{
    ...
    // ------------------------------------------------------------------------
    constexpr float dram_bw_derate_factor = 0.85f;
        ...
        for (const string &scope : scope_vec)
        {
            if (scope == "SYSMEM")
            {
                for(int i = 0; i < num_host_mem_channels; i++) {
                    ...
                    params[key] = std::to_string(host_mem::address_map::DEVICE_TO_HOST_MMIO_SIZE_BYTES);
            ...
            else if (scope == "DRAM")
            {
                ...
                params[key] = std::to_string((uint64_t)(get_dram_bw(arch) * dram_bw_derate_factor));
    ...
    bool store)
{
    ...
    if (!parse_env("TT_BACKEND_SKIP_SYSTEM_RUNTIME_PARAMS", false)){
        ...
        const bool perform_harvesting = true;
        const bool clean_system_resources = false;
        auto devices = tt_cluster::detect_available_devices(TargetDevice::Silicon, false); //if devices is empty, just populate db from file. else compare db with sys level params
        ...
        params[key] = std::to_string(devices.size());
