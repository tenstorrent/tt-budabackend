...
void tt_runtime::initialize_perf_overlay_decouple_epoch_command_mailbox() {
    for (auto device_it: cluster->get_sdesc_for_all_devices()) {
        ...
        log_info(tt::LogPerfInfra, "Initializing overlay-decouple epoch command sync ptrs for device {}. First_code = {}, translated_addr = {}, value_addr = {}, first_core_mailbox_addr = {}, first_core_addr = {}",
...
void tt_runtime::perf_overlay_decouplings_update_epoch_command_start(uint device_id) {
    ...
    current_epoch_id++;
    log_info(tt::LogPerfInfra, "Updating epoch command start signal for overlay-decoupling mode for device {}, core {}, epoch_id {}", device_id, first_core.str(), current_epoch_id);
...
void tt_runtime::initialize_device_perf() {
    perf::record_scope record("initialize-performance-trace");
    ...
    runtime_state(tt_runtime_state::Uninitialized)
{
    ...
    if (config.perf_desc.enabled() && (config.perf_desc.run_analyzer || config.perf_desc.overlay_decouplings.size() > 0)) {
    ...
    try {
        ...
        compile_threads.push_back(std::thread([&] {
            try {
                ...
                if (config.do_compile() or need_partial_recompile_during_run) {
                    // Create overlay binaries if compiling or if partial recompile is needed
                    string sdesc_to_use = config.output_dir + "/device_desc.yaml";
                    generate_pipegen_spec(netlist_path, config.output_dir, compiled_epochs, sdesc_to_use, cluster_descriptor_path);
                    create_graph_overlay_binaries();
                }
        ...
        if (result) {
            if (config.do_compile()) {
                ...
                log_trace(tt::LogCompileTrisc, "Compile skipped");
            }
        }


        for (auto th = 0; th < exceptions_all_threads.size(); th++) {
...
void tt_runtime::perform_harvesting() {
    ...
    if(!config.do_run()) {
        ...
        if(harvest_soc_descs or noc_translation_enabled) {
            ...
            generate_sdesc_yaml_for_pipegen(workload_target_device_ids);
        }
    }
    else {
...
void tt_runtime::create_graph_overlay_binaries() {
    wrap_graph_overlay_binaries_creation(
        [this](int temporal_epoch, const std::unordered_map<chip_id_t, buda_soc_description>& sdesc_per_chip) {
            this->create_temporal_epoch_overlay_binaries(temporal_epoch, sdesc_per_chip);
        });
}
...
void tt_runtime::wrap_graph_overlay_binaries_creation(std::function<void(int, const std::unordered_map<chip_id_t, buda_soc_description>& sdesc_per_chip)> creator_function) {
    ...
    perf::record_scope record(perf::HostEventType::PIPEGEN_RUNTIME);
    ...
    tt::parallel_for(0, num_temporal_epochs, [=, &sdesc_per_chip](int epoch) {
        creator_function(epoch, sdesc_per_chip);
    }, num_threads);

}

void tt_runtime::create_temporal_epoch_overlay_binaries(int temporal_epoch, const std::unordered_map<chip_id_t, buda_soc_description>& sdesc_per_chip) {
    ...
    run_pipegen(config.output_dir, *(graph_names.begin()), global_epoch_id, chip_ids, config.perf_desc, file_to_use,
                sdesc_per_chip);
}
                ...
                if (workload.queues.find(input_name) != workload.queues.end()){
                    ...
                    if (queue_info.loc == QUEUE_LOCATION::DRAM){
                        if (queue_info.target_device != graph_info.target_device){
                            log_info(tt::LogRuntime, "op {} has input queue {} target_device: {} that does not match graph's target_device: {}", op.first, queue_info.name, queue_info.target_device, graph_info.target_device);
...
void tt_runtime::create_graphs_and_init_queues() {
    ...
    for (auto &graph : workload.graph_order) {
        ...
        log_debug(tt::LogRuntime, "\t(graph-to-epoch) {} \t-> compiled epoch_id {}", epoch_program.name, epoch_program.epoch_id);
    }
...
void tt_runtime::drain_perf_dram_buffers(bool last_drain) {
    perf::record_scope record("drain-performance-buffers");
    if (config.mode != DEVICE_MODE::CompileOnly &&
        ...
        if (config.optimization_level == 0) {
            if (!last_drain) {
                ...
                log_info(tt::LogPerfInfra, "Draining all dram perf buffers for the current program since opt-level is set to 0");
            ...
            if (last_drain) {
                wait_for_idle({}, "drain_perf_buffers");
                log_info(tt::LogPerfInfra, "Draining all dram perf buffers since all programs have finished");
                cluster->dump_perf_counters(config.output_dir);
            }
        }
    }
...
void tt_runtime::finish_performance_trace() {
    ...
    if (config.mode != DEVICE_MODE::CompileOnly) {
        ...
        if (config.perf_desc.enabled()) {
            ...
            if (config.perf_desc.run_analyzer) {
...
tt::DEVICE_STATUS_CODE tt_runtime::run_program(const string &program_name, const std::map<string, string> &parameters) {
    ...
    try {
        perf::record_scope record(perf::HostEventType::RUN_PROGRAM);
    ...
    else if (instrn.opcode == INSTRUCTION_OPCODE::AllocateQueue)
    {
        ...
        for (const auto &var : instrn.vars) {
            ...
            if (qs_to_sync.size() > 0) {
                log_trace(tt::LogRuntime, "allocate queue = {} overlaps with dyamic queues: {}", q_name, fmt::join(qs_to_sync, ", "));
...
void tt_runtime::run_execute_instrn(netlist_program &program, tt_instruction_info &instrn) {
    ...
    perf::record_scope record(perf::HostEventType::RUN_EXECUTE_INSTRUCTION);
...
void tt_runtime::push_queue_side_effects(netlist_program &program) {
    ...
    if (qptrs_wrap.has_pending()) {
        perf::record_scope record(perf::HostEventType::PUSH_QUEUE_SIDE_EFFECTS);
...
void tt_runtime::wait_for_idle(const std::set<int> &target_devices, string caller, int cmds_thresh) {
    ...
    for (const int dev : devices) {
        ...
        log_debug(tt::LogRuntime, "\twait_for_epoch complete on device {}", dev);
    }

    log_info(tt::LogRuntime, "\tWait for idle complete on devices {}, caller = {}", s.str(), caller);
}
    ...
    if (config.mode == DEVICE_MODE::RunOnly) {
        if (fs::exists(runtime_data)) {
            ...
            if(runtime_data["noc_translation_enabled"].as<int>() != noc_translation_enabled) {
                log_info(tt::LogRuntime, "Recompiling overlay binaries since NOC translation is enabled on this machine.");
                need_partial_recompile_during_run = true;
            }
            if(arch_supports_harvesting) {
                if((rows_to_harvest.size() != runtime_data["harvested_rows_per_chip"].size())){
                    log_warning(tt::LogRuntime, "Harvesting configuration between compile and run do not match. Recompiling overlay binaries during run.");
                    need_partial_recompile_during_run = true;
                }
                // Checks only performed for grayskull and whb0
                // Error if one of the harvesting configs is empty and the other exists
                for(auto it : runtime_data["harvested_rows_per_chip"]){
                    if(rows_to_harvest.find(it.first.as<int>()) == rows_to_harvest.end()){
                        log_warning(tt::LogRuntime, "Device {} is not present in the runtime harvesting config but is present in the compile harvesting config. Recompiling overlay binaries during run.", it.first.as<string>());
                        need_partial_recompile_during_run = true;
                        break;
                    }
                    if(arch_name == tt::ARCH::GRAYSKULL) {
                        // Harvesting masks must match exactly between compile and run for GS
                        if(it.second.as<int>() != rows_to_harvest.at(it.first.as<int>())){
                            log_warning(tt::LogRuntime, "The harvesting config for device {} does not match harvesting config in compile. Recompiling overlay binaries during run.", it.first.as<string>());
                            need_partial_recompile_during_run = true;
                            break;
                        }
                    }
                        ...
                        if(std::bitset<32>(it.second.as<int>()).count() < std::bitset<32>(rows_to_harvest.at(it.first.as<int>())).count()) {
                            log_warning(tt::LogRuntime, "The harvesting config for device {} does not match harvesting config in compile. Recompiling overlay binaries during run.", it.first.as<string>());
                            need_partial_recompile_during_run = true;
                            break;
                        }
...
void tt_runtime::ensure_devices_present(const TargetDevice &target_type) {
    log_assert(target_type == TargetDevice::Versim or target_type == TargetDevice::Silicon, "Expected Versim or Silicon Backend");
...
void tt_runtime::enable_loop_on_device_if_eligible(netlist_program &program, int loop_count){
    ...
    if ((loop_count >= min_loop_iterations) || loader->get_in_loop_on_device()) {
        ...
        log_info(tt::LogRuntime, "Enabling program looping on device for Loop({}) in {} at pc: {}.", loop_count, program.get_name(), program.get_current_pc());
    }
}
