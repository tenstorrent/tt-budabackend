...
public:
    ...
    void assert_not_full() 
    {
        log_assert(
            !(this->full()), 
            "Trying to push to a full SimpleQueue: {},  current size = {}", this->name, this->size()
        );
    }

    void assert_not_empty() 
    {
        log_assert(!(this->empty()), "Trying to pop from an empty SimpleQueue: {}", this->name);
    }
    ...
    const T& read(int off) 
    {
        int rd_ptr = this->rd_ptr;
        log_assert(
            (off + rd_ptr + this->offset < this->size() and 0 <= rd_ptr), 
            "Trying to read queue {} with size {} out-of-bounds with index {}", this->name, this->size(), rd_ptr
        );

        return this->data_queue.at(off + rd_ptr + this->offset);
    }
