...
std::vector<tt::ARCH> tt_cluster::detect_available_devices(const TargetDevice &target_type, bool only_detect_mmio){
    ...
    log_assert(target_type == TargetDevice::Versim or target_type == TargetDevice::Silicon, "Expected device type Silicon or Versim.");
...
void tt_cluster::open_device(
    ...
    type = target_type;
    log_assert(type == TargetDevice::Versim or type == TargetDevice::Silicon, "Expected device type Silicon or Versim.");
...
void tt_cluster::close_device() {
    ...
    log_info(tt::LogRuntime, "Closed all devices successfully");
}
...
void tt_cluster::broadcast_write_to_all_dram_in_cluster(const vector<uint32_t>& vec, uint64_t addr, bool small_access, bool register_txn) {
    ...
    if(cluster_arch == tt::ARCH::GRAYSKULL) {
        ...
        device -> broadcast_write_to_cluster(vec.data(), vec.size() * 4, addr, chips_to_exclude, rows_to_exclude, cols_to_exclude_for_wormhole_dram_broadcast_group_1, tlb_to_use);
        // DRAM Group 2 on WH (column 0 with only DRAM rows)
        device -> broadcast_write_to_cluster(vec.data(), vec.size() * 4, addr, chips_to_exclude, rows_to_exclude_for_wormhole_dram_broadcast_group_2, 
                                            cols_to_exclude_for_wormhole_dram_broadcast_group_2, tlb_to_use);
    }
}
