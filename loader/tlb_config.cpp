...
void configure_static_tlbs(const std::uint32_t& chip, const buda_SocDescriptor& sdesc, std::shared_ptr<tt_device> device) {
    ...
    for (uint32_t tlb_id = DYNAMIC_TLB_BASE_INDEX; tlb_id < DYNAMIC_TLB_BASE_INDEX + DYNAMIC_TLB_COUNT; tlb_id++) {
        device -> configure_tlb(chip, tt_xy_pair(DEVICE_DATA.DRAM_CHANNEL_0_X, DEVICE_DATA.DRAM_CHANNEL_0_Y), tlb_id, peer_dram_offset, TLB_DATA::Posted);
    ...
    for(int i = 0; i < DEVICE_DATA.DRAM_LOCATIONS.size(); i++) {
        device -> configure_tlb(chip, DEVICE_DATA.DRAM_LOCATIONS[i],  EPOCH_CMD_QUEUE_TLBS[i], epoch_queue::get_epoch_alloc_queue_sync_addr(), TLB_DATA::Posted);
