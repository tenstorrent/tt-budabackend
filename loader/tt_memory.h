...
inline memory::address_t WriteBlob(
  ...
  log_assert(mem.base() <= start, "Incorrect start address in WriteBlob");
  const auto one_past_end = start + count * sizeof(memory::word_t);
  log_assert(one_past_end <= mem.limit(), "Incorrect end address in WriteBlob");
  const auto offset_from_base = start - mem.base();
  for (unsigned int idx = offset_from_base / sizeof(memory::word_t), span_pos = 0; span_pos < count; ++idx, ++span_pos) {
