...
memory::memory(address_t base, address_t size) : m_base(base), m_content(size / sizeof(word_t)) {
  log_assert(size % sizeof(word_t) == 0, "Memory size is not word aligned");
  assert_invariants();
}
...
void memory::assert_invariants() const {
  log_assert(m_base % sizeof(word_t) == 0, "Memory base is not word aligned");
  log_assert(m_content.size() <= (numeric_limits<address_t>::max() - m_base) / sizeof(word_t) + 1, "Contents do not fit in memory");
}

memory slice_memory(const memory &mem, memory::address_t base, memory::address_t start, memory::address_t end) {
  log_assert(mem.base() <= start, "Incorrect start address for slice");
  log_assert(mem.limit() >= end, "Incorrect end address for slice");
  log_assert(((end - start) % sizeof(memory::word_t)) == 0, "memory slice is not word aligned");

  auto num_words_to_copy = (end - start) / sizeof(memory::word_t);
  auto starting_word = (start - base) / sizeof(memory::word_t);
  log_assert(starting_word >= 0, "Incorrect starting word");
