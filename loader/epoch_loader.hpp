...
class tt_epoch_binary
{
    ...
    void assign_tensix_binaries_to_dram(int hex_id, int dram_channel, int dram_subchannel, uint64_t dram_start_addr);
...
struct tt_epoch_control
{
    ...
    static vector<uint32_t> get_valid_qcmd(tt_xy_pair dram_xy, uint64_t dram_addr, bool perf_en, uint16_t overlay_decouple_mask);
