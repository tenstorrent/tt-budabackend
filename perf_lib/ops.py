...
class MatmulAttr(OpBaseAttr):
    def __init__ (self, reblock_mode, config, bias=False, sparse_zero_freq=False, sparse_nz_counts=False):
        if bias:
            op_type = OpType.MatmulBias
        elif sparse_zero_freq:
            op_type = OpType.MatmulSparse
        elif sparse_nz_counts:
            op_type = OpType.MatmulSparseNzCounts
        else:
            op_type = OpType.Matmul
        super().__init__(op_type, reblock_mode, config)
        self.sparse_zero_freq = sparse_zero_freq
        self.sparse_nz_counts = sparse_nz_counts
        if config == "":
            return
    ...
    def get_attr_labels_and_alignment(self):
        ...
        if self.sparse_zero_freq:
            label = ("zero_strp_freq", "zero_ublock_freq", "zero_tile_freq") + label
            table_alignment  = "%-10s %-10s %-10s " + table_alignment

        if self.sparse_nz_counts:
            label = ("nz_strips", "nz_ublocks", "nz_tiles") + label
            table_alignment  = "%-10s %-10s %-10s " + table_alignment
    ...
    def get_attr_report_single_run(self, test_id):
        ...
        if self.sparse_zero_freq:
            attr = (self.zero_strip_freq, self.zero_ublock_freq, self.zero_tile_freq) + attr
        if self.sparse_nz_counts:
            attr = (self.nz_strips, self.nz_ublocks, self.nz_tiles) + attr
        attr = (test_id,) + attr
        return attr

class MatmulBiasAttr(MatmulAttr):
    def __init__ (self, reblock_mode, config):
        super().__init__(reblock_mode, config, bias=True)
        if config == "":
            return
...
class MatmulSparse(MatmulAttr):
    def __init__ (self, reblock_mode, config):
        super().__init__(reblock_mode, config, sparse_zero_freq=True)
        if config == "":
            return
...
class MatmulSparseNzCounts(MatmulAttr):
    def __init__ (self, reblock_mode, config):
        super().__init__(reblock_mode, config, sparse_nz_counts=True)
        if config == "":
            return
