...
record_scope::record_scope(const string& event_label) {
    this->recorder_func = [event_label]() { backend_profiler.record_loader_event(event_label); };
    this->recorder_func();
}

record_scope::record_scope(const string& event_label, uint64_t event_value) {
    this->recorder_func = [event_label, event_value]() { backend_profiler.record_loader_event(event_label, event_value); };
    this->recorder_func();
}

record_scope::record_scope(uint64_t event_id) {
    this->recorder_func = [event_id]() { backend_profiler.record_loader_event(event_id); };
    this->recorder_func();
}

record_scope::record_scope(uint64_t event_id, uint64_t event_value) {
    this->recorder_func = [event_id, event_value]() { backend_profiler.record_loader_event(event_id, event_value); };
    this->recorder_func();
}

record_scope::record_scope(const perf::HostEventType& event) {
    this->recorder_func = [event]() { backend_profiler.record_loader_event(event); };
    this->recorder_func();
}

record_scope::record_scope( const perf::HostEventType& event, uint device_id) {
    this->recorder_func = [event, device_id]() { backend_profiler.record_loader_event(event, device_id); };
    this->recorder_func();
}

record_scope::record_scope(const perf::HostEventType& event, uint device_id, uint epoch_id) {
    this->recorder_func = [event, device_id, epoch_id]() { backend_profiler.record_loader_event(event, device_id, epoch_id); };
    this->recorder_func();
}

record_scope::record_scope(const perf::HostEventType& event, uint device_id, uint epoch_id, uint program_id) {
    this->recorder_func = [event, device_id, epoch_id, program_id]() {backend_profiler.record_loader_event(event, device_id, epoch_id, program_id); };
    this->recorder_func();
}
