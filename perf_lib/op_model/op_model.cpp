...
void validate(const tt_op_model_desc& op_desc) {
    log_assert(op_desc.type != "", "Must specify op type");
    log_assert(op_desc.arch != "", "Must specify device arch");
...
uint32_t OpModel::get_op_cycles(const tt_op_model_desc& op_desc) {
    validate(op_desc);
    auto& op_model = OpModel::get(op_desc.arch);
...
std::map<std::string, OpModel>& OpModel::get_instances() {
    static std::map<std::string, OpModel> instances;
    return instances;
}

OpModel& OpModel::get(const std::string& arch_name) {
    auto& instances = get_instances();
    auto match = instances.find(arch_name);
    if (match == instances.end()) {
        match = instances.emplace(arch_name, OpModel(arch_name)).first;
    }
    return match->second;
}
...
float OpModel::get_op_param(const tt_op_model_desc& op_desc, const std::string& param_name) {
    ...
    OpModel& model = get(op_desc.arch);
    return model.get_param(op_name, param_name, weight_attr);
}

OpModel::OpModel(const std::string& arch_name) {
    std::string params_path = buda_home() + "perf_lib/op_model/params/" + arch_name + "_params.yaml";
    m_model_params = std::make_unique<OpModelParams>(params_path);
    if (!m_model_params) {
        log_fatal("Failed to load OpModelParams from {}", params_path);
    }

    if (arch_name == "wormhole_b0") {
        std::string sparse_params_path_lofi = buda_home() + "perf_lib/op_model/params/" + arch_name + "_sparse_params_lofi.yaml";
        std::string sparse_params_path_hifi2 = buda_home() + "perf_lib/op_model/params/" + arch_name + "_sparse_params_hifi2.yaml";
...
float OpModel::get_param(const std::string& op_name, const std::string& param_name, const std::string &param_attr) {
    return m_model_params->get_param(op_name, param_name, param_attr);
}
