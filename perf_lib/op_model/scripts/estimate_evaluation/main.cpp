...
int main(int argc, char* argv[]) {
    if (argc < 6) {
        log_fatal("Required arguments: perf sweep results file, output file, op type, op name, architecture");
    ...
    for (const auto& perf_sweep_result: perf_sweep_results) {
        ...
        uint32_t estimate = tt::get_op_cycles_for_perf_sweep_result(perf_sweep_result, op_type, op_name, arch_name, sparse_formula);
