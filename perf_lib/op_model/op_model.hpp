...
class OpModel {
   public:
    ...
    static std::map<std::string, OpModel>& get_instances();

    static OpModel& get(const std::string& arch_name);
   ...
   private:
    ...
    explicit OpModel(const std::string& params_path);
    float get_param(const std::string& op_name, const std::string& param_name, const std::string& param_attr = "");
