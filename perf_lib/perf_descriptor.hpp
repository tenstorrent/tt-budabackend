...
struct record_scope {
    record_scope(const string& event_label);

    record_scope(const string& event_label, uint64_t event_value);

    record_scope(uint64_t event_id);

    record_scope(uint64_t event_id, uint64_t event_value);

    record_scope(const perf::HostEventType& event);

    record_scope(const perf::HostEventType& event, uint device_id);

    record_scope(const perf::HostEventType& event, uint device_id, uint epoch_id);

    record_scope(const perf::HostEventType& event, uint device_id, uint epoch_id, uint program_id);
    ...
    std::function<void()> recorder_func;
};
