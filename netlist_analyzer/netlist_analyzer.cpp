...
void tt_netlist_analyzer::configure_analyzer_for_epoch(const int& epoch_id) {
    ...
    m_analyzer_per_epoch.emplace(epoch_id, dpnra::Analyzer(this->arch));
...
void tt_netlist_analyzer::run_analyzer_checks_for_epoch(const int& epoch_id) {
    ...
    log_assert(
        ...
        epoch_id);
    for (const auto& chip : m_chips_per_epoch.at(epoch_id)) {
        m_analyzer_per_epoch.at(epoch_id).analyze_chip(chip);
    }
}
...
void tt_netlist_analyzer::run_net2pipe_flow(const string &build_dir_path){
    ...
    for (int e = 0; e < m_netlist_parser.get_number_of_temporal_graphs(); e++) {
        ...
        for (const auto& chip_id : m_chips_per_epoch.at(e)) {
            const string analyzer_yaml_path = build_dir_path + "/analyzer_output_temporal_epoch_" + std::to_string(e) + "_chip_" + std::to_string(chip_id) + ".yaml";
            m_analyzer_per_epoch.at(e).serialize_chip(chip_id, analyzer_yaml_path);
            const string report_pdf_cmd = "netlist_analyzer/analyzer/pdf_report.py --input-chip-yaml " + analyzer_yaml_path;
            //if (system(report_pdf_cmd.c_str()) != 0) {
            //    log_fatal("Running report_pdf.py command failed: {}", report_pdf_cmd);
           // }
        }
    }
}
