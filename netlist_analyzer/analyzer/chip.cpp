...
std::shared_ptr<Node> Chip::getDramNode(int channel, int sub_channel) const {
    // TODO: assert channel <  num channels
    //cout << "channel: " << channel << " sub: " << sub_channel << endl;
    if(channel == 255) {
...
void Chip::outputYaml(const std::string &filename, int chip_id) const {
    ...
    out << YAML::Value << this->slowest_op_cycles;
    out << YAML::Key << "bw_limited_op_cycles";
    out << YAML::Value << this->bw_limited_op_cycles;
    out << YAML::Key << "nodes";
    out << YAML::Value;
    out << YAML::BeginSeq;
...
std::shared_ptr<Link> Chip::getWorstLink(int longest_op_cycles) {
