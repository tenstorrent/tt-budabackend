...
void tt_eltwise_binary_bare_op::set_hlk_args_t(
    ...
    if (m_config.op != BinaryOp::Add && m_config.op != BinaryOp::Multiply && m_config.op != BinaryOp::Subtract &&
        m_config.op != BinaryOp::Requantization && m_config.op != BinaryOp::Dequantization && m_config.op != BinaryOp::Quantization) {
...
void tt_eltwise_binary::utils::golden_model(
    ...
    log_assert(
        ...
        inputs[0]->get_shape(),
        inputs[1]->get_shape());
    log_debug(tt::LogOp, "Eltwise Binary Op: {}", binary_op_to_string(config.op));
