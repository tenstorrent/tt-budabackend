...
{
    return modify_blob_dw(
        blob_dw, STREAM_ONETIME_MISC_CFG_REG_INDEX, ~(1 << PHASE_AUTO_CONFIG), val << PHASE_AUTO_CONFIG);
}
...
{
    return ((src_in_order_fwd ? 1 : 0) << MSG_SRC_IN_ORDER_FWD) | (arb_group_size << MSG_ARB_GROUP_SIZE) |
...
void PhaseBlobFillerArchSpecific::setup_misc_cfg_reg(
    ...
    const bool data_auto_send /* not used on blackhole */,
    ...
    const unsigned int reg_update_vc_reg)
{
    ...
    phase_blob.append(
        configure_4_byte_register(
            STREAM_MISC_CFG_REG_INDEX,
                ...
                (local_sources_connected ? 1 : 0) << LOCAL_SOURCES_CONNECTED |
    ...
    phase_blob.append(
        configure_4_byte_register(
            STREAM_ONETIME_MISC_CFG_REG_INDEX,
    ...
    phase_blob.append(
        configure_4_byte_register(
            STREAM_REMOTE_DEST_TRAFFIC_REG_INDEX, 
...
{
    phase_blob.append(
        configure_4_byte_register(STREAM_REMOTE_DEST_MSG_INFO_BUF_SIZE_REG_INDEX, 1),
        "STREAM_REMOTE_DEST_MSG_INFO_BUF_SIZE_REG_INDEX");
}
...
void PhaseBlobFillerArchSpecific::setup_stream_wr_ptr_reg(BlobSection& phase_blob)
{
    ...
    phase_blob.append(
        configure_4_byte_register(STREAM_WR_PTR_REG_INDEX, (0 | 1 << MEM_WORD_ADDR_WIDTH)), "STREAM_WR_PTR_REG_INDEX");
}
...
void PhaseBlobFillerArchSpecific::setup_stream_remote_dest_traffic(BlobSection& phase_blob, const PhaseConfig& phase)
{
    ...
    phase_blob.append(
        configure_4_byte_register(STREAM_REMOTE_DEST_TRAFFIC_REG_INDEX, phase_helper.get_group_priority()),
        "STREAM_REMOTE_DEST_TRAFFIC_REG_INDEX");
}
