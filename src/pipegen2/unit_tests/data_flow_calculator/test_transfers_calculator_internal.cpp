...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        12 /* tiles_to_send */, 2010 /* max_tiles_per_phase */);
...
TEST_F(Pipegen2_TransfersCalculatorInternal_CalculatePhasesForRoot, LessThanMaxTilesPerPhaseTilesToSend)
{
    ...
    verify_phases( src_node->get_sending_phases(dest_node.get()), {
        PhaseInfo(c_start_phase_offset, c_data_offset, 12)
    });
}
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        5000 /* tiles_to_send */, 2010 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock();
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        6400 /* tiles_to_send */, 1280 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock();
...
protected:
    ...
    std::unique_ptr<DataFlowNodeMock> make_df_node_mock(unsigned int tiles_to_send,
    ...
    {
        return make_df_node_mock(tiles_to_send, max_tiles_per_phase,
                                 false /* is_dram_or_pcie_input */, false /* is_dram_parallel_fork */);
    }
    ...
    {
        return make_df_node_mock(tiles_to_send, max_tiles_per_phase,
                                 true /* is_dram_or_pcie_input */, false /* is_dram_parallel_fork */);
    }
    ...
    {
        return make_df_node_mock(tiles_to_send, max_tiles_per_phase,
                                 false /* is_dram_or_pcie_input */, true /* is_dram_parallel_fork */);
    }

    std::unique_ptr<DataFlowNodeMock> make_dest_node_mock(unsigned int max_tiles_per_phase)
    {
        return make_df_node_mock(0 /* tiles_to_send */, max_tiles_per_phase,
                                 false /* is_dram_or_pcie_input */, false /* is_dram_parallel_fork */);
    }
...
{
    std::unique_ptr<DataFlowNodeMock> prev_src_node = make_src_node_mock(
        4 /* tiles_to_send */, 1280 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> curr_src_node = make_src_node_mock(
        8 /* tiles_to_send */, 1280 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock(1280 /* max_tiles_per_phase */);

    connect_data_flow_graph({
        make_df_edge(prev_src_node, dest_node),
        make_df_edge(curr_src_node, dest_node)
    });
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_dram_or_pcie_src_mock(
        4 /* tiles_to_send */, 1280 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock(1280 /* max_tiles_per_phase */);
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_dram_parallel_fork_mock(
        4 /* tiles_to_send */, 1280 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock(1280 /* max_tiles_per_phase */);
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        4 /* tiles_to_send */, 32 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock();
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        4 /* tiles_to_send */, 32 /* max_tiles_per_phase */, 2 /* input_group_count */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock();
...
TEST_F(Pipegen2_TransfersCalculatorInternal_CalculateReceivingPhases, SameSenderCanAccumulateReceiverCanExpand)
{
    ...
    verify_phases(receiving_phases_per_input_group.at(0), {
...
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock(
        53 /* max_tiles_per_phase */);
    ...
    EXPECT_THAT(receiving_phases_per_input_group, Contains(Key(0)));
    verify_phases(receiving_phases_per_input_group.at(0), {
        PhaseInfo(c_start_phase_offset, 50 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 1, 50 /* num_msgs */)
    });
}

TEST_F(Pipegen2_TransfersCalculatorInternal_CalculateReceivingPhases, DifferentSender)
{
    std::unique_ptr<DataFlowNodeMock> src_node_1 = make_src_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> src_node_2 = make_src_node_mock(
        3 /* tiles_to_send */, 10 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node = make_dest_node_mock(
        77 /* max_tiles_per_phase */);

    connect_data_flow_graph({
        make_df_edge(src_node_1, dest_node, {} /* offsets */),
        make_df_edge(src_node_2, dest_node, {} /* offsets */)
    });

    configure_df_inputs(dest_node, {
        make_df_input(src_node_1, 0), make_df_input(src_node_1, 5), make_df_input(src_node_1, 10),
        make_df_input(src_node_2, 0), make_df_input(src_node_2, 3), make_df_input(src_node_2, 6),
    ...
    verify_phases(src_node_1->get_sending_phases(dest_node.get()), {
    ...
    verify_phases(src_node_2->get_sending_phases(dest_node.get()), {
...
TEST_F(Pipegen2_TransfersCalculatorInternal_CalculateReceivingPhases, DestHasDuplicatesSenderChangesDestination)
{
    std::unique_ptr<DataFlowNodeMock> src_node = make_src_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> dest_node_1 = make_dest_node_mock(
        77 /* max_tiles_per_phase */, 2 /* input_group_count */);
    std::unique_ptr<DataFlowNodeMock> dest_node_2 = make_dest_node_mock(
        77 /* max_tiles_per_phase */);
    ...
    int offset_dest_1 = data_flow_internal::calculate_receiving_phases(
        dest_node_1.get(), c_start_phase_offset);
    EXPECT_EQ(offset_dest_1, 1);

    int offset_dest_2 = data_flow_internal::calculate_receiving_phases(
        dest_node_2.get(), c_start_phase_offset + offset_dest_1);
    ...
    verify_phases(dest_2_phases.at(0), {
        PhaseInfo(c_start_phase_offset + 1, 5 /* num_msgs */)
    });

    EXPECT_EQ(dest_1_phases.size(), 2);
    EXPECT_THAT(dest_1_phases, Contains(Key(0)));
    verify_phases(dest_1_phases.at(0), {
        PhaseInfo(c_start_phase_offset, 5 /* num_msgs */)
    });
    EXPECT_THAT(dest_1_phases, Contains(Key(1)));
    verify_phases(dest_1_phases.at(1), {
        PhaseInfo(c_start_phase_offset + 2, 5 /* num_msgs */)
    });
}
...
{
    std::unique_ptr<DataFlowNodeMock> src_node_1 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> src_node_2 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);

    std::unique_ptr<DataFlowNodeMock> fork_1_1 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> fork_1_2 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> fork_1_3 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);

    std::unique_ptr<DataFlowNodeMock> fork_2_1 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> fork_2_2 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> fork_2_3 = make_df_node_mock(
        5 /* tiles_to_send */, 100 /* max_tiles_per_phase */);

    std::unique_ptr<DataFlowNodeMock> gather_1 = make_df_node_mock(
        40 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> gather_2 = make_df_node_mock(
        40 /* tiles_to_send */, 100 /* max_tiles_per_phase */);
    std::unique_ptr<DataFlowNodeMock> gather_3 = make_df_node_mock(
        15 /* tiles_to_send */, 100 /* max_tiles_per_phase */);

    std::unique_ptr<DataFlowNodeMock> union_node = make_df_node_mock(
        40 /* tiles_to_send */, 100 /* max_tiles_per_phase */, 2 /* input_group_count */);

    std::unique_ptr<DataFlowNodeMock> dest_1 = make_dest_node_mock(
        100 /* max_tiles_per_phase */, 2 /* input_group_count */);
    std::unique_ptr<DataFlowNodeMock> dest_2 = make_dest_node_mock(
        100 /* max_tiles_per_phase */, 1 /* input_group_count */);
    ...
    configure_df_inputs(gather_1, {
        make_df_input(fork_1_1, 0), make_df_input(fork_1_1, 5), make_df_input(fork_1_1, 10),
        make_df_input(fork_2_1, 10),make_df_input(fork_2_1, 15),
        make_df_input(fork_2_1, 0),
        make_df_input(fork_2_1, 0),
        make_df_input(fork_1_1, 40)
    });

    configure_df_inputs(gather_2, {
        make_df_input(fork_2_2, 0), make_df_input(fork_2_2, 5), make_df_input(fork_2_2, 10),
        make_df_input(fork_1_2, 10),make_df_input(fork_1_2, 15),
        make_df_input(fork_1_2, 0),
        make_df_input(fork_1_2, 0),
        make_df_input(fork_2_2, 40)
    });

    configure_df_inputs(gather_3, {
        make_df_input(fork_2_3, 0), make_df_input(fork_2_3, 5), make_df_input(fork_1_3, 10),
    });

    int start_phase_offset = c_start_phase_offset;

    start_phase_offset += data_flow_internal::calculate_phases(
        dest_1.get(), 0 /* data_offset */, nullptr /* dest */, start_phase_offset);

    start_phase_offset += data_flow_internal::calculate_phases(
        dest_1.get(), 0 /* data_offset */, nullptr /* dest */, start_phase_offset);

    start_phase_offset += data_flow_internal::calculate_phases(
        dest_2.get(), 0 /* data_offset */, nullptr /* dest */, start_phase_offset);

    // Verify all sending / receiving phases on all data flow nodes.
    // src_node_1
    verify_phases(src_node_1->get_sending_phases(fork_1_1.get()), {
        PhaseInfo(c_start_phase_offset, 0 /* data_offset */, 5 /* num_msgs */)
    });
    verify_phases(src_node_1->get_sending_phases(fork_1_2.get()), {
        PhaseInfo(c_start_phase_offset + 6, 0 /* data_offset */, 5 /* num_msgs */)
    });
    verify_phases(src_node_1->get_sending_phases(fork_1_3.get()), {
        PhaseInfo(c_start_phase_offset + 11, 0 /* data_offset */, 5 /* num_msgs */)
    });

    // src_node_2
    verify_phases(src_node_2->get_sending_phases(fork_2_1.get()), {
        PhaseInfo(c_start_phase_offset + 1, 0 /* data_offset */, 5 /* num_msgs */)
    });
    verify_phases(src_node_2->get_sending_phases(fork_2_2.get()), {
        PhaseInfo(c_start_phase_offset + 5, 0 /* data_offset */, 5 /* num_msgs */)
    });
    verify_phases(src_node_2->get_sending_phases(fork_2_3.get()), {
        PhaseInfo(c_start_phase_offset + 10, 0 /* data_offset */, 5 /* num_msgs */)
    });

    // fork_1_1
    verify_phases(fork_1_1->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset, 5 /* num_msgs */)
    });
    verify_phases(fork_1_1->get_sending_phases(gather_1.get()), {
        PhaseInfo(c_start_phase_offset, 0 /* data_offset */, 15 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 4, 40 /* data_offset */, 5 /* num_msgs */)
    });

    // fork_1_2
    verify_phases(fork_1_2->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 6, 5 /* num_msgs */)
    });
    verify_phases(fork_1_2->get_sending_phases(gather_2.get()), {
    ...
    // fork_1_3
    verify_phases(fork_1_3->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 11, 5 /* num_msgs */)
    });
    verify_phases(fork_1_3->get_sending_phases(gather_3.get()), {
        PhaseInfo(c_start_phase_offset + 11, 10 /* data_offset */, 5 /* num_msgs */),
    });

    // fork_2_1
    verify_phases(fork_2_1->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 1, 5 /* num_msgs */)
    });
    verify_phases(fork_2_1->get_sending_phases(gather_1.get()), {
    ...
    // fork_2_2
    verify_phases(fork_2_2->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 5, 0 /* data_offset */, 5 /* num_msgs */)
    });
    verify_phases(fork_2_2->get_sending_phases(gather_2.get()), {
        PhaseInfo(c_start_phase_offset + 5, 0 /* data_offset */, 15 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 9, 40 /* data_offset */, 5 /* num_msgs */)
    });

    // fork_2_3
    verify_phases(fork_2_3->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 10, 0 /* data_offset */, 5 /* num_msgs */)
    });
    verify_phases(fork_2_3->get_sending_phases(gather_3.get()), {
        PhaseInfo(c_start_phase_offset + 10, 10 /* num_msgs */),
    });

    // gather_1
    verify_phases(gather_1->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset, 15 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 1, 20 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 4, 5 /* num_msgs */),
    });
    verify_phases(gather_1->get_sending_phases(union_node.get()), {
        PhaseInfo(c_start_phase_offset, 40 /* num_msgs */),
    });

    // gather_2
    verify_phases(gather_2->get_receiving_phases_per_input_group().at(0), {
    ...
    verify_phases(gather_2->get_sending_phases(union_node.get()), {
        PhaseInfo(c_start_phase_offset + 5, 40 /* num_msgs */),
    });

    // gather_3
    verify_phases(gather_3->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 10, 10 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 11, 5 /* num_msgs */),
    });
    verify_phases(gather_3->get_sending_phases(dest_2.get()), {
        PhaseInfo(c_start_phase_offset + 10, 15 /* num_msgs */),
    });

    // union_node
    verify_phases(union_node->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset, 40 /* num_msgs */),
    });
    verify_phases(union_node->get_receiving_phases_per_input_group().at(1), {
        PhaseInfo(c_start_phase_offset + 5, 40 /* num_msgs */),
    });
    verify_phases(union_node->get_sending_phases(dest_1.get()), {
        PhaseInfo(c_start_phase_offset, 0 /* data_offset */, 40 /* num_msgs */),
        PhaseInfo(c_start_phase_offset + 5, 0 /* data_offset */, 40 /* num_msgs */),
    });

    // dest_1
    verify_phases(dest_1->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset, 40 /* num_msgs */),
    });
    verify_phases(dest_1->get_receiving_phases_per_input_group().at(1), {
        PhaseInfo(c_start_phase_offset + 5, 40 /* num_msgs */),
    });

    // dest_2
    verify_phases(dest_2->get_receiving_phases_per_input_group().at(0), {
        PhaseInfo(c_start_phase_offset + 10, 15 /* num_msgs */),
    });
}