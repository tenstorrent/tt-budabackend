...
TEST(Pipegen2_CoreResources, AllocateGatherStream_ProperAllocatedStreamStorage)
{
    ...
    worker_core_resources.allocate_gather_stream();
...
TEST(Pipegen2_CoreResources, AllocateMulticastStream_ProperAllocatedStreamStorage)
{
    ...
    worker_core_resources.allocate_multicast_stream();
...
TEST(Pipegen2_CoreResources, AllocateGeneralPurposeStream_ProperAllocatedStreamStorage)
{
    ...
    worker_core_resources.allocate_general_purpose_stream();
...
TEST(Pipegen2_CoreResources, AllocateL1ExtraTileHeadersSpace_AllocateUntilOutOfMemory)
{
    ...
    EXPECT_THROW(worker_core_resources.allocate_l1_extra_tile_headers_space(1), std::runtime_error);
}
...
TEST(Pipegen2_CoreResources, AllocateL1DataBuffer_AllocateUntilOutOfMemory)
{
    ...
    EXPECT_NO_THROW(
        l1_current_data_buffers_space_address = worker_core_resources.allocate_l1_data_buffer(available_space - 1));
    EXPECT_EQ(l1_current_data_buffers_space_address, l1_data_buffers_space_end_address - (available_space - 1));
    ...
    EXPECT_THROW(worker_core_resources.allocate_l1_data_buffer(1), std::runtime_error);
}
...
TEST(Pipegen2_CoreResources, AllocateKernelInput_RepeatedCallsUntilExceptionThrown)
{
    WorkerCoreResourcesGS worker_core_resources({0, 0, 0});
    ...
    test_function_repeated_calls_until_exception_thrown<unsigned int>(
        [&]() -> unsigned int
        {
...
TEST(Pipegen2_CoreResources, AllocateKernelOutput_RepeatedCallsUntilExceptionThrown)
{
    WorkerCoreResourcesGS worker_core_resources({0, 0, 0});
