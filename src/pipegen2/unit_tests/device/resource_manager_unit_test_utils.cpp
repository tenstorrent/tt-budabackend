...
{
    EXPECT_EQ(
        resource_manager->allocate_packer_stream(core_location, OPERAND_OUTPUT_START_INDEX), expected_return_value);
}
...
void verify_allocate_gather_stream(
    ...
    pipegen2::ResourceManager* resource_manager)
{
    tt_cxy_pair core_location = tt_cxy_pair(chip_id, soc_descriptor_file_mock->get_ethernet_cores().front());
...
void verify_allocate_multicast_stream(
    ...
    pipegen2::StreamId expected_return_value)
{
    tt_cxy_pair worker_core_location = tt_cxy_pair(chip_id, soc_descriptor_file_mock->get_worker_cores().front());
    ...
    pipegen2::StreamId expected_return_value)
{
    ...
    EXPECT_EQ(
        resource_manager->allocate_packer_multicast_stream(worker_core_location, operand_id), expected_return_value);
}
...
void verify_allocate_ethernet_stream(
    ...
    pipegen2::ResourceManager* resource_manager)
{
    tt_cxy_pair eth_core_location = tt_cxy_pair(chip_id, soc_descriptor_file_mock->get_ethernet_cores().front());
    ...
    unsigned int min_blob_size)
{
    ...
    EXPECT_EQ(resource_manager->allocate_l1_extra_overlay_blob_space(core_location, blob_size_in_bytes), nullptr);
