...
constexpr unsigned int dram_output_stream_max_write_issue_bytes = 6 * 256 * 32;

// TODO: Create a preprocessing wrapper that would do static assert that value of a constant is greater than 0. This
// would protect us from someone using a constant that is not supported in a given architecture.

// Minimal size of dram receiver stream in bytes.
constexpr unsigned int dram_rec_stream_min_size_bytes = 52 * 1024;
constexpr unsigned int eth_dram_rec_stream_min_size_bytes = 0;

// Maximal size of dram receiver stream in bytes.
constexpr unsigned int pipe_reduce_mem_usage_threshold_bytes = 100 * 1024;
constexpr unsigned int eth_pipe_reduce_mem_usage_threshold_bytes = 0;
// ---------------------------------------------------------------------------------------------------------------------
