...
private:
    ...
    std::map<tt_cxys_pair, const std::vector<NcriscConfig>*> collect_ncrisc_configs(const StreamGraph* stream_graph);
    ...
    // for the value.
    template <typename T>
    void write_optionally(
        std::string&& name, const std::optional<T>& opt, std::string (*converter_f)(const T&) = nullptr);

    // Writes parameter string to the blob yaml, with an optional converter function for the value.
    template <typename T>
    void write_param(std::string&& name, const T& value, std::string (*converter_f)(const T&) = nullptr);
    ...
    static typename std::enable_if<
        std::is_constructible<std::string, decltype(std::to_string(std::declval<T>()))>::value,
        std::string>::type
    get_string(T value);
    ...
    template <typename T>
    static std::string get_string(const std::vector<T>& values, std::string (*converter_f)(const T&) = nullptr);
