...
class L1DataBuffersFactory {
   public:
    ...
    static std::unique_ptr<TileHeaderL1Buffer> create_tile_header_buffer(const tt_cxy_pair& core_physical_location,
    ...
    static std::unique_ptr<StreamL1Buffer> create_stream_buffer(const StreamNode* stream_node,
                                                                const unsigned int buffer_address,
