...
namespace pipegen2
{
    ...
    {
    public:
        VirtualNode(const tt_cxy_pair& physical_location,
                    unsigned int size_tiles,
                    unsigned int tile_size,
                    unsigned int num_epoch_tiles,
                    unsigned int tiles_per_input) :
            RGBaseNode(-1, RGNodeType::Virtual, physical_location, size_tiles, tile_size,
                       num_epoch_tiles, tiles_per_input),
            m_parent_rg_node(nullptr)
        {
        }

        explicit VirtualNode(const RGBaseNode* rg_node) :
            VirtualNode(rg_node, rg_node->get_physical_location())
        {
        }

        VirtualNode(const RGBaseNode* rg_node, const tt_cxy_pair& physical_location) :
            RGBaseNode(-1, RGNodeType::Virtual, physical_location, rg_node->get_size_tiles(), rg_node->get_tile_size(),
                       rg_node->get_num_epoch_tiles(), rg_node->get_tiles_per_input(),
        ...
        explicit VirtualNode(NodeId node_id) :
            RGBaseNode(node_id, RGNodeType::Virtual, tt_cxy_pair(), 0, 0, 0, 0),
            m_parent_rg_node(nullptr)
        {
        }

        explicit VirtualNode(const tt_cxy_pair& physical_location) :
            RGBaseNode(-1, RGNodeType::Virtual, physical_location, 0, 0, 0, 0),
            m_parent_rg_node(nullptr)
        {
        }
