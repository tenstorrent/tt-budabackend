...
{
public:
    explicit PipeInput(const RGBaseNode* node) : m_node(node), m_offset(0) {}

    PipeInput(const RGBaseNode* node, unsigned int offset) : m_node(node), m_offset(offset) {}
    ...
    const RGBaseNode* get_non_virtual_node() const { return VirtualNode::get_non_virtual_node(m_node); }
...
public:
    ...
    void add_input_node(const RGBaseNode* node, unsigned int offset) { m_inputs.emplace_back(PipeInput(node, offset)); }
