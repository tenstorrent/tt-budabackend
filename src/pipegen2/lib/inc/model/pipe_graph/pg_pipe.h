...
public:
    ...
    {
    public:
        explicit Input(PGBuffer* buffer) : m_buffer(buffer), m_offset(0) {}

        Input(PGBuffer* buffer, unsigned int offset) : m_buffer(buffer), m_offset(offset) {}
    ...
    void set_pipe_periodic_repeat(unsigned int pipe_periodic_repeat) { m_pipe_periodic_repeat = pipe_periodic_repeat; }
    ...
    void set_is_mmio_pipe_downstream(bool is_mmio_pipe_downstream) { m_mmio_pipe_downstream = is_mmio_pipe_downstream; }
    ...
    const std::vector<tt_cxy_pair>& get_mcast_core_logical_locations() const { return m_mcast_core_logical_locations; }
    ...
    void add_output_buffer_id(const NodeId id) { m_output_buffers_ids.push_back({id}); }

    void add_input_buffer_id(const NodeId id) { m_input_buffers_ids.push_back(id); }
    ...
    void add_input_buffer(PGBuffer* buffer, unsigned int offset) { m_inputs.emplace_back(Input(buffer, offset)); }
