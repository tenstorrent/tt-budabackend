...
public:
    ...
    virtual unsigned int get_input_group_count() const { return is_union() ? m_sources.size() : 1; }

    // Returns the total number of inputs per one input group.
    unsigned int get_num_inputs_per_input_group() const { return get_total_num_inputs() / get_input_group_count(); }
    ...
    void add_sending_phase(
        DataFlowNode* destination, unsigned int phase_offset, unsigned int data_offset, unsigned int num_msgs);
    ...
    unsigned int get_current_input_group_index() const { return m_current_input_group_index % get_input_group_count(); }

    void add_input(DataFlowNode* source, unsigned int offset) { m_inputs.emplace_back(source, offset); }
...
private:
    ...
    bool can_expand_last_receiving_phase(
