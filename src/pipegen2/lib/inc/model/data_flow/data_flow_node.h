...
namespace pipegen2
{
    ...
    public:
        ...
        void set_is_on_direct_path(bool val) { m_is_on_direct_path = val; }

        bool is_on_direct_path() const { return m_is_on_direct_path; }

        bool is_union() const;

        bool is_scatter() const;

        bool is_dram_or_pcie_input() const
        {
            return try_get_dram_input_node() != nullptr || try_get_pcie_streaming_node() != nullptr;
        }

        bool is_dram_or_pcie_output() const
        {
        ...
        {
            return is_root_node() && is_leaf_node();
        }
        ...
        unsigned int get_num_epoch_tiles() const { return m_rg_node->get_num_epoch_tiles(); }

        unsigned int get_consume_granularity() const;

        unsigned int get_scatter_gather_num_tiles() const;
        ...
        unsigned int get_size_tiles() const { return m_rg_node->get_size_tiles(); }

        unsigned int get_num_tiles_per_input() const { return m_rg_node->get_tiles_per_input(); }
        ...
        uint32_t get_num_iterations() const { return m_num_iterations; }

        uint32_t get_subtree_common_divisor() const { return m_subtree_common_divisor; }
        ...
        void set_num_iterations(uint32_t val) { m_num_iterations = val; }

        void set_subtree_common_divisor(uint32_t val) { m_subtree_common_divisor = val; }
    ...
    private:
        ...
        bool is_dram_parallel_fork() const;
        ...
        uint32_t m_subtree_common_divisor;

        // Whether this node is on direct path or not. Note that node can be on multiple direct paths if it is forked in
        // such way.
        bool m_is_on_direct_path;
