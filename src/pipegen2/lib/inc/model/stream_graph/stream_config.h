...
#define ADD_FIELD_TO_MAP(Name)                                                                                       \
    ...
    {                                                                                                                \
        ...
        field_map->emplace(                                                                                          \
            #Name, make_tuple(&StreamConfig::get_##Name, &StreamConfig::set_##Name, &StreamConfig::opt_set_##Name)); \
    } while (0);
...
private:
    ...
    template <typename T>
    using FieldMap = std::unordered_map<
        std::string,
        std::tuple<
            const std::optional<T>& (StreamConfig::*)() const,
            void (StreamConfig::*)(T),
    ...
    using FieldMapBase = std::variant<
        FieldMap<bool>,
...
public:
    ...
    void apply(const StreamConfig& other_config, bool force_set = true)
    {
        for (const auto& kv : other_config.m_field_maps)
        {
            ...
            std::visit(
                [this, &type_index, &other_config, &force_set](auto&& arg)
                {
