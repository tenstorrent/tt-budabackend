...
{
public:
    StreamPhasesCommonConfig(StreamNode* stream_node) : m_stream_node(stream_node), m_common_config() {}

    StreamPhasesCommonConfig(StreamNode* stream_node, StreamConfig&& common_config) :
        m_stream_node(stream_node), m_common_config(std::move(common_config))
    {
    }
...
{
public:
    StreamNode(StreamType stream_type, const tt_cxy_pair& physical_location, unsigned int operand_id = 0);
    ...
    void add_phase_configs(std::vector<PhaseConfig>&& phase_configs, const unsigned int max_num_tiles_per_phase);
    ...
    // as the old one).
    void update_phases(const std::vector<PhaseInfo>& new_phases_info, const unsigned int new_max_num_tiles_per_phase);
    ...
    void update_phases(std::vector<PhaseConfig>&& new_phase_configs, const unsigned int new_max_num_tiles_per_phase);
    ...
    std::vector<PhaseConfig>& get_phase_configs() { return m_phase_configs; }

    // Gets a const reference to configs of all phases.
    const std::vector<PhaseConfig>& get_phase_configs() const { return m_phase_configs; }
    ...
    void set_ncrisc_configs(std::vector<NcriscConfig>&& configs) { m_ncrisc_configs = std::move(configs); }
    ...
    PhaseId get_last_phase_offset() const { return m_phase_configs.back().phase_id; }
