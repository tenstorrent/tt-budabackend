...
enum class NOC_ROUTE : unsigned int { NOC0 = 0, NOC1 = 1, INVALID = std::numeric_limits<unsigned int>::max() };
...
constexpr inline bool operator<(const tt_cxys_pair &left, const tt_cxys_pair &right) {
    return (left.chip < right.chip || (left.chip == right.chip && left.x < right.x) ||
