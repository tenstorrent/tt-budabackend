...
class BasePipegen2CompileException : public std::runtime_error {
   ...
   protected:
    ...
    BasePipegen2CompileException(const std::string& error_message, const std::optional<tt_cxy_pair>& logical_location,
...
class BasePipegen2IOException : public std::runtime_error {
   ...
   protected:
    BasePipegen2IOException(const std::string& error_message) : std::runtime_error(error_message) {}
};
...
class IllegalCoreResourceAllocationException : public BasePipegen2CompileException {
   public:
    ...
    IllegalCoreResourceAllocationException(const std::string& error_message, const tt_cxy_pair& physical_location,
                                           CoreResourceType core_resource_type)
        : BasePipegen2CompileException(error_message, std::nullopt /* logical_location */,
                                       std::make_optional(physical_location)),
          m_core_resource_type(core_resource_type) {}

    IllegalCoreResourceAllocationException(const std::string& error_message, const tt_cxy_pair& physical_location,
                                           const tt_cxy_pair& logical_location, CoreResourceType core_resource_type)
        : BasePipegen2CompileException(error_message, logical_location, std::make_optional(physical_location)),
          m_core_resource_type(core_resource_type) {}
...
class OutOfCoreResourcesException : public IllegalCoreResourceAllocationException {
   public:
    OutOfCoreResourcesException(const std::string& error_message, const tt_cxy_pair& physical_location,
                                CoreResourceType core_resource_type, unsigned int avaiable_core_resources,
    ...
    OutOfCoreResourcesException(const std::string& error_message, const tt_cxy_pair& physical_location,
                                const tt_cxy_pair& logical_location, CoreResourceType core_resource_type,
                                unsigned int avaiable_core_resources, unsigned int used_core_resources)
        : IllegalCoreResourceAllocationException(error_message, physical_location, logical_location,
                                                 core_resource_type),
          m_total_core_resources_available(avaiable_core_resources),
          m_core_resources_used(used_core_resources) {}
    ...
    OutOfCoreResourcesException(const std::string& error_message, const tt_cxy_pair& physical_location,
                                CoreResourceType core_resource_type, unsigned int avaiable_core_resources)
        : OutOfCoreResourcesException(error_message, physical_location, core_resource_type, avaiable_core_resources,
                                      avaiable_core_resources + 1 /* used_core_resources */) {}

    OutOfCoreResourcesException(const std::string& error_message, const tt_cxy_pair& physical_location,
                                const tt_cxy_pair& logical_location, CoreResourceType core_resource_type,
                                unsigned int avaiable_core_resources)
        : OutOfCoreResourcesException(error_message, physical_location, logical_location, core_resource_type,
                                      avaiable_core_resources, avaiable_core_resources + 1 /* used_core_resources */) {}
...
class InvalidPipeGraphSpecificationException : public BasePipegen2CompileException {
   public:
    ...
    InvalidPipeGraphSpecificationException(const std::string& error_message, const tt_cxy_pair& logical_location,
                                           const std::optional<tt_cxy_pair>& physical_location)
        : InvalidPipeGraphSpecificationException(error_message, std::make_optional(logical_location),
                                                 physical_location) {}
...
class InvalidPipegenYamlException : public BasePipegen2IOException {
   public:
    InvalidPipegenYamlException(const std::string& error_message) : BasePipegen2IOException(error_message) {}
};

// Exception type used for SoC info yaml parsing exceptions.
class InvalidSoCInfoYamlException : public BasePipegen2IOException {
   public:
    InvalidSoCInfoYamlException(const std::string& error_message) : BasePipegen2IOException(error_message) {}
};
