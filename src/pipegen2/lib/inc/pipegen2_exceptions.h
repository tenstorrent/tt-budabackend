...
protected:
    ...
    {
        log_assert(
            logical_location.has_value() || physical_location.has_value(),
...
public:
    ...
    IllegalCoreResourceAllocationException(
        const std::string& error_message, const tt_cxy_pair& physical_location, CoreResourceType core_resource_type) :
        BasePipegen2CompileException(
            error_message, std::nullopt /* logical_location */, std::make_optional(physical_location)),
...
{
public:
    InvalidPipeGraphSpecificationException(
        const std::string& error_message,
        const std::optional<tt_cxy_pair>& logical_location,
        const std::optional<tt_cxy_pair>& physical_location) :
    ...
    InvalidPipeGraphSpecificationException(const std::string& error_message, const tt_cxy_pair& logical_location) :
        InvalidPipeGraphSpecificationException(
            error_message, std::make_optional(logical_location), std::nullopt /* physical_location */)
    {
    }

    InvalidPipeGraphSpecificationException(
        const std::string& error_message, const tt_cxy_pair& logical_location, const tt_cxy_pair& physical_location) :
        InvalidPipeGraphSpecificationException(
            error_message, std::make_optional(logical_location), std::make_optional(physical_location))
    {
    }
};
