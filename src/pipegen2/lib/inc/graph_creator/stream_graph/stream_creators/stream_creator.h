...
public:
    ...
    void configure_dram_relay_stream(
    ...
    void configure_dram_to_unpacker_prefetch_post_tm_stream(
        const RGBasePipe* rg_pipe,
        const DataFlowInfo& data_flow_info,
        const NcriscConfig& base_ncrisc_config,
    ...
    // false otherwise.
    bool can_configure_dram_to_unpacker_stream(
        const RGBasePipe* rg_pipe,
        const NcriscConfig& base_ncrisc_config,
        const UnpackerOutputNode* unpacker_node,
    ...
    void configure_dram_to_unpacker_stream(
        const RGBasePipe* rg_pipe,
        const NcriscConfig& base_ncrisc_config,
        const UnpackerOutputNode* unpacker_node,
        StreamNode* stream);
    ...
    void configure_packer_to_noc_stream(
    ...
    void update_stream_for_dram_write(StreamNode* stream, const bool is_remote_write, const bool is_untilized_output);

    // Configures stream's parameters related to untilization.
    void configure_packer_stream_with_untilization_params(
        const RGBasePipe* rg_pipe, unsigned int packer_stride, StreamNode* stream);

    // Configures relay stream, with default buffer size.
    void configure_relay_stream_with_default_buffer_size(
    ...
    void configure_gather_relay_stream(StreamNode* relay_stream, const RGBasePipe* pipe);

    // Configures relay stream which is gathering from packer streams and writing to PCIe.
    void configure_gather_to_pcie_relay_stream(StreamNode* relay_stream, const RGBasePipe* pipe);
    ...
    void configure_prefetch_pre_tm_relay_stream(
        StreamNode* stream,
    ...
    void configure_dram_multicast_stream(
        const RGBasePipe* rg_pipe,
    ...
    std::vector<PhaseInfo> decompose_phases(
    ...
    std::vector<PhaseInfo> merge_phases(const std::vector<PhaseInfo>& phase_info_vec, unsigned int max_msgs_in_phase);

    // Configures stream configs source and destination fields based on given source and destination streams.
    void configure_stream_src_and_dest(StreamNode* stream, StreamNode* source_stream, StreamNode* destination_stream);

    // Configures stream configs source and destination fields based on given source and destination streams.
    void configure_stream_src_and_dest(
        StreamNode* stream,
    ...
    void configure_multicast_streams_src_and_dest(
    ...
    void update_stream_iterations_in_epoch(
    ...
    void update_resend_of_sending_stream(
    ...
    void set_common_stream_properties_from_pipe(
        StreamNode* stream, const RGBasePipe* rg_pipe, bool incoming_stream = true);
    ...
    void set_common_l1_to_noc_stream_properties(
    ...
    unsigned int calculate_pipe_reduce_mem_usage_threshold(
    ...
    void check_and_convert_unpacker_to_embedding_index_stream(
...
private:
    ...
    static unsigned int calculate_buf_space_available_ack_thr(
    ...
    static unsigned int calculate_dram_to_unpacker_buffer_size(
    ...
    static unsigned int calculate_default_relay_stream_buffer_size_tiles(
        const unsigned int num_unique_input_nodes, const bool is_ethernet_core = false);
