...
namespace pipegen2
{
    ...
    {
    public:
        ScatterPatternCompressionResult(unsigned long num_loops, unsigned long increment, std::size_t pattern_length)
        ...
        {
            const unsigned long my_compression_level = get_compression_level();
            const unsigned long other_compression_level = other.get_compression_level();
        ...
        unsigned long get_num_loops() const
        {
            return m_num_loops;
        }
        ...
        unsigned long get_increment() const
        {
            return m_increment;
        }
    ...
    {
    public:
        DramScatterOffsetCompressor(const std::vector<unsigned long>& dram_scatter_offsets);

        // Compresses DRAM scatter offsets.
        std::vector<unsigned long> compress_dram_scatter_offsets();
        ...
        std::vector<unsigned long> compress_dram_scatter_offsets_for_tilizer(
    ...
    private:
        ...
        bool is_valid_scatter_offset_increment(const unsigned long increment);

        // Returns encoded scatter offset loop magic number for a given scatter compression.
        unsigned long encode_dram_scatter_offset_loop(const ScatterPatternCompressionResult& scatter_compression);
        ...
        unsigned long get_compression_pattern_index_increment(
            const ScatterPatternCompressionResult& scatter_compression,
            const std::size_t pattern_start_index);
        ...
        static constexpr unsigned long c_dram_io_is_scatter_loop_flag = 0x8000000000000000ULL;

        // Flag indicating that the scatter offset is special in a way that it encodes scatter loop for tilizer.
        static constexpr unsigned long c_dram_io_is_scatter_loop_for_tilizer_flag = 0x0000000080000000ULL;
        ...
        // space on Grayskull.
        static constexpr unsigned long c_bad_increment_threshold = std::numeric_limits<unsigned int>::max();

        // Original scatter offsets, without compressions.
        const std::vector<unsigned long>& m_dram_scatter_offsets;

        // Resulting, compressed scatter offsets.
        std::vector<unsigned long> m_compressed_scatter_offsets;
    };
}