...
public:
    ...
    std::vector<NcriscConfig> configure_dram_ncrisc_reader(
    ...
    NcriscConfig configure_pcie_ncrisc_reader(
    ...
    NcriscConfig configure_prefetch_pre_tm_ncrisc_reader(
    ...
    NcriscConfig configure_dram_ncrisc_writer(
        const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe, const SoCInfo* soc_info);

    // Creates NCRISC config used to write data to PCIe.
    NcriscConfig configure_pcie_ncrisc_writer(const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe);

    // Creates NCRISC config used to write accumulated data from L1 to DRAM for DRAM output intermed stream.
    NcriscConfig configure_dram_output_intermed_ncrisc_writer(
...
private:
    ...
    unsigned int calculate_dram_pipe_scatter_chunk_size(const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe);

    // Calculates read chunk size for every input to the pipe and returns minimum of those.
    unsigned int calculate_dram_pipe_read_chunk_size(
        const DataFlowInfo& data_flow_info,
    ...
    unsigned int calculate_max_chunk_size_for_pipe(
    ...
    unsigned int get_dram_read_max_transfer_size_bytes(
    ...
    NcriscConfig create_dram_ncrisc_read_config(
    ...
    NcriscConfig create_ncrisc_read_config(
    ...
    NcriscConfig create_ncrisc_write_config(
    ...
    unsigned int calculate_dram_buf_write_chunk_size_tiles(
    ...
    static unsigned int get_transfer_chunk_size_tiles(
    ...
    static bool is_transfer_chunk_size_within_limits(
        unsigned int transfer_chunk_size_tiles,
        unsigned int tile_size_bytes,
        unsigned int max_transfer_size_tiles,
        unsigned int max_transfer_size_bytes);
    ...
    static std::uint64_t get_dram_buffer_noc_address(
    ...
    const unsigned int get_dram_buf_size_factor(
