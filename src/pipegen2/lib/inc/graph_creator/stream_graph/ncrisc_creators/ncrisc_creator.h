...
namespace pipegen2
{
    ...
    private:
        ...
        static unsigned long get_dram_buffer_noc_address(const DramNodeInterface* dram_node, ChipId chip_id,
                                                         const SoCInfo* soc_info);

        // Flag indicating that a given dram scatter offset is read from the padding buffer.
        constexpr static unsigned long c_is_offset_padding_address_flag = 0x4000000000000000ULL;
        ...
        constexpr static unsigned long c_invalid_dram_scatter_offset_flag = 0xFFFFFFFFUL;
    };
}