...
public:
    DramWriteCommonStreamsCreator(StreamCreator* stream_creator, NcriscCreator* ncrisc_creator) :
        m_stream_creator(stream_creator), m_ncrisc_creator(ncrisc_creator)
    {
    }

    // Setups given packer stream for DRAM write. Configures untilization parameters if untilized output is
    // required.
    void setup_packer_stream_for_dram_write(
    ...
    void setup_packer_stream_for_pcie_write(
        const StreamPhasesCommonConfig& stream_phases_common_config,
        const RGBaseNode* input_node,
        const RGBasePipe* pipe,
        const DataFlowInfo& data_flow_info);

    void setup_relay_stream_for_pcie_write(
...
private:
    ...
    void setup_stream_for_dram_or_pcie_write(
