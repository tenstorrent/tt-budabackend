...
namespace pipegen2
{
    ...
    public:
        ...
        std::vector<std::unique_ptr<StreamNode>> create_streams(
            const RGBasePipe* pipe,
            const DataFlowInfo& data_flow_info);

    protected:
        // Constructor, available only from derived classes for specific architecture.
        PipeStreamsCreator(std::unique_ptr<NcriscCreator> ncrisc_creator,
        ...
        virtual std::vector<std::unique_ptr<StreamNode>> create_streams_internal(
            const RGBasePipe* pipe,
            const DataFlowInfo& data_flow_info) = 0;
        ...
        virtual void assign_streams_to_pipe_outputs(const RGBasePipe* pipe,
