...
{
public:
    DramReadCommonStreamsCreator(StreamCreator* stream_creator) : m_stream_creator(stream_creator) {}
...
private:
    ...
    std::unique_ptr<StreamNode> create_dram_or_pcie_multicast_stream(
        const RGBasePipe* pipe, const NcriscConfig& base_ncrisc_config, const bool is_dram_prefetch);
    ...
    void scale_up_dram_receiving_stream(
        StreamNode* stream,
        const RGBasePipe* pipe,
        const DataFlowInfo& data_flow_info,
        unsigned int max_num_tiles_per_phase,
    ...
    unsigned int recalculate_max_num_tiles_per_phase(
