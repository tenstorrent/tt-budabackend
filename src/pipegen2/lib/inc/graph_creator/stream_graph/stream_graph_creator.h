...
private:
    ...
    static std::unordered_map<NodeId, StreamNode*> create_node_id_to_stream_mapping(const StreamGraph* stream_graph);

    // Moves created streams to stream graph.
    static void move_created_streams_to_graph(
    ...
    // same offset etc.
    void unroll_phases(StreamNode* stream_node, unsigned int num_iterations_in_epoch, unsigned int unroll_factor);
    ...
    bool should_skip_last_phase_for_ptrs_not_zero(
    ...
    void optimize_edge_phases(
        StreamNode* source_stream, StreamNode* dest_stream, const int start_phase_idx, const int end_phase_idx);
    ...
    void add_epoch_offset_to_stream_phases(const StreamGraphCollection* stream_graph_collection, const int epoch_num);
