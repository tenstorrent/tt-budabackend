...
namespace pipegen2
{
    ...
    {
        ASSERT(!new_phases_info.empty(), "Expecting at least one new phase");
        ASSERT(m_phase_configs.empty() || new_phases_info[0].phase_offset == m_phase_configs.at(0).phase_id,
    ...
    void StreamNode::update_phases(std::vector<PhaseConfig>&& new_phase_configs,
                                   const unsigned int new_max_num_tiles_per_phase)
    {
        ASSERT(!new_phase_configs.empty(), "Expecting at least one new phase config");
        ASSERT(m_phase_configs.empty() || new_phase_configs[0].phase_id == m_phase_configs.at(0).phase_id,
               "Expecting the same inital phase id when updating phases");
    ...
    {
        ASSERT(!m_ncrisc_configs.empty(), "Attempting to read NCRISC config from stream that has no NCRISC configs set");
    ...
    bool StreamNode::changes_destinations() const
    {
        ...
        for (const PhaseConfig& phase_config : m_phase_configs)
        {
            ...
            ASSERT(config.get_dest().has_value(), "Expected all phases to have a destination");
    ...
    {
        ASSERT(m_phase_configs.size() > 0, "Must be called after phases are set");
        if (m_phase_configs.size() == 1)
        {
            return false;
        }
        ...
        {
            const StreamConfig& config = phase_config.config;
            ASSERT(config.get_source().has_value(), "Expected all phases to have a source");
