...
StreamNode::StreamNode(StreamType stream_type, const tt_cxy_pair& physical_location, unsigned int operand_id) :
...
void StreamNode::add_phase_configs(
    const std::vector<PhaseInfo>& phases_info, const unsigned int max_num_tiles_per_phase)
{
...
void StreamNode::add_phase_configs(std::vector<PhaseConfig>&& phase_configs, const unsigned int max_num_tiles_per_phase)
{
    set_max_num_tiles_per_phase(max_num_tiles_per_phase);

    m_phase_configs.insert(
        m_phase_configs.end(),
...
void StreamNode::sort_phases() { std::sort(m_phase_configs.begin(), m_phase_configs.end()); }

void StreamNode::update_phases(
    const std::vector<PhaseInfo>& new_phases_info, const unsigned int new_max_num_tiles_per_phase)
{
    log_assert(!new_phases_info.empty(), "Expecting at least one new phase");
    log_assert(
        m_phase_configs.empty() || new_phases_info[0].phase_offset == m_phase_configs.at(0).phase_id,
...
void StreamNode::update_phases(
    std::vector<PhaseConfig>&& new_phase_configs, const unsigned int new_max_num_tiles_per_phase)
{
    log_assert(!new_phase_configs.empty(), "Expecting at least one new phase config");
    log_assert(
        m_phase_configs.empty() || new_phase_configs[0].phase_id == m_phase_configs.at(0).phase_id,
        "Expecting the same inital phase id when updating phases");
...
const NcriscConfig& StreamNode::get_base_ncrisc_config() const { return get_base_ncrisc_config(); }
...
std::vector<StreamNode*> StreamNode::get_unique_sources() const
{
    ...
    return std::vector<StreamNode*>(
        std::make_move_iterator(stream_sources.begin()), std::make_move_iterator(stream_sources.end()));
}
...
std::vector<StreamNode*> StreamNode::get_unique_destinations() const
{
    ...
    return std::vector<StreamNode*>(
        std::make_move_iterator(stream_destinations.begin()), std::make_move_iterator(stream_destinations.end()));
}
