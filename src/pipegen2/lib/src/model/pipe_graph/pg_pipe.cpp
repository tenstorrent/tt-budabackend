...
namespace pipegen2
{
    ...
    std::unordered_set<const PGBuffer*> PGPipe::get_unique_input_buffers() const
    {
        std::unordered_set<const PGBuffer*> unique_input_buffers;
        for (const PGPipe::Input& input : m_inputs)
        {
            unique_input_buffers.insert(input.get_buffer());
        }
    ...
    {
        ASSERT(has_single_output(), "Pipe does not have only one output buffer!");
    ...
    std::unordered_set<const PGBuffer*> PGPipe::get_unique_output_padding_buffers() const
    {
        std::unordered_set<const PGBuffer*> unique_output_padding_buffers;
        for (const auto& [scatter_index, output_padding_inputs] : m_scatter_index_to_output_padding_inputs)
        {
            ASSERT(output_padding_inputs.size() > 0,
            ...
            unique_output_padding_buffers.insert(output_padding_inputs[0].get_buffer());
        }

        return unique_output_padding_buffers;
    }

    std::unordered_set<const PGBuffer*> PGPipe::get_unique_input_buffers_including_output_padding() const
    {
        ...
        std::unordered_set<const PGBuffer*> all_unique_input_buffers = get_unique_input_buffers();

        for (const PGBuffer* output_padding_buffer : get_unique_output_padding_buffers())
        {
            all_unique_input_buffers.insert(output_padding_buffer);
        }
    ...
    void PGPipe::add_output_padding_buffer(PGBuffer* output_padding_buffer, std::size_t scatter_index)
    {
        ...
        ASSERT(num_msgs_per_scatter_index % output_padding_buffer->get_scatter_gather_num_tiles() == 0,
    ...
    {
        const std::unordered_set<const PGBuffer*> unique_input_buffers = get_unique_input_buffers();
    ...
    bool PGPipe::is_scatter_prefetch_post_tm() const
    {
        ...
        ASSERT(pipe_has_scatter_prefetch_post_tm_inputs != pipe_has_non_scatter_prefetch_post_tm_inputs,
