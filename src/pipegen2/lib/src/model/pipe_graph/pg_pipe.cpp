...
PGPipe::PGPipe(NodeId id) :
    ...
    m_incoming_noc_id(NOC_ROUTE::NOC0),
    m_incoming_noc_vc(0),
    m_outgoing_noc_id(NOC_ROUTE::NOC0),
    m_outgoing_noc_vc(0),
    m_mmio_pipe(false),
...
{
    return is_output_padding(scatter_index) ? m_scatter_index_to_output_padding_inputs.at(scatter_index) : m_inputs;
}
...
std::vector<const PGBuffer*> PGPipe::get_unique_output_padding_buffers() const
{
    ...
    for (const auto& [scatter_index, output_padding_inputs] : m_scatter_index_to_output_padding_inputs)
    {
        ...
        if (unique_output_padding_buffers_set.find(output_padding_buffer) == unique_output_padding_buffers_set.end())
        {
...
std::vector<const PGBuffer*> PGPipe::get_unique_input_buffers_including_output_padding() const
{
    ...
    std::copy(
        all_unique_input_buffers.begin(),
        all_unique_input_buffers.end(),
...
bool PGPipe::has_consumer_duplicates() const { return m_consumer_repeat > 1 || has_output_list_duplicates(); }
...
void PGPipe::add_output_padding_buffer(PGBuffer* output_padding_buffer, std::size_t scatter_index)
{
    ...
    log_assert(
        num_msgs_per_scatter_index % output_padding_buffer->get_scatter_gather_num_tiles() == 0,
...
bool PGPipe::is_connecting_l1_buffers() const
{
    ...
    const bool pipe_has_l1_source_buffers = std::all_of(
        unique_input_buffers.begin(),
        unique_input_buffers.end(),
        [](const PGBuffer* source_buffer) { return source_buffer->is_located_in_l1(); });

    const bool pipe_has_l1_dest_buffers = std::all_of(
        first_dest_buffers.begin(),
        first_dest_buffers.end(),
        [](const PGBuffer* dest_buffer) { return dest_buffer->is_located_in_l1(); });
...
bool PGPipe::has_single_buffer_input() const { return get_unique_input_buffers().size() == 1; }

bool PGPipe::is_direct_intermediate_pipe() const
{
    return (
        has_single_input() && m_inputs[0].get_buffer()->is_intermediate_operand() && has_single_output() &&
        m_output_buffers[0][0]->is_intermediate_operand());
}

bool PGPipe::is_join_intermediate_pipe() const
{
    return (
        has_single_output() && m_output_buffers[0][0]->is_intermediate_operand() && m_inputs.size() == 2 &&
        (m_inputs[0].get_buffer()->is_intermediate_operand() || m_inputs[1].get_buffer()->is_intermediate_operand()));
}
...
bool PGPipe::is_dram_prefetch_post_tm() const
{
    return std::any_of(
        m_inputs.begin(),
        m_inputs.end(),
        [](const PGPipe::Input& pipe_input) { return pipe_input.get_buffer()->is_dram_prefetch_post_tm(); });
}
