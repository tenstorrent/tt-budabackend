...
namespace pipegen2
{
    ...
    {
        ASSERT(m_inputs.size() > 1, "Expecting DramEmbeddingPipe to have at least 2 inputs");
        ...
        ASSERT(dram_embedding_table, "Expected DramEmbeddingTableInputNode as first input to DramEmbedding pipe");
    ...
    const VirtualNode* DramEmbeddingPipe::get_embedding_index_v_node() const
    {
        ...
        ASSERT(embedding_index, "Expected embedding index to be wrapped in virtual node");
