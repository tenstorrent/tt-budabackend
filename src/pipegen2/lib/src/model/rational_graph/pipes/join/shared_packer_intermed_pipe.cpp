...
namespace pipegen2
{
    ...
    {
        ASSERT(m_inputs.size() - m_inputs_without_virtual.size() == 1, "Shared virtual node doesn't exist");
    ...
    const std::vector<PipeInput>& SharedPackerIntermedPipe::get_inputs() const
    {
        ...
        {
            ASSERT(m_inputs.size() > 1, "Expected pipe to have more than one input");
