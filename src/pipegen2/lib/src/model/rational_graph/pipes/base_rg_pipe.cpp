...
{
    const DramInputNode* dram_input_node = dynamic_cast<const DramInputNode*>(get_inputs()[0].get_non_virtual_node());
...
void RGBasePipe::swap_output_nodes(
    const std::size_t first_output_node_index, const std::size_t second_output_node_index)
{
    log_assert(
        first_output_node_index >= 0 && first_output_node_index < m_output_nodes.size(),
        "Given first output node index is out of bounds");
    log_assert(
        second_output_node_index >= 0 && second_output_node_index < m_output_nodes.size(),
...
unsigned int RGBasePipe::get_min_num_tiles_to_transfer(const DataFlowInfo& data_flow_info) const
{
    ...
    for (const PipeInput& pipe_input : get_inputs())
    {
        min_num_tiles_to_transfer =
            std::gcd(min_num_tiles_to_transfer, data_flow_info.get_tiles_to_send(pipe_input.get_node()));
    }
