...
namespace pipegen2
{
    ...
    {
        ASSERT(!m_inputs.empty(), "Expecting at least one input to the pipe");
    ...
    {
        ASSERT(m_output_nodes.size() == 1, "Expecting only a single node at the pipe output");
    ...
    {
        ASSERT(first_output_node_index >= 0 && first_output_node_index < m_output_nodes.size(),
               "Given first output node index is out of bounds");
        ASSERT(second_output_node_index >= 0 && second_output_node_index < m_output_nodes.size(),
    ...
    std::vector<unsigned int> RGBasePipe::get_input_node_offsets(const RGBaseNode* input_node) const
    {
        ...
        ASSERT(!offsets.empty(), "Tried to get input offsets for the node which is not in pipe inputs");
