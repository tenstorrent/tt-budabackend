...
namespace pipegen2
{
RationalGraph::RationalGraph(
    std::vector<std::unique_ptr<RGBaseNode>>&& nodes,
    std::vector<std::unique_ptr<RGBasePipe>>&& pipes,
    const bool is_doing_pcie_transfer) :
    m_nodes(std::move(nodes)), m_pipes(std::move(pipes)), m_is_doing_pcie_transfer(is_doing_pcie_transfer)
{
}
...
void RationalGraph::depth_first_search(
