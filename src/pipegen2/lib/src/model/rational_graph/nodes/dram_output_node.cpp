...
namespace pipegen2
{
    ...
    unsigned int DramOutputNode::get_datums_row_size_bytes() const
    {
        return m_tile_size / m_untilized_output_tile_dim_r;
    }

    unsigned int DramOutputNode::get_stride_offset_bytes(const unsigned int stride_index) const
    {
        ASSERT(m_untilized_output, "Expecting call to get_stride_offset_bytes() only for untilized outputs");
    ...
    {
        ASSERT(m_untilized_output,
               "Expecting call to get_bytes_count_in_untilized_row_of_tiles() only for untilized outputs");
    ...
    {
        ASSERT(m_untilized_output,
               "Expecting call to get_bytes_count_in_untilized_row_of_cores() only for untilized outputs");
    ...
    {
        ASSERT(m_untilized_output,
               "Expecting call to get_bytes_count_in_untilized_output() only for untilized outputs");
