...
namespace pipegen2
{
    ...
    {
        ASSERT(m_sending_phases.count(destination), "Destination not found");
    ...
    {
        m_receiving_phases_per_input_group[m_current_input_group_index].emplace_back(phase_offset, num_msgs);
    }

    void DataFlowNode::update_receiving_phases(const unsigned int phase_offset,
                                               const unsigned int receiving_chunk_size)
    {
        std::vector<PhaseInfo>& current_input_group_receiving_phases =
            m_receiving_phases_per_input_group[m_current_input_group_index];

        ASSERT(!current_input_group_receiving_phases.empty(), "Receiving phases should not be empty when updating");
    ...
    {
        const auto it = m_sending_phases.find(destination);
        ASSERT(it != m_sending_phases.end(), "Sending phases to the given destination are not yet calculated");
