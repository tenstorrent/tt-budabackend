...
namespace pipegen2
{
DataFlowNode::DataFlowNode(const RGBaseNode* rg_base_node) : DataFlowNode()
{
...
NodeId DataFlowNode::get_id() const { return m_rg_node->get_id(); }
...
unsigned int DataFlowNode::get_serialization_factor() const
{
...
unsigned int DataFlowNode::get_repeat_factor() const { return m_rg_pipe ? m_rg_pipe->get_periodic_repeat() : 1; }

bool DataFlowNode::is_untilized_dram_output() const
{
    const DramOutputNode* output_node = try_get_dram_output_node();
    return (output_node != nullptr && output_node->is_untilized_output());
}

bool DataFlowNode::is_intermediate() const { return dynamic_cast<const BaseIntermedNode*>(m_rg_node) != nullptr; }

bool DataFlowNode::can_do_epoch_in_single_iteration() const { return is_untilized_dram_output() || is_intermediate(); }
...
unsigned int DataFlowNode::get_size_tiles() const { return m_rg_node->get_size_tiles(); }

unsigned int DataFlowNode::get_num_tiles_per_input() const { return m_rg_node->get_tiles_per_input(); }
...
{
    log_assert(
        m_current_input_group_index < m_number_of_unique_df_paths,
        "Current input group index out of range for node {}",
        get_id());

    const std::vector<DataFlowNodeInput>& df_inputs = get_all_inputs();
    const unsigned int num_inputs_per_group = get_num_inputs_per_input_group();

    return DataFlowNodeInputRange(
        df_inputs, get_current_input_group_index() * num_inputs_per_group, num_inputs_per_group);
}
...
bool DataFlowNode::is_dram_or_pcie_output() const
{
    return try_get_dram_output_node() != nullptr || try_get_pcie_streaming_node() != nullptr;
}
...
bool DataFlowNode::is_isolated_node() const { return is_root_node() && is_leaf_node(); }

bool DataFlowNode::try_update_last_sending_phase(DataFlowNode* destination)
{
    log_assert(destination, "Expecting valid destination as argument to DataFlowNode::try_update_last_sending_phase");
    log_assert(
        m_sending_phases.count(destination),
        "Did not find sending phases from node {} to destination node {}",
        get_id(),
        destination->get_id());
...
void DataFlowNode::add_sending_phase(
    DataFlowNode* destination,
    const unsigned int phase_offset,
...
bool DataFlowNode::can_expand_last_receiving_phase(
    const std::vector<PhaseInfo>& receiving_phases, unsigned int receiving_chunk_size) const
{
...
void DataFlowNode::expand_last_receiving_phase(
    std::vector<PhaseInfo>& receiving_phases, const unsigned int receiving_chunk_size)
{
...
void DataFlowNode::update_receiving_phases(const unsigned int phase_offset, const unsigned int receiving_chunk_size)
{
    std::vector<PhaseInfo>& current_input_group_receiving_phases = get_receiving_phases_for_current_input_group();

    log_assert(
        !current_input_group_receiving_phases.empty(),
        "Receiving phases of node {} should not be empty when updating",
        get_id());
...
std::vector<PhaseInfo> DataFlowNode::get_receiving_phases_for_all_input_groups() const
{
    ...
    {
        all_receiving_phases.insert(
            all_receiving_phases.end(), input_group_receiving_phases.begin(), input_group_receiving_phases.end());
    }
