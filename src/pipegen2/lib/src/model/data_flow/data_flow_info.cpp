...
const std::vector<PhaseInfo>& DataFlowInfo::get_edge_phases(const RGBaseNode* source, const RGBasePipe* dest) const
{
    ...
    log_assert(
        m_node_2_pipe_edges.is_valid_edge_phases_iterator(it),
        "Expecting to find edge phases from node {} to pipe {}",
        source->get_id(),
        dest->get_id());
...
const std::vector<PhaseInfo>& DataFlowInfo::get_edge_phases(const RGBasePipe* source, const RGBaseNode* dest) const
{
    ...
    log_assert(
        m_pipe_2_node_edges.is_valid_edge_phases_iterator(it),
        "Expecting to find edge phases from pipe {} to node {}",
        source->get_id(),
        dest->get_id());
...
unsigned int DataFlowInfo::get_max_num_tiles_per_phase(const RGBaseNode* node) const
{
    ...
    log_assert(
        it != m_node_2_max_num_tiles_per_phase.end(), "Expecting to find data flow info for node {}", node->get_id());
...
unsigned int DataFlowInfo::get_tiles_to_send(const RGBaseNode* node) const
{
    ...
    log_assert(it != m_node_2_msgs_to_send.end(), "Expecting to find data flow info for node {}", node->get_id());
...
unsigned int DataFlowInfo::get_num_iterations_in_epoch(const RGBasePipe* pipe) const
{
    ...
    log_assert(
        it != m_pipe_2_num_iterations_in_epoch.end(), "Expecting to find data flow info for pipe {}", pipe->get_id());
...
const unsigned int DataFlowInfo::get_subtree_divisor(const RGBasePipe* pipe) const
{
    ...
    log_assert(it != m_pipe_2_subtree_divisor.end(), "Expecting to find data flow info for pipe {}", pipe->get_id());
