...
L1DataBuffersMemoryLayout::L1DataBuffersMemoryLayout(const tt_cxy_pair &core_physical_location,
...
const L1Buffer *L1DataBuffersMemoryLayout::allocate_l1_tile_header_buffer(const unsigned int tile_size) {
    ...
    std::unique_ptr<TileHeaderL1Buffer> thb = L1DataBuffersFactory::create_tile_header_buffer(
        m_core_physical_location, thb_address, tile_size);
...
const L1Buffer *L1DataBuffersMemoryLayout::allocate_l1_stream_buffer(const StreamNode *stream_node,
    ...
    return store_buffer(
        L1DataBuffersFactory::create_stream_buffer(stream_node, buffer_address, buffer_size));
}
...
const L1Buffer *L1DataBuffersMemoryLayout::allocate_l1_ncrisc_fallback_buffer(const unsigned int buffer_size) {
    ...
    const unsigned int buffer_address = allocate_space_for_l1_data_buffer(buffer_size);
    return store_buffer(L1DataBuffersFactory::create_ncrisc_fallback_buffer(
        m_core_physical_location, buffer_address, buffer_size));
}

const L1Buffer *L1DataBuffersMemoryLayout::allocate_l1_extra_overlay_blob_space(const unsigned int total_blob_size,
...
void L1DataBuffersMemoryLayout::check_if_out_of_l1_data_buffers_memory() {
    ...
    if (m_l1_current_data_buffers_space_address <
        ...
        throw OutOfCoreResourcesException(
            ...
            m_core_physical_location, m_core_logical_location,
            OutOfCoreResourcesException::CoreResourceType::kL1DataBuffersMemory, total_available_space,
...
unsigned int L1DataBuffersMemoryLayout::get_tile_header_buffer_address(const unsigned int tile_size) {
    ...
    log_assert(it != m_allocated_tile_header_buffers.end(), "Core {}: Tile header buffer for tile size {} is missing",
               m_core_physical_location.str(), tile_size);
...
unsigned int L1DataBuffersMemoryLayout::calculate_extra_blob_space_to_allocate(unsigned int blob_size_in_bytes,
                                                                               const bool is_ethernet_core) {
