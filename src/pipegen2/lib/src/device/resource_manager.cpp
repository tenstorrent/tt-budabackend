        ...
        for (const tt_cxy_pair& core_physical_location : m_soc_info->get_worker_cores_physical_locations(chip_id))
        {
            ...
            m_worker_cores_resources.emplace(
                ...
                resource_manager_internal::create_worker_core_resources(
                    m_soc_info->get_device_arch(), core_physical_location, core_logical_location));
        }
...
WorkerCoreResources* ResourceManager::get_worker_core_resources(const tt_cxy_pair& core_physical_location) const
{
    ...
    log_assert(
        ...
        "ResourceManager: There is no worker core at a location: {}",
        core_physical_location.str());
...
EthernetCoreResources* ResourceManager::get_ethernet_core_resources(const tt_cxy_pair& core_physical_location) const
{
    ...
    log_assert(
        ...
        "ResourceManager: There is no ethernet core at a location: {}",
        core_physical_location.str());
...
CoreResources* ResourceManager::get_core_resources(const tt_cxy_pair& core_physical_location) const
{
    ...
    if (worker_core_resources_it == m_worker_cores_resources.end())
    {
        ...
        log_assert(
            eth_core_resources_it != m_ethernet_cores_resources.end(),
            "ResourceManager: There is no routing core at a location: {}",
            core_physical_location.str());
...
StreamId ResourceManager::allocate_unpacker_stream(
    const tt_cxy_pair& worker_core_physical_location, int operand_id) const
{
    return get_worker_core_resources(worker_core_physical_location)->allocate_unpacker_stream(operand_id);
}

StreamId ResourceManager::allocate_intermed_stream(
    const tt_cxy_pair& worker_core_physical_location, int operand_id) const
{
...
const L1Buffer* ResourceManager::allocate_l1_stream_buffer(
    const StreamNode* stream_node, const unsigned int buffer_size) const
{
    return get_core_resources(stream_node->get_physical_location())
        ->allocate_l1_stream_buffer(stream_node, buffer_size);
}

const L1Buffer* ResourceManager::allocate_l1_ncrisc_fallback_buffer(
...
const L1Buffer* ResourceManager::allocate_l1_extra_overlay_blob_space(
...
unsigned int ResourceManager::get_tile_header_buffer_address(
    const tt_cxy_pair& core_physical_location, const unsigned int tile_size) const
{
