                              ...
                              unsigned int max_num_phases_per_iteration)
{
    ...
    {
        calculate_num_iterations_for_multi_source_path(leaf_nodes, root_nodes);
    }
}
...
void calculate_num_iterations_for_multi_source_path(
                                   ...
                                   unsigned int max_num_tiles_per_phase)
{
    log_assert(root_node->is_root_node(),
               "Expecting root node argument to start with max_tiles_per_phase calculation");

    unsigned int num_tiles_per_input = root_node->get_num_tiles_per_input();

    auto set_max_num_tiles_per_phase = [&num_tiles_per_input, max_num_tiles_per_phase](DataFlowNode* df_node)
    {
        ...
        if (df_node->get_tiles_to_send() > max_num_tiles_per_phase)
        {
            common_divisor = std::lcm(common_divisor, df_node->get_subtree_common_divisor());
        }

        if (num_tiles_per_input <= max_num_tiles_per_phase)
        {
            unsigned int lcm_with_tiles_per_input = std::lcm(common_divisor, num_tiles_per_input);
            if (lcm_with_tiles_per_input > max_num_tiles_per_phase)
            {
        ...
        df_node->set_max_num_tiles_per_phase((max_num_tiles_per_phase / common_divisor) * common_divisor);
    };
