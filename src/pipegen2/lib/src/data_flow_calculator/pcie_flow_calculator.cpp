...
namespace pipegen2
{
unsigned int PCIeFlowCalculator::calculate_source_size_tiles(
    ...
    const unsigned int tile_size)
{
    ...
    while (src_size_tiles % 2 == 0 && src_size_tiles * tile_size > constants::dram_output_stream_max_write_issue_bytes)
    {
        src_size_tiles /= 2;
    }
...
unsigned int PCIeFlowCalculator::calculate_destination_size_tiles(
    const std::vector<PGBuffer*>& mmio_pipe_outputs, const unsigned int tile_size, const unsigned int src_size_tiles)
{
    // TODO: This is magic from pipegen1, we should eventually understand it and document it here.
