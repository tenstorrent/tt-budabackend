...
DataFlowCalculator::DataFlowCalculator(const RationalGraph* rational_graph, unsigned int max_num_tiles_per_phase) :
    m_rational_graph(rational_graph), m_max_num_tiles_per_phase(max_num_tiles_per_phase)
{
}
...
std::unique_ptr<SubgraphLeafGroups> DataFlowCalculator::find_leaf_groups(const std::vector<DataFlowNode*>& root_nodes)
{
...
void DataFlowCalculator::calculate_transfer_limits(
...
{
    // All data flow calculations conducted on separate DataFlowGraph will be summed up in `data_flow_info`.
    ...
    {
        if (df_graph->is_single_node_graph())
        {
            continue;
        }

        data_flow_info += get_data_flow_info(df_graph.get());
    }
...
DataFlowInfo DataFlowCalculator::create_data_flow_info(const DataFlowGraph* data_flow_graph)
{
    ...
    for (const std::unique_ptr<DataFlowNode>& df_node : data_flow_graph->get_nodes())
    {
        ...
        {
            result.set_edge_phases(
                df_node->get_rg_pipe(), df_node->get_rg_node(), df_node->get_receiving_phases_for_all_input_groups());
