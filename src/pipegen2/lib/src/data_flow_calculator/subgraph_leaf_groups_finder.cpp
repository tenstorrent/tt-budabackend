...
std::unique_ptr<SubgraphLeafGroups> SubgraphLeafGroupsFinder::find_leaf_groups(
    ...
    for (DataFlowNode* df_node : root_nodes) {
        data_flow_internal::find_leaf_subgraphs(df_node, next_available_subgraph_id,
                                                max_root_to_leaf_paths_per_subgraph);
    }
    ...
    for (DataFlowNode* df_node : root_nodes) {
        data_flow_internal::find_leaf_groups_per_subgraph(df_node, node_visit_count, 0 /* root_to_leaf_path_index */,
                                                          subgraph_leaf_groups.get());
    }
