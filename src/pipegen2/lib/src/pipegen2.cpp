...
std::unique_ptr<StreamGraphCollection> Pipegen2::create_stream_graphs(
    const std::string& pipegen_yaml_path, int epoch_num)
{
...
std::unique_ptr<ForkJoinGraphCollection> Pipegen2::create_fork_join_graphs()
{
    ...
    return fork_join_graph_creator.create_fork_join_graphs(
        m_pipe_graph.get(), m_stream_graphs.get(), m_resource_manager.get());
}
...
void Pipegen2::output_blob_yaml(
...
void Pipegen2::output_input_buffer_usage_analysis(
    const int epoch_num,
