...
std::vector<std::unique_ptr<RationalGraph>> RationalGraphCreator::create_rational_graphs(
    const PipeGraph& pipe_graph, ResourceManager* resource_manager)
{
...
std::unique_ptr<RationalGraph> RationalGraphCreator::create_rational_graph(
    const PGSubgraph* pg_subgraph, ResourceManager* resource_manager)
{
    std::vector<std::unique_ptr<RGBaseNode>> rational_graph_nodes =
        create_rational_graph_nodes(pg_subgraph->get_buffers(), resource_manager);

    std::vector<std::unique_ptr<RGBasePipe>> rational_graph_pipes;
    for (const PGPipe* subgraph_pipe : pg_subgraph->get_pipes())
    {
        create_rational_graph_pipes(
            subgraph_pipe, rational_graph_pipes, rational_graph_nodes, resource_manager->get_soc_info());
    }

    handle_forked_buffers(
        pg_subgraph->get_buffers(), rational_graph_pipes, rational_graph_nodes, resource_manager->get_soc_info());
    ...
    std::unique_ptr<RationalGraph> rational_graph = std::make_unique<RationalGraph>(
        std::move(rational_graph_nodes), std::move(rational_graph_pipes), is_subgraph_doing_pcie_transfer(pg_subgraph));
...
std::vector<std::unique_ptr<RGBaseNode>> RationalGraphCreator::create_rational_graph_nodes(
    const std::vector<const PGBuffer*>& subgraph_buffers, ResourceManager* resource_manager)
{
    std::vector<std::unique_ptr<RGBaseNode>> rational_graph_nodes;
...
std::unique_ptr<RGBaseNode> RationalGraphCreator::create_rational_graph_node(
    const PGBuffer* subgraph_buffer, ResourceManager* resource_manager)
{
    ...
    else
    {
        throw InvalidPipeGraphSpecificationException(
            "Unsupported input buffer type for buffer " + std::to_string(subgraph_buffer->get_id()) +
                ". This buffer was not recognized as either intermediate, "
                "input buffer, output buffer nor a relay buffer.",
            subgraph_buffer->get_logical_location());
    }
}

std::unique_ptr<BaseInputNode> RationalGraphCreator::create_input_node(
    const PGBuffer* input_buffer, ResourceManager* resource_manager)
{
    ...
    else if (input_buffer->is_output_operand())
    {
        return std::make_unique<PackerInputNode>(
            ...
            RGBaseNode::BlockingParams(
                input_buffer->get_mblock_m(),
    ...
    {
        throw InvalidPipeGraphSpecificationException(
            "Unsupported input buffer type for buffer " + std::to_string(input_buffer->get_id()) +
                ". This input buffer was not recognized as neither dram"
...
std::unique_ptr<BaseOutputNode> RationalGraphCreator::create_output_node(
    const PGBuffer* output_buffer, ResourceManager* resource_manager)
{
    ...
    {
        throw InvalidPipeGraphSpecificationException(
            "Unsupported output buffer type for buffer " + std::to_string(output_buffer->get_id()) +
                ". This input buffer was not recognized as neither dram"
...
std::unique_ptr<BaseIntermedNode> RationalGraphCreator::create_intermed_node(
    const PGBuffer* intermed_buffer, ResourceManager* resource_manager)
{
    ...
    {
        throw InvalidPipeGraphSpecificationException(
            "Unsupported intermed buffer type for buffer " + std::to_string(intermed_buffer->get_id()) +
                ". This buffer was not recognized as either gradient op "
...
std::unique_ptr<RGBaseNode> RationalGraphCreator::create_relay_node(
    const PGBuffer* relay_buffer, const SoCInfo* soc_info)
{
    ...
    const SoCInfo* soc_info)
{
    ...
    if (subgraph_pipe->is_pipe_scattering())
    {
        ...
        create_serial_fork_nodes_for_pipe_inputs(
            subgraph_pipe, created_rg_pipes_from_subgraph_pipe, soc_info, rational_graph_pipes, rational_graph_nodes);
    }
}
    ...
    const SoCInfo* soc_info)
{
    ...
    const tt_cxy_pair& pipe_physical_location = get_physical_location(subgraph_pipe, pipe_logical_location, soc_info);
    ...
    const bool should_decompose_inter_tensix_pipe =
        ...
        !can_directly_connect_l1_source_to_l1_dest(subgraph_pipe, soc_info) && !pipe_is_join_intermed;
    ...
    else if ((pipe_has_single_input || (pipe_reads_from_dram_after_reblocking && !pipe_has_single_output)))
    {
        ...
        pipe_reads_from_dram_after_reblocking || pipe_writes_to_dram || pipe_is_join_intermed || pipe_is_dram_embedding)
    {
        create_join_pipe(
            subgraph_pipe,
            scatter_index,
    ...
    {
        throw InvalidPipeGraphSpecificationException(
            "Unsupported pipe type for pipe " + std::to_string(subgraph_pipe->get_id()) + " and scatter index " +
                std::to_string(scatter_index) + ". This pipe was not recognized to be either" +
    ...
    std::vector<std::unique_ptr<RGBaseNode>>& rational_graph_nodes)
{
    ...
    if (is_tilizer_pipe)
    {
        direct_pipe = std::make_unique<DramTilizerPipe>(
    ...
    {
        direct_pipe = std::make_unique<DramPrefetchPostTMUnicastPipe>(
            std::move(rg_pipe_properties),
            subgraph_pipe->get_dram_pipe_total_readers(),
            subgraph_pipe->get_dram_pipe_reader_index(),
            subgraph_pipe->get_op_input_dram_io_buf_size_tiles(),
    ...
    {
        direct_pipe = std::make_unique<DramUnicastPipe>(
            std::move(rg_pipe_properties),
            subgraph_pipe->get_dram_pipe_total_readers(),
            subgraph_pipe->get_dram_pipe_reader_index(),
            subgraph_pipe->get_op_input_dram_io_buf_size_tiles(),
    ...
    {
        direct_pipe = std::make_unique<UnicastToDramPipe>(std::move(rg_pipe_properties), pipe_physical_location);
    }
    else if (subgraph_pipe->is_direct_intermediate_pipe())
    {
        ...
        {
            throw InvalidPipeGraphSpecificationException(
                "Unsupported intermediate pipe " + std::to_string(subgraph_pipe->get_id()) + " and scatter index " +
                    std::to_string(scatter_index) + ". The pipe is unsuppored because it is" +
    ...
    {
        direct_pipe = create_relay_pipe(
            std::move(rg_pipe_properties), pipe_physical_location, pg_pipe_inputs[0].get_buffer(), pg_pipe_output);
    }
    else
    {
...
std::unique_ptr<RGBasePipe> RationalGraphCreator::create_relay_pipe(
    ...
    const PGBuffer* relay_buffer)
{
    ...
    {
        log_assert(
            input_buffer->get_type() == BufferType::kEthernetRelay &&
                relay_buffer->get_type() == BufferType::kEthernetRelay,
            "Expecting Ethernet relay buffers for cross-chip relay pipe");
        return create_ethernet_relay_pipe(
            std::move(rg_pipe_properties), pipe_physical_location, relay_buffer->get_use_ethernet_fw_stream());
    }
    else
    {
...
std::unique_ptr<RGBasePipe> RationalGraphCreator::create_ethernet_relay_pipe(
    RGPipeProperties&& rg_pipe_properties, const tt_cxy_pair& pipe_physical_location, const bool is_fw_relay_pipe)
{
    if (is_fw_relay_pipe)
    {
    ...
    std::vector<std::unique_ptr<RGBaseNode>>& rational_graph_nodes)
{
    ...
    if (is_embedding_pipe)
    {
        ...
        join_pipe = std::make_unique<DramEmbeddingPipe>(
            std::move(rg_pipe_properties),
            subgraph_pipe->get_dram_pipe_total_readers(),
            subgraph_pipe->get_dram_pipe_reader_index(),
            subgraph_pipe->get_op_input_dram_io_buf_size_tiles(),
    ...
    {
        join_pipe = std::make_unique<DramPrefetchPostTMGatherPipe>(
            std::move(rg_pipe_properties),
            subgraph_pipe->get_dram_pipe_total_readers(),
            subgraph_pipe->get_dram_pipe_reader_index(),
            subgraph_pipe->get_op_input_dram_io_buf_size_tiles(),
    ...
    {
        join_pipe = std::make_unique<DramGatherPipe>(
            std::move(rg_pipe_properties),
            subgraph_pipe->get_dram_pipe_total_readers(),
            subgraph_pipe->get_dram_pipe_reader_index(),
            subgraph_pipe->get_op_input_dram_io_buf_size_tiles(),
    ...
    std::map<std::vector<PGBuffer*>, RGBasePipe*>& pg_pipe_dest_list_to_rg_pipe)
{
    ...
    RGBasePipe* sink_subgraph_input_pipe = get_sending_subgraph(
        subgraph_pipe,
        created_rg_pipes_from_subgraph_pipe,
        pg_pipe_inputs,
        pg_pipe_outputs,
...
void RationalGraphCreator::create_sending_subgraph(
    ...
    std::vector<std::unique_ptr<RGBaseNode>>& rational_graph_nodes)
{
    create_pipe_sending_to_destinations(
        subgraph_pipe,
        created_rg_pipes_from_subgraph_pipe,
        pg_pipe_inputs,
        pg_pipe_outputs,
    ...
    if (subgraph_pipe->has_output_list_duplicates())
    {
        ...
        std::unique_ptr<VirtualNode> union_output_node =
            std::make_unique<VirtualNode>(pipe_writing_to_l1->get_physical_location());
        connect_rg_pipe_with_input_node(pipe_writing_to_l1, union_output_node.get());

        create_union_pipe_for_rg_node(
            union_output_node.get(), created_rg_pipes_from_subgraph_pipe, rational_graph_pipes);
    ...
    std::vector<std::unique_ptr<RGBaseNode>>& rational_graph_nodes)
{
    ...
    if (pg_pipe_outputs.size() > 1)
    {
        ...
        derived_pipe = std::make_unique<MulticastPipe>(
            std::move(derived_pipe_properties),
            pipe_physical_location,
    ...
    else if (pg_pipe_outputs[0]->is_relay())
    {
        ...
        derived_pipe = create_relay_pipe(
            std::move(derived_pipe_properties),
            pipe_physical_location,
    ...
    std::map<std::vector<PGBuffer*>, RGBasePipe*>& pg_pipe_dest_list_to_rg_pipe)
{
    ...
    std::unique_ptr<RGBasePipe> pipe_sending_to_pcie = create_pipe_sending_to_pcie(mmio_pipe, scatter_index, soc_info);
    ...
    connect_pipe_reading_from_pcie_to_streaming_node(
        pipe_reading_from_pcie.get(),
        tile_size,
        epoch_tiles,
        src_size_tiles,
        dest_size_tiles,
        mmio_pipe->is_mmio_pipe_downstream(),
        mmio_pipe_outputs[0]->get_logical_location().chip,
        rational_graph_nodes,
        scatter_index,
        mmio_pipe->get_id());
...
std::unique_ptr<RGBasePipe> RationalGraphCreator::create_pipe_sending_to_pcie(
    const PGPipe* mmio_pipe, const unsigned int scatter_index, const SoCInfo* soc_info)
{
    const std::vector<PGPipe::Input>& mmio_pipe_inputs = mmio_pipe->get_inputs(scatter_index);
    ...
    const unsigned int scatter_index)
{
    ...
    {
        return connect_pipe_sending_to_pcie_to_union_pipe(
            pipe_sending_to_pcie,
            tile_size,
            epoch_tiles,
            src_size_tiles,
            dest_size_tiles,
    ...
    const unsigned int scatter_index)
{
    ...
    create_union_pipe_for_rg_node(pcie_streaming_node.get(), created_rg_pipes_from_subgraph_pipe, rational_graph_pipes);
    rational_graph_nodes.push_back(std::move(pcie_streaming_node));
    ...
    const SoCInfo* soc_info)
{
    ...
    tt_cxy_pair pipe_logical_location = tt_cxy_pair(
        mmio_pipe_outputs[0]->get_logical_location().chip, mmio_pipe_logical_location.x, mmio_pipe_logical_location.y);
    ...
    {
        pipe_reading_from_pcie = std::make_unique<PCIeMulticastPipe>(
            RGPipeProperties(
                pipe_id,
                mmio_pipe->get_incoming_noc_id(),
                mmio_pipe->get_incoming_noc_vc(),
    ...
    {
        pipe_reading_from_pcie = std::make_unique<PCIeUnicastPipe>(
            RGPipeProperties(
                pipe_id,
                mmio_pipe->get_incoming_noc_id(),
                mmio_pipe->get_incoming_noc_vc(),
...
tt_cxy_pair RationalGraphCreator::choose_location_of_pipe_sending_to_pcie(
    const std::vector<PGPipe::Input>& mmio_pipe_inputs, const unsigned int scatter_index, const SoCInfo* soc_info)
{
    ...
    const tt_cxy_pair& first_input_location = get_physical_location(mmio_pipe_inputs[0].get_buffer(), soc_info);
    ...
    {
        const tt_cxy_pair& current_input_location = get_physical_location(mmio_pipe_inputs[i].get_buffer(), soc_info);
...
void RationalGraphCreator::connect_rg_pipe_with_input_nodes(
    RGBasePipe* rg_pipe, const std::vector<PGPipe::Input>& pg_pipe_inputs, const unsigned int scatter_index)
{
...
void RationalGraphCreator::connect_rg_pipe_with_input_node(
    RGBasePipe* rg_pipe, const PGPipe::Input& pg_pipe_input, const unsigned int scatter_index)
{
...
void RationalGraphCreator::connect_rg_pipe_with_output_nodes(
    RGBasePipe* rg_pipe, const std::vector<PGBuffer*>& pg_pipe_outputs)
{
...
void RationalGraphCreator::create_union_pipe_for_rg_node(
    ...
    std::vector<std::unique_ptr<RGBasePipe>>& rational_graph_pipes)
{
    std::unique_ptr<UnionPipe> union_pipe =
        std::make_unique<UnionPipe>(union_pipe_output_node->get_physical_location());
    ...
    std::vector<std::unique_ptr<RGBaseNode>>& rational_graph_nodes)
{
    ...
    reconnect_forked_node(
        rg_node,
        std::move(fork_pipe),
        forked_pipes,
        rational_graph_pipes,
        rational_graph_nodes,
        false /* is_serial_fork */);
...
std::unique_ptr<ForkPipe> RationalGraphCreator::create_dram_parallel_fork_pipe_for_node(
    const PGBuffer* pg_buffer, const tt_cxy_pair& physical_location)
{
...
void RationalGraphCreator::reconnect_forked_node(
    RGBaseNode* rg_node,
    std::unique_ptr<ForkPipe> fork_pipe,
    ...
    const bool is_serial_fork)
{
    ...
    for (RGBasePipe* output_pipe : rg_node->get_output_pipes())
    {
        ...
        std::unique_ptr<VirtualNode> virtual_node = create_fork_node_for_rg_edge(rg_node, output_pipe, is_serial_fork);
...
std::unique_ptr<VirtualNode> RationalGraphCreator::create_fork_node_for_rg_edge(
    const RGBaseNode* input_node, const RGBasePipe* output_pipe, const bool is_serial_fork)
{
...
bool RationalGraphCreator::is_buffer_forking_to_optimized_packer_multicast(
    const PGBuffer* buffer, const SoCInfo* soc_info)
{
    if (!buffer->is_packer())
    {
        return false;
...
void RationalGraphCreator::ensure_optimized_packer_multicast_is_first_fork(RGBasePipe* fork_pipe)
{
    ...
    for (std::size_t i = 1; i < fork_pipe->get_output_nodes().size(); ++i)
    {
        ...
        log_assert(
            !found_optimized_packer_multicast_fork,
...
bool RationalGraphCreator::is_packer_multicast_optimization_enabled(
    const PGPipe* subgraph_pipe, const SoCInfo* soc_info)
{
...
bool RationalGraphCreator::can_directly_connect_l1_source_to_l1_dest(
    const PGPipe* subgraph_pipe, const SoCInfo* soc_info)
{
...
DramInputNodeType RationalGraphCreator::get_input_dram_buffer_type(const PGBuffer* input_dram_buffer)
{
    ...
    {
        throw InvalidPipeGraphSpecificationException(
            " Unsupported dram input buffer type for buffer " + std::to_string(input_dram_buffer->get_id()) +
                ". This dram buffer was not recognized as either dram io "
...
tt_cxy_pair RationalGraphCreator::get_physical_location(
    const PGPipe* pg_pipe, const tt_cxy_pair& pipe_logical_location, const SoCInfo* soc_info)
{
...
void RationalGraphCreator::create_relay_nodes(
    const std::vector<const PGBuffer*>& subgraph_buffers,
    std::vector<std::unique_ptr<RGBaseNode>>& rational_graph_nodes)
{
...
void RationalGraphCreator::manage_gather_optimizations(
...
void RationalGraphCreator::allocate_kernel_input_index(
    ResourceManager* resource_manager, std::vector<std::unique_ptr<RationalGraph>>& rational_graphs)
{
