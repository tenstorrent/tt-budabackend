...
namespace pipegen2
{
        ...
        const std::vector<StreamNode*>& all_streams)
    {
        ...
        for (StreamNode* stream_node: all_streams)
        {
            ...
            {
                ASSERT(phase_config.config.get_dest().has_value(),
                                                                                       ...
                                                                                       const unsigned int buffer_size)
    {
        ...
        ASSERT(base_stream_config.get_dram_buf_noc_addr().has_value(),
        ...
        const unsigned long local_dram_buf_noc_address = soc_info->get_local_dram_buffer_noc_address(
    ...
    bool WorkerCorePaddingTable::has_allocated_buffer(const unsigned int buffer_size,
                                                      const unsigned long dram_buf_noc_addr) const
    {
    ...
    unsigned int WorkerCorePaddingTable::get_allocated_buffer_address(const unsigned int buffer_size,
                                                                      const unsigned long dram_buf_noc_addr) const
    {
        ASSERT(has_allocated_buffer(buffer_size, dram_buf_noc_addr),
