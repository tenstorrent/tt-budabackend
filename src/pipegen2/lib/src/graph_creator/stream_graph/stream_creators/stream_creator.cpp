...
namespace pipegen2
{
                                                    ...
                                                    StreamNode* stream)
    {
        ...
        ASSERT(base_ncrisc_config.dram_buf_read_chunk_size_tiles.has_value(),
    ...
    {
        return calculate_buf_space_available_ack_thr(node->get_size_tiles(),
                                                     node->get_scatter_gather_num_tiles());
    }
    ...
    unsigned int StreamCreator::calculate_buf_space_available_ack_thr(unsigned int size_tiles,
    ...
    bool StreamCreator::can_configure_dram_to_unpacker_stream(const RGBasePipe* rg_pipe,
                                                              const NcriscConfig& base_ncrisc_config,
    ...
    unsigned int StreamCreator::calculate_dram_to_unpacker_buffer_size(const NcriscConfig& base_ncrisc_config,
                                                                       const UnpackerOutputNode* unpacker_node)
    {
    ...
    void StreamCreator::set_common_l1_to_noc_stream_properties(StreamNode* stream,
                                                               ...
                                                               const BaseInputNode* input_node)
    {
        ...
        {
            base_stream_config.set_buf_full_size_bytes(input_node->get_tile_size() *
                                                       input_node->get_size_tiles() *
                                                       input_node->get_num_scatter_chunks());
            base_stream_config.set_is_sending_tiles_out_of_order(true);
            base_stream_config.set_num_mblock_buffering(input_node->get_size_tiles() /
                                                        input_node->get_scatter_gather_num_tiles());
            base_stream_config.set_num_msgs_in_block(input_node->get_num_scatter_chunks() *
                                                     input_node->get_scatter_gather_num_tiles());
    ...
    void StreamCreator::configure_packer_to_noc_stream(StreamNode* stream,
                                                       const RGBasePipe* rg_pipe,
    ...
    void StreamCreator::update_stream_for_dram_write(StreamNode* stream,
    ...
    void StreamCreator::configure_packer_stream_with_untilization_params(const RGBasePipe* rg_pipe,
                                                                         const unsigned int packer_stride,
                                                                         StreamNode* stream)
    {
        StreamConfig& base_stream_config = stream->get_base_config();

        ASSERT(rg_pipe->get_output_nodes().size() == 1, "Expecting only a single dram output node");
        const DramOutputNode* dram_output_node = dynamic_cast<const DramOutputNode*>(
            *rg_pipe->get_output_nodes().begin());
        ASSERT(dram_output_node, "Unexpected downcast failure from RGBaseNode to DramOutputNode");
        ASSERT(dram_output_node->is_untilized_output(), "Expecting untilized output node");
        ...
        base_stream_config.set_skip_col_tile_row_bytes(
            dram_output_node->get_bytes_count_in_untilized_row_of_tiles());
        base_stream_config.set_skip_col_zrow_bytes(dram_output_node->get_bytes_count_in_untilized_row_of_cores());
        base_stream_config.set_skip_zcol_bytes(
            dram_output_node->get_bytes_count_in_untilized_row_for_single_producer());
        base_stream_config.set_stride(packer_stride);
        base_stream_config.set_stride_offset_size_bytes(
            dram_output_node->get_stride_offset_bytes(packer_stride));
    ...
    void StreamCreator::configure_relay_stream_with_default_buffer_size(StreamNode* relay_stream,
    ...
    void StreamCreator::configure_relay_stream(StreamNode* relay_stream,
    ...
    void StreamCreator::configure_gather_relay_stream(StreamNode* relay_stream,
                                                      const RGBasePipe* pipe)
    {
        ...
        configure_relay_stream_with_default_buffer_size(relay_stream, pipe->get_unique_input_nodes().size(),
                                                        buffer_tile_size);
    ...
    void StreamCreator::configure_gather_to_pcie_relay_stream(StreamNode* relay_stream,
                                                              const RGBasePipe* pipe)
    {
        const PackerInputNode* packer_input_node =
            dynamic_cast<const PackerInputNode*>(pipe->get_inputs()[0].get_non_virtual_node());
        ASSERT(packer_input_node, "Expecting packer input node at the input of gather to PCIe pipe");

        const RGBaseNode* output_node = pipe->get_output_node();

        configure_relay_stream(
            relay_stream,
            output_node->get_size_tiles() /* buffer_size_tiles */,
            output_node->get_tile_size());
    ...
    void StreamCreator::configure_multicast_stream(const RGBasePipe* rg_pipe,
    ...
    void StreamCreator::configure_dram_multicast_stream(const RGBasePipe* rg_pipe,
                                                        const bool is_dram_prefetch,
                                                        const NcriscConfig& base_ncrisc_config,
                                                        StreamNode* multicast_stream)
    {
    ...
    void StreamCreator::configure_address_offsets(StreamNode* stream, const RGBasePipe* pipe,
                                                  const BaseInputNode* input_node)
    {
        ...
        ASSERT(!node_pipe_input_offset_indexes.empty(), "Expecting pipe node input to have at least one input offset");
        ...
        for (std::size_t i = 1; i < node_pipe_input_offset_indexes.size(); ++i)
        {
            ...
            const bool is_offset_adjacent = (
                current_offset == prev_offset + adjacent_offsets_distance);
        ...
        ASSERT(stream->get_phase_configs().size() % non_adjacent_offsets.size() == 0,
    ...
    std::vector<PhaseInfo> StreamCreator::decompose_phases(const std::vector<PhaseInfo>& phase_info_vec,
                                                           unsigned int decomposed_phase_max_size)
    {
        ...
        for (const PhaseInfo& phase_info : phase_info_vec)
        {
            ...
            ASSERT(phase_info.num_msgs % decomposed_phase_size == 0,
    ...
    std::vector<PhaseInfo> StreamCreator::merge_phases(const std::vector<PhaseInfo>& phase_info_vec,
    ...
    void StreamCreator::configure_stream_src_and_dest(StreamNode* stream,
                                                      StreamNode* source_stream,
    ...
    unsigned int StreamCreator::calculate_default_relay_stream_buffer_size_tiles(
        ...
        const bool is_ethernet_core)
    {
        const unsigned int combined_min_tile_count = is_ethernet_core ?
            c_eth_sum_gather_input_min_size_tiles : c_sum_gather_input_min_size_tiles;

        unsigned int per_input_tile_count = is_ethernet_core ?
            c_eth_gather_streamed_read_input_buffer_num_tiles : c_worker_gather_streamed_read_input_buffer_num_tiles;
    ...
    bool StreamCreator::is_packer_stream_sending_to_pipe_output_noc(const RGBasePipe* rg_pipe,
                                                                    const PackerInputNode* packer_node)
    {
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        ASSERT(input_stream_node, "Input stream node for the pipe not found");
            ...
            first_phase_iter->phase_id == first_phase_id)
        {
            ...
            {
                const std::vector<PhaseInfo>& receiver_phases = data_flow_info.get_edge_phases(
                    rg_pipe, first_output_node);
                auto receiving_phase_iter = receiver_phases.begin();
                for (PhaseConfig& phase_config : input_stream_node->get_phase_configs())
                {
                    receiving_phase_iter = connect_input_stream_phase_to_receiving_stream(input_stream_node,
        ...
        const std::vector<PhaseInfo>& receiver_phases = data_flow_info.get_edge_phases(
            rg_pipe, first_output_node);
        ASSERT(!sender_phases.empty(), "Expecting at least one phase for the input stream");
        ASSERT(!receiver_phases.empty(), "Expecting at least one phase for the receiving streams");
        ...
        input_stream_node->add_phase_configs(std::move(sender_phase_configs),
                                             data_flow_info.get_max_num_tiles_per_phase(first_output_node));
        set_common_stream_properties_from_pipe(input_stream_node, rg_pipe);
        configure_scatter_loops(rg_pipe, input_stream_node);
        ...
        ASSERT(first_phase_iter != input_stream_node->get_phase_configs().end(),
        ...
        const std::vector<StreamNode*>& receiving_streams)
    {
        auto receiving_phase_iter = std::upper_bound(
            receiver_phase_start_iter, receiver_phases.end(), input_stream_phase_offset,
            [](const unsigned int phase_offset, const PhaseInfo& phase_info) {
                return phase_info.phase_offset > phase_offset;
            });
        ASSERT(receiving_phase_iter != receiver_phases.begin(), "Expecting at least one receiving phase to exist");
        --receiving_phase_iter;

        uint32_t receiving_stream_idx = std::distance(receiver_phases.begin(), receiving_phase_iter) %
                                        receiving_streams.size();
        ASSERT(receiving_stream_idx < receiving_streams.size(), "Expecting receiving stream index to be valid");

        uint32_t receiving_stream_phase_idx = std::distance(receiver_phases.begin(), receiving_phase_iter) /
                                              receiving_streams.size();
        ASSERT(receiving_stream_phase_idx < receiving_streams[receiving_stream_idx]->get_num_phases(),
    ...
    void StreamCreator::update_stream_iterations_in_epoch(StreamNode* stream,
                                                          const RGBasePipe* pipe,
                                                          const DataFlowInfo& data_flow_info)
    {
    ...
    void StreamCreator::update_resend_of_sending_stream(StreamNode* stream,
    ...
    std::vector<PhaseConfig>::iterator StreamCreator::find_phase_by_phase_id(StreamNode* stream, const PhaseId phase_id)
    {
        auto phase_iter = std::lower_bound(
            ...
            phase_id,
            [](const PhaseConfig& phase_config, const PhaseId phase_id)
            {
    ...
    void StreamCreator::set_common_stream_properties_from_pipe(StreamNode* stream,
                                                               const RGBasePipe* rg_pipe,
                                                               const bool incoming_stream)
    {
    ...
    void StreamCreator::convert_packer_to_intermed_stream(StreamNode* stream_node,
                                                          const SharedPackerIntermedNode* intermed_node)
    {
    ...
    void StreamCreator::configure_intermed_stream_properties_from_node(StreamNode* stream,
                                                                       const BaseIntermedNode* intermed_node)
    {
        StreamConfig& base_stream_config = stream->get_base_config();
    ...
    void StreamCreator::check_and_convert_unpacker_to_embedding_index_stream(StreamNode* stream_node,
                                                                             const UnpackerOutputNode* unpacker_node)
    {
