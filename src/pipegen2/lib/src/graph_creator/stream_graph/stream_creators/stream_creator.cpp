...
namespace pipegen2
{
                                                  ...
                                                  const BaseInputNode* input_node)
    {
        ...
        log_assert(
            !node_pipe_input_offset_indexes.empty(), "Expecting pipe node input to have at least one input offset");
        ...
        log_assert(stream->get_phase_configs().size() % non_adjacent_offsets.size() == 0,
                                                           ...
                                                           unsigned int decomposed_phase_max_size)
    {
        ...
        for (const PhaseInfo& phase_info : phase_info_vec)
        {
            ...
            log_assert(phase_info.num_msgs % decomposed_phase_size == 0,
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        log_assert(first_phase_iter != input_stream_node->get_phase_configs().end(),
        ...
        const std::vector<StreamNode*>& receiving_streams)
    {
        ...
        uint32_t receiving_stream_phase_idx = std::distance(receiver_phases.begin(), receiving_phase_iter) /
                                              receiving_streams.size();
        log_assert(receiving_stream_phase_idx < receiving_streams[receiving_stream_idx]->get_num_phases(),
