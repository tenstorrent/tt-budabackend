...
StreamIDsAllocator::StreamIDsAllocator(const ResourceManager* resource_manager) : m_resource_manager(resource_manager)
{
}

void StreamIDsAllocator::allocate_stream_ids(const StreamGraphCollection* stream_graph_collection) const
{
    for (StreamNode* stream_node : stream_graph_collection->get_streams())
    {
        ...
        {
            stream_node->assign_stream_id(m_resource_manager->allocate_packer_multicast_stream(
                stream_node->get_physical_location(), stream_node->get_operand_id()));
        }
        ...
        else if (stream_node->get_stream_type() == StreamType::Unpacker)
        {
            ...
            {
                stream_node->assign_stream_id(m_resource_manager->allocate_unpacker_stream(
                    stream_node->get_physical_location(), stream_node->get_operand_id()));
            }
        }
        ...
        {
            stream_node->assign_stream_id(m_resource_manager->allocate_intermed_stream(
                stream_node->get_physical_location(), stream_node->get_operand_id()));
        }
...
void StreamIDsAllocator::allocate_packer_stream_id(StreamNode* stream_node) const
{
    log_assert(
        stream_node->get_base_config().get_fork_streams().has_value(), "Packer stream must have fork streams set");
    ...
    for (std::size_t i = 0; i < fork_streams.size(); ++i)
    {
        ...
        if (fork_stream->has_stream_id())
        {
            ...
            log_assert(
                fork_stream->get_stream_type() == StreamType::PackerMulticast,
        ...
        if (is_first_packer_fork_stream)
        {
            ...
            {
                fork_stream->assign_stream_id(m_resource_manager->allocate_packer_multicast_stream(
                    fork_stream->get_physical_location(), fork_stream->get_operand_id()));
            }
            else
            {
                fork_stream->assign_stream_id(m_resource_manager->allocate_packer_stream(
                    fork_stream->get_physical_location(), fork_stream->get_operand_id()));
            }
        }
        else if (fork_stream->get_stream_type() == StreamType::Intermed)
        {
            fork_stream->assign_stream_id(m_resource_manager->allocate_intermed_stream(
                fork_stream->get_physical_location(), fork_stream->get_operand_id()));
        }
        else
        {
...
void StreamIDsAllocator::allocate_relay_stream_id(StreamNode* stream_node) const
{
    ...
    const bool eth_stream =
        base_config.get_eth_receiver().value_or(false) || base_config.get_eth_sender().value_or(false);
