...
StreamIDsAllocator::StreamIDsAllocator(const ResourceManager* resource_manager)
...
void StreamIDsAllocator::allocate_packer_stream_id(StreamNode* stream_node) const {
    log_assert(stream_node->get_base_config().get_fork_streams().has_value(),
    ...
    for (std::size_t i = 0; i < fork_streams.size(); ++i) {
        ...
        if (fork_stream->has_stream_id()) {
            ...
            log_assert(fork_stream->get_stream_type() == StreamType::PackerMulticast,
