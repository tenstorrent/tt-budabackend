...
StreamBuffersAllocator::StreamBuffersAllocator(const ResourceManager* resource_manager) :
...
unsigned int StreamBuffersAllocator::get_allocated_stream_buffers_address(StreamNode* stream_node)
{
    ...
    const unsigned int buffer_address =
        base_config.get_is_reading_from_padding_table().value_or(false)
            ? get_prologue_buffer_address_for_padding_table(stream_node, buffer_size)
            : m_resource_manager->allocate_l1_stream_buffer(stream_node, buffer_size)->get_address();
    ...
    const std::vector<StreamNode*>& all_streams)
{
    ...
    return std::vector<StreamNode*>(
        std::make_move_iterator(sorted_multicast_destinations_set.begin()),
        std::make_move_iterator(sorted_multicast_destinations_set.end()));
}

unsigned int StreamBuffersAllocator::get_prologue_buffer_address_for_padding_table(
    StreamNode* stream_node, const unsigned int buffer_size)
{
    const StreamConfig& base_stream_config = stream_node->get_base_config();
    log_assert(
        base_stream_config.get_dram_buf_noc_addr().has_value(),
    ...
    const std::uint64_t local_dram_buf_noc_address =
        soc_info->get_local_dram_buffer_noc_address(base_stream_config.get_dram_buf_noc_addr().value());
...
WorkerCorePaddingTable* StreamBuffersAllocator::get_worker_core_padding_table(const tt_cxy_pair& core_physical_location)
{
    ...
    {
        worker_core_padding_table_it =
            m_worker_core_padding_table.emplace(core_physical_location, std::make_unique<WorkerCorePaddingTable>())
                .first;
    }
...
bool WorkerCorePaddingTable::has_allocated_buffer(
...
void WorkerCorePaddingTable::set_allocated_buffer_address(
...
unsigned int WorkerCorePaddingTable::get_allocated_buffer_address(
    const unsigned int buffer_size, const std::uint64_t dram_buf_noc_addr) const
{
    log_assert(
        has_allocated_buffer(buffer_size, dram_buf_noc_addr),
