...
std::vector<std::uint64_t> DramScatterOffsetCompressor::compress_dram_scatter_offsets()
{
    std::size_t pattern_start_index = 0;

    while (pattern_start_index < m_dram_scatter_offsets.size())
    {
        const ScatterPatternCompressionResult pattern_compression = get_max_pattern_compression(pattern_start_index);
    ...
    const std::size_t pattern_start_index)
{
    ...
    {
        const ScatterPatternCompressionResult current_pattern_compression =
            get_scatter_pattern_compression(pattern_start_index, pattern_length);
...
ScatterPatternCompressionResult DramScatterOffsetCompressor::get_scatter_pattern_compression(
    const std::size_t pattern_start_index, const std::size_t pattern_length)
{
    ...
    const std::uint64_t max_num_loops =
        (m_dram_scatter_offsets.size() - pattern_start_index - pattern_length) / pattern_length;

    std::uint64_t num_loops = 0;
    std::optional<std::uint64_t> increment;
    bool pattern_has_matching_increment = true;

    for (std::uint64_t loop_index = 0; loop_index < max_num_loops && pattern_has_matching_increment; ++loop_index)
    {
        // For each element of the pattern, check if we can match it in the current loop iteration
        for (std::uint64_t pattern_el_index = 0; pattern_el_index < pattern_length; ++pattern_el_index)
        {
            const std::size_t prev_offset_index = pattern_start_index + pattern_el_index + loop_index * pattern_length;
            const std::size_t next_offset_index = prev_offset_index + pattern_length;

            const std::uint64_t prev_offset = m_dram_scatter_offsets[prev_offset_index];
            const std::uint64_t next_offset = m_dram_scatter_offsets[next_offset_index];

            const std::uint64_t current_el_increment = next_offset - prev_offset;
...
bool DramScatterOffsetCompressor::is_valid_scatter_offset_increment(const std::uint64_t increment)
{
    return increment <= c_bad_increment_threshold;
}

std::uint64_t DramScatterOffsetCompressor::encode_dram_scatter_offset_loop(
...
void DramScatterOffsetCompressor::insert_scatter_pattern(
    const std::size_t pattern_start_index, const std::size_t pattern_length)
{
...
std::size_t DramScatterOffsetCompressor::get_compression_scatter_pattern_length(
    const ScatterPatternCompressionResult& pattern_compression, const std::size_t pattern_start_index)
{
    if (pattern_compression.can_compress(c_minimum_compression_level))
    {
...
std::uint64_t DramScatterOffsetCompressor::get_compression_pattern_index_increment(
    const ScatterPatternCompressionResult& pattern_compression, const std::size_t pattern_start_index)
{
    if (pattern_compression.can_compress(c_minimum_compression_level))
    {
...
std::vector<std::uint64_t> DramScatterOffsetCompressor::compress_dram_scatter_offsets_for_tilizer(
    const unsigned int c_dim_size, const unsigned int tilize_mblock_n_loop_num_rows)
{
    std::size_t pattern_start_index = 0;
    ...
    while (pattern_start_index < m_dram_scatter_offsets.size())
    {
        ...
        if (pattern_end_index - pattern_start_index + 1 >= c_min_scatter_pattern_length_for_tilizer)
        {
            ...
            std::uint64_t special_offset = c_dram_io_is_scatter_loop_for_tilizer_flag | pattern_extra_count;
...
std::size_t DramScatterOffsetCompressor::find_arithmetic_progression_pattern_end_index(
    std::size_t pattern_start_index, const unsigned int c_dim_size, const unsigned int tilize_mblock_n_loop_num_rows)
{
    ...
    const std::uint64_t pattern_diff =
    ...
    while (pattern_end_index + 1 <= max_pattern_end_index &&
           m_dram_scatter_offsets[pattern_end_index + 1] - m_dram_scatter_offsets[pattern_end_index] == pattern_diff)
    {
        ++pattern_end_index;
    }
