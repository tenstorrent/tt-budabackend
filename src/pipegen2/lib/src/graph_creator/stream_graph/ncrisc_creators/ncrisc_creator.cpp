...
{
    return configure_dram_io_or_prefetch_post_tm_ncrisc_reader(
        data_flow_info,
        rg_pipe,
        input_total_readers,
        input_reader_index,
    ...
    const bool is_prefetch_post_tm)
{
    ...
    const unsigned int dram_scatter_chunk_size_tiles = calculate_dram_pipe_scatter_chunk_size(data_flow_info, rg_pipe);
    ...
    fill_dram_scatter_offsets(rg_pipe, input_dram_node_to_ncrisc_config, prepare_offsets_for_tilizer, ncrisc_configs);

    fill_dram_readers_properties(rg_pipe, input_total_readers, input_reader_index, input_dram_node_to_ncrisc_config);
...
void NcriscCreator::fill_tilizer_dram_scatter_offsets(
    const RGBasePipe* rg_pipe, std::vector<std::uint64_t>& merged_dram_scatter_offsets)
{
    ...
    log_assert(dram_embedding_table, "Expected DramEmbeddingTableInputNode as input node for tilizer pipe");
    ...
    std::vector<std::uint64_t>& merged_dram_scatter_offsets)
{
    for (const PipeInput& pipe_input : rg_pipe->get_inputs())
    {
        const DramInputNode* input_dram_node = dynamic_cast<const DramInputNode*>(pipe_input.get_non_virtual_node());
        log_assert(input_dram_node, "Dram pipe is expected to have dram nodes as inputs.");
        const NcriscConfig* ncrisc_config = input_dram_node_to_ncrisc_config.at(pipe_input.get_node());

        std::uint64_t dram_scatter_offset =
            ncrisc_config->dram_buf_noc_addr + pipe_input.get_offset() * ncrisc_config->msg_size;
...
void NcriscCreator::compress_dram_scatter_offsets(
    const RGBasePipe* rg_pipe, const bool compress_offsets_for_tilizer, NcriscConfig& ncrisc_config)
{
    const DramInputNode* dram_input_node =
        dynamic_cast<const DramInputNode*>(rg_pipe->get_inputs()[0].get_non_virtual_node());
    log_assert(dram_input_node, "Dram pipe is expected to have dram nodes as inputs.");
...
NcriscConfig NcriscCreator::configure_pcie_ncrisc_reader(
    const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe, const SoCInfo* soc_info)
{
    log_assert(rg_pipe->get_inputs().size() == 1, "Expecting only a single PCIe streaming node at the input");
    const PCIeStreamingNode* pcie_streaming_node =
        dynamic_cast<const PCIeStreamingNode*>(rg_pipe->get_inputs().front().get_node());
...
NcriscConfig NcriscCreator::configure_prefetch_pre_tm_ncrisc_reader(
    const RGBasePipe* rg_pipe, const SoCInfo* soc_info, const DramInputNode* dram_input_node)
{
    log_assert(
        dram_input_node && dram_input_node->is_dram_prefetch_pre_tm(),
...
NcriscConfig NcriscCreator::configure_dram_ncrisc_writer(
    const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe, const SoCInfo* soc_info)
{
    const DramOutputNode* dram_output_node = dynamic_cast<const DramOutputNode*>(rg_pipe->get_output_node());
    log_assert(dram_output_node, "Expecting a DRAM output node at the pipe output");
...
NcriscConfig NcriscCreator::configure_pcie_ncrisc_writer(const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe)
{
    const PCIeStreamingNode* pcie_streaming_node = dynamic_cast<const PCIeStreamingNode*>(rg_pipe->get_output_node());
    if (pcie_streaming_node == nullptr)
    {
        const VirtualNode* v_node = dynamic_cast<const VirtualNode*>(rg_pipe->get_output_node());
        log_assert(v_node, "Expecting either a Virtual or PCIe streaming node at the PCIe write pipe output");
        log_assert(
            v_node->has_output_pipes(),
        ...
        log_assert(pcie_streaming_node, "Expecting a PCIe streaming node at the union pipe output");
    }

    const unsigned int msg_size = pcie_streaming_node->get_tile_size();
    // PCIe NOC address depends on the stream's L1 buffer address, so we will set it later
    // after resource allocation is done.
    const std::uint64_t dram_buf_noc_addr = 0;
    const unsigned int dram_buf_size_tiles =
        pcie_streaming_node->get_num_queue_slots() * pcie_streaming_node->get_size_tiles();
    const unsigned int dram_buf_size_bytes = dram_buf_size_tiles * msg_size;
...
unsigned int NcriscCreator::calculate_dram_pipe_scatter_chunk_size(
    const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe)
{
    ...
    const unsigned int max_dram_input_buffer_size_tiles)
{
    ...
    for (const RGBaseNode* input_node : unique_input_dram_nodes)
    {
        ...
        log_assert(dram_input_node, "Expected dram input node as input to the pipe");
...
unsigned int NcriscCreator::calculate_dram_input_read_chunk_size(
    const DataFlowInfo& data_flow_info,
    const DramInputNode* dram_input_node,
    const unsigned int max_dram_input_buffer_size_tiles)
{
    ...
    chunk_size = get_transfer_chunk_size_tiles(
        ...
        min_chunk_size,
        tile_size,
        constants::dram_input_stream_max_pending_read_tiles,
...
unsigned int NcriscCreator::calculate_max_chunk_size_for_pipe(
    const RGBasePipe* rg_pipe, const DataFlowInfo& data_flow_info, const unsigned int max_num_tiles_per_phase)
{
    ...
    return tiles_to_transfer > max_num_tiles_per_phase ? std::gcd(max_num_tiles_per_phase, tiles_to_transfer)
                                                       : tiles_to_transfer;
}

unsigned int NcriscCreator::get_dram_read_max_transfer_size_bytes(
    const unsigned int dram_input_node_tile_size, const unsigned int max_dram_input_buffer_size_tiles)
{
    ...
    return std::min(
        constants::dram_input_stream_max_pending_read_bytes,
        max_dram_input_buffer_size_tiles * dram_input_node_tile_size);
}

void NcriscCreator::get_clear_granularity_of_downstream_buffers(
    const RGBasePipe* rg_pipe, const RGBaseNode* rg_node, unsigned int* clear_granularity)
{
    if (rg_pipe)
    {
...
NcriscConfig NcriscCreator::create_dram_ncrisc_read_config(
    ...
    const unsigned int dram_buf_size_factor)
{
    ...
    const std::uint64_t dram_buf_noc_addr =
        get_dram_buffer_noc_address(dram_input_node, dram_input_node->get_physical_location().chip, soc_info);
    const unsigned int dram_buf_size_tiles =
        dram_input_node->get_size_tiles() * dram_input_node->get_num_queue_slots() * dram_buf_size_factor;
    const unsigned int dram_buf_size_bytes = dram_buf_size_tiles * msg_size;
...
NcriscConfig NcriscCreator::create_ncrisc_read_config(
    const unsigned int num_msgs,
...
NcriscConfig NcriscCreator::create_ncrisc_write_config(
    ...
    const bool is_ram)
{
    ...
    return create_ncrisc_write_config(
        num_msgs,
        msg_size,
        dram_buf_noc_addr,
        dram_buf_size_tiles,
        dram_buf_size_bytes,
        dram_buf_size_q_slots,
        dram_buf_write_chunk_size_tiles,
        is_ram);
}
...
unsigned int NcriscCreator::calculate_dram_buf_write_chunk_size_tiles(
    const DataFlowInfo& data_flow_info, const RGBasePipe* rg_pipe, const DramOutputNode* dram_output_node)
{
    ...
    if (dram_output_node->is_untilized_output())
    {
        log_assert(packer_input_node, "Expected packer input node as input to the pipe when untilizing data");
    ...
    write_chunk_size_tiles = get_transfer_chunk_size_tiles(
        write_chunk_size_tiles,
        min_write_chunk_size_tiles,
        tile_size,
        constants::dram_output_stream_max_write_issue_tiles,
        max_transfer_size_bytes);
    log_assert(write_chunk_size_tiles > 0, "Failed to compute valid write_chunk_size_tiles");

    return write_chunk_size_tiles;
}

unsigned int NcriscCreator::get_transfer_chunk_size_tiles(
    unsigned int transfer_chunk_size_tiles,
...
bool NcriscCreator::is_transfer_chunk_size_within_limits(
    const unsigned int transfer_chunk_size_tiles,
    const unsigned int tile_size_bytes,
    const unsigned int max_transfer_size_tiles,
    const unsigned int max_transfer_size_bytes)
{
...
std::uint64_t NcriscCreator::get_dram_buffer_noc_address(
    const DramNodeInterface* dram_node, ChipId chip_id, const SoCInfo* soc_info)
{
    ...
    return dram_node->get_is_remote_io()
               ...
               : soc_info->get_dram_buffer_noc_address(
                     dram_node->get_dram_address(),
                     chip_id,
                     dram_node->get_dram_channel(),
                     dram_node->get_dram_subchannel());
}

const unsigned int NcriscCreator::get_dram_buf_size_factor(
    const DramInputNode* first_dram_input_node, bool is_prefetch_post_tm) const
{
