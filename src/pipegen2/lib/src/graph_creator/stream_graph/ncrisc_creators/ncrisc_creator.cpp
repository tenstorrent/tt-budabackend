...
namespace pipegen2
{
        ...
        std::vector<std::uint64_t>& merged_dram_scatter_offsets)
    {
        for (const PipeInput& pipe_input: rg_pipe->get_inputs())
        {
            ...
            std::uint64_t dram_scatter_offset = ncrisc_config->dram_buf_noc_addr +
    ...
    NcriscConfig NcriscCreator::configure_prefetch_pre_tm_ncrisc_reader(
        ...
        const DramInputNode* dram_input_node)
    {
        log_assert(dram_input_node && dram_input_node->is_dram_prefetch_pre_tm(),
                                                             ...
                                                             const RGBasePipe* rg_pipe)
    {
        ...
        if (pcie_streaming_node == nullptr)
        {
            ...
            log_assert(v_node->has_output_pipes(),
    ...
    std::uint64_t NcriscCreator::get_dram_buffer_noc_address(const DramNodeInterface* dram_node, ChipId chip_id,
                                                             const SoCInfo* soc_info)
    {
