    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    m_dram_write_common_streams_creator->setup_relay_stream_for_pcie_write(relay_stream.get(), pipe, data_flow_info);
...
void GatherToPCIeStreamsCreator::connect_inputs_to_relay_stream(
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info, StreamNode* relay_stream)
{
    ...
    for (const RGBaseNode* input_node : input_nodes)
    {
        ...
        log_assert(virtual_node != nullptr, "Expecting virtual node at the GatherToPCIe pipe input");
