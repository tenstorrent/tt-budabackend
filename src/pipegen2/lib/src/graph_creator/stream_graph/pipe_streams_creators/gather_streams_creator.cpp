    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    std::vector<std::unique_ptr<StreamNode>> relay_streams =
        create_gather_relay_streams(pipe, data_flow_info, num_gather_relay_streams, gather_relay_phases);
    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info, const std::vector<PhaseInfo>& gather_stream_phases)
{
    ...
    m_stream_creator->set_common_stream_properties_from_pipe(gather_stream.get(), pipe, false /* incoming_stream */);

    return gather_stream;
}

void GatherStreamsCreator::connect_relays_to_gather_stream(
    const std::vector<StreamNode*>& relay_streams, StreamNode* gather_stream)
{
...
void GatherStreamsCreator::connect_inputs_to_relay_streams(
