...
namespace pipegen2
{
DramMulticastStreamsCreator::DramMulticastStreamsCreator(
...
DramMulticastStreamsCreator::~DramMulticastStreamsCreator() = default;

std::vector<std::unique_ptr<StreamNode>> DramMulticastStreamsCreator::create_streams_internal(
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    std::vector<std::unique_ptr<StreamNode>> created_streams;

    const DramMulticastPipe* dram_multicast_pipe = dynamic_cast<const DramMulticastPipe*>(pipe);
    log_assert(dram_multicast_pipe, "DramMulticastStreamsCreator invoked for inadequate pipe");

    const DramInputNode* dram_input_node =
        dynamic_cast<const DramInputNode*>(pipe->get_inputs()[0].get_non_virtual_node());
    ...
    std::unique_ptr<StreamNode> multicast_stream = m_dram_read_common_streams_creator->create_dram_multicast_stream(
        pipe,
        data_flow_info,
        dram_multicast_pipe->get_max_dram_input_buffer_size_tiles(),
