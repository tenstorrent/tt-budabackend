    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    std::unique_ptr<StreamNode> multicast_stream = m_dram_read_common_streams_creator->create_pcie_multicast_stream(
    ...
    std::vector<std::unique_ptr<StreamNode>> unpacker_streams =
        m_dram_read_common_streams_creator->create_multicast_unpacker_streams(
            multicast_stream.get(), pipe, pipe->get_inputs().front().get_node(), data_flow_info, max_tiles_per_phase);
