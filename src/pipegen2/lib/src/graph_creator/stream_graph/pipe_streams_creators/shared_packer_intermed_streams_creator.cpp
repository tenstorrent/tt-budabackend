    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    log_assert(intermed_pipe != nullptr, "Expecting SharedPackerIntermedPipe in SharedPackerIntermedStreamsCreator");

    const VirtualNode* virtual_input_node =
        dynamic_cast<const VirtualNode*>(intermed_pipe->get_shared_packer_virtual_node().get_node());
    const SharedPackerIntermedNode* intermed_input_node =
        dynamic_cast<const SharedPackerIntermedNode*>(intermed_pipe->get_inputs()[0].get_node());
    const SharedPackerIntermedNode* intermed_output_node =
        dynamic_cast<const SharedPackerIntermedNode*>(intermed_pipe->get_output_nodes()[0]);
    log_assert(virtual_input_node != nullptr, "Expecting VirtualNode at SharedPackerIntermedPipe input");
    log_assert(intermed_input_node != nullptr, "Expecting SharedPackerIntermedNode at SharedPackerIntermedPipe input");
    log_assert(
