...
namespace pipegen2
{
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        ASSERT(in_virtual_node, "Expecting virtual node at the Relay pipe input");
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        ASSERT(relay_node != nullptr, "Expecting relay node at the Relay pipe output");

        const std::vector<PhaseInfo>& relay_phases = data_flow_info.get_edge_phases(
            relay_pipe, relay_pipe->get_output_nodes()[0]);
