    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    std::unique_ptr<StreamNode> relay_stream =
        create_relay_stream(pipe, data_flow_info, relay_phases, input_nodes.size());
...
void DormantRelayStreamsCreator::connect_inputs_to_relay_stream(
