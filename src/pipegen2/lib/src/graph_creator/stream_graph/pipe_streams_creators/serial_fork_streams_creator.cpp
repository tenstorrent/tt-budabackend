...
namespace pipegen2
{
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        ASSERT(serial_fork_pipe, "Expecting serial fork pipe to be used in SerialForkStreamsCreator");

        StreamNode* stream_node = get_input_stream_node(pipe);
        ASSERT(stream_node, "Input stream node for the SerialFork pipe not found");
        ...
        const std::vector<std::unique_ptr<StreamNode>>& created_streams)
    {
        ...
        for (const RGBaseNode* output_node : pipe->get_output_nodes())
        {
            ...
            ASSERT(out_serial_fork_node != nullptr, "Expecting serial fork node at the SerialFork pipe output");
