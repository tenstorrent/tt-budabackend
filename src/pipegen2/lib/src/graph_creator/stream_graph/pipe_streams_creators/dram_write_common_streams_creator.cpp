    ...
    const SoCInfo* soc_info)
{
    ...
    NcriscConfig ncrisc_config = m_ncrisc_creator->configure_dram_ncrisc_writer(data_flow_info, pipe, soc_info);

    setup_stream_for_dram_or_pcie_write(
        stream_phases_common_config.get_stream_node(),
...
void DramWriteCommonStreamsCreator::setup_relay_stream_for_pcie_write(
    StreamNode* relay_stream, const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    NcriscConfig ncrisc_config = m_ncrisc_creator->configure_pcie_ncrisc_writer(data_flow_info, pipe);

    setup_stream_for_dram_or_pcie_write(
        relay_stream, std::move(ncrisc_config), true /* is_remote_write */, false /* is_untilized_output */);
}

void DramWriteCommonStreamsCreator::setup_stream_for_dram_or_pcie_write(
    StreamNode* stream, NcriscConfig&& ncrisc_config, const bool is_remote_write, const bool is_untilized_output)
{
