...
namespace pipegen2
{
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        ASSERT(intermed_pipe != nullptr, "Expecting DramOutputIntermedPipe in DramOutputIntermedStreamsCreator");

        const DramOutputIntermedNode* intermed_input_node = dynamic_cast<const DramOutputIntermedNode*>(
            pipe->get_inputs()[0].get_node());
        const DramOutputIntermedNode* intermed_output_node = dynamic_cast<const DramOutputIntermedNode*>(
            pipe->get_output_nodes()[0]);
        ASSERT(intermed_input_node != nullptr, "Expecting DramOutputIntermedNode at DramOutputIntermedPipe input");
        ASSERT(intermed_output_node != nullptr, "Expecting DramOutputIntermedNode at DramOutputIntermedPipe input");
    ...
    void DramOutputIntermedStreamsCreator::add_ncrisc_config(StreamNode* stream,
                                                             const DramOutputIntermedNode* intermed_node)
    {
