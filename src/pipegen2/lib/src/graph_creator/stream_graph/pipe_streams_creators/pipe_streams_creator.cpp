...
namespace pipegen2
{
        ...
        const std::size_t output_index)
    {
        ...
        ASSERT(created_streams.size() >= pipe->get_output_nodes().size(), "Not enough created streams");
        std::size_t stream_index = created_streams.size() - pipe->get_output_nodes().size() + output_index;

        ASSERT(stream_index < created_streams.size(), "Index out of range");
    ...
    void PipeStreamsCreator::assign_stream_node_to_virtual_node(const VirtualNode* virt_node, StreamNode* stream_node)
    {
        ...
        ASSERT(emplace_result.second, "Virtual node already has a stream node");
    }

    void PipeStreamsCreator::assign_common_config_to_virtual_node_stream(const VirtualNode* virt_node,
                                                                         StreamConfig&& stream_node_static_config)
    {
        auto it = m_virt_node_to_stream_node->find(virt_node);
        ASSERT(it != m_virt_node_to_stream_node->end(), "Virtual node does not have a stream node assigned");
