    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    std::vector<std::unique_ptr<StreamNode>> unpacker_streams =
        create_unpacker_receiving_streams(multicast_pipe, data_flow_info);
...
bool MulticastStreamsCreator::should_create_multicast_stream(
    const StreamNode* input_stream, const MulticastPipe* multicast_pipe)
{
    if (is_direct_packer_multicast(input_stream, multicast_pipe))
    {
    ...
    const bool input_stream_on_same_core =
        input_stream->get_physical_location() == multicast_pipe->get_physical_location();
...
bool MulticastStreamsCreator::is_direct_packer_multicast(
    const StreamNode* input_stream, const MulticastPipe* multicast_pipe)
{
...
std::unique_ptr<StreamNode> MulticastStreamsCreator::create_multicast_stream(
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    m_stream_creator->set_common_stream_properties_from_pipe(multicast_stream.get(), pipe, false /* incoming_stream */);

    return multicast_stream;
}

void MulticastStreamsCreator::convert_input_stream_to_multicast(
    const MulticastPipe* multicast_pipe, StreamNode* input_stream)
{
    const bool is_direct_packer_multicast =
        MulticastStreamsCreator::is_direct_packer_multicast(input_stream, multicast_pipe);

    log_assert(
        input_stream->get_stream_type() == StreamType::Gather || input_stream->get_stream_type() == StreamType::Relay ||
