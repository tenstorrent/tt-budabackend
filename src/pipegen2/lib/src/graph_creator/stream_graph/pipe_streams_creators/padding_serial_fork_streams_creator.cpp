...
namespace pipegen2
{
        ...
        const DataFlowInfo& data_flow_info)
    {
        ...
        ASSERT(serial_fork_pipe, "Expecting serial fork pipe to be used in PaddingSerialForkStreamsCreator");
    ...
    void PaddingSerialForkStreamsCreator::assign_common_config_to_pipe_outputs(const RGBasePipe* pipe)
    {
        for (const RGBaseNode* output_node : pipe->get_output_nodes())
        {
            ...
            ASSERT(out_serial_fork_node != nullptr, "Expecting serial fork node at the PaddingSerialFork pipe output");
        ...
        const std::size_t output_index)
    {
        ...
        ASSERT(stream_it != created_streams.end(), "Did not find padding stream at the location of output node");
    ...
    std::unordered_map<tt_cxy_pair, StreamNode*> PaddingSerialForkStreamsCreator::create_padding_stream_per_output_core(
        const PaddingSerialForkPipe* pipe,
        std::vector<std::unique_ptr<StreamNode>>& created_streams)
    {
        ...
        ASSERT(!pipe_outputs.empty(), "Expecting at least one node at the PaddingSerialForkPipe output");

        StreamNode* base_padding_stream = get_input_stream_node(pipe);
        ASSERT(base_padding_stream, "Base padding stream not found");
        ...
        for (const RGBaseNode* output_node : pipe_outputs)
        {
            ...
            created_streams.push_back(
                create_padding_stream_for_core(pipe, base_padding_stream, core_physical_location));
    ...
    std::unique_ptr<StreamNode> PaddingSerialForkStreamsCreator::create_padding_stream_for_core(
        const PaddingSerialForkPipe* pipe,
        const StreamNode* base_padding_stream,
