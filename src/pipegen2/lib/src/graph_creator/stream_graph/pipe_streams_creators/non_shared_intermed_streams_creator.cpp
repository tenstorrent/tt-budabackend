    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    const NonSharedIntermedNode* intermed_input_node =
        dynamic_cast<const NonSharedIntermedNode*>(pipe->get_inputs()[0].get_node());
    const NonSharedIntermedNode* intermed_output_node =
        dynamic_cast<const NonSharedIntermedNode*>(pipe->get_output_nodes()[0]);
