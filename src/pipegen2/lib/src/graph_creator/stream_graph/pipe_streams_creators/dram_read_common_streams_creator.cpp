    ...
    std::vector<NcriscConfig>&& ncrisc_configs)
{
    ...
    if (unpacker_node)
    {
        created_streams.push_back(create_dram_prefetch_post_tm_to_unpacker_stream(
            pipe, unpacker_node, data_flow_info, std::move(ncrisc_configs)));
    }
    else
    {
        created_streams.push_back(
            create_dram_relay_stream(pipe, data_flow_info, ncrisc_configs[0], true /* is_dram_prefetch_post_tm */));
    }
    ...
    configure_dram_receiving_stream(
        created_streams[0].get(),
        std::move(ncrisc_configs),
        pipe,
        dram_input_node,
        data_flow_info,
        false /* should_scale_up_stream */,
    ...
    std::vector<NcriscConfig>&& ncrisc_configs)
{
    ...
    std::vector<std::unique_ptr<StreamNode>> created_streams = create_dram_or_pcie_to_unpacker_streams(
        pipe,
        ncrisc_configs[0],
        dram_input_node->is_dram_prefetch_post_tm(),
        max_dram_input_buffer_size_tiles,
        unpacker_node,
        data_flow_info);
    ...
    const bool should_scale_up_stream =
        can_use_single_unpacker_stream ? dram_input_node->is_dram_io() && !dram_input_node_is_embedding_index : true;
    ...
    if (created_streams.size() > 1)
    {
        ...
        configure_relay_to_unpacker_stream(
            created_streams[1].get(), pipe, dram_input_node, unpacker_node, data_flow_info, max_num_tiles_per_phase);
...
{
    std::vector<std::unique_ptr<StreamNode>> created_streams =
        create_dram_input_streams(pipe, data_flow_info, max_dram_input_buffer_size_tiles, std::move(ncrisc_configs));
    ...
    base_config.set_dram_embeddings_row_shift(
        dram_embedding_table->get_tile_size() * dram_embedding_table->get_untilized_input_c_dim() *
    ...
    base_config.set_dram_embeddings_row_tiles(
        chunk_size_tiles * dram_embedding_table->get_embedding_table_row_size_per_core() /
...
std::vector<std::unique_ptr<StreamNode>> DramReadCommonStreamsCreator::create_pcie_to_unpacker_streams(
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info, NcriscConfig&& ncrisc_config)
{
    ...
    const PCIeStreamingNode* pcie_streaming_node =
        dynamic_cast<const PCIeStreamingNode*>(pipe->get_inputs().front().get_node());
    ...
    std::vector<std::unique_ptr<StreamNode>> created_streams = create_dram_or_pcie_to_unpacker_streams(
        pipe,
        ncrisc_config,
        false /* is_dram_prefetch_post_tm */,
        0 /* max_dram_input_buffer_size_tiles */,
        unpacker_node,
        data_flow_info);
    ...
    configure_pcie_receiving_stream(
        created_streams[0].get(),
    ...
    unsigned int& max_num_tiles_per_phase)
{
    ...
    configure_dram_receiving_stream(
        multicast_stream.get(),
        std::move(ncrisc_configs),
        pipe,
        dram_input_node,
        data_flow_info,
        true /* should_scale_up_stream */,
        max_dram_input_buffer_size_tiles,
        max_num_tiles_per_phase);
    ...
    const unsigned int max_num_tiles_per_phase)
{
    ...
    for (std::size_t i = 0; i < pipe->get_output_nodes().size(); ++i)
    {
        ...
        log_assert(
            unpacker_node != nullptr, "Expecting unpacker node at the DramMulticast or PCIeMulticast pipe output");

        unpacker_streams.push_back(
            create_multicast_unpacker_stream(pipe, input_node, unpacker_node, data_flow_info, max_num_tiles_per_phase));
    }
    ...
    const unsigned int max_num_tiles_per_phase)
{
    ...
    const std::vector<PhaseInfo> decomposed_phases =
        m_stream_creator->decompose_phases(original_phases, decomposed_phase_max_size);

    const std::vector<PhaseInfo> merged_phases =
        m_stream_creator->merge_phases(decomposed_phases, max_num_tiles_per_phase);
...
const UnpackerOutputNode* DramReadCommonStreamsCreator::get_unpacker_output_node(
    const RGBasePipe* pipe, const std::size_t output_index)
{
    ...
    {
        log_assert(
            dynamic_cast<const RelayNode*>(output_node) != nullptr,
    ...
    const DataFlowInfo& data_flow_info)
{
    ...
    {
        created_streams.push_back(
            create_dram_relay_stream(pipe, data_flow_info, base_ncrisc_config, is_dram_prefetch_post_tm));
    }
    else
    {
...
std::unique_ptr<StreamNode> DramReadCommonStreamsCreator::create_dram_or_pcie_multicast_stream(
    const RGBasePipe* pipe, const NcriscConfig& base_ncrisc_config, const bool is_dram_prefetch_post_tm)
{
    std::unique_ptr<StreamNode> multicast_stream =
        std::make_unique<StreamNode>(StreamType::Multicast, pipe->get_physical_location());
    ...
    const UnpackerOutputNode* unpacker_node)
{
    std::unique_ptr<StreamNode> dram_to_unpacker_stream = std::make_unique<StreamNode>(
        StreamType::Unpacker, unpacker_node->get_physical_location(), unpacker_node->get_operand_id());
...
void DramReadCommonStreamsCreator::create_dram_to_unpacker_streams_with_relay(
    ...
    std::vector<std::unique_ptr<StreamNode>>& created_streams)
{
    std::unique_ptr<StreamNode> dram_relay_stream =
        create_dram_relay_stream(pipe, data_flow_info, base_ncrisc_config, is_dram_prefetch_post_tm);
...
{
    std::unique_ptr<StreamNode> dram_relay_stream =
        std::make_unique<StreamNode>(StreamType::Relay, pipe->get_physical_location());
...
std::unique_ptr<StreamNode> DramReadCommonStreamsCreator::create_relay_to_unpacker_stream(
    const RGBasePipe* pipe, const UnpackerOutputNode* unpacker_node)
{
    std::unique_ptr<StreamNode> unpacker_stream = std::make_unique<StreamNode>(
        StreamType::Unpacker, unpacker_node->get_physical_location(), unpacker_node->get_operand_id());
    ...
    const unsigned int tiles_per_input)
{
    ...
    bool should_recalculate = can_use_single_unpacker_stream && data_flow_info.get_tiles_to_send(output_node) >
                                                                    constants::general_max_num_tiles_per_phase;
    ...
    unsigned int& max_num_tiles_per_phase)
{
    ...
    {
        scale_up_dram_receiving_stream(
            dram_receiving_stream,
            pipe,
            data_flow_info,
            max_num_tiles_per_phase,
    ...
    check_dram_input_buffer_size_constraints(
        dram_receiving_stream,
        get_unpacker_output_node(pipe),
        max_dram_input_buffer_size_tiles,
        dram_input_node->get_tile_size());
}
    ...
    unsigned int& max_num_tiles_per_phase)
{
    ...
    if (data_flow_info.get_tiles_to_send(output_node) > constants::general_max_num_tiles_per_phase)
    {
        ...
        rec_stream_buffer_size_tiles = std::lcm(rec_stream_buffer_size_tiles, data_flow_info.get_subtree_divisor(pipe));
    }
    max_num_tiles_per_phase = recalculate_max_num_tiles_per_phase(
        input_node->get_tiles_per_input(), rec_stream_buffer_size_tiles, num_iterations_in_epoch);
...
void DramReadCommonStreamsCreator::scale_up_dram_receiving_stream(
    StreamNode* stream,
    const RGBasePipe* pipe,
    const DataFlowInfo& data_flow_info,
    unsigned int max_num_tiles_per_phase,
    ...
    unsigned int max_dram_input_buffer_size_tiles)
{
    ...
    if (max_dram_input_buffer_size_tiles > 0)
    {
        ...
        scale_factor =
            std::max(1u, max_dram_input_buffer_size_tiles * dram_input_node_tile_size_bytes / base_buffer_size_bytes);
    ...
    else
    {
        ...
        const unsigned int min_buffer_size = pipe->is_ethernet_pipe() ? constants::eth_dram_rec_stream_min_size_bytes
                                                                      : constants::dram_rec_stream_min_size_bytes;
    ...
    const unsigned int pipe_reduce_mem_usage_threshold = m_stream_creator->calculate_pipe_reduce_mem_usage_threshold(
...
void DramReadCommonStreamsCreator::scale_up_pcie_receiving_stream(
    StreamNode* stream, const PCIeStreamingNode* pcie_streaming_node)
{
...
unsigned int DramReadCommonStreamsCreator::recalculate_max_num_tiles_per_phase(
    unsigned int tiles_per_input, unsigned int buffer_size_tiles, unsigned int num_iters_in_epoch)
{
    ...
    log_assert(
        recalculated_max_num_tiles_per_phase > 0 &&
    ...
    const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    std::sort(
        input_phases.begin(),
        input_phases.end(),
        [](const PhaseInfo& left, const PhaseInfo& right) { return left.phase_offset < right.phase_offset; });
    ...
    const unsigned int max_num_tiles_per_phase)
{
    ...
    update_stream_with_merged_decomposed_phases(
        relay_to_unpacker_stream, input_node, unpacker_phases, data_flow_info, num_iterations, max_num_tiles_per_phase);
}
    ...
    const std::vector<std::unique_ptr<StreamNode>>& created_streams)
{
    ...
    m_stream_creator->configure_stream_src_and_dest(
        dram_or_pcie_receiving_stream, nullptr /* source_stream */, unpacker_stream);
    m_stream_creator->configure_stream_src_and_dest(
        unpacker_stream, dram_or_pcie_receiving_stream, nullptr /* destination_stream */);
}
    ...
    unsigned int dram_input_node_tile_size_bytes)
{
    ...
    unsigned int stream_node_buf_size_tiles =
        stream_node->get_base_config().get_buffer_size().value() / dram_input_node_tile_size_bytes;
    ...
    if (stream_node->get_stream_type() == StreamType::Relay)
    {
        if (stream_node_buf_size_tiles > max_dram_input_buffer_size_tiles)
        {
            log_warning(
                ...
                "the limit of {} tiles",
                stream_node->get_stream_id(),
                stream_node_buf_size_tiles,
                max_dram_input_buffer_size_tiles);
        }
    }
    else if (stream_node->get_stream_type() == StreamType::Unpacker)
    {
        ...
        unsigned int max_buf_size_tiles = std::max(max_dram_input_buffer_size_tiles, unpacker_node_buf_size_tiles);
