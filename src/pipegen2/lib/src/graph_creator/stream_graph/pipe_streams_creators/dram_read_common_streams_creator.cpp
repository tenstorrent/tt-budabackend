...
namespace pipegen2
{
        ...
        std::vector<NcriscConfig>&& ncrisc_configs)
    {
        ...
        ASSERT(dram_input_node != nullptr, "Expecting DRAM input node at the pipe input");
        ...
        {
            ASSERT(unpacker_node != nullptr, "Only dram to unpacker connection may induce multiple streams");
        ...
        std::vector<NcriscConfig>&& ncrisc_configs)
    {
        ...
        ASSERT(created_streams.size() == 1, "Expecting only one stream created for reading from DRAM embedding table");
    ...
    {
        ASSERT(pipe->get_inputs().size() == 1, "Expecting only a single PCIe streaming node at the input");
        ...
        const UnpackerOutputNode* unpacker_node = get_unpacker_output_node(pipe);
        ASSERT(unpacker_node != nullptr, "Expecting unpacker node at the PCIe pipe output");
            ...
            const unsigned int max_num_tiles_per_phase)
    {
        ...
        for (std::size_t i = 0; i < pipe->get_output_nodes().size(); ++i)
        {
            ...
            // log_assert(unpacker_node, "Expecting unpacker node at the DramMulticast or PCIeMulticast pipe output");
            ASSERT(unpacker_node != nullptr,
                                                                                     ...
                                                                                     const std::size_t output_index)
    {
        ...
        ASSERT(output_index < pipe->get_output_nodes().size(), "Index out of range");
        ...
        if (!unpacker_node)
        {
            ASSERT(dynamic_cast<const RelayNode*>(output_node) != nullptr,
        ...
        const unsigned int tiles_per_input)
    {
        ...
        return should_recalculate ?
            recalculate_max_num_tiles_per_phase(tiles_per_input, output_node->get_size_tiles()) :
        ...
        unsigned int& max_num_tiles_per_phase)
    {
        ...
        ASSERT(input_phases.size() > 0, "Expecting at least one phase to configure");
        ...
        const unsigned int buffer_size_tiles)
    {
        ...
        if (tiles_per_input <= constants::general_max_num_tiles_per_phase)
        {
            ...
            {
                // TODO report warning:
                // "pipegen cannot ensure divisibility with num_tiles_per_input - therefore fork-join paths may hang"
                // example netlist: netlist_219ee5a9de05292fbc8fbd1c6cd9b0bf.yaml
            }
            else
            {
        ...
        return (constants::general_max_num_tiles_per_phase / num_divisor_tiles) * num_divisor_tiles;
    }
