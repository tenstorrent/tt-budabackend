    ...
    const RGBaseNode* input_node, const RGBasePipe* pipe, const DataFlowInfo& data_flow_info)
{
    ...
    log_assert(
        prefetch_input_node && prefetch_input_node->is_dram_prefetch_pre_tm(),
        "Expecting prefetch Pre-TM DRAM input node at the DramPrefetchPreTMParalleFork pipe input");

    std::unique_ptr<StreamNode> prefetch_relay_stream =
        std::make_unique<StreamNode>(StreamType::Relay, prefetch_input_node->get_physical_location());
