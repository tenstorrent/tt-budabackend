...
namespace pipegen2
{
        ...
        const std::unordered_map<const VirtualNode*, StreamPhasesCommonConfig>& virt_node_to_stream_node)
    {
        ...
        ASSERT(pipes_writing_to_pcie.size() == pipes_reading_from_pcie.size(),
        ...
        for (std::size_t i = 0; i < pipes_writing_to_pcie.size(); ++i)
        {
            ...
            ASSERT(stream_reading_from_pcie,
                                                            ...
                                                            std::vector<const RGBasePipe*>& pipes_reading_from_pcie)
    {
        for (const std::unique_ptr<RGBaseNode>& node : rational_graph->get_nodes())
        {
            ...
            {
                ASSERT(pcie_streaming_node->get_input_pipe(),
        ...
        const std::unordered_map<const VirtualNode*, StreamPhasesCommonConfig>& virt_node_to_stream_node)
    {
        ...
        ASSERT(virtual_input_node, "Expected to find virtual node at the input of the pipe");

        auto it = virt_node_to_stream_node.find(virtual_input_node);
        ASSERT(it != virt_node_to_stream_node.end(), "Expected to find stream node assigned to virtual node");
    ...
    void StreamGraphCreator::optimize_unpacker_streams_fed_by_union(StreamGraph* stream_graph)
    {
        for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
        {
            ...
            ASSERT(!phase_configs.empty(), "At this point, stream node should have at least one phase config");
    ...
    PhaseId StreamGraphCreator::get_max_phase_in_stream_graph(const StreamGraph* stream_graph)
    {
        ...
        for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
        {
            ASSERT(stream_node->get_num_phases() > 0, "Expected phases to be created for stream node");
        ...
        const StreamGraphCollection* stream_graph_collection)
    {
        ...
        for (const std::unique_ptr<StreamGraph>& stream_graph : stream_graph_collection->get_stream_graphs())
        {
            ...
            for (const StreamLocation& core_location : stream_graph_core_locations)
            {
                ASSERT(phases_footprint_per_core.find(core_location) != phases_footprint_per_core.end(),
    ...
    void StreamGraphCreator::adjust_phase_transition_properties(StreamNode* stream_node)
    {
        ...
        ASSERT(stream_node->get_phase_configs().size() > 0, "At this point, stream must have at least one phase");
        ...
        ASSERT(first_phase_config.get_dest().value().size() > 0, "Expected at least one destination stream");
        ...
        {
            ASSERT(dest_stream->get_base_config().get_buffer_size().has_value(), "Expected buffer size to be set");
    ...
    void StreamGraphCreator::copy_phase_transition_properties(const std::vector<StreamNode*> destination_streams)
    {
        ...
        for (std::size_t dest_stream_idx = 1; dest_stream_idx < destination_streams.size(); ++dest_stream_idx)
        {
            ...
            ASSERT(dest_stream_configs.size() == first_dest_stream_configs.size(),
                                                  ...
                                                  const int segment_end_phase_idx)
    {
        ...
        ASSERT(dest_phase_iter != dest_phase_configs.begin(),
             ...
             ++dest_phase_iter)
        {
            ...
            ASSERT(dest_phase_source_stream, "Expected to find a source stream for the phase");
    ...
    void StreamGraphCreator::assign_pcie_streams_noc_addresses(const ResourceManager* resource_manager)
    {
        for (StreamNode* stream_writing_to_pcie : m_pcie_writing_streams)
        {
            ...
            unsigned long local_pcie_buffer_address = resource_manager->get_soc_info()->get_local_pcie_buffer_address(
            ...
            unsigned long dram_buf_noc_addr = resource_manager->get_soc_info()->get_buffer_noc_address_through_pcie(
