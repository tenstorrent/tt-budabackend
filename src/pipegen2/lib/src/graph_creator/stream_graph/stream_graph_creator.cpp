...
std::vector<PhaseConfig>::iterator StreamGraphCreator::find_phase_config_by_phase_id(
    std::vector<PhaseConfig>& phase_configs, const PhaseId phase_id)
{
    ...
    auto equal_or_higher = std::lower_bound(
        phase_configs.begin(),
        phase_configs.end(),
        phase_id,
        [](const PhaseConfig& phase_config, PhaseId phase_id) { return phase_config.phase_id < phase_id; });
    log_assert(
        equal_or_higher != phase_configs.end() && equal_or_higher->phase_id == phase_id,
        "Expected to find at least one phase config with phase_id equal to {}",
        phase_id);
...
void StreamGraphCreator::connect_pcie_streams(const StreamGraph* stream_graph)
{
    ...
    for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
    {
        ...
        auto it = pcie_streaming_node_id_to_stream_node.find(std::make_pair(
            pcie_streaming_node->get_mmio_pipe_id(), pcie_streaming_node->get_mmio_pipe_scatter_index()));
        ...
        {
            log_assert(
                pcie_streaming_node->is_pcie_reader(), "We expect PCIeStreaming node to be either writer or reader");
    ...
    StreamGraph* stream_graph)
{
    ...
    {
        PipeStreamsCreator* streams_creator =
            pipe_streams_creator_factory->get_pipe_streams_creator(pipe, resource_manager, virt_node_to_stream_node);

        std::vector<std::unique_ptr<StreamNode>> pipe_streams = streams_creator->create_streams(pipe, data_flow_info);
...
{
    std::unordered_map<NodeId, StreamNode*> buf_node_id_to_stream_node = create_node_id_to_stream_mapping(stream_graph);
    ...
    for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
    {
        ...
        log_assert(
            sharing_stream_it != buf_node_id_to_stream_node.end(),
    ...
    const StreamGraph* stream_graph)
{
    ...
    for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
    {
        ...
        {
            buf_node_id_to_stream_node.emplace(stream_node->get_base_config().get_buf_id().value(), stream_node.get());
        }
    }
...
void StreamGraphCreator::move_created_streams_to_graph(
    std::vector<std::unique_ptr<StreamNode>>& created_streams, StreamGraph* stream_graph)
{
...
void StreamGraphCreator::calculate_unrolled_phase_offset(const std::vector<std::unique_ptr<StreamGraph>>& stream_graphs)
{
    m_unrolled_phase_offset = 0;
    for (const std::unique_ptr<StreamGraph>& stream_graph : stream_graphs)
    {
        m_unrolled_phase_offset = std::max(m_unrolled_phase_offset, get_max_phase_in_stream_graph(stream_graph.get()));
    }
...
void StreamGraphCreator::insert_dummy_phases(StreamGraphCollection* stream_graph_collection)
{
    ...
    for (const std::unique_ptr<StreamGraph>& stream_graph : stream_graph_collection->get_stream_graphs())
    {
        for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
        {
            ...
            log_assert(
                phase_config.get_dest().has_value(),
            ...
            {
                auto phase_iter =
                    find_phase_config_by_phase_id(dest_stream->get_phase_configs(), phase_configs[0].phase_id);
    ...
    for (const std::unique_ptr<StreamGraph>& stream_graph : stream_graph_collection->get_stream_graphs())
    {
        for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
        {
            ...
            log_debug(
                ...
                chosen_noc_id,
                stream_node->get_physical_location().str(),
                stream_node->get_stream_type());
...
void StreamGraphCreator::set_op_names_on_cores(
    const StreamGraphCollection* stream_graph_collection, const ResourceManager* resource_manager)
{
    for (const tt_cxy_pair& core_location : stream_graph_collection->get_physical_locations())
    {
        for (const StreamNode* stream : stream_graph_collection->get_streams_on_core(core_location))
        {
            ...
            if (stream->get_stream_type() == StreamType::Packer || stream->get_stream_type() == StreamType::Unpacker)
            {
...
void StreamGraphCreator::unroll_stream_graph(StreamGraph* stream_graph, const unsigned int max_unroll_factor)
{
    ...
    for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
    {
        ...
        {
            iterations_limit_wrt_max_tiles_per_phase_for_graph = std::min(
                iterations_limit_wrt_max_tiles_per_phase_for_graph, iterations_limit_wrt_max_tiles_per_phase_for_node);
        }
    }
    ...
    const StreamGraphCollection* stream_graph_collection)
{
    ...
    for (const std::unique_ptr<StreamGraph>& stream_graph : stream_graph_collection->get_stream_graphs())
    {
        ...
        for (const StreamLocation& core_location : stream_graph_core_locations)
        {
            ...
            for (const StreamNode* stream_node : stream_graph_collection->get_streams_on_core(core_location))
            {
                ...
                phases_footprint_per_core[stream_node->get_physical_location()] +=
                    stream_node->get_phase_configs().size() * c_estimated_phase_physical_size_bytes * mblock_buffering;
            }
        }
        ...
        {
            log_assert(
                phases_footprint_per_core.find(core_location) != phases_footprint_per_core.end(),
                "Core location not found in phases_footprint_per_core map");
            const unsigned int overlay_blob_size =
                is_ethernet ? eth_l1_mem::address_map::OVERLAY_BLOB_SIZE : l1_mem::address_map::OVERLAY_BLOB_SIZE;
...
void StreamGraphCreator::unroll_phases(
    StreamNode* stream_node, const unsigned int num_iterations_in_epoch, const unsigned int unroll_factor)
{
    ...
    for (unsigned int unroll_iter = 1; unroll_iter < unroll_factor; ++unroll_iter)
    {
        for (unsigned int i = 0; i < initial_phase_count; ++i)
        {
            ...
            stream_node->add_phase_config(
                m_unrolled_phase_offset * unroll_iter + phase_config.phase_id, std::move(new_phase_config));
        }
    }
}
...
void StreamGraphCreator::adjust_phase_transition_properties(StreamNode* stream_node)
{
    ...
    {
        const bool is_dram_read =
            base_config.get_dram_io().value_or(false) || base_config.get_dram_streaming().value_or(false);
        ...
        const bool can_force_next_phase_src_change_opt =
            is_dram_read && rd_wr_pointers_aligned_with_zero &&
    ...
    auto is_dest_unpacker_with_2k_header_buffer_reset = [](StreamNode* dest_stream)
    {
        ...
        return c_use_2k_tile_header_buffer_reset && dest_stream->get_stream_type() == StreamType::Unpacker &&
    ...
    const bool is_source_scatter_send_from_l1_buffer = base_config.get_is_l1_producer().value_or(false) &&
    ...
    for (unsigned int i = 0; i < phase_configs.size(); ++i)
    {
        ...
        // phases.
        const bool is_dest_gather_relay =
            cur_dest->get_stream_type() == StreamType::GatherRelay || cur_dest->get_stream_type() == StreamType::Gather;
        ...
        const bool is_l1_scatter_buffer_new_unroll_iter =
            is_source_scatter_send_from_l1_buffer && is_different_unroll_iter;
        ...
        const bool stop_opt_for_max_tiles_exceeded =
            crossing_max_tiles_per_phase && !is_dest_unpacker_with_2k_header_buffer_reset(dest_stream_node);

        const bool should_stop_optimization = cur_dest != dest_stream_node || stop_opt_for_non_consecutive_phase ||
                                              stop_opt_for_max_tiles_exceeded || stop_opt_for_scatter_send;
...
void StreamGraphCreator::copy_phase_transition_properties(const std::vector<StreamNode*> destination_streams)
{
    ...
    for (std::size_t dest_stream_idx = 1; dest_stream_idx < destination_streams.size(); ++dest_stream_idx)
    {
        ...
        log_assert(
            dest_stream_configs.size() == first_dest_stream_configs.size(),
...
void StreamGraphCreator::compute_ptrs_not_zero_field(StreamNode* stream_node)
{
    ...
    else
    {
        ...
        if (should_skip_last_phase_for_ptrs_not_zero(
                phase_configs, last_phase_to_check_idx, num_iterations_in_epoch, unroll_factor))
        {
...
bool StreamGraphCreator::should_skip_last_phase_for_ptrs_not_zero(
    const std::vector<PhaseConfig>& phase_configs,
...
void StreamGraphCreator::optimize_edge_phases(
    ...
    const int segment_end_phase_idx)
{
    ...
    auto dest_phase_iter = std::upper_bound(
        dest_phase_configs.begin(),
        dest_phase_configs.end(),
        start_phase_id,
        [](const PhaseId phase_id, const PhaseConfig& phase_config) { return phase_config.phase_id > phase_id; });

    log_assert(
        dest_phase_iter != dest_phase_configs.begin(),
    ...
    for (; dest_phase_iter != dest_phase_configs.end() && dest_phase_iter->phase_id <= end_phase_id; ++dest_phase_iter)
    {
        ...
        if (started_new_subsegment)
        {
            ...
            auto source_stream_subsegment_end_it = std::upper_bound(
                source_phase_configs.begin(),
                source_phase_configs.end(),
                dest_phase_iter->phase_id,
...
void StreamGraphCreator::assign_pcie_streams_noc_addresses(const ResourceManager* resource_manager)
{
    for (StreamNode* stream_writing_to_pcie : m_pcie_writing_streams)
    {
        ...
        std::uint64_t dram_buf_noc_addr = resource_manager->get_soc_info()->get_buffer_noc_address_through_pcie(
            local_pcie_buffer_address, stream_reading_from_pcie->get_physical_location().chip);
