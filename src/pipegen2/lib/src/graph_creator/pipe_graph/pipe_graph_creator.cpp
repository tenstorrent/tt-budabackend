...
void PipeGraphCreator::find_pipe_graph_nodes_connections(PipeGraph& pipe_graph)
{
    for (std::unique_ptr<PGPipe>& pipe : pipe_graph.get_pipes())
    {
             ...
             scatter_index += pipe->get_consumer_repeat())
        {
            ...
            pipe_graph_creator_internal::connect_pipe_with_output_padding_buffer(
                scatter_index, buffer_id, pipe.get(), m_pipe_graph_info);
        }
             ...
             scatter_index += pipe->get_consumer_repeat())
        {
            for (NodeId buffer_id : output_buffer_ids[scatter_index])
            {
                pipe_graph_creator_internal::connect_pipe_with_output_buffer(
                    pipe.get(), scatter_index, buffer_id, m_pipe_graph_info);
            }
        }
    }
}
...
void PipeGraphCreator::patch_deficiencies(PipeGraph& pipe_graph)
{
    ...
    pipe_graph_handlers.push_back(
        std::move(std::make_unique<EmbeddingIndexHandler>(EmbeddingIndexHandler(&m_pipe_graph_info))));
    pipe_graph_handlers.push_back(
        std::move(std::make_unique<IntermediateBufferHandler>(IntermediateBufferHandler(&m_pipe_graph_info))));
    pipe_graph_handlers.push_back(std::move(std::make_unique<E2EQueueHandler>(E2EQueueHandler())));
    pipe_graph_handlers.push_back(std::move(std::make_unique<PostTMPipeHandler>(PostTMPipeHandler())));
