...
namespace pipegen2
{
    std::unique_ptr<PipeGraph> PipeGraphCreator::create_pipe_graph(const std::string& pipegen_yaml_path)
    {
        ...
        PipeGraphParser graph_parser(pipegen_yaml_path);
        graph_parser.parse_graph(*pipe_graph);
    ...
    void PipeGraphCreator::add_buffer_to_map_per_id(NodeId buffer_id, PGBuffer* buffer)
    {
        ...
        {
            ERROR("PipeGraphCreator: Buffer with id " << buffer_id << " already exists");
        }
    ...
    void PipeGraphCreator::find_pipe_graph_nodes_connections(PipeGraph& pipe_graph)
    {
        for (std::unique_ptr<PGPipe>& pipe : pipe_graph.get_pipes())
        {
                 ...
                 scatter_index += pipe->get_consumer_repeat())
            {
                ...
                if (buffer_id == c_no_output_padding_buffer_id)
                {
                    continue;
                }
    ...
    void PipeGraphCreator::handle_embedding_connections(PipeGraph& pipe_graph)
    {
        for (std::unique_ptr<PGPipe>& pipe : pipe_graph.get_pipes())
        {
            ...
            {
                ASSERT(pipe->get_output_buffers().size() == 1, "Embedding table pipe should have only one output");
                ...
                ASSERT(embedding_index_kernel_buffer != nullptr,
    ...
    void PipeGraphCreator::copy_embedding_index_properties(const PGBuffer* embedding_index_kernel_buffer)
    {
        ...
        ASSERT(embedding_index_pipe != nullptr,
                "Could not find embedding index pipe for embedding index kernel buffer");
        ASSERT(embedding_index_pipe->get_inputs().size() == 1,
        ...
        ASSERT(embedding_index_kernel_buffer_iter != m_buffers_per_id.end(),
    ...
    void PipeGraphCreator::connect_pipe_with_input_buffer(PGPipe* pipe, NodeId input_buffer_id)
    {
        ...
        {
            ERROR("PipeGraphCreator: Pipe with id " << pipe->get_id() << " has inexistent input buffer with id " <<
                  input_buffer_id);
        }
                                                                   ...
                                                                   PGPipe* pipe)
    {
        ...
        if (buffer_it == m_buffers_per_id.end())
        {
            ERROR("PipeGraphCreator: Pipe with id " << pipe->get_id() << " has inexistent output padding buffer with "
                  "id " << output_padding_buffer_id);
        }
                                                           ...
                                                           NodeId output_buffer_id)
    {
        ...
        if (buffer_it == m_buffers_per_id.end())
        {
            ERROR("PipeGraphCreator: Pipe with id " << pipe->get_id() << " has inexistent output buffer with id " <<
                  output_buffer_id);
        }
