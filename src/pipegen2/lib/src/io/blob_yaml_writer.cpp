...
BlobYamlWriter::BlobYamlWriter(const std::string& blob_yaml_path) : m_current_indentation_level(0)
{
    try
    {
...
BlobYamlWriter::~BlobYamlWriter() { m_blob_yaml_file_stream.close(); }

void BlobYamlWriter::write_blob_yaml(
    const StreamGraphCollection* stream_graph_collection, const PerfInfoManager& perf_info_manager)
{
...
std::map<tt_cxys_pair, const std::vector<NcriscConfig>*> BlobYamlWriter::collect_ncrisc_configs(
...
{
    std::map<PhaseId, std::map<tt_cxys_pair, const StreamConfig*>> phase_map = collect_phase_map(stream_graphs);
...
void BlobYamlWriter::collect_stream_graph_phases(
    const StreamGraph* stream_graph, std::map<PhaseId, std::map<tt_cxys_pair, const StreamConfig*>>& phase_map)
{
    for (const std::unique_ptr<StreamNode>& stream_node : stream_graph->get_streams())
    {
...
void BlobYamlWriter::write_phase_map(const std::map<PhaseId, std::map<tt_cxys_pair, const StreamConfig*>>& phase_map)
{
...
void BlobYamlWriter::write_stream_params(const StreamConfig& stream_config, const PhaseId phase_id)
{
    ...
    write_optionally(
        "scatter_list_stream_id", stream_config.get_scatter_list_stream_id(), &BlobYamlWriter::get_stream_id_string);
    ...
    write_optionally(
        "space_shared_with_stream",
        stream_config.get_space_shared_with_stream(),
...
template <typename T>
void BlobYamlWriter::write_optionally(
    std::string&& name, const std::optional<T>& opt, std::string (*converter_f)(const T&))
{
    if (opt.has_value())
    {
...
template <typename T>
void BlobYamlWriter::write_param(std::string&& name, const T& value, std::string (*converter_f)(const T&))
{
...
std::string BlobYamlWriter::get_string(NOC_ROUTE value) { return get_string(static_cast<unsigned int>(value)); }

std::string BlobYamlWriter::get_string(const std::string& value) { return value; }

template <typename T>
typename std::
    enable_if<std::is_constructible<std::string, decltype(std::to_string(std::declval<T>()))>::value, std::string>::type
    BlobYamlWriter::get_string(T value)
{
    return std::to_string(value);
}

template <typename T>
std::string BlobYamlWriter::get_string(const std::vector<T>& values, std::string (*converter_f)(const T&))
{
    std::stringstream result_stream;
    result_stream << "[";
    for (std::size_t i = 0; i < values.size(); ++i)
    {
        result_stream << (converter_f ? converter_f(values[i]) : get_string(values[i]))
                      << (i < values.size() - 1 ? ", " : "");
    }
    result_stream << "]";
...
std::string BlobYamlWriter::get_hex_string(const unsigned int& number) { return get_hex_string((std::uint64_t)number); }
...
std::string BlobYamlWriter::get_stream_dest_string(const std::vector<StreamNode*>& stream_dest)
{
    ...
    std::sort(
        sorted_stream_dest.begin(),
        sorted_stream_dest.end(),
    ...
    dest_str << "[" << get_string(sorted_stream_dest.front()) << "," << get_string(sorted_stream_dest.back()) << "]";
    return dest_str.str();
}

BlobYamlWriter::IndentYaml::IndentYaml(BlobYamlWriter* blob_yaml_writer) : m_blob_yaml_writer(blob_yaml_writer)
{
...
{
    m_blob_yaml_writer->m_current_indentation = m_blob_yaml_writer->m_current_indentation_level > 0
