...
for p in 1..num_phases
  ...
  $binary_streaming_test_graph[phase] = {}

  $binary_streaming_test_graph[phase][input0_data_stream] = {
    :input_index => 0,
    :auto_run => true,
  ...
  $binary_streaming_test_graph[phase][input1_data_stream] = {
    :input_index => 1,
    :auto_run => true,
...
for p in 1..num_phases
  ...
  if !$binary_streaming_test_graph[phase]
    $binary_streaming_test_graph[phase] = {}
  end

  resend = output0_no_resend ? false : (((p-1) % num_microblocks_in_buf) == 0 ? false : true)

  $binary_streaming_test_graph[phase][output0_data_stream] = {
    :output_index => 0,
    :auto_run => true,
    :buf_addr => buf_addr,
    :buf_size => buf_size,
