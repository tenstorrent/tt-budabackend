...
input1_data_buf_size = $PARAMS[:input1_data_buf_size] ?  ($PARAMS[:input1_data_buf_size]*input1_data_buf_msg_size) :  num_input1_data_buf_msgs*input1_data_buf_msg_size
...
for p in 1..num_phases
  ...
  $binary_test_graph[phase] = {}

  $binary_test_graph[phase][input0_data_stream] = {
    :input_index => 0,
    :auto_run => true,
  ...
  $binary_test_graph[phase][input0_park_stream] = {
    :auto_run => true,
...
#    :preload_data => (0..num_msgs-1).map { |i| (start_seed_0 + (i << 16)) },
  ...
  $binary_test_graph[phase][input1_data_stream] = {
    :input_index => 1,
    :auto_run => true,
  ...
  $binary_test_graph[phase][input1_park_stream] = {
    :auto_run => true,
...
#    :preload_data => (0..num_msgs-1).map { |i| (start_seed_0 + (i << 16)) },
  ...
  $binary_test_graph[phase][output0_data_stream] = {
    :output_index => 0,
    :auto_run => true,
...
#    :preload_data => (0..(output0_data_buf_size/msg_size)-1).map { |i| (start_seed_2 + (i << 16)) },
  ...
  $binary_test_graph[phase][output0_park_stream] = {
    :auto_run => true,
