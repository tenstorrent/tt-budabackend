...
$unary_tilize_streaming_test_graph = {}
...
for p in 1..num_phases
  ...
  $unary_tilize_streaming_test_graph[phase] = {}

  buf_addr = input0_data_base
  $unary_tilize_streaming_test_graph[phase][input0_data_stream] = {
    :input_index => 0,
    :auto_run => true,
    :buf_addr => buf_addr,
...
for p in 1..num_phases
  ...
  if !$unary_tilize_streaming_test_graph[phase]
    $unary_tilize_streaming_test_graph[phase] = {}
  end

  resend = output0_no_resend ? false : (((p-1) % num_microblocks_in_buf) == 0 ? false : true)

  $unary_tilize_streaming_test_graph[phase][output0_data_stream] = {
    :output_index => 0,
    :auto_run => true,
    :buf_addr => buf_addr,
    :buf_size => buf_size,
