...
def STREAM_GATHER(arb_group_size, src_in_order_fwd)
...
def STREAM_MSG_SIZE_FORMAT(offset, bits)
  return ((offset << MSG_HEADER_WORD_CNT_OFFSET) | (bits << MSG_HEADER_WORD_CNT_BITS))
end
...
def STREAM_SOURCE_ENDPOINT_NEW_MSG(addr, size)
  return ((addr << SOURCE_ENDPOINT_NEW_MSG_ADDR) | (size << SOURCE_ENDPOINT_NEW_MSG_SIZE))
end
  
def STREAM_SOURCE_ENDPOINT_MEM_BUF_NEW_MSGS_DATA(msgs, words)
...
def get_phase_blob(chip_id, x, y, stream_id, phase, prev_phase, next_phase, dest_phase, dram_blob, src_old_base, src_new_base, next_phase_is_dummy_phase)
      ...
      if first_phase
        if ($PARAMS[:noc_version] > 1)
          cfg_dw_list << blob_cfg_dw(STREAM_GATHER_REG_INDEX,
                                    STREAM_GATHER(phase[:arb_group_size] ? phase[:arb_group_size] : 1,
  ...
  if phase[:resend]
    ...
    cfg_dw_list << blob_cfg_dw(STREAM_WR_PTR_REG_INDEX, (phase[:buf_size] > all_msg_size ? all_msg_size : 0)/MEM_WORD_BYTES)
