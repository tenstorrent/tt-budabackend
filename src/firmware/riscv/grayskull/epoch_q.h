 ...
 */
    ...
    static constexpr std::int32_t GridSizeCol = 13;
 ...
 */
    ...
    static constexpr std::int32_t EPOCH_ALLOC_QUEUE_SYNC_ADDR = (EPOCH_TABLE_DRAM_ADDR) - (32*EPOCH_ALLOC_QUEUE_SYNC_SLOTS);
    ...
    // For statically allocated kernel cache.
    static constexpr std::int32_t TRISC_KERNEL_CACHE_NUM_SLOTS = 0;
    static constexpr std::int32_t TRISC_KERNEL_CACHE_EMPTY_SLOT_IDX = 0; // Index reserved for empty binary.
    ...
    static int32_t get_epoch_alloc_queue_sync_addr() {
        int32_t addr = (get_epoch_table_dram_addr()) - (32 * EPOCH_ALLOC_QUEUE_SYNC_SLOTS);
    ...
    static std::int32_t get_queue_update_blob_size_bytes() {
        return get_queue_update_blob_num_entries() * 8;
    }

    static std::int32_t get_kernel_cache_num_slots() {
