...
three_d_array_tile_src_map::three_d_array_tile_src_map(
    ...
    this->producer_data_format = data_format(
        ublock_tiles_r,
        ublock_tiles_c,
        mblock_ublocks_m,
        mblock_ublocks_n,
...
three_d_array_tile_src_map three_d_array_tile_src_map::vslice(int factor) {
    this->tm_assert(
        (this->dim_size(map_dims::rt) % factor) == 0,
        "vslice called with non-divisible factor = " + std::to_string(factor) +
            ", rt = " + std::to_string(this->dim_size(map_dims::rt)));

    three_d_array_tile_src_map result = this->copy_except_tile_map_vector();
    ...
    for (int out_t = 0; out_t < t_size; out_t++) {
        for (int out_rt = 0; out_rt < rt_size; out_rt++) {
            for (int out_ct = 0; out_ct < ct_size; out_ct++) {
                int in_t = out_t / factor;
                int in_ct = out_ct;
                int in_rt = (out_t % factor) * rt_size + out_rt;
                result.tile_map.set_tile_map(out_t, out_rt, out_ct, this->get_tile_map(in_t, in_rt, in_ct));
            }
        }
    }
...
three_d_array_tile_src_map three_d_array_tile_src_map::vstack(int factor) {
    this->tm_assert(
        (this->dim_size(map_dims::t) % factor) == 0,
        "vstack called with non-divisible factor = " + std::to_string(factor) +
            ", t = " + std::to_string(this->dim_size(map_dims::t)));
    ...
    for (int in_t = 0; in_t < t_size; in_t++) {
        for (int in_rt = 0; in_rt < rt_size; in_rt++) {
            for (int in_ct = 0; in_ct < ct_size; in_ct++) {
                int out_t = in_t / factor;
                int out_ct = in_ct;
                int out_rt = (in_t % factor) * rt_size + in_rt;
                result.tile_map.set_tile_map(out_t, out_rt, out_ct, this->get_tile_map(in_t, in_rt, in_ct));
            }
        }
    }
...
three_d_array_tile_src_map three_d_array_tile_src_map::hslice(int factor) {
    this->tm_assert(
        (this->dim_size(map_dims::ct) % factor) == 0,
        "hslice called with non-divisible factor = " + std::to_string(factor) +
            ", ct = " + std::to_string(this->dim_size(map_dims::ct)));
    ...
    for (int out_t = 0; out_t < t_size; out_t++) {
        for (int out_rt = 0; out_rt < rt_size; out_rt++) {
            for (int out_ct = 0; out_ct < ct_size; out_ct++) {
                int in_t = out_t / factor;
                int in_rt = out_rt;
                int in_ct = (out_t % factor) * ct_size + out_ct;
                result.tile_map.set_tile_map(out_t, out_rt, out_ct, this->get_tile_map(in_t, in_rt, in_ct));
            }
        }
    }
...
three_d_array_tile_src_map three_d_array_tile_src_map::hstack(int factor) {
    this->tm_assert(
        (this->dim_size(map_dims::t) % factor) == 0,
        "hstack called with non-divisible factor = " + std::to_string(factor) +
            ", t = " + std::to_string(this->dim_size(map_dims::t)));
    ...
    for (int in_t = 0; in_t < t_size; in_t++) {
        for (int in_rt = 0; in_rt < rt_size; in_rt++) {
            for (int in_ct = 0; in_ct < ct_size; in_ct++) {
                int out_t = in_t / factor;
                int out_rt = in_rt;
                int out_ct = (in_t % factor) * ct_size + in_ct;
                result.tile_map.set_tile_map(out_t, out_rt, out_ct, this->get_tile_map(in_t, in_rt, in_ct));
            }
        }
    }
...
three_d_array_tile_src_map three_d_array_tile_src_map::broadcast(map_dims dimension, int factor) {
    ...
    if (dimension == map_dims::t) {
        this->tm_assert(
            (factor == 1) || (this->dim_size(map_dims::t) == 1), "broadcast t-dim called with input that has t > 1");
    ...
    for (int out_t = 0; out_t < result_t_size; out_t++) {
        for (int out_rt = 0; out_rt < result_rt_size; out_rt++) {
            for (int out_ct = 0; out_ct < result_ct_size; out_ct++) {
                int in_t = (out_t % t_size);
                int in_rt = (out_rt % rt_size);
                int in_ct = (out_ct % ct_size);
                result.tile_map.set_tile_map(out_t, out_rt, out_ct, this->get_tile_map(in_t, in_rt, in_ct));
            }
        }
    }
...
three_d_array_tile_src_map three_d_array_tile_src_map::pad(int padding_rt, int padding_ct) {
    ...
    for (int out_t = 0; out_t < t_size; out_t++) {
        for (int out_rt = 0; out_rt < rt_size; out_rt++) {
            for (int out_ct = 0; out_ct < ct_size; out_ct++) {
                ;
                if (out_rt < dim_size(map_dims::rt) and out_ct < dim_size(map_dims::ct)) {
                    result.tile_map.set_tile_map(out_t, out_rt, out_ct, this->get_tile_map(out_t, out_rt, out_ct));
                } else {
                    result.tile_map.set_tile_map(out_t, out_rt, out_ct, padding_tile);
                }
            }
        }
    }
...
bool three_d_array_tile_src_map::need_phased_stack() {
    if (this->multi_t_reach_stack) {
        ...
        bool can_stack_from_single_producer_output_buf =
            (total_stack_factor == 1) || ((this->producer_output_buf_size_t % total_stack_factor) == 0) ||
...
void three_d_array_tile_src_map::check_phased_stack(
    ...
    this->tm_assert(
        stack_ok_for_single_t_buf,
        "TM order does't satisfy constraints for stacking with phased pipes, the number of buffered output mblocks on "
        "the producer (not counting double-buffering) must be a multiple of the total stack factor or producer t");
    ...
    n2p::Log() << "TM (producer = " << this->producer_name << ", consumer = " << this->consumer_name
    ...
    int producer_block_tiles_pre_stack_r =
        (this->dim_size(map_dims::rt) - this->r_padding_tiles) / this->r_stack_factor;
    int producer_block_tiles_pre_stack_c =
        (this->dim_size(map_dims::ct) - this->c_padding_tiles) / this->c_stack_factor;
    ...
    if (this->row_major_stack) {
        this->tm_assert(
            divisible_either_direction(total_slice_factor, this->c_stack_factor),
            "row major stack with c stack factor = " + std::to_string(this->c_stack_factor) +
                " not divisible in either direction with preceding total slice factor = " +
                std::to_string(total_slice_factor));
        ...
        if ((total_slice_factor % this->c_stack_factor) == 0) {
            ...
            this->tm_assert(
                divisible_either_direction(total_slice_factor_r_stack_cover, this->r_stack_factor),
                "row major stack with c stack factor = " + std::to_string(this->c_stack_factor) +
                    " and total slice factor = " + std::to_string(this->c_stack_factor) + " x " +
                    std::to_string(total_slice_factor_r_stack_cover) +
                    " -> the second factor is not divisible in either direction with r stack factor = " +
                    std::to_string(this->r_stack_factor));
        ...
        this->tm_assert(
            divisible_either_direction(effective_c_stack_factor, this->producer_output_buf_size_t),
            "row-major stack: effective c stack factor not divisible either way with producer output buffer size "
            "mblocks");
        ...
        this->tm_assert(
            divisible_either_direction(total_slice_factor, this->r_stack_factor),
        ...
        if ((total_slice_factor % this->r_stack_factor) == 0) {
            ...
            this->tm_assert(
                divisible_either_direction(total_slice_factor_c_stack_cover, this->c_stack_factor),
                "column major stack with r stack factor = " + std::to_string(this->r_stack_factor) +
                    " and total slice factor = " + std::to_string(this->r_stack_factor) + " x " +
                    std::to_string(total_slice_factor_c_stack_cover) +
                    " -> the second factor is not divisible in either direction with c stack factor = " +
                    std::to_string(this->c_stack_factor));
        ...
        this->tm_assert(
            divisible_either_direction(effective_r_stack_factor, this->producer_output_buf_size_t),
            "row-major stack: effective r stack factor not divisible either way with producer output buffer size "
            "mblocks");
    ...
    n2p::Log() << "  -> effective_c_stack_factor = " << effective_c_stack_factor
               << ", effective_r_stack_factor = " << effective_r_stack_factor << "\n";
    n2p::Log() << "  -> stacking_block_tiles_c = " << stacking_block_tiles_c
               << ", stacking_block_tiles_r = " << stacking_block_tiles_r << "\n";
    n2p::Log() << "  -> padding_ct = " << this->c_padding_tiles << ", padding_rt = " << this->r_padding_tiles << "\n";

    // Padding
    if (effective_r_stack_factor > 1) {
        this->tm_assert(
            this->r_padding_tiles % stacking_block_tiles_r == 0,
            "Padding rt (" + std::to_string(this->r_padding_tiles) + ") not divisible by stacking block r (" +
                std::to_string(stacking_block_tiles_r) + ")");
    ...
    if (effective_c_stack_factor > 1) {
        this->tm_assert(
            this->c_padding_tiles % stacking_block_tiles_c == 0,
            "Padding ct (" + std::to_string(this->c_padding_tiles) + ") not divisible by stacking block c (" +
                std::to_string(stacking_block_tiles_c) + ")");
    ...
    this->tm_assert(
        divisible_either_direction(consumer_num_cores_r, effective_r_stack_factor),
    ...
    this->tm_assert(
        divisible_either_direction(consumer_num_cores_c, effective_c_stack_factor),
    ...
    this->tm_assert(
        divisible_either_direction(stacking_block_tiles_r, consumer_mblock_ublocks_r * consumer_ublock_tiles_r),
        "stacking block r size tiles = " + std::to_string(stacking_block_tiles_r) +
            " not divisible in either direction with consumer mblock tiles r = " +
            std::to_string(consumer_mblock_ublocks_r * consumer_ublock_tiles_r));

    this->tm_assert(
        divisible_either_direction(stacking_block_tiles_c, consumer_mblock_ublocks_c * consumer_ublock_tiles_c),
        "stacking block c size tiles = " + std::to_string(stacking_block_tiles_c) +
            " not divisible in either direction with consumer mblock tiles c = " +
            std::to_string(consumer_mblock_ublocks_c * consumer_ublock_tiles_c));

    // Stacking blocks must align with consumer-side ublocks.
    this->tm_assert(
        divisible_either_direction(stacking_block_tiles_r, consumer_ublock_tiles_r),
        "stacking block r size tiles = " + std::to_string(stacking_block_tiles_r) +
            " not divisible in either direction with consumer ublock tiles r = " +
            std::to_string(consumer_ublock_tiles_r));

    this->tm_assert(
        divisible_either_direction(stacking_block_tiles_c, consumer_ublock_tiles_c),
        "stacking block c size tiles = " + std::to_string(stacking_block_tiles_c) +
            " not divisible in either direction with consumer ublock tiles c = " +
            std::to_string(consumer_ublock_tiles_c));
    ...
    if (stacking_block_within_consumer_ublock) {
        ...
        if (stacking_block_tiles_c < consumer_ublock_tiles_c) {
            ublock_level_stack_ok = row_major_stack && (stacking_block_tiles_r == 1) &&
                                    ((consumer_mblock_ublocks_c == 1) ||
                                     ((consumer_ublock_tiles_r == 1) &&
                                      (consumer_row_major_ublock_scan_order || (consumer_mblock_ublocks_r == 1))));
        } else if (stacking_block_tiles_c == consumer_ublock_tiles_c) {
            ublock_level_stack_ok =
                !row_major_stack && ((consumer_mblock_ublocks_c == 1) || !consumer_row_major_ublock_scan_order);
        } else {  // stacking_block_tiles_c > consumer_ublock_tiles_c, implies stacking_block_tiles_r <
                  // consumer_ublock_tiles_r since stacking_block_within_consumer_ublock==true
            ublock_level_stack_ok = false;
        }
        this->tm_assert(
            ublock_level_stack_ok,
            "stacking within consumer ublock that does not follow consumer-side tile scan order");
    } else {
        ...
        if (consumer_row_major_ublock_scan_order) {
            if (stacking_block_tiles_c < (consumer_mblock_ublocks_c * consumer_ublock_tiles_c)) {
                this->tm_assert(
                    row_major_stack || (effective_r_stack_factor == 1),
                    "stacking within consumer mblock in column-major direction while ublock scan order is row-major");
                ...
                this->tm_assert(
                    stacking_block_tiles_r == consumer_ublock_tiles_r,
                    "hstack within consumer mblock in row-major scan order -> input block r dimension must be one "
                    "consumer ublock_r");
            }
            ...
            if (stacking_block_tiles_r < (consumer_mblock_ublocks_r * consumer_ublock_tiles_r)) {
                this->tm_assert(
                    !row_major_stack || (effective_c_stack_factor == 1),
                    "stacking within consumer mblock in row-major direction while ublock scan order is column-major");
                this->tm_assert(
                    stacking_block_tiles_c == consumer_ublock_tiles_c,
                    "vstack within consumer mblock in column-major scan order -> input block c dimension must be one "
                    "consumer ublock_c");
            }
        }
    }
...
consumer_to_producer_tile_map three_d_array_tile_src_map::get_op_matmul_col_input(
    int kernel_bcast_tiles,
    bool kernel_bcast_tiles_per_t,
    int consumer_t,
    int consumer_ublock_tiles_k,
    int consumer_ublock_tiles_c,
    int consumer_col_input_block_ublocks_k,
    ...
    if (kernel_bcast_tiles) {
        this->tm_assert(
            (this->producer_output_buf_size_t == 1 && this->producer_data_format.t == 1) || kernel_bcast_tiles_per_t,
            "with kernel broadcast that's not per-t, producer must have t = 1 and buf_size_mb = 1 or 2");
        // FIXME imatosevic - should we have this check? presently there is a test which brings in ublocks row by row
        // with kernel bcast this->tm_assert(kernel_bcast_tiles % consumer_ublock_tiles == 0,
    ...
    this->tm_assert(
        (consumer_rt == this->dim_size(map_dims::rt)) && (consumer_ct == this->dim_size(map_dims::ct)) &&
            (consumer_t == this->dim_size(map_dims::t)),
        "incompatible dimensions:  consumer -> " + dim_to_str(consumer_t, consumer_rt, consumer_ct) +
            ", post-TM producer -> " +
    ...
    int producer_core_output_buf_size_tiles =
        this->producer_data_format.mblock_size_tiles() * this->producer_output_buf_size_t;
    ...
    for (int consumer_col = 0; consumer_col < consumer_num_cores_c; consumer_col++) {
        ...
        for (int consumer_col_tile_index = 0; consumer_col_tile_index < consumer_total_tiles_per_col;
            ...
            int consumer_ublock_r = consumer_row_major_ublock_scan_order
                                        ? (consumer_ublock_index / consumer_mblock_ublocks_n)
                                        : (consumer_ublock_index % consumer_col_input_block_ublocks_k);
            int consumer_ublock_c = consumer_row_major_ublock_scan_order
                                        ? (consumer_ublock_index % consumer_mblock_ublocks_n)
                                        : (consumer_ublock_index / consumer_col_input_block_ublocks_k);
            ...
            tile_to_core_index_map curr_tile_map =
                this->get_tile_map(consumer_curr_t, consumer_in_rt, consumer_in_ct, consumer_curr_input);
            if (curr_tile_map.tile_index >= producer_core_output_buf_size_tiles) {
                ...
                if (kernel_bcast_tiles_per_t) {
                    if (consumer_mblock_col_tile_index >= kernel_bcast_tiles) {
                        this->tm_assert(
                            curr_tile_map == result.pipes[pipe_index].tile_map.at(
                                                 (consumer_curr_t * kernel_bcast_tiles) +
                                                 (consumer_col_tile_index % kernel_bcast_tiles)),
                            "input using kernel_broadcast_per_t but post-TM input canonical form is not periodic for t "
                            "= " +
                                std::to_string(consumer_curr_t));
                        continue;
                    }
                } else {
                    if (consumer_col_tile_index >= kernel_bcast_tiles) {
                        this->tm_assert(
                            curr_tile_map ==
                                result.pipes[pipe_index].tile_map.at(consumer_col_tile_index % kernel_bcast_tiles),
    ...
    if (this->need_phased_stack()) {
        ...
        this->check_phased_stack(
            effective_c_stack_factor,
            effective_r_stack_factor,
            consumer_ublock_tiles_k,
            consumer_ublock_tiles_c,
            consumer_col_input_block_ublocks_k,
            consumer_mblock_ublocks_n,
            1,
            consumer_num_cores_c,
            consumer_row_major_ublock_scan_order);
        ...
        if ((effective_c_stack_factor > 1) || (effective_r_stack_factor > 1)) {
            result.apply_stack(
                effective_c_stack_factor,
                effective_r_stack_factor,
                this->row_major_stack,
                this->c_stack_padding,
                this->r_stack_padding);
        }
    }
    result.check_producer_tiles_out_of_order(
        this->producer_data_format.num_cores_r,
        this->producer_data_format.num_cores_c,
...
consumer_to_producer_tile_map three_d_array_tile_src_map::get_op_matmul_row_input(
    int kernel_bcast_tiles,
    bool kernel_bcast_tiles_per_t,
    int consumer_t,
    int consumer_ublock_tiles_r,
    int consumer_ublock_tiles_k,
    int consumer_mblock_ublocks_m,
    int consumer_row_input_block_ublocks_k,
    int consumer_num_cores_r,
    int consumer_num_cores_c,
    bool consumer_row_major_ublock_scan_order) {
    ...
    for (int consumer_row = 0; consumer_row < consumer_num_cores_r; consumer_row++) {
        ...
        for (int consumer_row_tile_index = 0; consumer_row_tile_index < consumer_total_tiles_per_row;
            ...
            int consumer_ublock_r = consumer_row_major_ublock_scan_order
                                        ? (consumer_ublock_index / consumer_row_input_block_ublocks_k)
                                        : (consumer_ublock_index % consumer_mblock_ublocks_m);
            int consumer_ublock_c = consumer_row_major_ublock_scan_order
                                        ? (consumer_ublock_index % consumer_row_input_block_ublocks_k)
                                        : (consumer_ublock_index / consumer_mblock_ublocks_m);
                ...
                if (kernel_bcast_tiles_per_t) {
                    if (consumer_row_block_tile_index >= kernel_bcast_tiles) {
                        this->tm_assert(
                            curr_tile_map == result.pipes[pipe_index].tile_map.at(
                                                 (consumer_curr_t * kernel_bcast_tiles) +
                                                 (consumer_row_tile_index % kernel_bcast_tiles)),
                            "input using kernel_broadcast_per_t but post-TM input canonical form is not periodic for t "
                            "= " +
                                std::to_string(consumer_curr_t));
                        continue;
                    ...
                    if (consumer_row_tile_index >= kernel_bcast_tiles) {
                        this->tm_assert(
                            curr_tile_map ==
                                result.pipes[pipe_index].tile_map.at(consumer_row_tile_index % kernel_bcast_tiles),
                            "input using kernel_broadcast but post-TM input canonical form is not periodic");
                        continue;
                    }
                }
    ...
    if (this->need_phased_stack()) {
        ...
        this->check_phased_stack(
            ...
            consumer_ublock_tiles_r,
            consumer_ublock_tiles_k,
            consumer_mblock_ublocks_m,
            consumer_row_input_block_ublocks_k,
            consumer_num_cores_r,
            1,
            consumer_row_major_ublock_scan_order);
...
consumer_to_producer_tile_map three_d_array_tile_src_map::get_embedding_table_input(
    ...
    this->tm_assert(
        consumer_num_cores_r >= this->producer_data_format.num_cores_r &&
    ...
    this->tm_assert(
        this->producer_data_format.mblock_ublocks_n >= core_c_div &&
...
consumer_to_producer_tile_map three_d_array_tile_src_map::get_untilized_input(
    ...
    if (this->producer_data_format.row_major_ublock_scan_order) {
        this->tm_assert(
            this->producer_data_format.mblock_ublocks_m >= core_r_div &&
...
consumer_to_producer_tile_map three_d_array_tile_src_map::get_op_eltwise_input(
    int kernel_bcast_tiles,
    bool kernel_bcast_tiles_per_t,
    int consumer_t,
    int consumer_ublock_tiles_r,
    ...
    this->tm_assert(
        (consumer_data_format.r_size_tiles() == this->dim_size(map_dims::rt)) &&
            (consumer_data_format.c_size_tiles() == this->dim_size(map_dims::ct)) &&
            (consumer_t == this->dim_size(map_dims::t)),
    ...
    for (int consumer_core_r = 0; consumer_core_r < consumer_num_cores_r; consumer_core_r++) {
        for (int consumer_core_c = 0; consumer_core_c < consumer_num_cores_c; consumer_core_c++) {
            ...
            for (int consumer_core_tile_index = 0; consumer_core_tile_index < consumer_total_tiles_per_core;
                 consumer_core_tile_index++) {
                int consumer_curr_input =
                    (consumer_core_tile_index / consumer_data_format.mblock_size_tiles()) / consumer_t;
                int consumer_curr_t =
                    (consumer_core_tile_index / consumer_data_format.mblock_size_tiles()) % consumer_t;
                    ...
                    if (kernel_bcast_tiles_per_t) {
                        if (consumer_mblock_tile_index >= kernel_bcast_tiles) {
                            this->tm_assert(
                                curr_tile_map == result.pipes[pipe_index].tile_map.at(
                                                     (consumer_curr_t * kernel_bcast_tiles) +
                                                     (consumer_core_tile_index % kernel_bcast_tiles)),
                                "input using kernel_broadcast_per_t but post-TM input canonical form is not periodic "
                                "for t = " +
                                    std::to_string(consumer_curr_t));
                            continue;
                        }
                        ...
                        if (consumer_core_tile_index >= kernel_bcast_tiles) {
                            this->tm_assert(
                                curr_tile_map ==
                                    result.pipes[pipe_index].tile_map.at(consumer_core_tile_index % kernel_bcast_tiles),
                                "input using kernel_broadcast but post-TM input canonical form is not periodic");
                            continue;
                        }
                    }
...
three_d_array_tile_src_map::get_prolog_matmul_col_input(
    int kernel_bcast_tiles,
    bool kernel_bcast_tiles_per_t,
    int consumer_t,
    int consumer_ublock_tiles_k,
    ...
    consumer_to_producer_tile_map result(
        ...
        consumer_column_num_chunks,
        consumer_num_cores_c);
    result.scatter_granularity = this->producer_output_buf_size_t *
                                 this->producer_data_format.mblock_size_tiles();  // initialize to maximum granularity

    // Create mapping which is a vector of pipes w/ metadata
    // This is the chunk gather/multicast map
    consumer_to_producer_tile_map result2(
        consumer_column_num_chunks, consumer_num_cores_c, consumer_num_cores_r, consumer_num_cores_c);
    result2.scatter_granularity = num_tiles_per_chunk;  // Will always be contiguous tiles

    tile_to_core_index_map prev_tile_map;

    const int producer_core_output_buf_size_tiles =
        this->producer_data_format.mblock_size_tiles() * this->producer_output_buf_size_t;
    ...
    for (int consumer_col = 0; consumer_col < consumer_num_cores_c; consumer_col++) {
        ...
        for (int consumer_col_tile_index = 0; consumer_col_tile_index < consumer_total_tiles_per_col_post_kernel_bcast;
             consumer_col_tile_index++) {
            ...
            n2p::Log() << "col: " << consumer_col << " tile idx: " << consumer_col_tile_index << " --> "
                       << consumer_in_rt << ", " << consumer_in_ct << std::endl;
            tile_to_core_index_map curr_tile_map =
                this->get_tile_map(consumer_curr_t, consumer_in_rt, consumer_in_ct, consumer_curr_input);
            if (curr_tile_map.tile_index >= producer_core_output_buf_size_tiles) {
                // We've reached past the end of producer core output buffer.
                ...
                if (kernel_bcast_tiles_per_t) {
                    if (curr_t_tile_index >= kernel_bcast_tiles) {
                        this->tm_assert(
                            curr_tile_map ==
                                result.pipes[pipe_index].tile_map.at(
                                    (consumer_curr_t * kernel_bcast_tiles) + (curr_t_tile_index % kernel_bcast_tiles)),
                            "input using kernel_broadcast_per_t but post-TM input canonical form is not periodic for t "
                            "= " +
                                std::to_string(consumer_curr_t));
                        continue;
            ...
            result2.pipes[consumer_col].tile_map.push_back(
                tile_to_core_index_map(consumer_chunk, consumer_col, chunk_index++));
        }
...
void consumer_to_producer_tile_map::check_producer_tiles_out_of_order(
    int producer_grid_r, int producer_grid_c, int producer_mblock_tiles) {
    this->producer_tiles_out_of_order = false;
    std::vector<std::vector<int>> producer_core_next_tile_index;
    std::vector<std::vector<int>> producer_core_dest_pipe_id;

    producer_core_dest_pipe_id.resize(producer_grid_r);
    producer_core_next_tile_index.resize(producer_grid_r);
    for (int r = 0; r < producer_grid_r; r++) {
        producer_core_dest_pipe_id[r].resize(producer_grid_c);
        producer_core_next_tile_index[r].resize(producer_grid_c);
    ...
    for (auto it : this->pipes) {
        ...
        for (tile_to_core_index_map tile_core_index : curr_pipe.tile_map) {
            if (tile_core_index.tile_index < 0) {
                assert(tile_core_index.is_padding_tile);
                this->producer_tiles_out_of_order = true;
                return;
            }
            ...
            if (producer_core_next_tile_index[tile_core_index.core_r][tile_core_index.core_c] !=
                tile_core_index.tile_index) {
                this->producer_tiles_out_of_order = true;
                return;
            }
...
void consumer_to_producer_tile_map::apply_stack(
    int c_stack_factor, int r_stack_factor, bool row_major_stack, int c_stack_padding, int r_stack_padding) {
    ...
    if ((row_major_stack && (c_stack_factor > 1)) || (!row_major_stack && (r_stack_factor == 1))) {
        single_dest_repeat =
            (c_stack_factor > effective_consumer_num_cores_c) ? (c_stack_factor / effective_consumer_num_cores_c) : 1;
        if ((r_stack_factor > 1) && (effective_consumer_num_cores_r == 1) && (effective_consumer_num_cores_c > 1) and
            (r_stack_padding == 0)) {
            dest_period = c_stack_factor;
        }
    } else {
        single_dest_repeat =
            (r_stack_factor > effective_consumer_num_cores_r) ? (r_stack_factor / effective_consumer_num_cores_r) : 1;
        if ((c_stack_factor > 1) && (effective_consumer_num_cores_c == 1) && (effective_consumer_num_cores_r > 1) and
            (c_stack_padding == 0)) {
            dest_period = r_stack_factor;
        }
    }

    bool multi_core_stack = ((r_stack_factor > 1) && (effective_consumer_num_cores_r > 1)) ||
                            ((c_stack_factor > 1) && (effective_consumer_num_cores_c > 1));
    ...
    for (int s = 0; s < total_stack_factor; s++) {
        ...
        for (int p = 0; p < num_pipes; p++) {
            ...
            std::pair<int, int> curr_stack_iter_dest = std::make_pair(curr_stack_iter_dest_r, curr_stack_iter_dest_c);
            ...
            if (include_dest) {
                ...
                bool padded_output =
                    rs >= (r_stack_factor - r_stack_padding) or cs >= (c_stack_factor - c_stack_padding);
    ...
    for (int t = 0; t < this->get_size(map_dims::t); t++) {
        ...
        for (int rt = 0; rt < this->get_size(map_dims::rt); rt++) {
            ...
            for (int ct = 0; ct < this->get_size(map_dims::ct); ct++) {
                printf(
                    "[%2d, %2d -> %4d] ",
                    this->get_tile_map(t, rt, ct).core_r,
                    this->get_tile_map(t, rt, ct).core_c,
                    this->get_tile_map(t, rt, ct).tile_index);
                if (((ct + 1) % this->producer_data_format.mblock_c_size_tiles()) == 0) {
                    printf(" |  ");
                }
            }
...
int consumer_to_producer_tile_map::max_producer_core_fan_out() {
    std::vector<std::vector<std::vector<std::vector<int>>>> producer_consumer_core_adj_map;
    std::vector<std::vector<int>> producer_core_fanout;

    producer_core_fanout.resize(this->producer_num_cores_r);
    producer_consumer_core_adj_map.resize(this->producer_num_cores_r);
    for (int pr = 0; pr < this->producer_num_cores_r; pr++) {
        producer_core_fanout[pr].resize(this->producer_num_cores_c);
        producer_consumer_core_adj_map[pr].resize(this->producer_num_cores_c);
        for (int pc = 0; pc < this->producer_num_cores_c; pc++) {
            producer_core_fanout[pr][pc] = 0;
            producer_consumer_core_adj_map[pr][pc].resize(this->consumer_num_cores_r);
            for (int cr = 0; cr < this->consumer_num_cores_r; cr++) {
                producer_consumer_core_adj_map[pr][pc][cr].resize(this->consumer_num_cores_c);
    ...
    for (auto it : this->pipes) {
        ...
        for (std::pair<int, int> dest_core : pipe.dest_cores) {
            ...
            for (tile_to_core_index_map tile_map_it : pipe.tile_map) {
                if (tile_map_it.is_padding_tile) {
                    continue;
                }
    ...
    for (int pr = 0; pr < this->producer_num_cores_r; pr++) {
        for (int pc = 0; pc < this->producer_num_cores_c; pc++) {
            for (int cr = 0; cr < this->consumer_num_cores_r; cr++) {
                for (int cc = 0; cc < this->consumer_num_cores_c; cc++) {
                    // printf("  --- p %d,%d (%d/%d) -> c %d,%d (%d/%d) -> %d\n", pr, pc, this->producer_num_cores_r,
                    // this->producer_num_cores_c, cr, cc, this->consumer_num_cores_r, this->consumer_num_cores_c,
                    // producer_consumer_core_adj_map[pr][pc][cr][cc]);
...
bool phase_pipe_tile_map::check_tile_map_periodic(
    int consumer_tile_clear_granularity, int& period, int& periodic_repeat) {
...
bool phase_pipe_tile_map::validate_padding() {
    return not std::all_of(
        this->tile_map.begin(), this->tile_map.end(), [](auto& index_map) { return index_map.is_padding_tile; });
}

int consumer_to_producer_tile_map::max_producer_core_phases(
    int consumer_tile_clear_granularity, bool producer_output_single_buffer) {
    ...
    for (auto it : this->pipes) {
        ...
        for (int pr = 0; pr < this->producer_num_cores_r; pr++) {
            for (int pc = 0; pc < this->producer_num_cores_c; pc++) {
                int curr_pipe_producer_core_phases = three_d_array_tile_src_map::int_div_with_ceil(
                    pipe_producer_core_tiles[pr][pc], pipe_scatter_granularity);
                if (!pipe_periodic) {
                    producer_core_phases[pr][pc] += curr_pipe_producer_core_phases;
                } else {
                    producer_core_phases[pr][pc] +=
                        three_d_array_tile_src_map::int_div_with_ceil(curr_pipe_producer_core_phases, periodic_repeat);
                }
            }
        }
    }
...
int consumer_to_producer_tile_map::max_dram_queue_input_indexes(int consumer_tile_clear_granularity) {
    ...
    for (auto it : this->pipes) {
        ...
        int periodic_repeat;
        bool pipe_periodic = pipe.check_tile_map_periodic(consumer_tile_clear_granularity, period, periodic_repeat);
        int curr_pipe_consumer_core_indexes =
            three_d_array_tile_src_map::int_div_with_ceil(pipe_consumer_core_tiles[cr][cc], pipe_scatter_granularity);
        if (!pipe_periodic) {
            consumer_core_indexes[cr][cc] += curr_pipe_consumer_core_indexes;
        } else {
            consumer_core_indexes[cr][cc] +=
                three_d_array_tile_src_map::int_div_with_ceil(curr_pipe_consumer_core_indexes, periodic_repeat);
        }
    }
...
void get_input_reblock_gather(
    OpClass consumer_op_class, int input_id, bool& reblock_r, bool& reblock_c, bool& gather_r, bool& gather_c) {
...
void three_d_array_tile_src_map::estimate_resource_usage(
    OpClass consumer_op_class,
    int input_id,
    int consumer_t,
    // consumer grid & output block dimensions (for eltwise, same as input dimensions)
    int consumer_grid_size_r,
    int consumer_grid_size_c,
    int consumer_mblock_ublocks_m,
    int consumer_mblock_ublocks_n,
    int consumer_ublock_tiles_r,
    int consumer_ublock_tiles_c,
    ...
    int consumer_input_block_size_tiles_r =
        (gather_r ? (consumer_mblock_ublocks_k * consumer_ublock_tiles_k)
                  : (consumer_mblock_ublocks_m * consumer_ublock_tiles_r));
    int consumer_input_block_size_tiles_c =
        (gather_c ? (consumer_mblock_ublocks_k * consumer_ublock_tiles_k)
                  : (consumer_mblock_ublocks_n * consumer_ublock_tiles_c));
    ...
    bool no_reblocking =
        ...
        (this->producer_data_format.mblock_ublocks_m ==
         (consumer_input_block_size_tiles_r / consumer_input_ublock_tiles_r)) &&
        (this->producer_data_format.mblock_ublocks_n ==
         (consumer_input_block_size_tiles_c / consumer_input_ublock_tiles_c)) &&
    ...
    if (this->tm_sequence.size() == 0 && no_reblocking) {
        ...
        int effective_producer_mblock_tiles_r =
            (this->tm_has_transpose ? this->producer_data_format.mblock_c_size_tiles()
                                    : this->producer_data_format.mblock_r_size_tiles()) /
            this->r_slice_factor;
        int effective_producer_mblock_tiles_c =
            (this->tm_has_transpose ? this->producer_data_format.mblock_r_size_tiles()
                                    : this->producer_data_format.mblock_c_size_tiles()) /
            this->c_slice_factor;

        bool ublock_size_preserved = (this->producer_data_format.ublock_tiles_r == consumer_input_ublock_tiles_r) &&
                                     (this->producer_data_format.ublock_tiles_c == consumer_input_ublock_tiles_c);

        // Variables for estimating pipe implementation efficiency
        // (initialized to the most pessimistic value):
        int consumer_min_tiles_from_same_producer_core =
            1;  // How many back-to-back tiles at consumer are received from the same producer core?
        int producer_core_min_contiguous_tiles_sent =
            1;  // How many tiles from each producer core are sent out from contiguous locations?
        ...
        if ((effective_producer_mblock_tiles_c % consumer_input_ublock_tiles_c) == 0) {
            ...
            producer_core_min_contiguous_tiles_sent =
                std::gcd(consumer_input_ublock_tiles_c, this->producer_data_format.ublock_tiles_c);
            if ((effective_producer_mblock_tiles_r % consumer_input_ublock_tiles_r) == 0) {
                ...
                if (consumer_input_ublock_tiles_c == this->producer_data_format.ublock_tiles_c) {
                    producer_core_min_contiguous_tiles_sent *=
                        std::gcd(consumer_input_ublock_tiles_r, this->producer_data_format.ublock_tiles_r);
                }
                // If ublock scan order is the same, we can get contiguous tiles from multiple ublocks
                int cont_consumer_ublocks_r =
                    std::gcd(effective_producer_mblock_tiles_r, consumer_input_block_size_tiles_r) /
                    consumer_input_ublock_tiles_r;
                int cont_consumer_ublocks_c =
                    std::gcd(effective_producer_mblock_tiles_c, consumer_input_block_size_tiles_c) /
                    consumer_input_ublock_tiles_c;
        ...
        int effective_consumer_grid_size_r_reblock =
            tm_has_transpose ? effective_consumer_grid_size_c : effective_consumer_grid_size_r;
        int effective_consumer_grid_size_c_reblock =
            tm_has_transpose ? effective_consumer_grid_size_r : effective_consumer_grid_size_c;
        ...
        if ((this->producer_data_format.num_cores_r % effective_consumer_grid_size_r_reblock) == 0) {
            ...
            reblock_tm_fork_r_factor_int =
                effective_consumer_grid_size_r_reblock / this->producer_data_format.num_cores_r;
        } else {
            ...
            reblock_tm_fork_r_factor_int =
                int_div_with_ceil(effective_consumer_grid_size_r_reblock, this->producer_data_format.num_cores_r) + 1;
        }
        ...
        if ((this->producer_data_format.num_cores_c % effective_consumer_grid_size_c_reblock) == 0) {
            ...
            reblock_tm_fork_c_factor_int =
                effective_consumer_grid_size_c_reblock / this->producer_data_format.num_cores_c;
        } else {
            reblock_tm_fork_c_factor_int =
                int_div_with_ceil(effective_consumer_grid_size_c_reblock, this->producer_data_format.num_cores_c) + 1;
        }
        ...
        int consumer_effective_grid_size =
            (reblock_r ? consumer_grid_size_r : 1) * (reblock_c ? consumer_grid_size_c : 1);
        ...
        producer_max_scatter_stream_phases_used_per_core = this->producer_data_format.mblock_size_tiles() *
                                                           (2 * this->producer_output_buf_size_t) *
                                                           this->c_stack_factor * this->r_stack_factor * bcast_factor;
        ...
        producer_max_scatter_stream_phases_used_per_core = int_div_with_ceil(
            producer_max_scatter_stream_phases_used_per_core, producer_core_min_contiguous_tiles_sent);
        ...
        int gather_pipe_period_tiles =
            this->producer_output_buf_size_t * total_slice_factor * consumer_input_block_size_tiles;

        // Need to multiply with 2 since we're using 2-stage gather for performance.
        consumer_max_gather_stream_phases_used_per_core =
            2 * int_div_with_ceil(gather_pipe_period_tiles, consumer_min_tiles_from_same_producer_core);
        return;
    }
}

void three_d_array_tile_src_map::estimate_dram_input_perf_resource_usage(
    OpClass consumer_op_class,
    int input_id,
    int consumer_t,
    // consumer grid & output block dimensions (for eltwise, same as input dimensions)
    ...
    if (this->tm_sequence.size() == 0 && no_reblocking) {
        ...
        // dimension, we're guaranteed a minimum granularity for both the above variables.
        if ((effective_producer_mblock_tiles_c % consumer_input_ublock_tiles_c) == 0) {
            producer_contiguous_tiles_read =
                std::gcd(consumer_input_ublock_tiles_c, this->producer_data_format.ublock_tiles_c);
            if ((effective_producer_mblock_tiles_r % consumer_input_ublock_tiles_r) == 0) {
                // If the other dimension is also divisible, we get whole consumer ublocks from a single producer
                // buffer. To get contiguous producer-side tiles, we need c dimension to be the same
                if (consumer_input_ublock_tiles_c == this->producer_data_format.ublock_tiles_c) {
                    producer_contiguous_tiles_read *=
                        std::gcd(consumer_input_ublock_tiles_r, this->producer_data_format.ublock_tiles_r);
                }
...
tile_to_core_index_map three_d_array_tile_src_map::get_tile_map(int t, int rt, int ct, int input_index) {
    ...
    if (input_index > 0) {
        ...
        result.tile_index +=
            (input_index * this->producer_data_format.t * this->producer_data_format.mblock_size_tiles());
    }
    return result;
}
