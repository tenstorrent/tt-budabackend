...
std::optional<tt_cxy_pair> choose_valid_pipe_and_buffer_location(const Router &router, pipe_segment_id_t const& pipe_segment_id, unique_id_t buffer_id, chip_id_t chip, const std::vector<tt_cxy_pair> &preferred_cores, const std::unordered_set<tt_cxy_pair> &invalid_cores = {}, bool balanced_mode = false) {
    ...
    for (const auto &c : router.get_soc_descriptor().workers) {
...
void dump_ethernet_core_resources(const Router &router, chip_id_t chip) {
    const auto &soc_descriptor = router.get_soc_descriptor();
...
static void log_ethernet_channel_pair_allocation_failure_messages(
    ...
    auto log_allocation_failure_reason = [&](const tt::buffer_info &buffer_info, const BufferAttributes &buffer_attributes, chip_id_t chip, ethernet_channel_t channel) -> void  {
...
static std::optional<std::pair<ethernet_channel_t, ethernet_channel_t>> get_ethernet_channel_pair_able_to_allocate_ethernet_buffer(
    ...
    auto can_fit_buffer_on_eth_channel = [&](const tt::buffer_info &buffer_info, const BufferAttributes &buffer_attributes, chip_id_t chip, ethernet_channel_t channel) -> bool {
        const tt_xy_pair &core = soc_descriptor.ethernet_cores.at(channel);
        const HwCoreAttributes &core_attributes = cluster_resource_model.get_core_attributes(tt_cxy_pair(chip, core));
    ...
    for (const auto &[sender_channel, receiver_channel] : directly_connected_channels) {
        bool can_add_buffer_to_sender_side = can_fit_buffer_on_eth_channel(sender_buffer_info, sender_buffer_attrs_with_eth, sender_chip, sender_channel);
        log_debug(tt::LogRouter, "Can't add buffer to sender size on channel {}", sender_channel);
        bool can_add_buffer_to_receiver_side = can_fit_buffer_on_eth_channel(receiver_buffer_info, receiver_buffer_attrs_with_eth, receiver_chip, receiver_channel);
    ...
    log_ethernet_channel_pair_allocation_failure_messages(
        directly_connected_channels, cluster_resource_model, sender_chip, sender_buffer_info, sender_buffer_attrs_with_eth, receiver_chip, receiver_buffer_info, receiver_buffer_attrs_with_eth, soc_descriptor
    );
...
void reorder_ethernet_links_by_priority(
    ...
    auto get_channel_allocated_buffer_count = [&](chip_id_t chip, int channel) -> int {
        const tt_xy_pair &core = soc_descriptor.ethernet_cores.at(channel);
        return cluster_resource_model.get_core_attributes(tt_cxy_pair(chip, core)).get_buffer_count();
    };
...
void decompose_chip_to_chip_gather_into_unicast_chip_to_chips(router::Router &router, const ClusterGraph &cluster_graph, unique_id_t pipe_id, std::vector<unique_id_t> &buffers_to_delete) {
    ...
    if (do_gather_on_producer_chip) {
        ...
        TT_ASSERT(!router.get_soc_descriptor().is_ethernet_core(tt_xy_pair(gather_pipe_location.value().x, gather_pipe_location.value().y)));
        ...
        TT_ASSERT(!router.get_soc_descriptor().is_ethernet_core(tt_xy_pair(src_chip_gather_core_location.x, src_chip_gather_core_location.y)));
...
std::vector<std::pair<ethernet_channel_t, ethernet_channel_t>> choose_ethernet_channels_along_route(
    ...
    for (size_t i = 0; i < route.size(); i++) {
	    ...
	    reorder_ethernet_links_by_priority(src_dest_channel_pairs, sender_chip, receiver_chip, router.get_cluster_resource_model(), router.get_soc_descriptor());
...
std::tuple<unique_id_t, unique_id_t> create_eth_link_pipe_and_endpoint_buffers(
    ...
    auto sender_buf_id = sender_side_buffer_already_there ? 
        ...
        generate_eth_link_endpoint_buffer(router, soc_descriptor, eth_buffer_info, sender_chip, sender_chan);
    ...
    auto receiver_buf_id = receiver_side_buffer_already_there ? 
        ...
        generate_eth_link_endpoint_buffer(router, soc_descriptor, eth_buffer_info, receiver_chip, receiver_chan);
...
void create_implicit_buffers_for_ethernet_datacopy_op(
    ...
    if (!buffers_already_directly_connected) {
        ...
        ethernet_channel_t input_buffer_channel =
            soc_descriptor.get_channel_of_ethernet_core(input_buffer.core_location());
        ...
        unique_id_t input_buffer_neighbour_buffer_id = generate_eth_link_endpoint_buffer(
            router, soc_descriptor, input_buffer.info(), input_connected_chip, input_connected_channel);

        ethernet_channel_t unpacker_buffer_channel =
            soc_descriptor.get_channel_of_ethernet_core(tt_xy_pair(unpacker_buffer.core_location().x, unpacker_buffer.core_location().y));
        ...
        unique_id_t unpacker_buffer_neighbour_buffer_id = generate_eth_link_endpoint_buffer(
            router, soc_descriptor, unpacker_buffer.info(), unpacker_connected_chip, unpacker_connected_channel);
...
bool pipe_already_routed_over_ethernet_to_connected_channel(const router::Router &router, const ClusterGraph &cluster_graph, unique_id_t pipe_id) {
    ...
    const tt_xy_pair &input_buffer_xy_location = tt_xy_pair(input_buffer_core_location.x, input_buffer_core_location.y);
    const tt_cxy_pair &output_buffer_core_location = router.get_buffer_location(pipe.output_buffer_ids().at(0));
    const tt_xy_pair &output_buffer_xy_location = tt_xy_pair(output_buffer_core_location.x, output_buffer_core_location.y);
    bool input_buffer_is_on_ethernet_core = router.get_soc_descriptor().is_ethernet_core(tt_xy_pair(input_buffer_core_location.x, input_buffer_core_location.y));
    bool output_buffer_is_on_ethernet_core = router.get_soc_descriptor().is_ethernet_core(tt_xy_pair(output_buffer_core_location.x, output_buffer_core_location.y));
    ...
    const auto input_buffer_eth_channel = router.get_soc_descriptor().get_channel_of_ethernet_core(input_buffer_xy_location);
    const auto output_buffer_eth_channel = router.get_soc_descriptor().get_channel_of_ethernet_core(output_buffer_xy_location);
