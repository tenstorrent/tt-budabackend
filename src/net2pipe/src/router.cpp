...
int compute_pipe_segment_gather_extra_input_buffering_size(
    ...
    bool consumer_is_relay = !consumer_buffer.is_queue() && consumer_buffer.info().type() == RouterBufferType::Relay;
    bool consumer_is_tensix = !consumer_buffer.is_queue() && (!consumer_buffer.core_location_assigned() || router.get_soc_descriptor().is_worker_core({consumer_buffer.core_location().x, consumer_buffer.core_location().y}));
    bool input_from_dram = producer_buffer.is_queue();
...
bool Router::is_ethernet_pipe(unique_id_t pipe_id) const {
    ...
    return soc_desc.is_ethernet_core(tt_xy_pair(input_buffer.core_location().x, input_buffer.core_location().y)) &&
        soc_desc.is_ethernet_core(tt_xy_pair(output_buffer.core_location().x, output_buffer.core_location().y));
}
        ...
        for (const auto &[queue_grid_r, col_operand_index_buffer] : fork_buf_unique_id_map) {
            for (const auto &[queue_grid_c, operand_index_buffer] : col_operand_index_buffer) {
                for (const auto [fork_index, buffer_id] : operand_index_buffer) {
                    ...
                    const auto &core_location = tt_cxy_pair(chip, soc_descriptor.get_routing_core(worker_core));
        ...
        for (const auto &[queue_grid_r, col_operand_index_buffer] : fork_buf_unique_id_map) {
            for (const auto &[queue_grid_c, operand_index_buffer] : col_operand_index_buffer) {
                for (const auto [fork_index, buffer_id] : operand_index_buffer) {
                    ...
                    const tt_xy_pair &worker_core =
                        map_prolog_queue_grid_entry_to_worker_coordinate(queue_grid_r, queue_grid_c, queue_name, consumer_op);
                    int consumer_routing_core_r = soc_descriptor.worker_log_to_routing_y.at(worker_core.y);
                    int consumer_routing_core_c = soc_descriptor.worker_log_to_routing_x.at(worker_core.x);
...
Router::constructor_initialize_pipe_resource_usage() {
    ...
    for (const auto &[pipe_id, pipe]: this->get_pipes()) {
        ...
        if (soc_descriptor.ethernet_cores.size() > 0) {
            if (is_chip_to_chip_pipe(get_pipe(pipe_id), *this)) {
                auto endpoint_cores = collect_pipe_endpoint_cores(*this, pipe_id);
...
Router::Router(
    const buda_SocDescriptor &_soc_descriptor,
    ...
    soc_descriptor(_soc_descriptor),
    chip_ids(_chip_ids),
    cluster_graph(_cluster_graph),

    dram_channels(Router::initialize_dram_core_map(_soc_descriptor)),
    address_map(ArchAddressMap::of(_soc_descriptor.arch)),
    cluster_resource_model(Router::initialize_hw_cores(_soc_descriptor, address_map, _chip_ids)),
    ...
    hronological_unique_id_list(_hronological_list_ids)
{
    ...
    for (auto chip : chip_ids) {
        ...
        ss << "COLUMNS:";
        for (int c = 0; c < soc_descriptor.grid_size.x; c++) {
           ss << "   " << c << "  |";
        }
        log_debug(tt::LogRouter, "{}", ss.str());
        
        for (int r = 0; r < soc_descriptor.grid_size.y; r++) {
            auto ss2 = std::stringstream();
            ss2 << "ROW (" << r << "): ";
            for (int c = 0; c < soc_descriptor.grid_size.x; c++) {
                bool core_defined = soc_descriptor.cores.find(tt_xy_pair(c,r)) != soc_descriptor.cores.end();
...
tt_cxy_pair Router::get_buffer_location_tensix_grid(unique_id_t buffer_id) const {
  ...
  result.chip = buffer_loc_routing.chip;
  result.x = this->get_soc_descriptor().routing_x_to_worker_x.at(buffer_loc_routing.x);
  result.y = this->get_soc_descriptor().routing_y_to_worker_y.at(buffer_loc_routing.y);
  return result;
} 
...
tt_cxy_pair Router::get_pipe_location_tensix_grid(unique_id_t pipe_id, int output_index = 0) const {
  ...
  result.x = this->get_soc_descriptor().routing_x_to_worker_x.at(pipe_loc_routing.x);
  result.y = this->get_soc_descriptor().routing_y_to_worker_y.at(pipe_loc_routing.y);
  return result;
}
...
bool Router::buffer_is_on_ethernet_core(unique_id_t buf_id, chip_id_t target_chip, ethernet_channel_t target_channel) const {
    ...
    const auto &soc_descriptor = this->get_soc_descriptor();
 ...
 */
std::unordered_map<tt_xy_pair, dram_channel_t> Router::initialize_dram_core_map(const buda_SocDescriptor &soc_descriptor) {
    auto dram_channels = std::unordered_map<tt_xy_pair, dram_channel_t>{};
    const int num_channels = soc_descriptor.dram_cores.size();
    for (int channel = 0; channel < num_channels; channel++) {
        const auto &subchannel_cores = soc_descriptor.dram_cores[channel];
 ...
 * management.
 */
std::unordered_map<tt_cxy_pair, std::unique_ptr<CoreResources>> Router::initialize_hw_cores(const buda_SocDescriptor &soc_descriptor, const AddressMap &address_map, const std::vector<chip_id_t> &chip_ids) {
...
int Router::get_noc_distance(int noc_id, int src_y_noc0_routing, int src_x_noc0_routing, int dest_y_noc0_routing, int dest_x_noc0_routing) const {
    return (noc_id == 1) ? get_noc1_distance(src_y_noc0_routing, src_x_noc0_routing, dest_y_noc0_routing, dest_x_noc0_routing)
                         : get_noc0_distance(src_y_noc0_routing, src_x_noc0_routing, dest_y_noc0_routing, dest_x_noc0_routing);
}

int Router::get_noc0_distance(int src_y_noc0_routing, int src_x_noc0_routing, int dest_y_noc0_routing, int dest_x_noc0_routing) const {

  int noc0_x_distance = dest_x_noc0_routing - src_x_noc0_routing;
  if (noc0_x_distance < 0) {
    noc0_x_distance += this->get_soc_descriptor().grid_size.x;
  }
  int noc0_y_distance = dest_y_noc0_routing - src_y_noc0_routing;
  if (noc0_y_distance < 0) {
    noc0_y_distance += this->get_soc_descriptor().grid_size.y;
  }
  return noc0_x_distance + noc0_y_distance;
}


int Router::get_noc1_distance(int src_y_noc0_routing, int src_x_noc0_routing, int dest_y_noc0_routing, int dest_x_noc0_routing) const {
  ...
  int noc1_x_distance = (noc0_x_distance == 0) ? 0 : (this->get_soc_descriptor().grid_size.x - noc0_x_distance);
  int noc1_y_distance = (noc0_y_distance == 0) ? 0 : (this->get_soc_descriptor().grid_size.y - noc0_y_distance);
  return noc1_x_distance + noc1_y_distance;
}
  
  
int Router::get_shortest_path_noc_id(int src_y_noc0_routing, int src_x_noc0_routing, int dest_y_noc0_routing, int dest_x_noc0_routing) const {
  int noc0_hops = get_noc0_distance(src_y_noc0_routing, src_x_noc0_routing, dest_y_noc0_routing, dest_x_noc0_routing);
  int noc1_hops = get_noc1_distance(src_y_noc0_routing, src_x_noc0_routing, dest_y_noc0_routing, dest_x_noc0_routing);
