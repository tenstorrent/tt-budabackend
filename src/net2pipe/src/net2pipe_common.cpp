...
if (get_op_class(op_info) == OpClass::MatMul) {
    return (op_info.attributes.identity) || (op_info.attributes.bias) || (op_info.gradient_op) || (op_info.attributes.requant) || (op_info.attributes.dequant)  ||
            (op_info.attributes.m_k > 1) || op_info.attributes.accumulate;
}
if (get_op_class(op_info) == OpClass::Depthwise){
    return (op_info.attributes.m_k > 1) || (op_info.attributes.bias);
}
...
return (get_op_class(op_info) == OpClass::FusedOp) ? op_info.fused_op_info.num_intermediates : 1;
}
...
return (op_has_output_buf(op_info) && op_has_intermediate_buf(op_info) && !op_info.untilize_output &&
        !op_is_fused(op_info) && (op_info.intermed_data_format == op_info.output_data_format)) ||
