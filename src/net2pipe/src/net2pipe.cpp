...
Net2Pipe::Net2Pipe(
    ...
    const std::string &soc_descriptor_file,
    ...
    this->soc_descriptor = load_soc_descriptor_from_yaml(soc_descriptor_file); // TODO:: Remove this eventually.
...
void Net2Pipe::output_pipes() {
    validate::validate_netlist_fits_on_arch(this->parsed_netlist, *(this->soc_descriptor.get()));
    ...
    for (auto it : this->parsed_netlist.queue_map) {
        ...
        for (int r = 0; r < queue_info.grid_size.r; r++) {
            for (int c = 0; c < queue_info.grid_size.c; c++) {
                ...
                if (queue_info.loc == QUEUE_LOCATION::HOST && (this->soc_descriptor->arch == tt::ARCH::WORMHOLE ||
                                                               this->soc_descriptor->arch == tt::ARCH::WORMHOLE_B0)) {
    ...
    for (const auto& [op_name, op_info] : graph_info.op_map) {
        ...
        for (int input_num = 0; input_num < num_op_inputs; input_num++) {
            ...
            if(is_name_prolog_queue(input_name, epoch_context)) {
                ...
                const auto kernel_in_buf_tiles = get_op_kernel_input_size_tiles(op_info, input_num);
    ...
    for (const auto &[buf_id, buffer] : epoch_context.buffer_map) {
        ...
        if (is_relay_buffer) {
            ...
            auto potential_eth_core_iter = std::find(
                this->soc_descriptor->ethernet_cores.begin(),
                this->soc_descriptor->ethernet_cores.end(),
                tt_xy_pair(routing_c, routing_r));
            bool is_eth_core_relay = potential_eth_core_iter != this->soc_descriptor->ethernet_cores.end();
            ...
            int r = is_eth_core_relay ? routing_r : this->soc_descriptor->routing_y_to_worker_y.at(routing_r);
            int c = is_eth_core_relay ? routing_c : this->soc_descriptor->routing_x_to_worker_x.at(routing_c);
            ...
            if (is_eth_core_relay) {
                size_t ethernet_chan =
                    std::distance(this->soc_descriptor->ethernet_cores.begin(), potential_eth_core_iter);
                TT_ASSERT(ethernet_chan < this->soc_descriptor->ethernet_cores.size(), "Invalid ethernet channel ", ethernet_chan, " doesn't exist for arch");
...
bool Net2Pipe::is_input_adjacent_mcast(std::string producer_name, std::string consumer_name, int& adjacent_noc_id, const temporal_epoch_context& epoch_context) const {
    ...
    if (op_graph_map.at(producer_name).target_device != op_graph_map.at(consumer_name).target_device) {
        return false;
    }
    ...
    bool result = n2p::producer_size_and_placement_for_direct_mcast(this->soc_descriptor->arch,
                                                                    producer_op_info, consumer_op_info,
                                                                    consumer_input_row_mcast, consumer_input_col_mcast,
                                                                    consumer_input_noc1_mcast,
                                                                    this->soc_descriptor->worker_grid_size.x, this->soc_descriptor->worker_grid_size.y,
                                                                    adjacent_noc_id);
            ...
            if (name_is_queue(producer_name)) {
                ...
                for (int op_input_index = 0; op_input_index < op_num_inputs; op_input_index++) {
                    ...
                    if (op_input_name == producer_name) {
                        ...
                        const int op_input_buf_size_tiles = get_op_kernel_input_size_tiles(output_op_info, op_input_index);
...
int Net2Pipe::get_queue_dram_subchannel(std::uint64_t q_buf_id, const temporal_epoch_context& epoch_context) const {
    ...
    int subchannel = std::get<1>(this->soc_descriptor->dram_core_channel_map.at(tt_xy_pair(dram_core.x, dram_core.y)));
    return subchannel;
}
...
void Net2Pipe::emit_queue(
    ...
    // <<  "\n";
    bool ethernet_chip_to_chip_supported = this->soc_descriptor->ethernet_cores.size() > 0;
      ...
      for (int r = 0; r < queue_info.grid_size.r; r++) {
        ...
        for (int c = 0; c < queue_info.grid_size.c; c++) {
          ...
          if (queue_info.loc == QUEUE_LOCATION::HOST) {
            ...
            TT_ASSERT(this->soc_descriptor->arch != tt::ARCH::GRAYSKULL || dram_chan == 0, "Encountered loc:HOST queue in Grayskull with memory ch > 0: not supported.");
            dram_addr += (dram_chan * PEER_REGION_SIZE);

            // Wormhole needs offset into host address for MMIO transactions
            if (this->soc_descriptor->arch == tt::ARCH::WORMHOLE ||
                this->soc_descriptor->arch == tt::ARCH::WORMHOLE_B0) {
              dram_addr += 0x800000000;
            }
...
void Net2Pipe::emit_kernel_bufs(
    ...
    auto get_op_core_routing_cxy = [this](const std::string &op_name, const tt_op_info &op_info, int r, int c, bool is_ethernet_datacopy, int &y_coord, int &x_coord) -> void {
        if (is_ethernet_datacopy) {
            ...
            const tt_xy_pair channel_xy = this->soc_descriptor->ethernet_cores.at(dest_channel);
    ...
    for (int i = 0; i < op_info.grid_size_logical_r(); i++) {
        for (int j = 0; j < op_info.grid_size_logical_c(); j++) {
            ...
            for (int k = 0; k < num_input_bufs; k++) {
                ...
                if (is_ethernet_datacopy) {
                    out << SET_KEY_VAL("ethernet_chan", this->soc_descriptor->get_channel_of_ethernet_core(tt_xy_pair(x_coord, y_coord)));
                }
                out << SET_KEY_VAL(
                    "chip_id",
                    YAML::Flow << YAML::BeginSeq << epoch_context.buffer_map.at(input_unique_id).core_location().chip
                               << YAML::EndSeq);
                out << SET_KEY_VAL("core_coordinates", SET_COORD2(y_coord, x_coord));
            ...
            if (n2p::op_has_output_buf(op_info)) {
                ...
                out << SET_KEY_VAL(
                    "chip_id",
                    YAML::Flow << YAML::BeginSeq << epoch_context.buffer_map.at(output_unique_id).core_location().chip
                               << YAML::EndSeq);
                out << SET_KEY_VAL("core_coordinates", SET_COORD2(y_coord, x_coord));
                out << SET_KEY_VAL("size_tiles", output_size_tiles);
                out << SET_KEY_VAL("scatter_gather_num_tiles", output_scatter_gather_num_tiles);
    ...
    for (int d = 0; d < num_mcast_lines; d++) {
      for (int chunk = 0; chunk < layout.num_chunks_to_gather; chunk++) {
        ...
        const auto &routing_core_coordinates = tt_cxy_pair(
            this->op_graph_map.at(op_name).target_device,
            this->soc_descriptor->worker_log_to_routing_x.at(core_log_x),
            this->soc_descriptor->worker_log_to_routing_y.at(core_log_y)
        );
...
void Net2Pipe::collect_kernel_buf_info(const std::string &op_name, int input_count, temporal_epoch_context& epoch_context) const {
    ...
    auto get_op_core_routing_cxy =
        ...
        if (is_ethernet_datacopy) {
            ...
            return tt_cxy_pair(
                this->op_graph_map.at(op_name).target_device,
                this->soc_descriptor->worker_log_to_routing_x.at(x_coord),
                this->soc_descriptor->worker_log_to_routing_y.at(y_coord));
        }
    };
            ...
            for (int k = 0; k < num_input_bufs; k++) {
                ...
                if (is_ethernet_datacopy) {
                    ...
                    const auto &sender_buffer_location = get_eth_datacopy_sender_chip_relay_buffer_location(
                        i, j, op_info, channels_assigned, *(this->soc_descriptor.get()));
...
void Net2Pipe::get_op_input(
    ...
    size_tiles = get_op_kernel_input_size_tiles(op_info, index);
...
int Net2Pipe::get_op_kernel_input_size_tiles(tt_op_info op_info, int index) const {
...
void Net2Pipe::get_op_output(
    ...
    if ((op_info.dest_accumulate_data_format == tt::DataFormat::Float32) &&
        (this->soc_descriptor->arch == tt::ARCH::GRAYSKULL)) {
      ERROR(std::string("Op ") + op_info.name + " has dest accumulate data format incompatible with arch = " + get_arch_str(this->soc_descriptor->arch));
    }
            ...
            if (op_info.attributes.identity) {
                if (input_num == 0) {
                    ...
                    if(is_name_prolog_queue(op_info.input_names.at(input_num), epoch_context)) {
                        ...
                        int kernel_buf_size = get_op_kernel_input_size_tiles(op_info, input_num);
                        ...
                        log_debug(tt::LogNet2Pipe, "eltwise prolog for op '{}': enabled: '{}', input{}: '{}'; max num tiles prologed: {}, kernel_buf_size: {}",
                         op_name, max_num_tiles <= get_op_kernel_input_size_tiles(op_info, input_num),
                ...
                if (input_num == 0) {
                    ...
                    const int kernel_buf_size = get_op_kernel_input_size_tiles(op_info, input_num);
...
*/
                    ...
                    const bool unroll_and_post_tm_prolog = is_name_prolog_queue(op_info.input_names.at(input_num), epoch_context) and
                                                           max_num_tiles <= get_op_kernel_input_size_tiles(op_info, input_num);
    ...
    for (int input_index = 0; input_index < num_input_bufs; input_index++) {
        ...
        auto pipe_coord_lookup_function = [this](const std::tuple<chip_id_t,int,int> &pipe_coord, const tt_op_info &op_info, bool is_ethernet_datacopy) -> tt_cxy_pair {
            if (is_ethernet_datacopy) {
                ...
                return tt_cxy_pair(
                    std::get<0>(pipe_coord), 
                    this->soc_descriptor->worker_log_to_routing_x.at(std::get<2>(pipe_coord)), 
                    this->soc_descriptor->worker_log_to_routing_y.at(std::get<1>(pipe_coord))
                );
            }
        };
    ...
    for (auto &[pipe_unique_id, pipe] : epoch_context.pipes) {
        ...
        if (!pipe.is_scatter()) {
            ...
            if (not is_ethernet_pipe) {
                if (this->soc_descriptor->routing_y_to_worker_y.find(y) != this->soc_descriptor->routing_y_to_worker_y.end() &&
                    this->soc_descriptor->routing_x_to_worker_x.find(x) != this->soc_descriptor->routing_x_to_worker_x.end()) {
                    y = this->soc_descriptor->routing_y_to_worker_y.at(y);
                    x = this->soc_descriptor->routing_x_to_worker_x.at(x);
                } else {
                    ...
                    TT_ASSERT(this->soc_descriptor->is_ethernet_core(tt_xy_pair(x,y)));
                    ethernet_chan = this->soc_descriptor->get_channel_of_ethernet_core(tt_xy_pair(x,y));
                }
            }
            if (ethernet_chan >= 0) {
                out << SET_KEY_VAL("ethernet_chan", ethernet_chan);
            ...
            const tt_xy_pair &first_consumer_core_xy = tt_xy_pair(pipe.core_locations().at(0).x, pipe.core_locations().at(0).y);
            bool consumer_is_worker_core = this->soc_descriptor->is_worker_core(first_consumer_core_xy);
            TT_ASSERT(consumer_is_worker_core || this->soc_descriptor->is_ethernet_core(first_consumer_core_xy));
            int ethernet_chan = consumer_is_worker_core ? -1 : this->soc_descriptor->get_channel_of_ethernet_core(first_consumer_core_xy);

            int p = 0;
            for (const auto &it : pipe.core_locations()) {
                const tt_xy_pair &core_xy = tt_xy_pair(it.x, it.y);
                bool is_worker_core = this->soc_descriptor->is_worker_core(core_xy);
                ...
                const tt_cxy_pair &mcast_core = tt_cxy_pair(
                    it.chip, 
                    is_worker_core ? this->soc_descriptor->get_worker_core(core_xy) : core_xy
                );

                if (!is_worker_core) {
                    TT_ASSERT(this->soc_descriptor->is_ethernet_core(core_xy));
                    ...
                    TT_ASSERT(ethernet_chan == this->soc_descriptor->get_channel_of_ethernet_core(core_xy), "Net2pipe doesn't currently support pipes that scatter to multiple different ethernet cores");
                }
...
void Net2Pipe::get_op_core_routing_cxy(const temporal_epoch_context& epoch_context, const std::string &op_name, int r_logical, int c_logical, int &y_coord, int &x_coord) const {
    ...
    if (is_ethernet_datacopy) {
        ...
        x_coord = soc_descriptor->worker_log_to_routing_x.at(x_coord);
        y_coord = soc_descriptor->worker_log_to_routing_y.at(y_coord);
    }
};  
        ...
        if (use_last_input_pipe_coords) {
            ...
            if (name_is_op(input_name, epoch_context)) {
              ...
              if (netlist_utils::is_valid_ethernet_op(input_op_info.type)) {
                ...
                const tt_xy_pair &eth_datacopy_egress_core = this->soc_descriptor->ethernet_cores.at(ethernet_channel);
            ...
            if (netlist_utils::is_valid_ethernet_op(op_info.type)) {
                ...
                const tt_xy_pair &eth_datacopy_ingress_core = this->soc_descriptor->ethernet_cores.at(ethernet_channel);
...
void Net2Pipe::run_router(int temporal_epoch, temporal_epoch_context& epoch_context, ClusterDescription cluster_description) const {
    ...
    TT_ASSERT(this->soc_descriptor->ethernet_cores.size() > 0 || epoch_context.buffer_map.size() > 0);
    TT_ASSERT(this->soc_descriptor->ethernet_cores.size() > 0 || epoch_context.pipes.size() > 0);
...
void Net2Pipe::set_ring_start_end_device_ids() {
    ...
    if (this->soc_descriptor->arch != tt::ARCH::WORMHOLE && this->soc_descriptor->arch != tt::ARCH::WORMHOLE_B0) {
