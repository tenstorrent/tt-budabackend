...
uint kernel_main(uint* params) {
    ...
    llk_unpack_A_init<BroadcastType::SRC_B_BROADCAST_TYPE>(TRANSPOSE_XY, TRANSPOSE_XY);
        ...
        for (int b = 0; b < block_cnt; ++b) {
            ...
            for (int r = 0; r < dst_tile_rows; ++r) {
                for (int c = 0; c < dst_tile_cols; ++c) {
                    llk_unpack_A<BroadcastType::SRC_B_BROADCAST_TYPE>(0, r * dst_tile_cols + c, TRANSPOSE_XY);
                }
            }
