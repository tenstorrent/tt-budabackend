...
uint kernel_main(uint* params) {
    ...
    llk_unpack_A_init<BroadcastType::SRC_B_BROADCAST_TYPE, TRANSPOSE_XY, true>();
        ...
        for (int b = 0; b < block_cnt; ++b) {
            ...
            for (int r = 0; r < dst_tile_rows; ++r) {
                for (int c = 0; c < dst_tile_cols; ++c) {
                    for (uint input = 0; input < num_inputs; ++input) {
                        llk_unpack_A<BroadcastType::SRC_B_BROADCAST_TYPE, TRANSPOSE_XY, true>(input, r * dst_tile_cols + c);
                    }
                }
            }
