...
class ckernel_template
{
    ...
    uint
        m_loop0_last_instr; // In the last iteration of the outer loop, this instruction replaces the inner loop instruction, if constructed with one inner loop instruction
                            // or the second inner loop instruction, if constructed with two inner loop instructions.
    uint
...
public:
    ...
    static constexpr uint DEF_ZEROSRCA = TT_OP_UNPACR_NOP(0, p_unpacr::UNP_ZEROSRC);
    static constexpr uint DEF_NINFSRCA = TT_OP_UNPACR_NOP(0, p_unpacr::UNP_NEGINFSRC);
    static constexpr uint DEF_UNPACR_NOP = TT_OP_UNPACR_NOP(0, p_unpacr::UNP_NOP);
