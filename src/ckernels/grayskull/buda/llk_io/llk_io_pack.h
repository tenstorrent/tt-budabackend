...
inline void llk_setup_outputs() {
    ...
    for (std::uint32_t n = 0; n < EPOCH_INFO_PTR->num_outputs; n++) {
        epoch_stream_info_t tt_l1_ptr *l1_outputs = EPOCH_INFO_PTR->outputs[n];
        std::uint32_t stream_id = l1_outputs->stream_id;
        std::uint32_t fifo_addr = l1_outputs->buf_base_addr/MEM_WORD_BYTES;
        std::uint32_t fifo_size = l1_outputs->buf_full_size_bytes/MEM_WORD_BYTES;
        std::uint32_t fifo_size_tiles = l1_outputs->buf_size_tiles;
        ...
        std::uint8_t legacy_pack = l1_outputs->legacy_pack;
        std::uint32_t dram_output_no_push = (l1_outputs->flags & STREAM_DRAM_NO_PUSH) != 0 ? 1 : 0;
        ...
        if (is_intermediate_operand) {
           std::uint8_t packer_operand = l1_outputs->packer_operand;
        ...
        std::uint16_t num_fork_streams = l1_outputs->num_fork_streams;
...
#pragma GCC unroll 0
            for (std::uint32_t k = 0; k < num_fork_streams; k++) {
                volatile epoch_stream_info_t tt_l1_ptr * fork_stream_info = EPOCH_INFO_PTR->active_streams[l1_outputs->fork_idxs[k]];
    ...
    if (is_output_operand_decoupled(OPERAND_OUTPUT_START_INDEX, overlay_output_decouple_mask)) {
...
inline void llk_wait_for_free_tiles(const std::int32_t operand, const std::int32_t num_tiles) {
    ...
    if constexpr (wait_for_blocks) {
        num_words = num_tiles * GET_L1_TILE_SIZE<true>((uint)pack_dst_format[output]);
    }
    ...
    if constexpr (push_blocks) {
        ...
        if (outputs[output].f.block_tile_cnt < outputs[output].f.block_tile_dim) {
          return; //row of ublocks not ready yet
           ...
           num_words = num_tiles_in_block * GET_L1_TILE_SIZE<true>((uint)pack_dst_format[output]);
