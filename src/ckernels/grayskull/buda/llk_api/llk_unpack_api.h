...
template <std::uint32_t operandA_kernel_broadcast=0, std::uint32_t operandB_kernel_broadcast=0 /*not used*/>
...
inline void llk_unpack_AB_matmul(
    ...
    for (std::uint32_t rt=0; rt<rt_dim; rt++) {
        ...
        if constexpr (operandA_kernel_broadcast > 0) {
            offset_address_a = MUL_TILE_SIZE_AND_INDEX(unpA_src_format, ((tile_index_a + rt*kt_dim)%operandA_kernel_broadcast));
        }
        std::uint32_t address_a = base_address_a + offset_address_a;
        ...
        for (std::uint32_t ct=0; ct<ct_dim; ct++) {
            ...
            if constexpr (operandB_kernel_broadcast > 0) {
                offset_address_b = MUL_TILE_SIZE_AND_INDEX(unpB_src_format, ((tile_index_b+ct)%operandB_kernel_broadcast));
            }
            std::uint32_t address_b = base_address_b + offset_address_b;
