...
#include "ctest_db_pack_params.h"
#include "llk_pack.h"
...
#define OUT_OF_ORDER_OUTPUT false
...
uint kernel_main(uint* params) {
    ...
    llk_pack_init<UNTILIZE_EN, ZERO_OUTPUT, DstTileFaceLayout::RowMajor>(16);
    ...
    llk_pack_dest_init<DstSync::SyncHalf, DstTileFaceLayout::RowMajor, UNTILIZE_EN, FP32_DEST_ACC_EN>();
        ...
        for (int b = 0; b < block_cnt; ++b) {
            ...
            for (int r = 0; r < dst_tile_rows; ++r) {
                for (int c = 0; c < dst_tile_cols; ++c) {
                    llk_pack<OUT_OF_ORDER_OUTPUT, DstSync::SyncHalf, UNTILIZE_EN, FP32_DEST_ACC_EN>(r * dst_tile_cols + c, 16);
                }
            }
            if (not ZERO_OUTPUT) {
                llk_pack_dest_section_done<DstSync::SyncHalf, FP32_DEST_ACC_EN>();
            }
