...
inline void llk_unpack_AB_hw_configure(const llk_unpack_AB_params_t *unpack_AB_params, const int within_face_16x16_transpose) {
    constexpr bool is_row_pool = false;
    constexpr bool transpose_xy_srca = within_face_16x16_transpose;
...
inline void llk_unpack_AB_hw_configure_disaggregated(
    ...
    llk_unpack_AB_hw_configure<is_fp32_dest_acc_en>(&unpack_AB_params, within_face_16x16_transpose);
}
...
inline void llk_unpack_AB_init(const std::uint32_t unpA_operand, const std::uint32_t unpB_operand, const std::uint32_t transpose=0, const std::uint32_t acc_to_dest=0) {
    llk_unpack_AB_mop_config<BType>(transpose>0);
