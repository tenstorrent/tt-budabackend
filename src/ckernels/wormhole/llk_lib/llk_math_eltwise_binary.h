...
inline void llk_math_eltwise_binary_impl(const std::uint32_t num_faces_A, const std::uint32_t num_faces_B, uint dst_index, bool clear_dest_acc) {
...
inline void llk_math_eltwise_binary(uint dst_index, bool clear_dest_acc = false) {
    llk_math_eltwise_binary_impl<eltwise_binary_type, src_b_bcast_type, Dst, NUM_FIDELITY_PHASES, binary_reuse_dest, is_fp32_dest_acc_en>(4, 4, dst_index, clear_dest_acc);
}
...
template <
    ...
    EltwiseBinaryReuseDestType binary_reuse_dest = EltwiseBinaryReuseDestType::NONE,
    bool is_fp32_dest_acc_en = false>
inline void llk_math_eltwise_binary(const std::uint32_t operand_A, const std::uint32_t operand_B, uint dst_index, bool clear_dest_acc = false) {
    // Todo: get num_faces based on operands
    llk_math_eltwise_binary_impl<eltwise_binary_type, src_b_bcast_type, Dst, NUM_FIDELITY_PHASES, binary_reuse_dest, is_fp32_dest_acc_en>(4, 4, dst_index, clear_dest_acc);
}
...
inline void llk_math_eltwise_binary_init_impl(const std::uint32_t num_faces_A, const std::uint32_t num_faces_B, const std::uint32_t transpose, const std::uint32_t acc_to_dest) {
    ...
    if constexpr (
        (eltwise_binary_type == ELWADD) || (eltwise_binary_type == ELWSUB) || (eltwise_binary_type == ELWMUL)) {
        eltwise_binary_configure_mop<eltwise_binary_type, src_b_bcast_type, 0, binary_reuse_dest>();
    } else {
        FWASSERT("Unsupported op!", false);
    }
