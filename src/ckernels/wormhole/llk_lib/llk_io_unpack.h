...
inline void llk_wait_tiles(int operand, std::int32_t num_tiles) {
    ...
    if (record_perf_events && perf_events_target_idx == 1 && operand <= 1) {
        num_tiles_unpack[operand] += num_tiles_u;
    }
    ...
    if (wait_for_tile_en) {
        ...
        if (num_tiles_recv < num_tiles_u) {
            ...
            uint leave_space = perf_events_target_idx == 1 ? 9 : 3; 
            record_timestamp_64b(event_id, leave_space); // Leave space for first-unpack-instruction and tile count op0, tile count op1 for first input
            do {
                tiles_received = (std::uint16_t) reg_read_barrier((std::uint32_t)tiles_received_ptr);
                uint16_t num_tiles_recv = tiles_received - operands[input].f.tiles_acked;
