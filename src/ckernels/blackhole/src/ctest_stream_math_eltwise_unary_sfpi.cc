...
uint kernel_main(uint* params) {
    ...
    llk_math_pack_sync_init<SyncTile2, FP32_DEST_ACC_EN>();
        ...
        for (int b = 0; b < block_cnt; ++b) {
            for (int r = 0; r < dst_tile_rows; ++r) {
                for (int c = 0; c < dst_tile_cols; ++c) {
                    ...
                    llk_math_eltwise_unary_datacopy<MATH_DATACOPY_OP, BroadcastType::SRC_B_BROADCAST_TYPE, SyncTile2, FP32_DEST_ACC_EN>(
                        0);
                    ...
                    } else if constexpr (SfpiTestType::SFPU_TYPE == SfpiTestType::test15) {
                        llk_math_eltwise_unary_sfpi_test15<SyncTile2>(0);
                    } else if constexpr (SfpiTestType::SFPU_TYPE == SfpiTestType::test16) {
                        llk_math_eltwise_unary_sfpi_test16<SyncTile2>(0);
                    } else if constexpr (SfpiTestType::SFPU_TYPE == SfpiTestType::test17) {
                        llk_math_eltwise_unary_sfpi_test17<SyncTile2>(0);
                    } else if constexpr (SfpiTestType::SFPU_TYPE == SfpiTestType::test18) {
                        llk_math_eltwise_unary_sfpi_test18<SyncTile2>(0);
                    } else if constexpr (SfpiTestType::SFPU_TYPE == SfpiTestType::test19) {
                        llk_math_eltwise_unary_sfpi_test19<SyncTile2>(0);
                    } else {
                        static_assert_sfpi_type_dependent<SfpiTestType::SFPU_TYPE>();
                    }

                    llk_math_dest_section_done<SyncTile2, FP32_DEST_ACC_EN>();
                }
            }
        }
    }
