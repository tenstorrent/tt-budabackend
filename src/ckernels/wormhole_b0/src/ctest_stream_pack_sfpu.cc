...
#include "ctest_stream_pack_sfpu_params.h"
...
uint kernel_main(uint* params) {
    ...
    llk_pack_dest_init<DstSync::SYNC_TYPE, DstTileFaceLayout::RowMajor, UNTILIZE_EN, FP32_DEST_ACC_EN>();
        ...
        for (int b = 0; b < block_cnt; ++b) {
            for (int r = 0; r < dst_tile_rows; ++r) {
                for (int c = 0; c < dst_tile_cols; ++c) {
                    ...
                    llk_pack<false, DstSync::SYNC_TYPE, UNTILIZE_EN, FP32_DEST_ACC_EN>(0, 16);
