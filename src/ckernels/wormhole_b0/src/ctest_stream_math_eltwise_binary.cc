...
#include "ctest_stream_math_eltwise_binary_params.h"
...
uint kernel_main(uint* params) {
    ...
    llk_math_eltwise_binary_init<EltwiseBinaryType::ELTWISE_BINARY_TYPE, BroadcastType::SRC_B_BROADCAST_TYPE, HF, ACC_TO_DEST>();
    llk_math_pack_sync_init<SyncTile16, FP32_DEST_ACC_EN>();
        ...
        for (int b = 0; b < block_cnt; ++b) {
            for (int r = 0; r < dst_tile_rows; ++r) {
                for (int c = 0; c < dst_tile_cols; ++c) {
                    ...
                    llk_math_eltwise_binary<EltwiseBinaryType::ELTWISE_BINARY_TYPE, BroadcastType::SRC_B_BROADCAST_TYPE, SyncTile16, HF, ACC_TO_DEST, FP32_DEST_ACC_EN>(0);
                    llk_math_dest_section_done<SyncTile16, FP32_DEST_ACC_EN>();
                }
            }
        }
    }
