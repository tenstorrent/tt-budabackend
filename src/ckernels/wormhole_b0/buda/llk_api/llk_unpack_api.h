...
template <std::uint32_t operandA_kernel_broadcast=0, std::uint32_t operandB_kernel_broadcast=0>
inline void llk_unpack_AB_matmul_mop_config(const bool transpose, const std::uint32_t ct_dim, const std::uint32_t rt_dim, const std::uint32_t kt_dim, const bool unpA_partial_face, const bool unpB_partial_face) {
    // in0 - loaded to SrcB
    // in1 - loaded to SrcA
    _llk_unpack_AB_matmul_mop_config_<operandA_kernel_broadcast, operandB_kernel_broadcast>(
        transpose, 
        ct_dim, 
        rt_dim, 
        kt_dim, 
...
__attribute__((always_inline)) inline void llk_unpack_AB_matmul_init(const std::uint32_t operandA, const std::uint32_t operandB, const std::uint32_t transpose=0, const std::uint32_t ct_dim=1, const std::uint32_t rt_dim=1, const std::uint32_t kt_dim=1) {
    ...
    const uint32_t unpA_num_faces = unpA_partial_face ? 1 : get_operand_num_faces(operandA_id);
    const uint32_t unpB_num_faces = unpB_partial_face ? 1 : get_operand_num_faces(operandB_id); // if partial face -> unpack face by face

    _llk_unpack_AB_matmul_init_<operandA_kernel_broadcast, operandB_kernel_broadcast>(
        transpose, 
        ct_dim, 
        rt_dim, 
        kt_dim, 
...
inline void llk_unpack_AB_matmul(
    ...
    _llk_unpack_AB_matmul_<operandA_kernel_broadcast, operandB_kernel_broadcast>(
        base_address_a, 
        base_address_b, 
        tile_index_a, 
        tile_index_b, 
...
inline void llk_unpack_thread_halt() {
    TT_LLK_DUMP("llk_unpack_thread_halt()");
    dbg_thread_halt<ThreadId::UnpackThreadId>();
}

inline void llk_unpack_thread_unhalt() {
    TT_LLK_DUMP("llk_unpack_thread_unhalt()");
    dbg_thread_unhalt<ThreadId::UnpackThreadId>();
}

inline void llk_unpack_dump_hw_cfg() {
    for (std::uint32_t n=0; n<dbg_cfgreg::HW_CFG_SIZE; n++) {
        std::uint32_t val = dbg_read_cfgreg(dbg_cfgreg::HW_CFG_0, n);
        TT_LLK_DUMP("CFGREG[{}] = {}", n, val);
    }
}

inline void llk_unpack_dump_thread_cfg() {
    for (std::uint32_t n=0; n<dbg_cfgreg::THREAD_CFG_SIZE; n++) {
        std::uint32_t val = dbg_read_cfgreg(ThreadId::UnpackThreadId, n);
        TT_LLK_DUMP("THREAD[{}][{}] = {}", ThreadId::UnpackThreadId, n, val);
    }
}
