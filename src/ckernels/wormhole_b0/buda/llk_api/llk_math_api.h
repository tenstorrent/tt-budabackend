...
template <DataCopyType type, BroadcastType src_b_bcast_type = BroadcastType::NONE, bool tilize = false /*unused*/, bool is_fp32_dest_acc_en = false, bool is_int_fpu_en = false>
...
inline void llk_math_eltwise_unary_datacopy_init(const std::uint32_t transpose_of_faces=0 /*unused*/, const std::uint32_t within_face_16x16_transpose=0 /* unused */, const std::uint32_t operand = 0) {
    TT_LLK_DUMP("llk_math_eltwise_unary_datacopy_init<{}, {}, {}, {}, {}>({}, {}, {})", type, src_b_bcast_type, is_fp32_dest_acc_en, is_int_fpu_en, transpose_of_faces, within_face_16x16_transpose, operand);
    const std::uint32_t operand_id = get_operand_id(operand);
...
inline void llk_math_calculate_sfpu(const int iterations = ITERATIONS, uint param0 = 0, uint param1 = 0, uint param2 = 0, uint param3 = 0, uint param4 = 0, uint param5 = 0)
{
    if constexpr (operation == SfpuType::exponential) {
        ...
        sfpu::_bitonic_topk_phases_steps<APPROXIMATION_MODE, ITERATIONS>(param0, param1, param2, param3, param4);
    }
    else if constexpr (operation == SfpuType::topk_merge) {
        sfpu::_bitonic_topk_merge<APPROXIMATION_MODE, ITERATIONS>(param0, param1);
    }
    else if constexpr (operation == SfpuType::topk_rebuild) {
        sfpu::_bitonic_topk_rebuild<APPROXIMATION_MODE, ITERATIONS>(param0, param1, param2, param3, param4);
    }
}
...
template <bool APPROXIMATE>
inline void llk_math_eltwise_unary_sfpu_topk_init(const uint operand) {
    llk_math_eltwise_unary_sfpu_init<SfpuType::topk_local_sort, APPROXIMATE>(operand);
}
...
inline void llk_math_dump_srcb_row(const std::uint32_t row_addr) {
    uint32_t row_data[8];
    for (int n=0; n<2; n++) {
        dbg_get_array_row(dbg_array_id::SRCB, 2*row_addr+n, row_data);
        for (int i=0; i<8; i++) {
            TT_LLK_DUMP("SRC_B[{}][{}] = 0x{:x}", row_addr, n*16+2*i, LOWER_HALFWORD(row_data[i]));
            TT_LLK_DUMP("SRC_B[{}][{}] = 0x{:x}", row_addr, n*16+2*i+1, UPPER_HALFWORD(row_data[i]));
        }
    }
}
