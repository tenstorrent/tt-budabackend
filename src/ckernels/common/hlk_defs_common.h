...
template<SfpuOp sfpu_op>
TT_HLK_ALWAYS_INLINE void hlk_unary_sfpu_op(tt_core* core_ptr, int stream, int dst_index, int dim, unsigned int sfpu_specific_param_0, unsigned int sfpu_specific_param_1) {
    SFPU_THREAD(llk_math_eltwise_unary_sfpu<get_sfpu_type_from_sfpu_op<sfpu_op>(), APPROX, DST_SYNC, IS_INT_FPU_EN>(stream, dst_index, dim, sfpu_specific_param_0, sfpu_specific_param_1));
}
