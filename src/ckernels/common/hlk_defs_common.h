...
constexpr bool IS_TILIZE_INPUT_EN = false;
...
TT_HLK_ALWAYS_INLINE void hlk_tilize_init_short(tt_core* core_ptr, int stream, int ct_dim) {
    ...
    MATH(llk_math_eltwise_unary_datacopy_init<A2D, BroadcastType::NONE, IS_TILIZE_INPUT_EN, IS_FP32_DEST_ACC_EN, IS_INT_FPU_EN>(0, 0, 0));
}
...
TT_HLK_ALWAYS_INLINE void hlk_mm_block(
    ...
    int inner_d_dim) {
    UNPACK_IF_NOT_MATH_DECOUPLED(
        llk_unpack_AB_matmul<lstream_kernel_broadcast, rstream_kernel_broadcast>(lstream, rstream, lindex, rindex, inner_c_dim, inner_r_dim, inner_d_dim));
...
TT_HLK_ALWAYS_INLINE void hlk_mm_tile(
    ...
    UNPACK_IF_NOT_MATH_DECOUPLED(llk_unpack_AB_matmul<lstream_kernel_broadcast, rstream_kernel_broadcast>(lstream, rstream, lindex, rindex, inner_c_dim));
...
void setup_kernel() {
    ...
    PACK(llk_pack_init<IS_UNTILIZE_OUTPUT_EN, false, MATMUL_AND_KERNEL_SETUP_DST_FACE_LAYOUT, true, IS_TILIZE_INPUT_EN>(16));
