...
void generate_all_fw(
    ...
    if (!compile_for_perf_only) {
        ...
        if (device_name == "wormhole" || device_name == "wormhole_b0") {
...
void generate_brisc_fw(const perf::PerfDesc& perf_desc, const std::string& arch_name, const std::string& out_dir_path) {
    ...
    if (is_default_brisc_bin && fs::exists(brisc_build_path)) {
        ...
        log_info(tt::LogCompileTrisc,"Using Default BRISC Bin");
        ...
        if (!loaded_brisc_bin) {
            log_info(tt::LogCompileTrisc, "Compiling BRISC firmware");
...
void generate_erisc_fw(const perf::PerfDesc& perf_desc, const std::string& arch_name, const std::string& out_dir_path) {
    ...
    uint32_t perf_dump_level = perf_desc.perf_dump_level;
    bool is_overlay_decoupled = is_perf_dump_en && perf_desc.overlay_decouplings.size() > 0;
    bool use_default_erisc_fw = default_epoch_q_num_slots && firmware_num_loop_iterations == 0;
    force_create_build_dir(erisc_build_dir);
    if (use_default_erisc_fw) {
        log_info(tt::LogCompileTrisc, "Using Default ERISC Bin");
        fs::path erisc_build_path(root + "build/src/firmware/riscv/targets/erisc_app/out/");
        ...
        log_info(tt::LogCompileTrisc, "Compiling ERISC firmware");
...
void generate_ncrisc_fw(
    ...
    bool is_default_ncrisc_bin = !is_perf_dump_en && !is_perf_spill_dram && perf_dump_level == 0 &&
                                 ...
                                 firmware_num_loop_iterations == 0 && !is_overlay_decoupled;
    ...
    if (is_default_ncrisc_bin && fs::exists(ncrisc_build_path)) {
        log_info(tt::LogCompileTrisc, "Using Default NCRISC Bin");
        ...
        log_info(tt::LogCompileTrisc, "Compiling NCRISC firmware");
        ...
        log_assert(
            (arch_name.compare("grayskull") == 0) || (arch_name.compare("wormhole") == 0) ||
            (arch_name.compare("wormhole_b0") == 0), "Invalid arch name");
...
UniqueCompileConfigMap get_unique_compilation_configs(
    ...
    for (auto&& [graph_name, graph] : graphs) {
        ...
        for (const std::pair<const string, tt_op_info>& op_it : graph.my_graph_info.op_map) {
            if (netlist_utils::is_non_tensix_op(op_it.second.type)) {
                log_debug(tt::LogCompileTrisc, "Skipping non-tensix op: {}", op_it.first);
                op_index++;
                continue;
            }
            if (compile_for_perf_only) {
    ...
    bool dump = true;
    if (dump) {
        log_debug(tt::LogCompileTrisc, "Unique Compilation Instances:");
        uint32_t count = 0;
        for (auto& it : compilation_map) {
            log_debug(tt::LogCompileTrisc, "[{}]: {} -> ", count, it.first);
            for (auto& e : it.second) {
                log_debug(tt::LogCompileTrisc, "{}, ", e);
            }
            log_debug(tt::LogCompileTrisc, "\n");
            count++;
        }
    }
...
void compile_for_op(
    ...
    log_debug(tt::LogCompileTrisc, "Compiling Op {}", op_info);
    log_debug(tt::LogCompileTrisc, "   index: {} Grid shape: {}", op_index, op->get_grid_shape());
...
void print_placement(const netlist_workload_data& workload, const string& build_dir_path) {
    ...
    for (const auto& graph_it : workload.graphs) {
        ...
        log_debug(tt::LogCompileTrisc, "Core utilization for each op type:");
...
void generate_trisc_fw(
    ...
    log_info(tt::LogCompileTrisc, "Compiling TRISC kernels");
    ...
    log_info(tt::LogCompileTrisc, "Compiling TRISC kernels, Done!");
}
...
void compile_ckernels_for_trisc(
    ...
    log_debug(tt::LogCompileTrisc, "Make compile cmd: {}", compile_cmd);
    ...
    log_debug(tt::LogCompileTrisc, "Make link cmd: {}", link_cmd);
...
void generate_perf_events_target_inputs(
    ...
    log_debug(tt::LogRuntime, "Recording performance trace for input indices: {}", fmt::join(target_inputs, ", "));
...
bool try_load_fw_bin(
    ...
    if (fs::exists((fs::path(hex_output)))) {
        log_info(tt::LogCompileTrisc, "Found {} cached bin at {}! Loading...", risc_name, load_bin_dir);
...
void try_dump_fw_bin(const string& risc_name, const string& dump_bin_dir, const string& fw_out_dir) {
    ...
    if (fs::exists(dump_bin_dir_path) && (!fs::is_directory(dump_bin_dir_path) || !fs::is_empty(dump_bin_dir_path))) {
        log_info(tt::LogCompileTrisc, "Skipping {} bin dump, {} is not empty.", risc_name, dump_bin_dir);
    } else {
        log_info(tt::LogCompileTrisc, "Dumping {} bin to {}...", risc_name, dump_bin_dir);
