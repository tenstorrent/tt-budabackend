...
void golden_digraph::add_all_nodes(std::map<string, tt_queue_wrap>& queues, tt_graph_info& graph_info) {
    ...
    log_debug(tt::LogGolden, "All op nodes created for graph: {}", graph_info.name);
    // add all queue nodes
    for (auto& q_wrap_it : queues) {
        insert_queue_node(&q_wrap_it.second);  // q pointer null
    }
    log_debug(tt::LogGolden, "All queue nodes created for graph: {}", graph_info.name);
}
    ...
    for (unsigned int i = 0; i < topo_order.size(); ++i) {
        ...
        if (graph[topo_order[i]].op_not_queue) {
            ...
            for (unsigned int j = 0; j < input_nodes.size(); ++j) {
                ...
                log_debug(tt::LogGolden, "in {} tensor dims {}", j, tm_output_tensors[j].get_shape());
            }
                ...
                for (const auto& tm : it.second) {
                    ...
                    if(tm_name == "pad") {
                        ...
                        log_assert(
                            ...
                            "Can only fo a full_broadcast if it is the bias input of the matmul");
                        log_assert(
                            (config.op != TmOp::TileBroadcast) or
                                ((input == 1) and
                                (netlist_utils::is_valid_binary_op(graph[topo_order[i]].my_op_info_ptr->type))),
                            "Can only fo a tile_broadcast if it is the second input of a binary op");
                        log_debug(tt::LogGolden, "Running TM OP {} on input {}", tm_name, input);
            ...
            if (graph[topo_order[i]].my_op_info_ptr->gradient_op) {
                if (graph[topo_order[i]].my_golden_output_ptr != nullptr) {
                    ...
                    log_debug(tt::LogGolden, "Using previous accumulator for op={}", op_ptr->name);
                    log_assert(
            ...
            if (graph[topo_order[i]].my_op_info_ptr->gradient_op) {
                ...
                if (op_acc_output_tensor_ptr != nullptr) {
                    if (output_queue.my_io->zero) {
                        log_debug(tt::LogGolden, "Zeroing accumulator for op={}", op_ptr->name);
            ...
            if (q_info.type == IO_TYPE::Queue) {
                log_debug(tt::LogGolden, "Queue: {}", q_info.name);
                log_debug(tt::LogGolden, "Queue.local_rd_ptr(): {}", q_wrap->my_io->get_local_rd_ptr());
                log_debug(tt::LogGolden, "Queue.global_rd_ptr(): {}", q_wrap->my_io->get_global_rd_ptr());
                log_debug(tt::LogGolden, "Queue.entries(): {}", q_wrap->my_io->entries);
            } else {
                log_debug(tt::LogGolden, "RAM: {}", q_info.name);
                log_debug(tt::LogGolden, "ram.global_wr_ptr(): {}", q_wrap->my_io->get_global_wr_ptr());
                log_debug(tt::LogGolden, "ram.global_rd_ptr(): {}", q_wrap->my_io->get_global_rd_ptr());
                log_debug(tt::LogGolden, "ram.entries(): {}", q_wrap->my_io->entries);
            }
                ...
                if (q_info.input_tm_ops.size() == 0) {
                    ...
                    for (const auto& tm : q_info.input_tm_ops[0]) {
                        ...
                        if(tm_name == "pad") {
                            ...
                            log_debug(tt::LogGolden, "Running TM OP {}", tm_name);
                            //
                            // Create input tensor vector for TM op
