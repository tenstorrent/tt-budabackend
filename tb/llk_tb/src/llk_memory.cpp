...
memory memory::from_discontiguous_risc_hex(std::istream &in, memory::risc_type_t risc_type) {
    ...
    switch (risc_type) {
        ...
        case NCRISC:
            ...
            break;
        case BRISC: is_brisc = true; break;
        default: throw runtime_error("Invalid risc core type.");
    }
    ...
    read_discontiguous_hex_file(in, [&](memory::address_t word_addr, memory::word_t value) {
        if (result.size() == 0) {  // first call
            ...
            if ((is_trisc || is_ncrisc) &&
                (((word_addr << 2) & 0xfff00000) == l1_mem::address_map::RISC_LOCAL_MEM_BASE)) {
                word_addr = (word_addr & ((l1_mem::address_map::MAX_L1_LOADING_SIZE >> 2) - 1)) +
                            risc_l1_local_memory_base_addr;
            } else if ((is_ncrisc) && (((word_addr << 2) & 0xfff00000) == l1_mem::address_map::NCRISC_IRAM_MEM_BASE)) {
                word_addr = (word_addr & ((l1_mem::address_map::MAX_L1_LOADING_SIZE >> 2) - 1)) +
                            (l1_mem::address_map::NCRISC_FIRMWARE_BASE >> 2);
            }
            if (is_ncrisc) {
                // For ncrisc, first block of hex data is L1 resident and goes @ address 0x9000. But the base address
                // for ncrisc hex image in L1 has to be 0x5000 which maps to l1_mem::address_map::NCRISC_FIRMWARE_BASE
                // and is 16 KB in size from 0x5000 to 0x8FFF in L1. Followed by L1 residend ncrisc code @ 0x9000 in L1.
        ...
        if ((is_trisc || is_ncrisc) && (((word_addr << 2) & 0xfff00000) == l1_mem::address_map::RISC_LOCAL_MEM_BASE)) {
            word_addr =
                (word_addr & ((l1_mem::address_map::MAX_L1_LOADING_SIZE >> 2) - 1)) + risc_l1_local_memory_base_addr;
        } else if ((is_ncrisc) && (((word_addr << 2) & 0xfff00000) == l1_mem::address_map::NCRISC_IRAM_MEM_BASE)) {
            word_addr = (word_addr & ((l1_mem::address_map::MAX_L1_LOADING_SIZE >> 2) - 1)) +
                        (l1_mem::address_map::NCRISC_FIRMWARE_BASE >> 2);
        }
...
void memory::write_hex(std::ostream &os, bool write_address_line) const {
    discontiguous_hex_file_writer writer(os);

    writer.add(m_base / sizeof(word_t), m_content.begin(), m_content.end(), write_address_line);
}
