...
void tests::multi_in::generate_stimulus_and_expected_tensors(
    ...
    for (uint n = 0; n < 4; n++) {
        ...
        input0[n].populate_with_uniform_distribution_data(
            ...
            input_seed);
        // input0[n].populate_with_debug_data();
    ...
    // Add randomization
    llk::Tensor input0_0;
    input0_0 = Tensor(test_config.tensor_configs["output0"].dims, test_config.tensor_configs["input0"].data_format);

    // From ctest_params_template/ctest_unpack_A_multi_in_params.h
    uint input_operand[] = {0, 1, 2, 3, 0, 1, 2, 3};
    uint input_tile_index[] = {0, 0, 0, 0, 1, 1, 1, 1};
        ...
        for (auto z = 0; z < input0_0.tile_tensor[w].size(); z++) {
            for (auto ct = 0; ct < input0_0.tile_tensor[w][z].size(); ct++) {
                for (auto rt = 0; rt < input0_0.tile_tensor[w][z][ct].size(); rt++) {
                    input0_0.tile_tensor[w][z][ct][rt] =
                        input0[input_operand[z]].tile_tensor[w][input_tile_index[z]][ct][rt];
                }
            }
        }
    }
...
void tests::multi_in::update_kernel_parameter_mapping(TestConfig &config) {
    xy_pair coord = llk::xy_pair(0, 0);  // TODO: Handle multi-core scenario? but up to user

    // Pull information from config.
    string num_tiles = to_string(
        config.tensor_configs["input0"].dims.num_tiles() + config.tensor_configs["input1"].dims.num_tiles() +
        config.tensor_configs["input2"].dims.num_tiles() + config.tensor_configs["input3"].dims.num_tiles());
    ...
    string num_input0_tiles = to_string(config.tensor_configs["input0"].dims.num_tiles());
    string num_input1_tiles = to_string(config.tensor_configs["input1"].dims.num_tiles());
    string num_input2_tiles = to_string(config.tensor_configs["input2"].dims.num_tiles());
    string num_input3_tiles = to_string(config.tensor_configs["input3"].dims.num_tiles());
    ...
    update_kernel_parameters(config, coord, KernelType::UNPACK, "__num_output_tiles__", num_tiles);
    update_kernel_parameters(config, coord, KernelType::UNPACK, "__num_input0_tiles__", num_input0_tiles);
    update_kernel_parameters(config, coord, KernelType::UNPACK, "__num_input1_tiles__", num_input1_tiles);
    update_kernel_parameters(config, coord, KernelType::UNPACK, "__num_input2_tiles__", num_input2_tiles);
    update_kernel_parameters(config, coord, KernelType::UNPACK, "__num_input3_tiles__", num_input3_tiles);
...
map<string, TestConfig> tests::multi_in::generate_test_configs(
 ...
 */
std::map<std::string, bool> tests::multi_in::test_main(TestArgs args) {
    // Basic test infra instantiation
    string test_name = "multi_in";
    Device device;
