// SPDX-FileCopyrightText: © 2024 Tenstorrent AI ULC
//
// SPDX-License-Identifier: Apache-2.0
#pragma once
#include "test_args.h"
#include "test_config.h"
#include "yaml-cpp/yaml.h"

namespace tests {
namespace multi_out {
void generate_stimulus_and_expected_tensors(
    tests::TestConfig test_config,
    llk::Tensor &input0,
    llk::Tensor &expected_output0,
    llk::Tensor &expected_output1,
    llk::Tensor &expected_output2,
    llk::Tensor &expected_output3);
void update_kernel_parameter_mapping(tests::TestConfig &config, std::vector<std::map<string, int>> output_tile_configs);
std::map<std::string, tests::TestConfig> generate_test_configs(
    tests::TestArgs args, std::string test_name, YAML::Node &test_descriptor);
std::map<std::string, bool> test_main(tests::TestArgs args);
}  // namespace multi_out
}  // namespace tests
