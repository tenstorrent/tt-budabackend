...
void tests::tilize::generate_stimulus_and_expected_tensors(
...
void tests::tilize::update_kernel_parameter_mapping(TestConfig &config) {
    ...
    int tile_size = 2048;
    if (config.tensor_configs["input0"].data_format == DataFormat::Float32) {
       tile_size = 4096;
    }
    
    update_overlay_config(config.overlay_config, "input0_data_buf_tile_size", to_string(config.tensor_configs["input0"].dims.rt*tile_size));
    update_overlay_config(config.overlay_config, "num_input0_data_buf_msgs", to_string(config.tensor_configs["input0"].dims.ct));
    ...
    update_kernel_parameters(config, coord, KernelType::UNPACK, "__block_c_dim__", to_string(32*config.tensor_configs["input0"].dims.rt));
...
map<string, TestConfig> tests::tilize::generate_test_configs(
 ...
 */
std::map<std::string, bool> tests::tilize::test_main(TestArgs args) {
    // Basic test infra instantiation
    string test_name = "tilize";
    Device device;
    ...
    for (auto const &it : test_configs) {
        ...
        try {
            ...
            // Write Inputs
            device.write_tensor(input0, target_coord, tensor_address_map[input0.name], llk::xy_pair(1, 1), true);
            device.run();
