...
struct test_address_map {
    ...
    static constexpr std::int32_t EXTRA_TILE_HEADER_BUFFER_SIZE =
        2 * 32 *
        1024;  // This is enough space for all 3 data types (bfp8, fp16, fp32). If we use more this should be increased.

    static constexpr std::int32_t INPUT_DATA_BASE =
        l1_mem::address_map::DATA_BUFFER_SPACE_BASE + EXTRA_TILE_HEADER_BUFFER_SIZE;
