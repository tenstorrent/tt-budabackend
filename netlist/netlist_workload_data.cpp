...
const std::vector<std::string> &netlist_workload_data::get_programs() {
    static std::vector<std::string> program_vector = {};
    if (!program_vector.size()) {
        for (const auto& prog_md : programs) {
            program_vector.push_back(prog_md.first);
        }
    }
...
void netlist_workload_data::parse_queue_producer_info() {
    ...
    for (auto &queue_it : queues) {
        ...
        if (!netlist_parser::is_queue_fed_by_op(queue_info)) {
            ...
            prod_info.is_tilized = queue_info.layout == IO_LAYOUT::Tilized;
