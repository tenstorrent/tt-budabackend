...
void netlist_program::set_variable(const string& variable_string, int value, VARIABLE_TYPE type) {
    ...
    if (not has_variable(variable_string)) {
        ...
        log_debug(tt::LogNetlist, "set_variable variable={} to value={}", variable_string, value);
...
void netlist_program::set_params(const std::map<std::string, std::string> &parameters) {
    for (const auto &parameter_it : parameters) {
        log_debug(tt::LogNetlist, "set_parameter parameter={} to value={}", parameter_it.first, parameter_it.second);
...
void netlist_program::operator++() {
    ...
    if (current_instruction.opcode == INSTRUCTION_OPCODE::EndLoop) {
        ...
        if (loop++) {
            ...
            loop_stack.pop_back();
            program_counter++;
            log_debug(tt::LogNetlist, "Loop Done: PC={}", program_counter);
        }
    } else {
        program_counter++;
    }
...
void netlist_program::run_instruction_with_callbacks(
    ...
    log_debug(tt::LogNetlist, "Running {}", instrn);
    ...
    if (instrn.opcode == INSTRUCTION_OPCODE::Var) {
            ...
            if (this->ignore_runtime_parameters) {
                ...
                if (this->parameters.find(variable_string) != this->parameters.end()) {
                    if (is_immediate(parameters.at(variable_string))) {
                        ...
                        log_debug(tt::LogNetlist, "Program {} param {}={}", this->name, variable_string, param_value);
        ...
        if (instrn.varinst_opcode == VAR_INSTRUCTION_OPCODE::IncWrap) {
            ...
            log_debug(tt::LogNetlist, "IncWrap: {}=({}+{})\%{}", result, old_val, increment, mod_wrap);
        ...
        log_debug(tt::LogNetlist, "AllocateQueue: {}", fmt::join(queues, ", "));
        ...
        log_debug(tt::LogNetlist, "DeallocateQueue: {}", fmt::join(queues, ", "));
        ...
        log_debug(tt::LogNetlist, "Loop Start: PC={} loop_count:{} = {}", program_counter, instrn.loop_count, loop_count);
        if (loop_count < 1) {
