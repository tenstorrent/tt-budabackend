        ...
        if (tm_it.Type() == YAML::NodeType::Scalar) {
            ...
            if (tm_name == "transpose") {
                if (consumer_type == "matmul") {
                    ...
                    transpose ^= true;
                } else if (consumer_type == "nop" or consumer_type == "datacopy" or consumer_type == "buffer" or is_valid_sfpu_op(consumer_type)) {
                    transpose ^= true;
                } else if (
                    (relative_input_index == 0) and is_valid_binary_op(consumer_type) &&
                    !is_valid_binary_quantisation_op(consumer_type) && !binary_int32_add) {
...
void netlist_parser::parse_tm_ops(const YAML::Node &tms, const string &tm_key, tt_op_info &op_info) {
    bool is_int32_binary_add = false;
    if (netlist_utils::is_valid_binary_op(op_info.type) && BinaryOp::Add == netlist_utils::get_valid_binary_op(op_info.type)) {
        for (const auto &input_df : op_info.input_data_formats) {
            if (input_df == DataFormat::Int32) {
                is_int32_binary_add = true;
                break;
            }
        }
    }
...
void netlist_parser::parse_file(string file) {
    log_info(tt::LogNetlist, "Parsing Netlist from file: {}", file);
    try {
...
void netlist_parser::verify_queues() {
    ...
    log_debug(tt::LogNetlist, "Queues Parsed In Netlist");
    for (auto it : queue_map) {
        log_debug(tt::LogNetlist, "Queue {}: \n{}", it.first, it.second);
        try {
            verify(it.second);
        } catch (const std::exception &e) {
            log_error("{}", e.what());
...
void netlist_parser::verify_graphs() {
    ...
    log_debug(tt::LogNetlist, "Graph Info Parsed In Netlist:");
...
void netlist_parser::verify_programs() {
    ...
    log_debug(tt::LogNetlist, "Program Info Parsed In Netlist:");
    for (auto it : program_map) {
        log_debug(tt::LogNetlist, "Program {}: \n{}", it.first, it.second);
        try {
            verify(it.second);
        } catch (const std::exception &e) {
            log_error("{}", e.what());
...
void netlist_parser::verify_fused_ops() {
    log_debug(tt::LogNetlist, "Fused Ops Parsed In Netlist:");
    for (auto it : fused_ops_op_map) {
        log_debug(tt::LogNetlist, "Fused Ops {}: \n{}", it.first, it.second);
        try {
                ...
                for (int i = 0; i < op_it.second.input_names.size(); i++) {
                    ...
                    for (auto &schedule : fused_op_info.schedules) {
                        for (auto &scheduled_op : schedule.scheduled_ops) {
                            ...
                            if (scheduled_op.type == "matmul") {
                                if (buff_size_scaler < scheduled_op.m_k) {
                                    buff_size_scaler = scheduled_op.m_k;
                                }
                            }
                        }
                    }
...
void netlist_parser::verify_complex_settings() {
    ...
    for (const auto &graph_it : graph_map) {
        for (const auto& op_it : graph_it.second.op_map) {
            ...
            bool is_dense_matmul = netlist_utils::is_valid_matmul_op(op_info.type) && !op_info.attributes.identity;
            bool is_identity_matmul = netlist_utils::is_valid_matmul_op(op_info.type) && op_info.attributes.identity;
            ...
            for (int i = 0; i < op_info.input_tile_dims.size(); i++) {
                ...
                switch (input_tile_dim) {
                    case TileDim::Dim1x32: {
                        log_assert(
                            (is_dense_matmul && (i == 0 || ((i == 2) && op_info.attributes.bias))) || is_unary_or_binary,
                            "TileDim 1x32 is supported on dense matmul on input0 or input 2 (with bias attribute) or "
                    ...
                    case TileDim::Dim8x32: {
                        log_assert(
                            (is_dense_matmul && (i == 0)) || is_unary_or_binary,
                            "TileDims 2x32, 4x32, 8x32 and 16x32 are supported on dense matmul on input0 or on "
                            "datacopy or on sfpu or on binary ops. Tried to use it on op: name {} type {} input index "
                            "{}.",
                            op_info.name,
                            op_info.type,
                            i);
                    ...
                    case TileDim::Dim16x32: {
                        log_assert(
                            (is_dense_matmul && (i == 0 || i == 1)) || is_unary_or_binary,
                            "TileDim 16x32 is supported on dense matmul on inputs 0 and 1, or on datacopy or on sfpu "
                            "or on binary ops. Tried to use it on op: name {} type {} input index {}.",
                            op_info.name,
                            op_info.type,
                            i);
                        break;
                    ...
                    case TileDim::Dim32x16: {
                        log_assert(
                            (is_identity_matmul && (i == 1)) || (is_dense_matmul && (i == 0)) || is_unary_or_binary,
                            "TileDim 32x16 is supported on identity matmul input1, on dense matmul input 0 or on "
                            "datacopy or on sfpu or on binary ops. Tried to use it on op: name {} type {} input index "
                            "{}.",
                            op_info.name,
                            op_info.type,
            ...
            if (is_dense_matmul) {
                // In case of dense matmul, output can't be 32x16
                log_assert(
                    op_info.output_tile_dim != TileDim::Dim32x16,
                    "In case of dense matmul, op output tile dims can't be 32x16. Op name: {}",
                    op_info.name);
            ...
            if (is_identity_matmul) {
                ...
                if (input_1_tile_dims == TileDim::Dim32x16) {
                    log_assert(
                        ...
                        "In case of identity matmul, if input1 has 32x16 tile dims, output must be 32x16. Op name: {} "
                        "Output tile dims: {}",
                        op_info.name,
                        get_string(output_tile_dim));
                }
                ...
                if (output_tile_dim == TileDim::Dim32x16) {
                    log_assert(
                        ...
                        "In case of identity matmul, if output tile dims are 32x16, input 1 tile dims must be 32x16. "
    ...
    for (const auto &graph_it : graph_map) {
        for (const auto &op_it : graph_it.second.op_map) {
            ...
            if (op_info.attributes.l1_acc) {
                log_assert(device_info.arch == tt::ARCH::WORMHOLE_B0,
            ...
            for (const auto input_format : op_info.input_data_formats) {
                if ((input_format == DataFormat::Int8) || (input_format == DataFormat::Int32)) {
                    is_input_int = true;
                    break;
                }
            }
            ...
            if (is_input_int) {
                log_assert(
                    op_info.output_data_format == DataFormat::Int8 || op_info.output_data_format == DataFormat::Int32 || 
                        op_info.attributes.dequant || is_dequant || is_fused_op,
                    "If Int8 or Int32 format is used, out data format must be Int8 or Int32 if dequant is not enabled, "
                    "output format = {} is not Int8/Int32",
                    op_info.output_data_format);
                ...
                log_assert(
                    ...
                    "intermed_data_format = {} is not Int8/Int32",
                ...
                if (is_valid_binary_op(op_info.type) && !(is_quant || is_requant || is_dequant)) {
                    ...
                    for (DataFormat input_format : op_info.input_data_formats) {
                        if (op_type == BinaryOp::Add) {
                            log_assert(
                                input_format == DataFormat::Int8 || input_format == DataFormat::Int32,
                                "Add can have only Int8 or Int32 inputs for integer version of this op input = {} is "
                                "not Int8/Int32",
                                input_format);
                        }
                        else {
                            log_assert(
                                ...
                                "Sub/Mul can have only Int8 inputs for integer version of these ops input = {} is not "
                                "Int8",
                                input_format);
                        }
                    }
                    ...
                    for (DataFormat input_format : op_info.input_data_formats) {
                        log_assert(
                            input_format == DataFormat::Int8 || input_format == DataFormat::Int32 ||
                                input_format == DataFormat::RawUInt32 || is_fused_op || is_requant || is_dequant ||
                                op_info.attributes.dequant || op_info.attributes.requant,
                            "If Int8 or Int32 format is used, all input data formats in op must be Int8 or Int32, input = "
                            "{} is not Int8/Int32",
                            input_format);
                    }
                }
                ...
                log_assert(
                    op_info.math_fidelity == MathFidelity::HiFi4, "If Int8 or Int32 is used, fidelity must be Hifi4");

                log_assert(
                    op_info.dest_accumulate_data_format == DataFormat::Int32,
                    "If Int8 or Int32 is used, dest accumulation format (acc_df) must be Int32");

                bool is_matmul = is_valid_matmul_op(op_info.type);
                bool is_ident_matmul = op_info.attributes.identity;
                if (is_matmul) {
                    log_assert(
                        op_info.input_data_formats[0] == DataFormat::Int8,
                        "Matmul with int inputs must have input 0 in Int8 format.");
                    log_assert(
                        op_info.input_data_formats[1] == DataFormat::Int8,
                    ...
                    if (op_info.attributes.dequant || op_info.attributes.requant) {
                        int scalars_input_index = op_info.attributes.bias ? 3 : 4;
                        log_assert(
