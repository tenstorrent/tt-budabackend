...
TopKSort netlist_utils::get_topk_sort_enum(string sort) {
    if (sort == "max") {
        return TopKSort::Max;
    } else if (sort == "argmax") {
        return TopKSort::ArgMax;
    } else {
...
TopKOp netlist_utils::get_topk_op(string topk_op) {
    TopKOp op = TopKOp::Invalid;
    if (topk_op == "topk") {
        op = TopKOp::TopK;
    }
    return op;
...
TopKOp netlist_utils::get_valid_topk_op(string topk_op) {
    TopKOp op = get_topk_op(topk_op);
    if (op == TopKOp::Invalid)
        log_fatal("Topk Op={} not supported in netlist", topk_op);
    return op;
}
...
bool netlist_utils::is_valid_topk_op(string topk) {
    TopKOp op = get_topk_op(topk);
    return (op != TopKOp::Invalid);
}
...
bool netlist_utils::is_int_format(DataFormat df) {
    return (df == DataFormat::Int8 || df == DataFormat::UInt8 || df == DataFormat::Int32 || df == DataFormat::UInt16);
...
std::shared_ptr<tt_op> netlist_utils::create_op(
    ...
    log_trace(tt::LogNetlist, "create_op: {}", op_info_ptr->type);
    ...
    } else if (is_valid_topk_op(op_info_ptr->type)) {
        new_tt_op = std::static_pointer_cast<tt_op>(std::make_shared<tt_topk_op>(
            op_info_ptr->name,                          // string name
            op_info_ptr->type,                          // string type
            grid_shape,                                 // tt_grid_shape grid_shape
            grid_loc,                                   // tt_grid_shape grid_loc
