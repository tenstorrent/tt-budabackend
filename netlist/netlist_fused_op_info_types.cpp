...
void verify(const tt_scheduled_op_info& t, tt::ARCH arch, const tt_fused_op_info& fused_op_info, bool last_op_in_schedule) {
    ...
    if (netlist_utils::is_valid_matmul_op(t.type)) {
        log_assert(
            t.output_dim.mblock_n == 1,
            "matmul op {} cannot have {} as input 1, matmul input 1 has be a column matrix of ubloks",
            t.name,
            t.input_names[1]);
                ...
                if (is_input(t.input_names[input_index])) {
                    if (netlist_utils::get_unary_op(t.type) == UnaryOp::Datacopy ||
                        ...
                        log_assert(
                            ...
                            "In int fused ops, add and datacopy sub op inputs have to be Int8 or Int32");
                    } else {
                        log_assert(
    ...
    for (const auto& schedule_info : t.schedules) {
        for (const auto& scheduled_op_info : schedule_info.scheduled_ops) {
            for (const auto& [input, tm_ops] : scheduled_op_info.input_tm_ops) {
                ...
                // lamdba function to get input dimension and assert if input does not exist
                auto get_input_dim = [&input_dim_info, &input_name]() -> const tt_dim_info& {
                    auto it = input_dim_info.find(input_name);
                    log_assert(it != input_dim_info.end(), "Input {} not found", input_name);
                    return it->second;
                };

                std::unordered_map<TmOp, std::vector<int>> tm_op_map;
                for (const auto& [tm_name, params] : tm_ops) {
                    TmOp tm_op = netlist_utils::get_tm_op(tm_name);
                    tm_op_map.insert({tm_op, params});
                }

                log_assert(
                    tm_op_map.find(TmOp::zBroadcast) == tm_op_map.end(),
                    "Fused op does not support internal z broadcast");

                if (tm_op_map.find(TmOp::rBroadcast) != tm_op_map.end() ||
                    tm_op_map.find(TmOp::cBroadcast) != tm_op_map.end()) {
                    log_assert(
                ...
                if (tm_op_map.find(TmOp::rBroadcast) != tm_op_map.end()) {
                    ...
                    const auto& input_dim = get_input_dim();
                    log_assert(
                ...
                if (tm_op_map.find(TmOp::cBroadcast) != tm_op_map.end()) {
                    const auto& input_dim = get_input_dim();
                ...
                if (netlist_utils::is_valid_matmul_op(scheduled_op_info.type)) {
                    log_assert(
                        tm_op_map.find(TmOp::rBroadcast) == tm_op_map.end(),
                        "Feature gap (fused_ops) - internal R broadcast not supported for matmul");
                }
...
void verify(const tt_fused_op_info& t, tt::ARCH arch) {
    ...
    if (is_int_fused_op) {
        if (t.schedules.back().scheduled_ops.back().type == "dequantization") {
            log_assert(
                t.output_data_format == DataFormat::Float32,  "If last sub op is quantization, output must be FP32");
        } else {
            log_assert(
