...
typedef struct {
  uint16_t  training_bit_status;     // post code, see dram_status_bit_e
  uint8_t   error_bit_status;        // bit0/1 reserved for read/writing training fail, bit 2-7 for all other error code, see dram_error_code_e
  uint8_t   channel_status;          // see dram_status_e in defines.h
...
typedef struct {
  ...
  uint32_t  gddr6_wr;
  uint32_t  gddr6_rl;
  uint32_t  gddr6_crcrl;
  uint32_t  gddr6_crcwl;
  uint32_t  spare[10];
} dram_phy_timing_t; // 15 * 4 = 60B
...
typedef struct {
  ...
  uint32_t  pcie_acpi_en     : 1;
  uint32_t  reg_hot_en       : 1;
  uint32_t  pwr_brake_en     : 1;
  uint32_t  bb_debug_en      : 1;
...
typedef struct {
  ...
  uint16_t  curr_arcaxi_state;                    // 0 = low, 1 == med, 2 == high
  uint16_t  targ_arcaxi_state;                    // 0 = low, 1 == med, 2 == high
  ...
  uint16_t  efuse_vf_init;
  uint16_t  spipmon_vf_init;
  uint16_t  voltage_at_fmax;                      // this is the Max frequency specified by that particular product, provided in from the SPIROM
  uint16_t  frequency_at_product_vmax;            // this is the Max voltage specified by that particular proudct, provided in from the SPIROM
  uint16_t  frequency_0p95;                       // this will be used to determine BOOST
  uint16_t  aiclk_boost_en;
  uint16_t  frequency_at_inflection;
  uint16_t  spare1;
  uint32_t  voltage_switch_time;
  uint32_t  spare[60-26-ARB_COUNT*2];             // This structure needs to sum up to 60 for now until we remove all hardcoded indexing from the infrastructure specifically related to ARC Mailbox
} aiclk_ppm_t; // 60 * 4 = 240B

typedef struct {
  uint32_t  curr_fan;
  uint32_t  spare[3-1];
} fan_ctrl_ppm_t; // 3 * 4 = 12B
...
typedef struct {
  ...
  uint16_t  pmon[17];
  uint16_t  idd;
  uint32_t  harvesting_ovr;
  uint8_t   harvesting_ovr_en;
  uint8_t   ft_stage[3];
  uint8_t   ft1_exit_code[3];
  uint8_t   ft2_exit_code[3];
  int16_t   frequency_margin;        // Stored as Mhz as an integer
  uint32_t  ft_version[4];
  float     aiclk_step_size;
  uint16_t  ddr_frequency;
  uint16_t  spare1;
  uint8_t   max_pcie_speed;
  uint8_t   spare2[3];
  uint32_t  ft_build_date;           // 0xYYYYMMDD
  uint32_t  rom_patch_num;
} spi_table_t; // 107 * 4 = 428B

typedef union {
  uint32_t     val[sizeof(spi_table_t)/sizeof(uint32_t)];
  spi_table_t  f;
} spi_table_u; // 107 * 4 = 428B
...
typedef struct {
  ...
  uint32_t  pcie_error_count;
  uint8_t   bb_debug_level_flag[6];
  uint16_t  bb_debug_index;
  uint16_t  pcie_mutex_acquire;
  uint8_t   pcie_dstate;
  uint8_t   spare0;
  uint32_t  spare[12];
} debug_t; // 58 * 4 = 232B
...
} gddr_vendor_id_t; // 1 * 4 = 4B

typedef struct {
  uint32_t  gddr_temperature;
} gddr_temperature_t; // 1 * 4 = 4B
...
typedef struct {
  ...
  fan_ctrl_ppm_t               fan_ctrl_ppm;                            // 12B
  tmon_t                       tmon;                                    // 40B
  ...
  spi_table_u                  spi_table;                               // 428B
  uint32_t                     arc_state;                               // 4B
  dram_feature_enable_bit_u    dram_feature_enable_bit;                 // 4B
  VFData                       vf_data;                                 // 40B
  dram_phy_timing_t            dram_phy_timing;                         // 60B
  dram_ac_timing_t             dram_ac_timing;                          // 120B
  gddr_vendor_id_t             dram_vendor_id[NUM_DRAM_INSTANCE];       // 24B
  gddr_temperature_t           dram_temperature[NUM_DRAM_INSTANCE];     // 24B
} mcore_mb_t; // 708 * 4 = 2832B

typedef union {
  uint32_t    val[sizeof(mcore_mb_t)/sizeof(uint32_t)];
  mcore_mb_t  f;
} mcore_mb_u; // 708 * 4 = 2832B
