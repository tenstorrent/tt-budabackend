...
TT_HLK_ALWAYS_INLINE void hlk_setup_kernel(tt_core* core_ptr, const hlk_args_t *args) {
    ...
    if constexpr (kernel_broadcast[1]) {
        ...
        inner_c_dim = args->num_tiles_per_n_sub_block;
    }
    int inner_r_dim;
    if constexpr (kernel_broadcast[0] | kernel_broadcast[1]) {
        inner_r_dim = 1;
    } else {
        inner_r_dim = args->num_tiles_per_m_sub_block;
    }
            ...
            if (min_in1_buffer) {
                ...
                for (int out_c = 0; out_c < outer_c; out_c++) {
                    ...
                    for (int out_r = 0; out_r < outer_r; out_r++) {
                        ...
                        for (int in_d = 0; in_d < inner_d; in_d++) {
                            ...
                            for (int in_r = 0; in_r < in_r_size; in_r++) {
                                int in0_index = out_r*inner_r*inner_d + in_d + (kernel_broadcast[1] ? in_r*inner_d : 0);
                                int in1_index = in_d*inner_c;
                                if constexpr (kernel_broadcast[0]) {
                                    in0_index = (oid*inner_r*inner_d*outer_r + out_r*inner_r*inner_d + in_r*inner_d + in_d)%kernel_broadcast[0];
                                } 
                                for (int in_c = 0; in_c < (kernel_broadcast[1] ? inner_c : 1); in_c++) {
                                    if constexpr (kernel_broadcast[1]) {
                                        in1_index = (oid*inner_c*inner_d*outer_c + out_c*inner_c*inner_d + in_d*inner_c + in_c)%kernel_broadcast[1];
                                    }
                                    hlk_mm_block(
                                        core_ptr, 
                                        HlkOperand::in0, 
                                        ...
                                        dst_index, 
                                        args->transpose,
                                        kernel_broadcast[1] ? 1 : inner_c,
                                        (kernel_broadcast[0] | kernel_broadcast[1])  ? 1 : inner_r,
                                        inner_d);

                                    dst_index+=((kernel_broadcast[1])? 1 : inner_c);    
                                }        
                            }
                        }
                ...
                for (int out_r = 0; out_r < outer_r; out_r++) {
                    ...
                    for (int out_c = 0; out_c < outer_c; out_c++) {
                        ...
                        if (enable_reload) {
                            ...
                            if (!l1_acc) {
                                ...
                                hlk_mm_block_init_short(core_ptr,
                                                        ...
                                                        HlkOperand::in1,
                                                        args->transpose, 
                                                        kernel_broadcast[1] ? 1 : args->num_tiles_per_n_sub_block, 
                                                        (kernel_broadcast[0] | kernel_broadcast[1])  ? 1 : args->num_tiles_per_m_sub_block, 
                                                        args->block_tile_dim);
                                hlk_reconfig_unpacker_df(core_ptr, HlkOperand::intermed0, HlkOperand::in1, HlkOperand::in0, HlkOperand::in0); //reconfig df for src A register
                            }    
                            hlk_pop_tiles(core_ptr, HlkOperand::intermed0, out_block_tile_cnt);
                        ...
                        for (int in_d = 0; in_d < inner_d; in_d++) {
                            int dst_index = 0;
                            for (int in_r = 0; in_r <  ((kernel_broadcast[0] or kernel_broadcast[1]) ? inner_r : 1); in_r++) {
                                int in0_index = (min_in0_buffer ? 0 : out_r*inner_r*inner_d) + in_d + (kernel_broadcast[1] ? in_r*inner_d : 0);
                                ...
                                for (int in_c = 0; in_c < (kernel_broadcast[1] ? inner_c : 1); in_c++) {
                                    ...
                                    hlk_mm_block(
                                        ...
                                        args->transpose,
                                        kernel_broadcast[1] ? 1 : inner_c,
                                        (kernel_broadcast[0] or kernel_broadcast[1])  ? 1 : inner_r,
                                        inner_d);
        ...
        if (bias) {
            ...
            if (args->adv_features_en != 0) {
                hlk_mm_block_init_short(core_ptr,
                        ...
                        args->transpose,
                        kernel_broadcast[1] ? 1 : args->num_tiles_per_n_sub_block,
                        (kernel_broadcast[0] or kernel_broadcast[1])  ? 1 : args->num_tiles_per_m_sub_block, 
                        args->block_tile_dim);
            } else {
