...
#include "hlks/inc/quantization_common.h"
    ...
    for (int batch = 0; batch < args->batch_cnt; batch++) {
                    ...
                    for (int in_d = 0; in_d < inner_d; in_d++) {
                        ...
                        for (int in_r = 0; in_r <  ((kernel_broadcast[0] or kernel_broadcast[1]) ? inner_r : 1); in_r++) {
                            int in0_index = (min_in0_buffer ? 0 : out_r * inner_r * inner_d) + in_d +
                                            (kernel_broadcast[1] ? in_r * inner_d : 0);
        ...
        if (requant) {
            ...
            for (int block = 0; block < out_block_cnt; block++) {
                ...
                quantization_ops_inner_loop<BinaryOp::Requantization>(
                    core_ptr,
            ...
            for (int block = 0; block < out_block_cnt; block++) {
                ...
                quantization_ops_inner_loop<BinaryOp::Dequantization>(
                    core_ptr,
                    HlkOperand::intermed0,
                    dq_operand,
                    HlkOperand::out0,
