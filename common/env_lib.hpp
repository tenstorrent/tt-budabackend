...
inline std::string buda_home(std::string default_home = "./") {
    std::string buda_home = parse_env<std::string>("BUDA_HOME", default_home);
    // some users rely on blackslash being present
    if (buda_home.back() != '/') {
        buda_home += "/";
    }
    return buda_home;
