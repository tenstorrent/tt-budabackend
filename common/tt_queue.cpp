...
void tt_queue::push(tt_queue::queue_entry in) {
    ...
    log_trace(tt::LogBackend, "Pushing entry {} to {}", global_wr_ptr, name);
...
void tt_queue::pop() {
    ...
    log_trace(tt::LogBackend, "Popping entry {} from {}", global_rd_ptr, name);
    if (global_rd_ptr == (2 * entries)) {
        global_rd_ptr = 0;
    }
    global_rd_ptr++;
...
tt_queue::queue_entry tt_queue::get() {
    log_trace(tt::LogBackend, "Get entry {} to {}", local_rd_ptr, name);
