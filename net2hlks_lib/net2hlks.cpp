...
void Net2Hlks::insert_hlks_header(ofstream &hlks, bool insert_quantization_header) {
    hlks << "#include <cstdint>" << endl;
    hlks << "#include \"hlk_api.h\"" << endl;
    if (insert_quantization_header) {
        hlks << "#include \"quantization_common.h\"" << endl;
    }
...
void Net2Hlks::insert_pack_relu_config(ofstream &hlks, const tt_scheduled_op_info &curr_op) {
    ...
    if (curr_op.relu_en) {
        std::uint32_t relu_threshold =
            (netlist_utils::is_exp_a_format(curr_op.output_data_format)
...
void Net2Hlks::insert_wait_tiles(
    ...
    if (mblock_bcast) {
        ...
        if ((is_reduce_matmul || is_bcast_matmul) && (input == 1)) {
            block_scaler = curr_op.m_k > 1 ? "(oid+1)*" : block_scaler;
            hlks << "if (m_block == 0) {" << endl;
            hlks << "hlk_wait_tiles(core_ptr, HlkOperand::" << in.at(curr_op.input_names.at(input)) << ", "
                 << block_scaler << block_tile_dim << ");" << endl;
            hlks << "}" << endl;
        ...
        if (mblock_bcast) {
            ...
            if (input == 0) {
                ...
                if (is_reduce_matmul || is_bcast_matmul) {
                    ...
                    hlks << "if ((m_block == " << curr_op.output_dim.mblock_m - 1 << ")" << curr_oid << " {" << endl;
        ...
        if (reset_state || curr_op.type != prev_op_in_schedule.type ||
            ...
            if (!is_quantization_op) {
                ...
                if (binary_op == BinaryOp::Quantization) {
                    ...
                    hlk_init = "hlk_sfpu_add_int32_init";
                } else {
                    log_fatal("Unknown binary op type");
                }

                if (binary_op == BinaryOp::Add) {
                    hlks << hlk_init << "(core_ptr);" << endl;
                } else {
                    if (!is_input_dest_reg(curr_op, 0)) {
                        union {
                            float f;
                            int32_t i;
                        } f2i = {.f = zero_point};
...
void Net2Hlks::insert_matmul(
    ...
    if (!is_input_kernel_broadcasted(curr_op.input_names.at(1))) {
        hlks << "int in1_index = (" << in1_offset << "in_d*" << inner_c << ");" << endl;
    }
    if (is_input_kernel_broadcasted(curr_op.input_names.at(1))) {
        hlks << "for (int in_c = 0; in_c < " << inner_c << "; in_c++) {" << endl;
        ...
        hlks << "in_d*" << inner_c << " + in_c)";
        hlks << "% kernel_broadcast[" << input_index[1] << "];" << endl;
    }

    hlks << "hlk_mm_tile(core_ptr, HlkOperand::" << in.at(curr_op.input_names.at(0))
         << " , HlkOperand::" << in.at(curr_op.input_names.at(1)) << ", in0_index, in1_index, dst_index_" << id << ""
         << (is_input_kernel_broadcasted(curr_op.input_names.at(1)) ? " + in_c" : "") << ", false, "
         << (is_input_kernel_broadcasted(curr_op.input_names.at(1)) ? 1 : inner_c) << ");" << endl;
    if (is_input_kernel_broadcasted(curr_op.input_names.at(1))) {
        hlks << "}" << endl;
    }
    hlks << "}" << endl;
...
void Net2Hlks::insert_binary_op(
    ...
    if (!is_quantization_op) {
        ...
        if (!input_from_dest) {
            binary_op = binary_op == BinaryOp::Add ? BinaryOp::AddInt32 : binary_op;
            hlks << "quantization_ops_inner_loop<BinaryOp::" << netlist_utils::binary_op_to_string(binary_op)
            ...
            // quantization_ops_inner_loop will reconfigure unpacker a internally.
            last_input_unpackerA = in.at(curr_op.input_names.at(1));
        } else {
            int input_index = is_input_dest_reg(curr_op, 0) ? 1 : 0;
            hlks << "quantization_ops_inner_loop_from_dest<BinaryOp::" << netlist_utils::binary_op_to_string(binary_op)
...
void Net2Hlks::insert_unary_op(
    ...
    if (is_input_kernel_broadcasted(curr_op.input_names.at(0))) {
        ...
        in_index = "(batch*" + to_string(out_block_tile_dim * out_block_cnt) + " + m_block*" +
                   to_string(curr_op.output_dim.mblock_n * out_block_tile_dim) + " + n_block*" +
                   to_string(out_block_tile_dim) + " + " + in_index + ") % kernel_broadcast[" +
                   to_string(input_index) + "]";
    }
...
// quantisation ops and binary add with int32 inputs are implemented as a binary SFPU ops
...
static bool is_binary_sfpu_op(const tt_fused_op_info &fused_op, const tt_scheduled_op_info &curr_op) {
    bool is_quantization_op = false;

    is_quantization_op = netlist_utils::is_valid_binary_quantisation_op(curr_op.type);
    if (netlist_utils::is_valid_binary_op(curr_op.type) &&
        netlist_utils::get_binary_op(curr_op.type) == BinaryOp::Add) {
        for (const auto &input_name : curr_op.input_names) {
            if (fused_op.get_input_data_format(input_name) == DataFormat::Int32) {
                is_quantization_op = true;
            }
        }
    }
    return is_quantization_op;
}
...
void Net2Hlks::insert_hlks_body(ofstream &hlks, const tt_fused_op_info &fused_op) {
    ...
    for (const auto &schedule : fused_op.schedules) {
        ...
        for (const auto &curr_op : schedule.scheduled_ops) {
            ...
            bool is_quantization_op = is_binary_sfpu_op(fused_op, curr_op);
            ...
            if (!is_output_dest_reg(prev_op_in_schedule) && !is_quantization_op) {
                hlks << "hlk_acquire_dst(core_ptr);" << endl;
            }

            if (is_binary_op) {
                insert_binary_op(hlks, in, curr_op, prev_op_in_schedule, schedule_has_matmul_loop(schedule), is_quantization_op);
            } else if (
                netlist_utils::is_valid_sfpu_op(curr_op.type) ||
                (netlist_utils::get_unary_op(curr_op.type) == UnaryOp::Datacopy)) {
...
void Net2Hlks::output_hlks() {
    ...
    for (auto fused_op_it : this->parsed_netlist.fused_ops_op_map) {
        ...
        if (!fused_op_info.custom_kernel_path.empty()) {
            ...
            log_info(tt::LogNet2Hlks, "Skipping fused op {} kernel generation as it has custom kernel path", fused_op_id);
            continue;
        }
        ...
        insert_hlks_header(hlks, has_quant_op);
