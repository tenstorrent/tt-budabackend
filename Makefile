...
OPT_LEVEL = -O3
DEBUG_FLAGS += -DDEBUG -DTT_DEBUG_LOGGING
else ifeq ($(CONFIG), assert)
OPT_LEVEL = -O3 -g
DEBUG_FLAGS += -DDEBUG -DTT_DEBUG_LOGGING
else ifeq ($(CONFIG), asan)
OPT_LEVEL = -O3 -g
DEBUG_FLAGS += -DDEBUG -DTT_DEBUG_LOGGING -fsanitize=address
CONFIG_LDFLAGS += -fsanitize=address
else ifeq ($(CONFIG), ubsan)
OPT_LEVEL = -O3 -g
DEBUG_FLAGS += -DDEBUG -DTT_DEBUG_LOGGING -fsanitize=undefined
...
LDFLAGS ?= $(CONFIG_LDFLAGS) -Wl,-rpath,$(PREFIX)/lib:umd/build/lib -L$(LIBDIR) -Lumd/build/lib -ldl
...
include netlist_analyzer/tests/module.mk
