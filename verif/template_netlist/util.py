...
try:
    from yaml import Loader #If we can find the loader module, use it since its safer. May not always find it due to version incompatibilities
except:
    pass
...
def get_num_input_operands(test_type: PerfTestType):
    if (
        test_type == PerfTestType.Unary or
        test_type == PerfTestType.TM or
        test_type == PerfTestType.Dram or
        test_type == PerfTestType.Pcie or
        test_type == PerfTestType.ReduceMax or
        test_type == PerfTestType.Tilizer
    ):
        return 1
    elif (
        test_type == PerfTestType.Matmul or
        test_type == PerfTestType.Binary
    ):
        return 2
    elif (
        test_type == PerfTestType.MatmulBias or
        test_type == PerfTestType.Fused0 or
        test_type == PerfTestType.MatmulSparse or
        test_type == PerfTestType.MatmulSparseNzCounts or
        test_type == PerfTestType.Splice
    ):
        return 3
    elif (
        test_type == PerfTestType.Fused2
    ):
        return 5
    else:
...
def is_matmul_op(test_type):
    if (
        ...
        test_type == PerfTestType.MatmulBias or
        test_type == PerfTestType.MatmulSparse or
        test_type == PerfTestType.MatmulSparseNzCounts
    ):
        return True
    else:
        return False
...
def has_inner_dim(test_type):
    if (
        is_matmul_op(test_type) or
        test_type == PerfTestType.ReduceMax
    ):
        return True
    else:
        return False
...
def run_tm_test(*, netlist_yaml, outdir, num_inputs=1, dry_run=False, determinism_tests = False, num_loops = None):
    cmd = []
    
    with open(os.path.join(outdir, netlist_yaml), "r") as file:
        try:
            netlist_info = yaml.load(file, Loader = Loader) #Use the loader if its been imported (safer)
        except:
...
def run_test_cmd(cmd, outdir, dry_run):
    ...
    reset_sub_proc = subprocess.run(
        ...
        ], env=env, cwd=get_git_root(), capture_output=True, universal_newlines=True
    )
...
def create_netlist_from_single_config(*, template_yaml, output_dir, config, test_id, use_shared_dir=False):
...
def create_netlists(*, template_yaml, output_dir, configs, use_shared_dir=False, use_hash=1, command=None):
    def get_subs(template_dict):
        return {f"TEMPLATE_{key}": val for (key,val) in template_dict.items()}
    test_list = []
    with open(template_yaml, 'r') as file:
...
def generate_test_commands(netlist_test_info, test_type=TestType.TmTest, determinism_tests = False, num_loops = None):
    cmds = []
    print(netlist_test_info)
    for odir, yaml in netlist_test_info:
        cmdline, _ = (run_tm_test(netlist_yaml=yaml, outdir=odir, dry_run=True, determinism_tests = determinism_tests, num_loops = num_loops)
                      if test_type == TestType.TmTest else
                      run_graph_test(netlist_yaml=yaml, outdir=odir, dry_run=True))
        cmds.append(cmdline)
    return cmds
...
def default_dram_string(num_buffers, channel, min_buffer_size=0x800000, generate_random_addr=True, check_buf_size=None, start_addr=None, dram_assignment=None):
    if generate_random_addr:
        return "[" + ", ".join([f'[{chan}, 0x{addr:08x}]' for chan, addr in generate_dram_channels_and_addresses(num_buffers, min_buffer_size, channel, check_buf_size)]) + "]"
    else:
        dram_assignment_valid = [True]
        return "[" + ", ".join([f'[{chan}, 0x{addr:08x}]' for chan, addr in generate_dram_channels_and_deterministic_addresses(num_buffers, min_buffer_size, channel, check_buf_size, start_addr, dram_assignment, dram_assignment_valid)]) + "]", dram_assignment_valid[0]
...
def check_addresses_for_overlap(chan_addr_map, min_size, upper_limit):
    ...
    for p in chan_addr_map:
        ...
        for addr in chan_map:
            if buf_addr >= addr:
                assert buf_addr >= (addr + min_size), f"Overlap found for buffer at addr 0x'{buf_addr:x}', size=0x'{min_size:x}', with buffer 0x'{addr:x}', size='0x{min_size:x}'" 
            else:
                assert buf_addr <= (addr - min_size), f"Overlap found for buffer at addr 0x'{buf_addr:x}', size=0x'{min_size:x}', with buffer 0x'{addr:x}', size='0x{min_size:x}'" 
            assert buf_addr + min_size < upper_limit, f"Buffer at addr 0x'{buf_addr:x}, size 0x'{min_size:x}' exceeds max dram channel space"
...
def generate_dram_channels_and_addresses(num_buffers, min_buffer_size, channels, check_buf_size):
    ...
    indices = [x + min_buffer_size for x in positive_integers_with_sum(num_buffers, (upper_limit - lower_limit) - num_buffers * min_buffer_size)]
    start = lower_limit
    for i in indices:
...
def generate_dram_channels_and_deterministic_addresses(num_buffers, min_buffer_size, channels, check_buf_size, start_addr_each_channel, dram_assignment, dram_assignment_valid):
