...
template_yaml = os.path.join(
    get_git_root(),
    "verif/template_netlist/templates/fused_op/test_fused_op_matmul_reduce_c_bcast.template.yaml",
)


class FusedOpMatmulReduceCBcastTest(FusedOpTestBase):
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0) -> Solver:
    ...
    fused_op_test = FusedOpMatmulReduceCBcastTest(
        solver,
        svars,
