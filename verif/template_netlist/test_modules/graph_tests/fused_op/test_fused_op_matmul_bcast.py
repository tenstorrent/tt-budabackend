...
template_yaml = os.path.join(
    get_git_root(),
    "verif/template_netlist/templates/fused_op/test_fused_op_matmul_bcast.template.yaml",
)


class FusedOpMatmulBcastTest(FusedOpTestBase):
    ...
    def export_sweep_vars(self) -> list[SweepVarsGroup]:
        ...
        # each possible broadcast factor for each broadcast in the netlist, total:
        # 3 bcasts * (MAX_BROADCAST_FACTOR - MIN_BROADCAST_FACTOR + 1) * CONFIGS_PER_COMBINATION.
        sweep_vars = [
            SweepVarsGroup(
                {tm.tm_arg.sexpr(): list(range(MIN_BROADCAST_FACTOR, MAX_BROADCAST_FACTOR + 1, 1))},
...
def _create_general_limits(device_arch: DeviceArchitecture) -> GeneralLimits:
    """Create general limits for fused op matmul broadcast graph test constraints.
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0) -> Solver:
    """Add constraints for fused op matmul broadcast graph tests.
    ...
    fused_op_test = FusedOpMatmulBcastTest(
        solver,
        svars,
