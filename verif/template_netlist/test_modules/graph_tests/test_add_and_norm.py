...
template_yaml = f"{TEMPLATE_DIR_PATH}/test_add_and_norm.template.yaml"
...
add_and_norm_test: GraphTestBase = None
sweep_var_names = []
...
class AddAndNormTest(GraphTestBase):
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0) -> Solver:
    ...
    """
    global add_and_norm_test, template_yaml

    device_architecture = DeviceArchitecture.create_from_string(arch_name, harvested_rows)

    add_and_norm_test = AddAndNormTest(
        solver,
        svars,
    ...
    add_and_norm_test.constraint_model()

    sweep_var_names.extend(add_and_norm_test.export_sweep_vars())
...
def extra_config_callback(model_vars: dict) -> dict:
    ...
    """
    global add_and_norm_test
    assert add_and_norm_test is not None

    add_and_norm_test.postprocess(model_vars)

    input_queue = add_and_norm_test.get_input_queues()[0]
    model_vars[MEAN_CONSTANT_KEY] = 1.0 / model_vars[input_queue.size_c.sexpr()]
    model_vars[ARCH_VAR_NAME] = str(add_and_norm_test.arch)
...
def valid_config_callback(model_vars: dict, logger: Logger) -> bool:
    ...
    global add_and_norm_test
    assert add_and_norm_test is not None
    return add_and_norm_test.valid_config_callback(model_vars, logger)