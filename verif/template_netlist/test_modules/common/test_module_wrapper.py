...
class TestModuleWrapper:
    ...
    compile_template_path:
        Function which calculates the name and path of the template .yaml file used in the test
        module.
    """
    ...
    compile_template_path: Optional[Callable] = None
    ...
    def create_from_search_config(
        ...
        # TODO: Rethink the test module API to better encapsulate the parameters of
        # the public facing test modules.
        # If compile_template_path exists in the test module, use that, else return template_yaml
        compile_template_path: Callable = getattr(test_module, "compile_template_path", None)
        if not compile_template_path:
            compile_template_path = lambda: getattr(test_module, "template_yaml", None)
