...
class TemplateNetlistTestBase(ABC):
    ...
    def constrain_op_node(self, op_node: Node) -> None:
        ...
        op_node:
            Op node.
        """
        constrain_op_impl = TemplateNetlistTestBase._get_op_callable(op_node, self.op_constrainers)
    ...
    def get_op_input_buffer_size_tiles(self, op_node: Node, input_idx: int) -> z3.Var:
        ...
        input_idx:
            Input index.
        """
        get_op_input_buffer_size_tiles_impl = TemplateNetlistTestBase._get_op_callable(
            op_node, self.op_input_buffer_size_tiles_calculator
        )
    ...
    def constrain_op_input_dims(self, op_node: Node) -> Callable:
        ...
        op_node:
            Op node.
        """
        constrain_op_input_dims_impl = TemplateNetlistTestBase._get_op_callable(
            op_node, self.op_input_dims_constrainers
        )
    ...
    def get_op_consumer_config(self, op_node: Node, input_idx: int) -> ConsumerOpConfiguration:
        ...
        input_idx:
            Input index.
        """
        get_op_consumer_config_impl = TemplateNetlistTestBase._get_op_callable(
            op_node, self.op_consumer_confiugration_providers
        )
    ...
    def get_op_ethernet_links_usage(self, op_node: Node, input_idx: int) -> int:
        ...
        input_idx:
            Input index.
        """
        get_op_ethernet_links_usage_impl = TemplateNetlistTestBase._get_op_callable(
            op_node, self.op_ethernet_links_usage_calculator
        )
        ...
        for consumer_op in self.ops:
            ...
            if consumer_op.type == NodeType.FusedOp:
                ...
                for input_name in consumer_op.inputs:
                    ...
                    scheduled_consumer_op = self.get_fused_op_input_consumer_op(
                        consumer_op, input_name
                    )
    ...
    def create_op_input(
        ...
        op_input = OpInputTMs(
            prod_fullt_buf=self.add_var(f"{consumer.name}_prod_fullt_buf_{input_idx}"),
            p2p_buf_size_mb=self.add_var(f"{consumer.name}_buf_size_mb_{input_idx}"),
            row_major_stack=self.add_var(f"{consumer.name}_row_major_stack_{input_idx}"),
            total_slice_factor=self.add_var(f"{consumer.name}_total_slice_factor_{input_idx}"),
            total_stack_factor=self.add_var(f"{consumer.name}_total_stack_factor_{input_idx}"),
            ...
            stacking_block_ct=self.add_var(f"{consumer.name}_stacking_block_ct_{input_idx}"),
            stacking_block_rt=self.add_var(f"{consumer.name}_stacking_block_rt_{input_idx}"),
    ...
    def create_op_input_single_tm(
        self, producer: Node, consumer: Node, tm_idx: int, tm_config: NetlistTMConfiguration
    ) -> TMInfo:
        ...
        input_idx = self.get_consumers_producer_input_index(producer, consumer)
        tm_info = TMInfo(
            tm_type=self.add_var(f"{consumer.name}_tm_type_{input_idx}_{tm_idx}"),
            tm_arg=self.add_var(f"{consumer.name}_tm_arg_{input_idx}_{tm_idx}"),
            ...
            post_tm_t=self.add_var(f"{consumer.name}_post_tm_t_{input_idx}_{tm_idx}"),
            c_bcast_factor=self.add_var(f"{consumer.name}_c_bcast_factor_{input_idx}_{tm_idx}"),
            r_bcast_factor=self.add_var(f"{consumer.name}_r_bcast_factor_{input_idx}_{tm_idx}"),
            t_bcast_factor=self.add_var(f"{consumer.name}_t_bcast_factor_{input_idx}_{tm_idx}"),
            hslice_factor=self.add_var(f"{consumer.name}_hslice_factor_{input_idx}_{tm_idx}"),
            vslice_factor=self.add_var(f"{consumer.name}_vslice_factor_{input_idx}_{tm_idx}"),
            hstack_factor=self.add_var(f"{consumer.name}_hstack_factor_{input_idx}_{tm_idx}"),
            vstack_factor=self.add_var(f"{consumer.name}_vstack_factor_{input_idx}_{tm_idx}"),
            adjusted_slice_factor=self.add_var(
                f"{consumer.name}_adjusted_slice_factor_{input_idx}_{tm_idx}"
            ),
            multi_t_reach_stack=self.add_var(
                f"{consumer.name}_multi_t_reach_stack_{input_idx}_{tm_idx}"
            ),
        )
    ...
    def read_queues(self) -> None:
        ...
        for q_name, q_dict in self.template_dict["queues"].items():
            ...
            if TemplateNetlistTestBase.is_field_template_var(ublock_order_netlist_field):
                ...
                q_node.ublock_order_template_str = q_dict["ublock_order"][
                    len(TEMPLATE_VAR_PREFIX) :
                ]
            else:
                ...
                self.solver.add(
                    q_node.ublock_order == UblockOrder[ublock_order_netlist_field].value
                )
        ...
        for g_name, g_dict in self.template_dict["graphs"].items():
            for op_name, op_params in g_dict.items():
                ...
                for input_index in range(len(op.inputs)):
                    op.add_template_attribute(
                        f"in{input_index}_df", op_params["in_df"][input_index]
                    )
                ...
                if TemplateNetlistTestBase.is_field_template_var(ublock_order_netlist_field):
                    op.ublock_order_template_str = ublock_order_netlist_field[
                        len(TEMPLATE_VAR_PREFIX) :
                    ]
                    ...
                    self.solver.add(
                        op.ublock_order == UblockOrder[ublock_order_netlist_field].value
                    )
    ...
    def parse_op_pad_unpad(self, op_node: Node, op_params: Dict[str, any]) -> None:
        ...
        for input_idx, input_name in enumerate(op_node.inputs):
            ...
            if unpad_rt:
                op_node.add_constrained_attribute(
                    f"input_{input_idx}_unpad_rt", unpad_rt, self.solver, 0
                )
            if unpad_ct:
                op_node.add_constrained_attribute(
                    f"input_{input_idx}_unpad_ct", unpad_ct, self.solver, 0
                )
            if pad_rt:
                op_node.add_constrained_attribute(
                    f"input_{input_idx}_pad_rt", pad_rt, self.solver, 0
                )
            if pad_ct:
                op_node.add_constrained_attribute(
                    f"input_{input_idx}_pad_ct", pad_ct, self.solver, 0
                )
            ...
            if consumer_node.type == NodeType.FusedOp:
                ...
                for scheduled_consumer_op in consumer_node.scheduled_ops:
                    for producer_name in scheduled_consumer_op.inputs:
                        producer_node = self.nodes[producer_name]
                        self.constrain_point_to_point_connection(
                            producer_node, scheduled_consumer_op
                        )
    ...
    def constrain_op_input_buf_min_size_tiles(self, op_node: Node) -> None:
        ...
        for input_idx in range(len(op_node.inputs)):
            ...
            self.solver.add(input_buf_min_size_tiles_var <= self.arch.max_input_buf_size_tiles)
    ...
    def constrain_op_untilize(self, op_node: Node) -> None:
        ...
        if op_node.is_output:
            self.solver.add(
                op_node.untilize >= Untilize.false.value, op_node.untilize <= Untilize.true.value
            )
            self.solver.add(
                Implies(
    ...
    def constrain_buf_size_mb(self, op_node: Node) -> None:
        ...
        self.solver.add(
            If(
                ...
                divisible_either_direction(op_node.buf_size_mb / 2, op_node.t_dim),
            )
        )
    ...
    def constrain_op_padding(self, op_node: Node) -> None:
        ...
        for input_idx, input_name in enumerate(op_node.inputs):
            ...
            supports_r_padding, supports_c_padding = self.op_input_supports_padding(
                op_node, input_idx
            )
    ...
    def constrain_op_l1_usage(self, op_node: Node) -> None:
        ...
        intermed_buffers_required_space = self.get_num_intermediate_buffers(
            op_node
        ) * self.get_intermediate_buffer_size_bytes(op_node)
        ...
        self.solver.add(
            ...
            >= (
                input_buffers_required_space
                + intermed_buffers_required_space
                + prologue_buffers_required_space
                + output_buffer_required_space
            )
        )
    ...
    def constrain_op_input_along_dim(self, op_node: Node, input_idx: int, dim: Dimension) -> None:
        ...
        self.solver.add(
            size_along_dim == op_node.get_tiles_along_dim(dim), input_dims.t == op_node.t_dim
        )
    ...
    def get_scheduled_matmul_op_input_buffer_size_tiles(
        self, op_node: Node, input_idx: int
    ) -> z3.Var:
    ...
    def get_eltwise_op_consumer_config(
        self, op_node: Node, input_idx: int
    ) -> ConsumerOpConfiguration:
    ...
    def get_matmul_op_consumer_config(
        self, op_node: Node, input_idx: int
    ) -> ConsumerOpConfiguration:
    ...
    def get_scheduled_matmul_op_consumer_config(
        self, op_node: Node, input_idx: int
    ) -> ConsumerOpConfiguration:
    ...
    def get_reduce_op_consumer_config(
        self, op_node: Node, input_idx: int
    ) -> ConsumerOpConfiguration:
        """
        Returns consumer config for eltwise op for a given input index.
    ...
    def get_fused_op_consumer_config(
        self, op_node: Node, input_idx: int
    ) -> ConsumerOpConfiguration:
    ...
    def constrain_dram_channel(self, q_node: Node) -> None:
        ...
        self.solver.add(
            tiles_per_channel
            >= (
                q_node.get_horizontal_size_var()
                * q_node.get_vertical_size_var()
                * q_node.t_dim
    ...
    def constrain_queue_prologue(self, q_node: Node) -> None:
        ...
        for graph_name in self.graphs:
            ...
            if (
                q_node.type == NodeType.InputQueue
                or q_node.type == NodeType.Epoch2EpochQueue
                or "opt" in graph_name
            ):
                self.solver.add(prologue == Prologue.false.value)
            elif (
                q_node.type == NodeType.ConstantQueue
                or grad_queue
                and graph_name == feeder_op.graph
            ):
                ...
                self.solver.add(prologue >= Prologue.false.value, prologue <= Prologue.true.value)
    ...
    def constrain_output_queue(self, q_node: Node) -> None:
        ...
        self.solver.add(q_node.loc >= BufferLoc.host.value, q_node.loc <= BufferLoc.dram.value)
        ...
        self.solver.add(
            Implies(q_node.loc == BufferLoc.host.value, q_input_op.untilize == Untilize.true.value)
        )
    ...
    def constrain_output_queue_grid_size(self, q_node: Node) -> None:
        ...
        self.solver.add(
            If(
                ...
                And(
                    q_node.grid_size_y == 1,
                    q_node.grid_size_x == 1,
                    q_node.ub_r == 1,
                    q_node.ub_c == 1,
                ),
                And(
                    q_node.grid_size_y == q_input_op.grid_size_y,
    ...
    def constrain_output_queue_size(self, output_queue: Node) -> None:
        ...
        self.solver.add(output_queue.get_vertical_size_var() == feeder_op.get_vertical_size_var())
    ...
    def constrain_producer_consumer_same_size(self, producer: Node, consumer: Node) -> None:
        ...
        # Micro block scan order has to match.
        self.solver.add(producer.ublock_order == consumer.ublock_order)
    ...
    def constrain_data_format(self) -> None:
        ...
        for op_node in self.ops:
            ...
            if self.arch.supports_float32_accumulation and (
                op_node.type == NodeType.ReduceOp
                or (op_node.type == NodeType.FusedOp and op_node.has_reduce_scheduled_op)
            ):
                self.solver.add(
                    Implies(
                        op_node.acc_df == DataFormat.Float32.value,
                        And(
                            self.data_format != DataFormat.Bfp8.value,
    ...
    def fix_output_ops_untilize(self, model_vars: Dict[str, any]) -> None:
        ...
        for op_node in self.ops:
            if op_node.is_output:
                ...
                model_vars[untilize_key] = TemplateNetlistTestBase.get_bool_var_string(
                    model_vars, untilize_key
                )
    ...
    def export_producer_queue_for_resource_constraints_from_model(
        self, node: Node, model_vars: dict
    ) -> ResourceConstrNode:
        """
    ...
    def export_producer_queue_for_resource_constraints_from_z3(
        self,
        node: Node,
    ) -> ResourceConstrNode:
        """
    ...
    def export_producer_node_for_resource_constraints_from_z3(
        self, node: Node
    ) -> ResourceConstrNode:
        """
    ...
    def export_consumer_op_for_resource_constraints_from_model(
        self, node: Node, model_vars: dict, input_index: int
    ) -> ResourceConstrNode:
        ...
        return ResourceConstrNode(
            ...
            ublock_scan_order=consumer_op_config.get_attr_from_model(
                "ublock_scan_order", model_vars
            ),
    ...
    def export_consumer_op_for_resource_constraints_from_z3(
        self, node: Node, input_index: int
    ) -> ResourceConstrNode:
    ...
    def get_node_tms(
        self, node: Node, input_idx: int, model_vars: Optional[Dict]
        ...
        if model_vars is not None:
            for tm in op_input_tms:
                if model_vars[tm.tm_type.sexpr()] == TMS.transpose.value:
                    tms.append({"tm_type": "transpose", "factor": True})
                else:
                    tms.append(
                        {
        ...
        return (
            ...
            + ", ".join(
                [
                    f"[{channel}, {hex(addr)}]"
                    for addr in range(
                        start_addr, start_addr + buffer_count * buffer_size, buffer_size
                    )
                ]
            )
            + "]"
    ...
    def is_temporary_buffer(buffer_name: str):
        return TemplateNetlistTestBase.is_intermediate_buffer(
            buffer_name
        ) or TemplateNetlistTestBase.is_dest_register(buffer_name)

    @staticmethod
    def get_node_type_from_netlist(
        op_params: Dict[str, any], is_scheduled: bool = False
    ) -> NodeType:
        ...
        return (
            NodeType.FusedOp
            if op_params["type"] == "fused_op"
            else (
                matmul_op_type
                if op_params["type"] == "matmul"
                else (reduce_op_type if op_params["type"] == "reduce" else (NodeType.EltwiseOp))
            )
        )
    ...
    def constrain_fork_streams(self, op: Node) -> None:
        ...
        """
        total_op_queues_fanout_z3 = self.get_op2queues_resource_usage(op, self.queues)
        (
        ...
        total_fork_streams_used_per_core_z3 = (
            total_op_queues_fanout_z3 + producer_max_fork_streams_used_per_core_z3
        )

        op.add_raw_attribute("total_fork_streams_used_per_core")

        self.solver.add(op.total_fork_streams_used_per_core == total_fork_streams_used_per_core_z3)
        self.solver.add(
    ...
    def constrain_pipe_inputs(self, op: Node) -> None:
        ...
        added only when needed).
        """
        total_queues_op_pipe_inputs_z3 = self.get_queues2op_resource_usage_z3_expr(
            op, self.queues, self.solver
        )

        op.add_raw_attribute("total_queues_op_pipe_inputs")

        self.solver.add(op.total_queues_op_pipe_inputs == total_queues_op_pipe_inputs_z3)
        self.solver.add(
    ...
    def get_op2queues_resource_usage(self, op: Node, queues: List[Node]) -> int:
        """
        ...
        """
        queues_fed_by_this_op = list(filter(lambda q: q.input == op.name, queues))

        return len(queues_fed_by_this_op)

    def get_op2consumer_ops_resource_usage(
        self, op: Node, ops: List[Node], model_vars: Dict
    ) -> Tuple[int, int]:
        """
        ...
        ops_fed_by_this_op = list(filter(lambda consumer_op: op.name in consumer_op.inputs, ops))
        ...
        for consumer_op in ops_fed_by_this_op:
            ...
            input_indexes = [
                idx for idx, input_name in enumerate(consumer_op.inputs) if input_name == op.name
            ]
            ...
            for input_index in input_indexes:
                (
                    ...
                    self.export_producer_op_for_resource_constraints_from_model(
                        node=op, model_vars=model_vars
                    ),
                    self.export_consumer_op_for_resource_constraints_from_model(
                        node=consumer_op, model_vars=model_vars, input_index=input_index
                    ),
                )

                total_producer_fork_streams_used_per_core += producer_max_fork_streams_used_per_core
                total_producer_stream_phases_used_per_core += (
                    producer_max_scatter_stream_phases_used_per_core
                )
    ...
    def get_queues2op_resource_usage(self, op: Node, queues: list[Node], model_vars: dict) -> int:
        """
        ...
        for queue in queues_feeding_this_op:
            ...
            queue_op_pipe_inputs = get_queue2op_resource_usage(
                self.export_producer_queue_for_resource_constraints_from_model(
                    node=queue,
                    model_vars=model_vars,
                ),
                self.export_consumer_op_for_resource_constraints_from_model(
                    node=real_op, model_vars=model_vars, input_index=input_index
                ),
            )
            ...
            is_queue_prologue_read = (
                queue_prologue_var_for_op_graph is not None
                and model_vars[queue_prologue_var_for_op_graph.sexpr()]
            )
            if is_queue_prologue_read > 0:
                grid_diff_across_x = math.ceil(
                    model_vars[queue.grid_size_x.sexpr()] / model_vars[op.grid_size_x.sexpr()]
                )
                grid_diff_across_y = math.ceil(
                    model_vars[queue.grid_size_y.sexpr()] / model_vars[op.grid_size_y.sexpr()]
                )
    ...
    def get_producer_ops2op_resource_usage(
        self, op: Node, ops: list[Node], model_vars: dict
    ) -> int:
        """
        ...
        ops_feeding_this_op = list(filter(lambda producer_op: producer_op.name in op.inputs, ops))
        ...
        for producer_op in ops_feeding_this_op:
            ...
            (
                ...
                self.export_producer_op_for_resource_constraints_from_model(
                    node=producer_op, model_vars=model_vars
                ),
                self.export_consumer_op_for_resource_constraints_from_model(
                    node=real_op, model_vars=model_vars, input_index=input_index
                ),
            ...
            if self.is_multi_grid_node(
                producer_op, model_vars
            ) and self.has_reblocking_on_connection(producer_op, op, model_vars):
    ...
    def get_op2consumer_ops_resource_usage_z3_expr(
        self, op: Node, ops: List[Node], solver: z3.Solver
    ) -> Tuple[z3.Var, z3.Var]:
        """
        ...
        for consumer_op in ops_fed_by_this_op:
            ...
            for input_index in input_indexes:
                (
                    ...
                    self.export_producer_op_for_resource_constraints_from_z3(node=op),
                    self.export_consumer_op_for_resource_constraints_from_z3(
                        node=consumer_op, input_index=input_index
                    ),
                    solver,
                )
    ...
    def get_queues2op_resource_usage_z3_expr(
        self, op: Node, queues: List[Node], solver: z3.Solver
    ) -> z3.Var:
        """
        Helper function for check_resource_usage which calculates number of
        pipe inputs for queues -> op connections.
        ...
        for queue in queues_feeding_this_op:
            ...
            queue_op_pipe_inputs = get_queue2op_resource_usage_z3(
                self.export_producer_queue_for_resource_constraints_from_z3(node=queue),
                self.export_consumer_op_for_resource_constraints_from_z3(
                    node=real_op, input_index=input_index
                ),
                solver,
            )
    ...
    def get_producer_ops2op_resource_usage_z3_expr(
        self, op: Node, ops: List[Node], solver: z3.Solver
    ) -> z3.Var:
        """
        Helper function for check_resource_usage which calculates stream
        ...
        for producer_op in ops_feeding_this_op:
            ...
            (
                ...
                self.export_producer_op_for_resource_constraints_from_z3(node=producer_op),
                self.export_consumer_op_for_resource_constraints_from_z3(
                    node=real_op, input_index=input_index
                ),
                solver,
    ...
    def check_resource_usage(self, model_vars: Dict, logger: Logger) -> bool:
        ...
        for op in self.ops:
            ...
            (
                producer_max_fork_streams_used_per_core,
                producer_max_scatter_stream_phases_used_per_core,
            ) = self.get_op2consumer_ops_resource_usage(op, self.ops, model_vars)

            total_stream_phases_used_per_core += producer_max_scatter_stream_phases_used_per_core
            ...
            ) = self.get_queues2op_resource_usage(op, self.queues, model_vars)
            ...
            ) = self.get_producer_ops2op_resource_usage(op, self.ops, model_vars)

            total_stream_phases_used_per_core += consumer_max_gather_stream_phases_used_per_core
            ...
            if total_extra_streams_user_per_core > self.arch.max_extra_streams_used_per_core:
                logger.warning(
            ...
            if total_queues_op_pipe_inputs > self.arch.max_queues_op_pipe_inputs:
                logger.warning(
            ...
            if total_stream_phases_used_per_core > self.arch.max_stream_phases_per_core:
                logger.warning(
    ...
    def has_reblocking_on_connection(
        self, producer: Node, consumer: Node, model_vars: dict
    ) -> bool:
        """Returns true if the reblocking is present on a producer->consumer connection."""
        reblocking_var_names = [
            "grid_size_x",
            "grid_size_y",
            "mb_m",
            "mb_n",
        ...
        return (
            any(
                model_vars[getattr(producer, var_name).sexpr()]
                != model_vars[getattr(consumer, var_name).sexpr()]
    ...
    @staticmethod
    def get_template_var_field(
        params_dict: Dict[str, any], key_name: str, default_value: any = None
    ) -> Optional[any]:
        """
