...
from test_modules.common.worker_manager import WorkerSolutionHandler
...
from util import PerfOpSweepConfig, dump_config_yaml, get_git_root
...
class SolutionHandler(ABC):
    def __init__(
        ...
        self.__arch = search_config.arch
        self.__run_compiler = search_config.run_compiler
    ...
    def __postprocess_solver_solution(
        ...
        if self.__run_compiler:
            template_yaml = self._module_wrapper.compile_template_path()
            logger = self._module_wrapper.logger
            if not CompilationPipelineRunner.run_compiler_pipeline_on_configs(
                template_yaml, config, self.__arch, logger
            ):
                return PostprocessedSolverSolution(False, config, solver_vars_values)
...
class FreeSearchSolutionHandler(SolutionHandler):
    ...
    def _handle_valid_config(self, postprocessed: PostprocessedSolverSolution) -> None:
        ...
        dump_config_yaml(postprocessed.postprocessed_solution, self._config_dir_path)
...
class ParallelSweepSolutionHandler(SolutionHandler):
    ...
    def _handle_valid_config(self, postprocessed: PostprocessedSolverSolution) -> None:
        ...
        self.__worker_solution_handler.process_sat(
            self.get_last_generated_config(),
            str(self.__sweep_vars_combination),
