...
from test_modules.common.enums import (TMS, Dimension, TileBroadcastDimension,
                                       UblockOrder)
...
class TMInfo:
    ...
    def constrain_valid_tm(self, solver: z3.Solver) -> None:
        ...
        valid_tm_conditions = [self.tm_type == tm_type.value for tm_type in MULTI_TM_TESTS_VALID_TMS]
...
class OpInputTMs(PipeOpInputBase):
    ...
    def initialize_variables(self, solver: z3.Solver) -> None:
        ...
        # stack factor is
        solver.add(
            self.stacking_block_ct > 0, self.stacking_block_rt > 0,
            self.effective_hstack_factor > 0, self.effective_vstack_factor > 0,
            self.post_padding_stacking_block_ct > 0, self.post_padding_stacking_block_rt > 0,
            self.post_padding_effective_hstack_factor > 0, self.post_padding_effective_vstack_factor > 0
        )
    ...
    def stack_factor(self) -> Dict[Dimension, z3.Var]:
        ...
        """
        return {
            Dimension.Horizontal: self.hstack_factor,
            Dimension.Vertical: self.vstack_factor
        }
    ...
    def slice_factor(self) -> Dict[Dimension, z3.Var]:
        ...
        """
        return {
            Dimension.Horizontal: self.hslice_factor,
            Dimension.Vertical: self.vslice_factor
        }
    ...
    def bcast_factor(self) -> Dict[Dimension, z3.Var]:
        ...
        """
        return {
            Dimension.Horizontal: self.c_bcast_factor,
            Dimension.Vertical: self.r_bcast_factor
        }
    ...
    def total_slice_stack_ratio(self) -> Dict[Dimension, z3.Var]:
        ...
        """
        return {
            dim: self.total_slice_factor / self.stack_factor[dim]
    ...
    def constrain_tm_sequence(self, solver: z3.Solver) -> None:
        ...
        for tm_info in self.tms:
            solver.add(
                Implies(tm_info.tm_type == TMS.c_broadcast.value,
                    And(
                        tm_info.post_tm_ct == tm_info.tm_arg * input_ct,        # out_ct = broadcast_factor * in_ct
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.r_broadcast.value,
                    And(
                        tm_info.post_tm_rt == tm_info.tm_arg * input_rt,        # out_rt = broadcast_factor * in_rt
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.t_broadcast.value,
                    And(
                        input_t == 1,                                           # in_t   = 1
                        tm_info.post_tm_t == tm_info.tm_arg * input_t,          # out_t  = broadcast_factor * in_t
                        tm_info.post_tm_ct == input_ct,                         # out_ct = in_ct
                        tm_info.post_tm_rt == input_rt,                         # out_rt = in_rt
                    )
                )
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.transpose.value,
                    And(
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.hslice.value,
                    And(
                        tm_info.post_tm_t == tm_info.tm_arg * input_t,          # out_t  = hslice_factor * in_t
                        input_ct == tm_info.tm_arg * tm_info.post_tm_ct,        # out_ct = in_ct / hslice_factor and in_ct % hslice_factor = 0
                        tm_info.post_tm_rt == input_rt,                         # out_rt = in_rt
                    )
                )
            )
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.vslice.value,
                    And(
                        tm_info.post_tm_t == tm_info.tm_arg * input_t,          # out_t  = vslice_factor * in_t
                        input_rt == tm_info.tm_arg * tm_info.post_tm_rt,        # out_rt = in_rt / vslice_factor and in_rt % vslice_factor = 0
                        tm_info.post_tm_ct == input_ct,                         # out_ct = in_ct
                    )
                )
            )
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.hstack.value,
                    And(
                        tm_info.post_tm_ct == tm_info.tm_arg * input_ct,        # out_ct = hstack_factor * in_ct
                        input_t == tm_info.post_tm_t * tm_info.tm_arg,          # out_t  = in_t / hstack_factor and in_t % hstack_factor == 0
                        tm_info.post_tm_rt == input_rt,                         # out_rt = in_rt
                    )
                )
            )
            ...
            solver.add(
                Implies(tm_info.tm_type == TMS.vstack.value,
                    And(
                        tm_info.post_tm_rt == tm_info.tm_arg * input_rt,        # out_rt = vstack_factor * in_rt
                        input_t == tm_info.post_tm_t * tm_info.tm_arg,          # out_t  = in_t / vstack_factor and in_t % vstack_factor == 0
                        tm_info.post_tm_ct == input_ct,                         # out_ct = in_ct
                    )
                )
            )
        ...
        solver.add(
            If(
                ...
                And(
                    ...
                    self.post_padding_effective_vstack_factor == self.effective_vstack_factor + (self.pad_unpad_params.pad_rt / self.stacking_block_rt),
                    self.post_padding_stacking_block_rt == self.stacking_block_rt
                ),
                And(
                    self.post_padding_stacking_block_rt == self.stacking_block_rt + self.pad_unpad_params.pad_rt,
        ...
        solver.add(
            If(
                ...
                And(
                    ...
                    self.post_padding_effective_hstack_factor == self.effective_hstack_factor + (self.pad_unpad_params.pad_ct / self.stacking_block_ct),
                    self.post_padding_stacking_block_ct == self.stacking_block_ct
                ),
                And(
                    self.post_padding_stacking_block_ct == self.stacking_block_ct + self.pad_unpad_params.pad_ct,
    ...
    def get_need_phased_stack_constraints(self) -> z3.Var:
        ...
        return And(
            Or (
                self.producer.buf_size_mb == 1,
                self.producer.buf_size_mb == 2
            ),
                    ...
                    If(
                        Or(
                            ...
                            And(
                                tm.tm_type == TMS.hstack.value,
                                Or(
                                    prev_tm.tm_type != TMS.vstack.value,
                                    prev_tm.tm_arg == 1
                                )
                            )
                        ),
    ...
    def get_dim_major_stacking_constraints(self, dim: Dimension) -> z3.Var:
        ...
        return And(
            divisible_either_direction(
                self.total_slice_factor, self.stack_factor[dim]
            ),
            ...
            If(
                ...
                And(
                    ...
                    self.stacking_block_tiles[dim] == self.producer_block_tiles_pre_stack[dim] * self.stack_factor[dim],
                    self.effective_stack_factor[dim] == 1,

                    If((self.total_slice_stack_ratio[dim]) % self.stack_factor[opposite_dim] == 0,
                        # In this case, one original producer t covers the total stacking factor
                        And(
                            self.stacking_block_tiles[opposite_dim] == self.producer_block_tiles_pre_stack[opposite_dim] * self.stack_factor[opposite_dim],
                            self.effective_stack_factor[opposite_dim] == 1,
                        ),

                        # else: stack_factor_opposite_dim % (total_slice_factor/stack_factor_dim) == 0
                        And(
                            self.stacking_block_tiles[opposite_dim] == self.producer_block_tiles_pre_stack[opposite_dim] * self.total_slice_stack_ratio[dim],
                            self.effective_stack_factor[opposite_dim] == self.stack_factor[opposite_dim] / self.total_slice_stack_ratio[dim]
                        )
                    )
                ),
                ...
                And (
                    self.stacking_block_tiles[dim] == self.producer_block_tiles_pre_stack[dim] * self.total_slice_factor,
                    self.stacking_block_tiles[opposite_dim] == self.producer_block_tiles_pre_stack[opposite_dim],
                    self.effective_stack_factor[dim] == self.stack_factor[dim]/ self.total_slice_factor,
    ...
    def constrain_consumer_grid_blocking(self, solver: z3.Solver) -> None:
        ...
        solver.add(
            Implies(
                ...
                And(
                    divisible_either_direction(
                        self.post_padding_stacking_block_ct, self.consumer_config.mblock_c * self.consumer_config.ublock_c
                    ),
                    divisible_either_direction(
                        self.post_padding_stacking_block_rt, self.consumer_config.mblock_r * self.consumer_config.ublock_r
                    ),
                    ...
                    divisible_either_direction(self.post_padding_stacking_block_ct, self.consumer_config.ublock_c),
                    divisible_either_direction(self.post_padding_stacking_block_rt, self.consumer_config.ublock_r)
                )
            )
        )
