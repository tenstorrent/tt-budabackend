...
class SweepCombination:
    ...
    def add_new_config(self, config: Dict[str, int]) -> SweepCombination:
        ...
        """
        if (not self.already_found(config)) and len(
            self.__generated_configs
        ) < self.__max_configs_per_combination:
            self.__generated_configs = self.__generated_configs + [config]

        return self

    def already_found(self, config: Dict[str, int]) -> bool:
        return config in self.__generated_configs
