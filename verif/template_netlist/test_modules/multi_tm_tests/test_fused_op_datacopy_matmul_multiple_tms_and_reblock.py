...
from test_modules.multi_tm_tests.multi_tm_test_base import MultiTmFusedOpTestBase
...
template_yaml = os.path.join(
    get_git_root(), 
    'verif/template_netlist/templates/test_fused_op_datacopy_matmul_multiple_tms_and_reblock.template.yaml'
)
...
class FusedOpDatacopyMatmulMultiTmTest(MultiTmFusedOpTestBase):
...
multi_tm_test: FusedOpDatacopyMatmulMultiTmTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = FusedOpDatacopyMatmulMultiTmTest(
        solver=solver,
        svars=svars,
