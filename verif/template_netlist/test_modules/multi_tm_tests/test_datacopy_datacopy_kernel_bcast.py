...
test_type_extension = "_multichip" if IS_MULTICHIP_TEST else ""
template_yaml = os.path.join(
    get_git_root(),
    f'verif/template_netlist/templates/test_datacopy_datacopy_kernel_bcast{test_type_extension}.template.yaml'
)
...
class DatacopyDatacopySingleTileBroadcastTest(MultiTMKernelBroadcastTestBase):
...
multi_tm_test: DatacopyDatacopySingleTileBroadcastTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = DatacopyDatacopySingleTileBroadcastTest(
        solver=solver,
        svars=svars,
