...
def compile_template_path():
    return os.path.join(
        get_git_root(),
        f"verif/template_netlist/templates/test_datacopy_fork{MultiTMTestParameters.get_fork_factor()}_datacopy_multiple_tms_and_reblock.template.yaml",
    )
...
class DatacopyForkDatacopyMultiTmTest(MultiTMForkingTestBase):
...
multi_tm_test: DatacopyForkDatacopyMultiTmTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = DatacopyForkDatacopyMultiTmTest(
        solver=solver,
        svars=svars,
