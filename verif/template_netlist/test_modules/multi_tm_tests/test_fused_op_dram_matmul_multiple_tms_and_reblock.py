...
template_yaml = os.path.join(
    get_git_root(), 
    'verif/template_netlist/templates/test_fused_op_dram_matmul_multiple_tms_and_reblock.template.yaml'
)
...
class FusedOpDramMatmulMultiTmTest(MultiTmFusedOpTestBase):
...
multi_tm_test: FusedOpDramMatmulMultiTmTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = FusedOpDramMatmulMultiTmTest(
        solver=solver,
        svars=svars,
