...
IS_MULTICHIP_TEST = int(os.getenv("MULTICHIP_TEST", 0))
IS_MATMUL_BIAS_TEST = int(os.getenv("MATMUL_BIAS", 0))
multichip_extension = "_multichip" if IS_MULTICHIP_TEST else ""
bias_extension = "_with_bias" if IS_MATMUL_BIAS_TEST else ""
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/test_datacopy_matmul{bias_extension}_kernel_bcast{multichip_extension}.template.yaml",
)
...
MAX_INPUT0_TENSOR_IN_TILES = int(os.getenv("MAX_INPUT0_TENSOR_IN_TILES", 16 * 16 * 64))
MAX_INPUT1_TENSOR_IN_TILES = int(os.getenv("MAX_INPUT1_TENSOR_IN_TILES", 16 * 16 * 64))
MAX_OUTPUT_TENSOR_IN_TILES = int(os.getenv("MAX_OUTPUT_TENSOR_IN_TILES", 16 * 16 * 64))


class DatacopyMatmulSingelTileBroadcastTest(MultiTMKernelBroadcastTestBase):
    def additional_constraints(self) -> None:
        super().additional_constraints()

        self.constrain_tensor_size(self.nodes["input0_dram"], MAX_INPUT1_TENSOR_IN_TILES)
        self.constrain_tensor_size(self.nodes["input1_dram"], MAX_INPUT1_TENSOR_IN_TILES)
...
multi_tm_test: DatacopyMatmulSingelTileBroadcastTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = DatacopyMatmulSingelTileBroadcastTest(
        solver=solver,
        svars=svars,
