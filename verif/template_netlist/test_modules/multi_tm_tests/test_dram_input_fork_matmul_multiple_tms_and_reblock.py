...
template_yaml = os.path.join(
    get_git_root(), 
    f'verif/template_netlist/templates/test_dram_input_fork{FORK_FACTOR}_matmul_multiple_tms_and_reblock.template.yaml'
)

# Number of inputs of the OP with multiple TMs on input(s)
NUM_INPUTS = 2 * FORK_FACTOR
...
MAX_INPUT1_TENSOR_IN_TILES = os.getenv('MAX_INPUT1_TENSOR_IN_TILES', 16*16*64)
MAX_OUTPUT_TENSOR_IN_TILES = os.getenv('MAX_OUTPUT_TENSOR_IN_TILES', 16*16*64)


class DramInputForkMatmulMultiTmTest(MultiTMForkingTestBase):
...
multi_tm_test: DramInputForkMatmulMultiTmTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = DramInputForkMatmulMultiTmTest(
        solver=solver,
        svars=svars,
