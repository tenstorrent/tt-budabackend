...
test_type_extension = "_prologue" if IS_PROLOGUE_TEST else ""
template_yaml = os.path.join(
    get_git_root(),
    f'verif/template_netlist/templates/test_dram_input_datacopy_kernel_bcast{test_type_extension}.template.yaml'
)
...
class DramInputDatacopySingleTileBrodacastTest(MultiTMKernelBroadcastTestBase):
...
multi_tm_test: DramInputDatacopySingleTileBrodacastTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = DramInputDatacopySingleTileBrodacastTest(
        solver=solver,
        svars=svars,
