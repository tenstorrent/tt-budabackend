...
template_yaml = os.path.join(
    get_git_root(),
    'verif/template_netlist/templates/test_fused_op_datacopy_matmul_kernel_bcast.template.yaml'
)
...
class FusedOpDatacopyMatmulKernelBroadcastTest(MultiTMFusedOpKernelBroadcastTestBase):
...
multi_tm_test: FusedOpDatacopyMatmulKernelBroadcastTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    multi_tm_test = FusedOpDatacopyMatmulKernelBroadcastTest(
        solver=solver,
        svars=svars,
