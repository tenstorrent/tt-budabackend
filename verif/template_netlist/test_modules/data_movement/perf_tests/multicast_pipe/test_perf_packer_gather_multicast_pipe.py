...
class PackerGatherMulticastPipePerfTest(DataMovementPerfTestBase):
    ...
    def additional_constraints(self) -> None:
        ...
        self.solver.add(feeder0.grid_size_x == self.in0_gather_buffers, feeder0.grid_size_y == 1)
        self.solver.add(feeder1.grid_size_x == 1, feeder1.grid_size_y == self.in1_gather_buffers)
        self.solver.add(
...
pipe_perf_test: PackerGatherMulticastPipePerfTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_perf_test = PackerGatherMulticastPipePerfTest(
        solver=solver,
        svars=svars,
