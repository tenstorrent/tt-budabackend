...
class DramGatherPipePerfTest(DataMovementPerfTestBase):
    ...
    def additional_constraints(self) -> None:
        self.in0_gather_buffers = self.add_var("in0_gather_buffers")
        self.in1_gather_buffers = self.add_var("in1_gather_buffers")
        ...
        self.solver.add(
            self.in0_gather_buffers >= 2, self.in0_gather_buffers <= MAX_GATHER_SOURCE_BUFFERS
        )
        self.solver.add(
            self.in1_gather_buffers >= 2, self.in1_gather_buffers <= MAX_GATHER_SOURCE_BUFFERS
        )
        self.solver.add(
            self.in0_multicast_dest_buffers >= 2,
            self.in0_multicast_dest_buffers <= MAX_MULTICAST_DESTINATION_BUFFERS,
        )
        self.solver.add(
            self.in1_multicast_dest_buffers >= 2,
        ...
        self.solver.add(
            dram_input0.grid_size_x == self.in0_gather_buffers, dram_input0.grid_size_y == 1
        )
        self.solver.add(
            dram_input1.grid_size_x == 1, dram_input1.grid_size_y == self.in1_gather_buffers
        )
        self.solver.add(
    ...
    def export_sweep_vars(self) -> List[SweepVarsGroup]:
        in0_gather_range = list(range(1, MAX_GATHER_SOURCE_BUFFERS + 1))
        in1_gather_range = list(range(1, MAX_GATHER_SOURCE_BUFFERS + 1))
        ...
        return [
            SweepVarsGroup(
                var_names_range_dict={
                    self.in0_gather_buffers.sexpr(): in0_gather_range,
                    self.in1_gather_buffers.sexpr(): in1_gather_range,
                },
