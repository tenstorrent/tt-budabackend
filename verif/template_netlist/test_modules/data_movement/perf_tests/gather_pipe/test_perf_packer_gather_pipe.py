...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/perf_tests/gather_pipe/test_perf_packer_gather_pipe.template.yaml",
)


class PackerGatherPipePerfTest(DataMovementPerfTestBase):
...
pipe_perf_test: PackerGatherPipePerfTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_perf_test = PackerGatherPipePerfTest(
        solver=solver,
        svars=svars,
