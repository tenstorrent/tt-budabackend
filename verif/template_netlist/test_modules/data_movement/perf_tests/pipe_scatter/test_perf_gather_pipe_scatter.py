...
class GatherPipeScatterPerfTest(DataMovementPerfTestBase):
    ...
    def additional_constraints(self) -> None:
        ...
        self.solver.add(feeder0.grid_size_x == 1, feeder0.grid_size_y == self.gather_source_buffers)
        self.solver.add(
    ...
    def export_sweep_vars(self) -> List[SweepVarsGroup]:
        gather_source_bufers_range = list(range(2, MAX_GATHER_SOURCE_BUFFERS + 1))
        pipe_scatter_dest_range = list(range(2, MAX_PIPE_SCATTER_DESTINATION_BUFFERS + 1))

        return [
            SweepVarsGroup(
                var_names_range_dict={
                    self.gather_source_buffers.sexpr(): gather_source_bufers_range,
                },
...
pipe_perf_test: GatherPipeScatterPerfTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_perf_test = GatherPipeScatterPerfTest(
        solver=solver,
        svars=svars,
