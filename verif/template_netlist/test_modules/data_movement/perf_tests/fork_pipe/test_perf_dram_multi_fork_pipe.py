...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/perf_tests/fork_pipe/test_perf_dram_multi_fork_pipe.template.yaml",
)
...
class DramMultiForkPipePerfTest(DataMovementPerfTestBase):
    ...
    def additional_constraints(self) -> None:
        ...
        for node in self.nodes.values():
            self.solver.add(node.t_dim == 1, node.grid_size_x == 1, node.grid_size_y == 1)

        for op in self.ops:
            self.solver.add(op.buf_size_mb == 2)

        dram_fork_input = self.nodes["input0_dram"]
        for target_op_idx in range(NUM_FORKS):
            target_op = self.nodes[f"target_op{target_op_idx}"]
            self.solver.add(target_op.grid_loc_x == 0, target_op.grid_loc_y == target_op_idx)

            # TODO: Uncomment the line bellow to test the case where there is no reblocking between the
            # dram input queue and the fork destinations.
            # constrain_no_reblocking_on_connection(self.solver, dram_fork_input, target_op)
        ...
        return [
            SweepVarsGroup(
                var_names_range_dict={
                    dram_fork_input.mb_m.sexpr(): mblock_range,
                    dram_fork_input.mb_n.sexpr(): mblock_range,
                },
...
pipe_perf_test: DramMultiForkPipePerfTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_perf_test = DramMultiForkPipePerfTest(
        solver=solver,
        svars=svars,
