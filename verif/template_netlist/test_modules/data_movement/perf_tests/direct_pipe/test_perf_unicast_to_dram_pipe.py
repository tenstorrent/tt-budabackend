...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/perf_tests/direct_pipe/test_perf_unicast_to_dram_pipe.template.yaml",
)


class UnicastToDramPipePerfTest(DataMovementPerfTestBase):
...
pipe_perf_test: UnicastToDramPipePerfTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_perf_test = UnicastToDramPipePerfTest(
        solver=solver,
        svars=svars,
