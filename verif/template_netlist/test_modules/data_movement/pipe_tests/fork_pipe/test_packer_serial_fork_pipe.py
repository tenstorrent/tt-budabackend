...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/fork_pipe/test_packer_serial_fork_pipe.template.yaml",
)


class PackerSerialForkPipe(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        ...
        self.solver.add(feeder0.buf_size_mb == 1)

        vstack_factor = self.op_input_tm_map["drainer0"]["feeder0"].get_tms()[0].tm_arg
        self.solver.add(vstack_factor > 1)
...
pipe_test: PackerSerialForkPipe = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = PackerSerialForkPipe(
        solver=solver,
        svars=svars,
