...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/fork_pipe/test_dram_parallel_fork_pipe.template.yaml",
)


class DramParallelForkPipe(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        ...
        dram_input = self.nodes["input0_dram"]
...
pipe_test: DramParallelForkPipe = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = DramParallelForkPipe(
        solver=solver,
        svars=svars,
