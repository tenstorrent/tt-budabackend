...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/fork_pipe/test_packer_parallel_fork_pipe.template.yaml",
)


class PackerParallelForkPipe(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        ...
        force_reblocking_on_connection(self.solver, feeder0, self.nodes["drainer0"])
...
pipe_test: PackerParallelForkPipe = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = PackerParallelForkPipe(
        solver=solver,
        svars=svars,
