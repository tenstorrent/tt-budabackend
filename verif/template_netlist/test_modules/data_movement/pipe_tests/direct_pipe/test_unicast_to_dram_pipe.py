...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/direct_pipe/test_unicast_to_dram_pipe.template.yaml",
)


class UnicastToDramPipeTest(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        ...
        self.solver.add(output_queue.grid_size_x == 1, output_queue.grid_size_y == 1)
...
pipe_test: UnicastToDramPipeTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = UnicastToDramPipeTest(
        solver=solver,
        svars=svars,
