...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/direct_pipe/test_dram_prefetch_post_tm_pipe.template.yaml",
)


class DramPrefetchPostTmTest(DataMovementPipeTestBase):
    # @override
    ...
    def export_sweep_vars(self) -> List[SweepVarsGroup]:
        mblock_range = list(range(2, 16, 2))
...
pipe_test: DramPrefetchPostTmTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = DramPrefetchPostTmTest(
        solver=solver,
        svars=svars,
