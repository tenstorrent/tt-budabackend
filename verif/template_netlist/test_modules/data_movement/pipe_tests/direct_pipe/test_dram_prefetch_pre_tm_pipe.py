...
from test_modules.common.enums import Prologue
...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/direct_pipe/test_dram_prefetch_pre_tm_pipe.template.yaml",
)

MIN_BROADCAST_FACTOR = 4


class DramPrefetchPreTmTest(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        ...
        dram_input: Node = self.nodes["input0_dram"]
        self.solver.add(dram_input.grid_size_x == 1, dram_input.grid_size_y == 1)
        prologue = dram_input.get_prologue_var("test_op")
        self.solver.add(prologue == Prologue.true.value)

        # By forcing broadcast we make net2pipe choose PRE-TM prefetch instead of POST-TM.
        broadcast_factor = self.op_input_tm_map["feeder0"]["input0_dram"].get_tms()[-1].tm_arg
        self.solver.add(broadcast_factor > MIN_BROADCAST_FACTOR)

        force_reblocking_on_connection(self.solver, dram_input, feeder0)

    # @override
    def export_sweep_vars(self) -> List[SweepVarsGroup]:
        mblock_range = list(range(8, 16, 2))
...
pipe_test: DramPrefetchPreTmTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = DramPrefetchPreTmTest(
        solver=solver,
        svars=svars,
