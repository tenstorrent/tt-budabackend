...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/gather_pipe/test_gather_to_dram_pipe.template.yaml",
)


class GatherToDramPipeTest(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        ...
        constrain_no_reblocking_on_connection(self.solver, self.nodes["input0_dram"], feeder0)

        # Constrain output queue to a single grid tile.
        output_queue = self.nodes["output_dram"]
        constrain_node_grid_size_equal_to_one(self.solver, output_queue)
...
pipe_test: GatherToDramPipeTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = GatherToDramPipeTest(
        solver=solver,
        svars=svars,
