...
from test_modules.common.enums import Untilize
...
template_yaml = os.path.join(
    get_git_root(),
    f"verif/template_netlist/templates/data_movement/pipe_tests/multicast_pipe/test_dram_multicast_pipe.template.yaml",
)


class DramGatherMulticastPipeTest(DataMovementPipeTestBase):
    # @override
    def additional_constraints(self) -> None:
        self.solver.add(self.data_format == DataFormat.Float16.value)

        dram_input0 = self.nodes["input0_dram"]
        constrain_node_grid_size_greater_than_one(self.solver, dram_input0)

        target_op = self.nodes["target_op0"]
        self.solver.add(target_op.grid_size_x > 1)
        self.solver.add(target_op.untilize == Untilize.false.value)

    # @override
    def export_sweep_vars(self) -> List[SweepVarsGroup]:
        mblock_range = list(range(4, 64, 4))

        input_0 = self.nodes["input0_dram"]
        return [
            SweepVarsGroup(
                var_names_range_dict={
                    input_0.mb_m.sexpr(): mblock_range,
                    input_0.mb_n.sexpr(): mblock_range,
                },
...
pipe_test: DramGatherMulticastPipeTest = None
...
def constraint_model(solver: Solver, svars: dict, arch_name: str, harvested_rows: int = 0):
    ...
    pipe_test = DramGatherMulticastPipeTest(
        solver=solver,
        svars=svars,
