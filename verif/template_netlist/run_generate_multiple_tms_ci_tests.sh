...
SHORT_OPTS=m:,o:,a:,s:,t:,r:,n:,c:,w:,x:,y:,z,l,h
...
LONG_OPTS+=timeout:,num-retries-on-timeout:,enable-strategy,run-compiler,help
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo ""
      echo "<run_compiler>      - run compiler to verify the resulting netlists as parts of test generation. Default: $RUN_COMPILER"
      echo ""
...
echo "  RUN_COMPILER=$RUN_COMPILER"
#exit 1
# ----------------------------------------------------------------------------------------------------------
