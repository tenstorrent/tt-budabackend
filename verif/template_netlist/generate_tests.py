...
def generate_all_configs(
    ...
    search_config = SolutionSearchConfig.create_from_search_type(search_type=search_type,
...
def full_test_driver(
    ...
    tests = create_netlists(template_yaml=test_module.template_yaml,
                            output_dir=f"{get_git_root()}/{output_dir}",
                            configs=config_list, use_hash=use_hash)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch", default="grayskull", type=str, help="Name of device architecture, one of: ['grayskull', 'wormhole', 'wormhole_b0']")
    parser.add_argument('--random-seed', default=0, type=int, help="Random seed for solver")
    parser.add_argument('--max-num-configs', default=None, type=int, help="Number of test configs to generate")
    parser.add_argument('--search-type', type=str, default="default", help="One of: ['default', 'serial-sweep', 'parallel-sweep']")
    ...
    parser.add_argument('--dry-run', action="store_true", default=False, help="Don't actually run tests")
    parser.add_argument('--dump-config-yaml', action="store_true", default=False, help="Write generated configs to a yaml")
    parser.add_argument('--timeout', default=0, type=int, help="Timeout in seconds for z3 solver, zero==inf")
    parser.add_argument('--num-retries-on-timeout', default=0, type=int, help="Enables retry with different seed if z3 solver timed out")
    parser.add_argument('--enable-strategy', action="store_true", default=False, help="Instantiate z3 solver with predefined strategy (as it can improve z3 solver performance)")
    parser.add_argument('--harvested-rows', default=0, type=int, help="Number of harvested rows on the architecture")

    args = parser.parse_args(sys.argv[1:])

    if args.arch not in ["grayskull", "wormhole", "wormhole_b0"]:
        raise argparse.ArgumentError(None,
                                     "--arch must be one of: ['grayskull', 'wormhole', 'wormhole_b0']")

    if args.search_type not in ["default", "serial-sweep", "parallel-sweep"]:
        raise argparse.ArgumentError(None,
                                     "--search-config must be one of: ['default', 'serial-sweep', 'parallel-sweep']")
