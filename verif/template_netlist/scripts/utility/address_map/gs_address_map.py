...
class GSAddressMap(AddressMap):
    ...
    NCRISC_L1_CONTEXT_BASE = NCRISC_FIRMWARE_BASE + 0x20  # TODO this region is weird, has no size defines
    NCRISC_L1_DRAM_POLLING_CTRL_BASE = NCRISC_FIRMWARE_BASE + 0x40  # TODO this one also

    NCRISC_PERF_QUEUE_HEADER_SIZE = 8 * 8
    NCRISC_PERF_QUEUE_HEADER_ADDR = (
        NCRISC_FIRMWARE_BASE + NCRISC_L1_SCRATCH_SIZE
    )  # TODO This overlaps with scratch, should it?
    NCRISC_L1_PERF_BUF_BASE = NCRISC_PERF_QUEUE_HEADER_ADDR + NCRISC_PERF_QUEUE_HEADER_SIZE  # TODO this one also
    ...
    TRISC_BASE = (
        NCRISC_FIRMWARE_BASE + NCRISC_FIRMWARE_SIZE
    )  # TODO completely define this region with its start and end addr, same for others, this is hard to track
    TRISC0_BASE = NCRISC_FIRMWARE_BASE + NCRISC_FIRMWARE_SIZE  # TODO put it on TRISC_BASE not like this, duplicate calc
    ...
    # TODO obviously THB is missing here completely
    ...
    def get_regions(self):
        ...
        regions = [
            ...
            MemoryRegion("NCRISC_L1_SCRATCH", self.NCRISC_L1_SCRATCH_BASE, self.NCRISC_L1_SCRATCH_SIZE),
            MemoryRegion(
            ...
            MemoryRegion("TRISC0", self.TRISC0_BASE, self.TRISC0_SIZE, alignment=self.get_alignment()),
            MemoryRegion("TRISC1", self.TRISC1_BASE, self.TRISC1_SIZE, alignment=self.get_alignment()),
            MemoryRegion("TRISC2", self.TRISC2_BASE, self.TRISC2_SIZE, alignment=self.get_alignment()),
            MemoryRegion(
                "TILE_HEADER_BUFFER",
            ...
            MemoryRegion(
                "DATA_BUFFERS", self.DATA_BUFFER_SPACE_BASE, 10240
            ...
            MemoryRegion(
                "MATH_PERF_BUF", self.MATH_PERF_BUF_BASE_ADDR, self.MATH_PERF_BUF_SIZE, alignment=self.get_alignment()
            ),
            MemoryRegion(
