...
from scripts.utility.run_net2pipe_pipegen import run_net2pipe, run_single_test, TestResult
...
def compute_kernel_broadcast_factor(
    ...
    for pipe in pipes_feeding_consumer_op:
        ...
        kernel_broadcast_factor_upper_bound = min(
            kernel_broadcast_factor_upper_bound, __get_scatter_offsets_size_tiles(unrolled_input_list)
        )
...
def is_netlist_passing_backend_compilation(netlist_path: str, arch: str, out_dir: str) -> bool:
    ...
    result: TestResult = run_single_test(
