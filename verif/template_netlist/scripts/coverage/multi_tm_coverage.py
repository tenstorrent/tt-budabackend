...
__supported_tms = ['transpose', 'r_broadcast', 'c_broadcast', 't_broadcast', 'hslice', 'vslice', 'hstack', 'vstack']
...
class DramInputDatacopyConfig(MultiTmTestConfig):
    def __init__(self) -> None:
        super().__init__(
            test_op_selector="graphs.test_multi_tm_dram_datacopy.datacopy0",
            input_count=1
        )
...
class DramInputMatmulConfig(MultiTmTestConfig):
    def __init__(self) -> None:
        super().__init__(
            test_op_selector="graphs.test_multi_tm_dram_matmul.op0",
            input_count=2
        )
...
class DatacopyDatacopyConfig(MultiTmTestConfig):
    def __init__(self) -> None:
        super().__init__(
            test_op_selector="graphs.test_multi_tm_datacopy_datacopy.op0",
            input_count=1
        )
...
def make_empty_tm_coverage_dict(num_tms: int) -> TMCoverageDict:
    coverage_dict = {}
    for tm_key in itertools.product(__supported_tms, repeat=num_tms):
        coverage_dict[tm_key] = 0
    return coverage_dict

def read_tms_from_op(netlist_dict: NetlistNode, test_op: str, input_idx: int) -> TMKey:
    test_op_node = read_netlist_node(netlist_dict, test_op)
    input_tms = test_op_node[f"input_{input_idx}_tms"]
    return tuple(
        [tm_dict_to_tm_str(tm_dict) for tm_dict in input_tms]
    )

def process_single_netlist(netlist_path: str, input_count: int, test_op: str) -> List[TMKey]:
...
def reduce_coverage(netlist_input_coverage: List[TMKey], input_count: int, num_tms: int) -> List[TMCoverageDict]:
    total_counts = [0, 0]
    coverage_result = [make_empty_tm_coverage_dict(num_tms) for _ in range(input_count)]
...
def multi_tm_test_coverage(test_dir: str, input_count: int, test_op: str, num_tms: int) -> List[TMCoverageDict]:
    ...
    return reduce_coverage(netlist_input_coverage, input_count, num_tms)
...
def print_coverage_result(coverage_result: List[TMCoverageDict], input_count: int) -> None:
    ...
    # print header
    print("{:50s} | ".format("TM combination") + " | ".join(["{:15s}".format(f" Frequency [{input_idx}] ") for input_idx in range(input_count)]))
    print((56 + 18 * input_count) * "=")

    # print coverage table
    for tm in coverage_result[0].keys():
        print(f"{format_tm(tm):50s} | " + " | ".join([f"{format_val(coverage_result[input_idx][tm])}" for input_idx in range(input_count)]))

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--test-dir", required=True, type=str)
    parser.add_argument("--test-type", required=True, type=str)
    parser.add_argument("--num-tms", default=2, type=int)
