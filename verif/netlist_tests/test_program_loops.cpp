...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_program/netlist_program_loops.yaml");
...
int main(int argc, char** argv) {
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        for (const auto &input_name : input_names) {
            ...
            inputs.insert({input_name, std::vector<tt_tensor>(q_desc.input_count*num_program_loops)});
            for (int b = 0; b < q_desc.input_count*num_program_loops; ++b) {
        ...
        // RUN_PROGRAM
        std::map <std::string, std::string> program_parameters = {{"$num_loops", to_string(num_program_loops)}};
        ...
        expected.insert({"output", std::vector<tt_tensor>(q_desc.input_count*num_program_loops)});
