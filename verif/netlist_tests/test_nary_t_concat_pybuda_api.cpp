...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    help_string += "--skip-check               : Runs only the target backend and does not verify correctness of the output\n";
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_nary_t_concat.yaml");
        ...
        verif_args::get_command_option_and_remaining_args(input_args, "--inputs", "q0,q1,q2,q3");

        tt::args::split_string_into_vector(args.inputs, cmdline_arg, ",");
...
int main(int argc, char** argv) {
    ...
    auto config =
        verif::test_config::read_from_yaml_file(args.netlist_path, "verif/netlist_tests/default_test_config.yaml");
    // Get Stimulus Config
    ...
    for (int n = 0; (n < args.num_loops) && pass; ++n) {
        ...
        std::vector input_names = args.inputs;  // Derive inputs from cmd line
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                expected.at("output").at(b) = tt_tensor(
                    {.rt = 4, .ct = 2, .z = 4, .w = 1}, DataFormat::Float32);  // FIXME: Remove this fake expected

                generate_tensor(
                    {.type = StimulusType::DebugTileId, .debug_tile_id_base = 0.0f, .debug_tile_id_step = 1.0f},
                    expected.at("output").at(b));
            }
        }
