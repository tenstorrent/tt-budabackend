...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_fused_op.yaml");
...
int main(int argc, char** argv) {
    ...
    for (int n = 0; (n < args.num_loops) && pass; ++n) {
        log_info(tt::LogTest, "Test loop {}", n);
        // DRIVE INPUTS -- Driven however test wants to set it up
        std::vector input_names = {"q0", "q1", "r0"};  // INPUTS:: FILL-ME
        ...
        for (const auto& input_name : input_names) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                log_info(tt::LogTest, "Pushing tensor {} num {}", input_name, b);
                verif::push_tensor(*target_backend, input_name, &inputs.at(input_name).at(b), 0);
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                auto output_op0 = inputs.at("q0").at(b).matmul(inputs.at("r0").at(0));
                tt_tensor intermed0;
                if (inputs.at("q1").at(b).same_shape(output_op0)) {
                    intermed0 = inputs.at("q1").at(b).add(output_op0);
                } else {
                    tt_shape shape = inputs.at("q1").at(b).get_shape();
                    shape.ct *= 4;  
                    intermed0 = inputs.at("q1").at(b).broadcast(shape, Dim::C).add(output_op0);
                }
                auto intermed1 = intermed0.reciprocal();
                intermed0 = intermed0.sqrt();
                intermed0 = intermed0.exp();
                expected.at("output").at(b) = intermed0.multiply(intermed1);
                if ((q_desc.ublock_ct*q_desc.mblock_n*q_desc.bufq_grid_dim_c) != expected.at("output").at(b).metadata.shape.ct) {
                    expected.at("output").at(b) = expected.at("output").at(b).reduce(ReduceFunc::Max, Dim::C, false);
                }
            }
        }
        ...
        for (const auto& output_name : output_names) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                log_info(tt::LogTest, "Checking Entry idx={} for output={} Loop={}", b, output_name, n);
                if (not compare_tensors(
                        expected.at(output_name).at(b),
                        output,
                        verif::comparison::get_config(output_name, comparison_configs))) {
                    pass = false;
                    log_error("Entry idx={} for output={} Mismatched Loop={}", b, output_name, n);
                    break;
                }
            }
        }
