...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_ram/netlist_accumulation.yaml");
...
int main(int argc, char** argv) {
    ...
    std::vector output_names = {"weight0"};  // OUTPUTS:: FILL-ME
    std::unordered_map<std::string, std::vector<tt_tensor>> expected(output_names.size());
    tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("weight0");
    tt_tensor_metadata md = get_tensor_metadata_for_queue(q_desc);
    expected.insert({"weight0", std::vector<tt_tensor>(q_desc.input_count)});
    log_info(tt::LogTest, "Generating tensor for weight0 input_count {}", q_desc.input_count);
    for (int b = 0; b < q_desc.input_count; ++b) {
        expected.at("weight0").at(b) = tt_tensor(md);
        generate_tensor(
            {.type = StimulusType::Constant, .constant_value = 0.0f},
            expected.at("weight0").at(b));  // Zero out parameters
    }

    // TEST START, do custom stuff if you want
    for (int n = 0; n < args.num_loops; ++n) {
        // DRIVE INPUTS -- Driven however test wants to set it up
        std::vector input_names = {"gradient0"};  // INPUTS:: FILL-ME
        ...
        for (const auto& input_name : input_names) {
            ...
            log_info(tt::LogTest, "Generating tensor for input {} input_count {}", input_name, q_desc.input_count);
            for (int b = 0; b < q_desc.input_count; ++b) {
        ...
        bool zero_weights = (n % 2 == 0);
        if (zero_weights) {
            generate_tensor(
                {.type = StimulusType::Constant, .constant_value = 0.0f},
                expected.at("weight0").at(0));  // Zero out parameters
            log_info(tt::LogTest, "Zeroing weight0 params for loop {}", n);
            verif::push_tensor(*target_backend, "weight0", &expected.at("weight0").at(0), 0);
            if (compare_to_backend) {
                verif::push_tensor(*compare_to_backend, "weight0", &expected.at("weight0").at(0), 0);
            }
        }
        // RUN_PROGRAM
        ...
        if (compare_to_backend) {
            ...
            tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("gradient0");
            for (int b = 0; b < q_desc.input_count; ++b) {
                expected.at("weight0").at(0) =
                    expected.at("weight0").at(0).add(inputs.at("gradient0").at(b));  // FIXME: Remove this fake expected
            }
        }
