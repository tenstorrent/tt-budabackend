...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_template.yaml");
...
int main(int argc, char** argv) {
    ...
    VerifTestConfig test_config = verif::test_config::read_from_yaml_file(args.netlist_path, "verif/netlist_tests/default_test_config.yaml");

    bool pass = true; 

    // INITIALIZATION OF BACKENDS   
    std::shared_ptr<tt_backend> target_backend = tt_backend::create(args.netlist_path, backend_config);
    tt::DEVICE_STATUS_CODE status_code = tt::DEVICE_STATUS_CODE::Success;
    status_code = target_backend->initialize();
    log_assert(status_code == tt::DEVICE_STATUS_CODE::Success, "target backend initialization failed");
    std::shared_ptr<tt_backend> compare_to_backend;
    if (compare_to == tt::DEVICE::Golden) {
        compare_to_backend = tt_backend::create(args.netlist_path, compare_to_config);
        status_code = compare_to_backend->initialize();
        log_assert(status_code == tt::DEVICE_STATUS_CODE::Success, "compare_to_backend initialization failed");
    }

    // TEST START, do custom stuff if you want
    // DRIVE INPUTS -- Driven however test wants to set it up
    std::vector input_names = {"input0"};  // INPUTS:: FILL-ME
    ...
    for (const auto &input_name : input_names) {
        ...
        for (int b = 0; b < q_desc.input_count; ++b) {
            ...
            generate_tensor(verif::stimulus::get_config(input_name, test_config.stimulus_configs), inputs.at(input_name).at(b));
    ...
    status_code = target_backend->run_program("program0", program_parameters);    
    log_assert(status_code == tt::DEVICE_STATUS_CODE::Success, "run_program failed");

    // GENERATE EXPECTED OR RUN_PROGRAM FOR COMPARE_TO 
    std::vector output_names = {"output0"}; // OUTPUTS:: FILL-ME
    std::unordered_map<std::string, std::vector<tt_tensor>> expected(output_names.size());
    tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("output0");
    expected.insert({"output0", std::vector<tt_tensor>(q_desc.input_count)});

    if (compare_to_backend) {
        compare_to_backend->run_program("program0", program_parameters);  
    } else {
        for (int b = 0; b < q_desc.input_count; ++b) {
            // Expected Calculation
            expected.at("output0").at(b) = inputs.at("input0").at(b); 
            tt_shape shape = expected.at("output0").at(b).get_shape();
            shape.z *= 4;  // FIXME: get from tm op
            expected.at("output0").at(b) = expected.at("output0").at(b).broadcast(shape, Dim::Z);
            expected.at("output0").at(b) = expected.at("output0").at(b).exp();
        }
    }
    ...
    for (const auto &output_name : output_names) {
        ...
        for (int b = 0; b < q_desc.input_count; ++b) {
            ...
            if (not compare_tensors(
                    ...
                    output,
                    verif::comparison::get_config(output_name, test_config.comparison_configs))) {
                pass = false;
