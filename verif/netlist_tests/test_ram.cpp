...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_ram/netlist_ram.yaml");
...
int main(int argc, char** argv) {
    ...
    std::vector input_names = {"r0"};  // INPUTS:: FILL-ME
    ...
    for (const auto& input_name : input_names) {
        ...
        for (int b = 0; b < q_desc.input_count; ++b) {
            ...
            verif::push_tensor(*target_backend, input_name, &inputs.at(input_name).at(b), b);
            if (compare_to_backend) {
                verif::push_tensor(*compare_to_backend, input_name, &inputs.at(input_name).at(b), b);
            }
        }
    }
    ...
    for (int b = 0; b < q_desc.input_count; ++b) {
        ...
        expected.at("r0").at(b) = inputs.at("r0").at(b);  // FIXME: Remove this fake expected
    }

    for (int n = 0; n < args.num_loops; ++n) {
        bool reinitialize_ram = (n % 7 == 0);
        if (reinitialize_ram) {
            generate_tensor(
                verif::stimulus::get_config("r0", stimulus_configs),
                expected.at("r0").at(0));  // Zero out parameters
            verif::push_tensor(*target_backend, "r0", &expected.at("r0").at(0), 0);
            if (compare_to_backend) {
                verif::push_tensor(*compare_to_backend, "r0", &expected.at("r0").at(0), 0);
            }
        }
        // RUN_PROGRAM
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                // Expected Calculation
                expected.at("r0").at(b) = expected.at("r0").at(b).sqrt();  // FIXME: Remove this fake expected
            }
        }
