...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_binary.yaml");
...
int main(int argc, char** argv) {
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        std::vector input_names = {"q0", "q1"};  // INPUTS:: FILL-ME
        ...
        for (const auto& input_name : input_names) {
            ...
            verif::push_tensors(*target_backend, input_name, inputs.at(input_name));
            if (compare_to_backend) {
                verif::push_tensors(*compare_to_backend, input_name, inputs.at(input_name));
            }
        }
        // RUN_PROGRAM
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                expected.at("output").at(b) =
                    inputs.at("q0").at(b).add(inputs.at("q1").at(b));  // FIXME: Remove this fake expected
            }
        }
        ...
        for (const auto& output_name : output_names) {
            ...
            std::vector<tt_tensor> outputs(q_desc.input_count);
            verif::get_and_pop_tensors(*target_backend, output_name, outputs);
            if (compare_to_backend) {
                verif::get_and_pop_tensors(*compare_to_backend, output_name, expected.at(output_name));
            }
            for (int b = 0; b < q_desc.input_count; ++b) {
