...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_program/netlist_host_loops_with_multiple_independent_programs.yaml");
...
int main(int argc, char** argv) {
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        std::vector input_names = {"q0","q1","q2"};  // INPUTS:: FILL-ME
        ...
        target_backend->run_program("program1", program_parameters);    
        target_backend->run_program("program2", program_parameters);    

        // GENERATE EXPECTED OR RUN_PROGRAM FOR COMPARE_TO 
        std::vector output_names = {"output0","output1","output2"}; // OUTPUTS:: FILL-ME
        ...
        for (const auto &output_name : output_names) {
            ...
            expected.insert({output_name, std::vector<tt_tensor>(q_desc.input_count)});
        }

        if (compare_to_backend) {
            compare_to_backend->run_program("program0", program_parameters);  
            compare_to_backend->run_program("program1", program_parameters);  
            compare_to_backend->run_program("program2", program_parameters);  
        } else {
            tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("q0");
            for (int b = 0; b < q_desc.input_count; ++b) {
                // Expected Calculation
                expected.at("output0").at(b) = inputs.at("q0").at(b);
                expected.at("output1").at(b) = inputs.at("q1").at(b).exp();
                expected.at("output2").at(b) = inputs.at("q2").at(b);
            }
        }
