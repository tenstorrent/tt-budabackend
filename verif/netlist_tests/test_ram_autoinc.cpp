...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_ram/netlist_ram_autoinc.yaml");
...
int main(int argc, char** argv) {
    ...
    tt_dram_io_desc input_q_desc = target_backend->get_queue_descriptor("q0");
    tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("r0");
    expected.insert({"r0", std::vector<tt_tensor>(q_desc.bufq_num_slots)});
    for (int b = 0; b < q_desc.bufq_num_slots; ++b) {
        // Expected Calculation
        expected.at("r0").at(b) = inputs.at("q0").at(b);  // FIXME: Remove this fake expected
    }
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.bufq_num_slots; ++b) {
                // Expected Calculation
                expected.at("r0").at(b) = inputs.at("q0").at(b).sqrt();  // FIXME: Remove this fake expected
            }
        }
