...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_flushless_accumulation.yaml");
...
int main(int argc, char** argv) {
    ...
    // Initialize expected
    std::vector output_names = {"r0"}; // OUTPUTS:: FILL-ME
    std::unordered_map<std::string, std::vector<tt_tensor>> expected(output_names.size());
    tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("r0");
    int num_ram_buffers = q_desc.bufq_num_slots;
    tt_tensor_metadata md = get_tensor_metadata_for_queue(q_desc); 
    expected.insert({"r0", std::vector<tt_tensor>(q_desc.input_count)});
    for (int b = 0; b < q_desc.input_count; ++b) {
        expected.at("r0").at(b) = tt_tensor(md);
        generate_tensor({.type = StimulusType::Constant, .constant_value = 0.0f}, expected.at("r0").at(b)); // Zero out parameters 
        verif::push_tensor(*target_backend, "r0", &expected.at("r0").at(b), b);
    }
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                expected.at("r0").at(0) = expected.at("r0").at(0).add(inputs.at("q0").at(b));  // FIXME: Remove this fake expected
            }
        }
    }
    ...
    for (const auto &output_name : output_names) {
        ...
        tt_tensor output(md);
        verif::get_and_pop_tensor(*target_backend, output_name, &output, (args.num_loops)%num_ram_buffers); 
        if (compare_to_backend) {
            expected.at(output_name).at(0) = tt_tensor(md);
            verif::get_and_pop_tensor(*compare_to_backend, output_name, &expected.at(output_name).at(0), (args.num_loops)%num_ram_buffers); 
        }
        log_info(tt::LogTest, "Checking Entry idx={} for output={}", (args.num_loops)%num_ram_buffers, output_name);
        if (not compare_tensors(
                expected.at(output_name).at(0),
                output,
                verif::comparison::get_config(output_name, comparison_configs))) {
            pass = false;
            log_error("Entry idx={} for output={} Mismatched", (args.num_loops)%num_ram_buffers, output_name);
        }
