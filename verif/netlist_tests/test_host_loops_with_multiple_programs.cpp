...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_program/netlist_host_loops_with_multiple_programs.yaml");
...
int main(int argc, char** argv) {
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        target_backend->run_program("program3", program_parameters);  
        target_backend->run_program("program4", program_parameters);    
        ...
        if (compare_to_backend) {
            ...
            compare_to_backend->run_program("program3", program_parameters);  
            compare_to_backend->run_program("program4", program_parameters);  
        } else {
            for (int b = 0; b < q_desc.input_count; ++b) {
                // Expected Calculation
                expected.at("output").at(b) = inputs.at("q0").at(b).sqrt().exp().sqrt().exp();
            }
        }
