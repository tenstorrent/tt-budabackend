...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_unary_with_repeat_broadcast_tm.yaml");
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                expected.at("output").at(b) = inputs.at("q0").at(b);  // FIXME: Remove this fake expected
                tt_shape shape = expected.at("output").at(b).get_shape();
                shape.ct *= 4;  // FIXME: get from tm op
                expected.at("output").at(b) = expected.at("output").at(b).broadcast_tiles(shape, Dim::C);
                expected.at("output").at(b) = expected.at("output").at(b).exp();  // FIXME: Remove this fake expected
            }
        }
