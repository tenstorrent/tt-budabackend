...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_ram/netlist_gradient_accumulation.yaml");
...
int main(int argc, char** argv) {
    ...
    std::vector output_names = {"acc.gradients"}; // OUTPUTS:: FILL-ME
    std::unordered_map<std::string, std::vector<tt_tensor>> expected(output_names.size());
    tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("acc.gradients");
    tt_tensor_metadata md = get_tensor_metadata_for_queue(q_desc); 
    expected.insert({"acc.gradients", std::vector<tt_tensor>(q_desc.input_count)});
    for (int b = 0; b < q_desc.input_count; ++b) {
        expected.at("acc.gradients").at(b) = tt_tensor(md);
        generate_tensor({.type = StimulusType::Constant, .constant_value = 0.0f}, expected.at("acc.gradients").at(b)); // Zero out parameters 
    }

    // TEST START, do custom stuff if you want
    for (int n = 0; n < args.num_loops; ++n) {
        // DRIVE INPUTS -- Driven however test wants to set it up
        std::vector input_names = {"matmul_bwd0", "matmul_bwd1"};  // INPUTS:: FILL-ME
        ...
        bool zero_params = (n%2 == 0);
        // RUN_PROGRAM
        std::map <std::string, std::string> program_parameters = {{"$zero_accumulated_gradients", to_string(zero_params)}};
        target_backend->run_program("program0", program_parameters);    
        ...
        if (compare_to_backend) {
            ...
            if (zero_params) {
                generate_tensor({.type = StimulusType::Constant, .constant_value = 0.0f}, expected.at("acc.gradients").at(0)); // Zero out parameters 
            }
            tt_dram_io_desc q_desc = target_backend->get_queue_descriptor("matmul_bwd0");
            for (int b = 0; b < q_desc.input_count; ++b) {
                auto matmul_output = inputs.at("matmul_bwd0").at(b).matmul(inputs.at("matmul_bwd1").at(b));  // FIXME: Remove this fake expected
                expected.at("acc.gradients").at(0) = expected.at("acc.gradients").at(0).add(matmul_output);
            }
        }

        // READ OUTPUTS 
        for (const auto &output_name : output_names) {
            log_info(tt::LogTest, "Reading and Checking Output Queue={}", output_name);
            tt_dram_io_desc q_desc = target_backend->get_queue_descriptor(output_name);
            tt_tensor_metadata md = get_tensor_metadata_for_queue(q_desc);
            // Checking for q_info.input_count
            for (int b = 0; b < q_desc.input_count; ++b) {
                tt_tensor output(md);
                verif::get_and_pop_tensor(*target_backend, output_name, &output, 0); 
                if (compare_to_backend) {
                    expected.at(output_name).at(b) = tt_tensor(md);
                    verif::get_and_pop_tensor(*compare_to_backend, output_name, &expected.at(output_name).at(b), 0); 
                }
