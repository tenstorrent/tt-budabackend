...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic_ram/netlist_parameter_queue.yaml");
...
int main(int argc, char** argv) {
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        int num_loops_in_program = 2;
        expected.insert({"output", std::vector<tt_tensor>(q_desc.input_count*num_loops_in_program)});

        if (compare_to_backend) {
            compare_to_backend->run_program("working_program", program_parameters);  
        } else {
            for (int b = 0; b < q_desc.input_count*num_loops_in_program; ++b) {
                // Expected Calculation
                expected.at("output").at(b) = inputs.at("r0").at(0);
            }
        }
