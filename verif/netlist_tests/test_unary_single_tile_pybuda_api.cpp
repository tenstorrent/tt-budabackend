...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_unary_single_tile.yaml");
    ...
    for (int n = 0; (n < args.num_loops) && pass; ++n) {
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                auto shape = tt_shape({.rt = 1, .ct = 1, .z = 2, .w = 1});
                expected.at("output").at(b) = inputs.at("q0").at(b).broadcast_tiles(shape, Dim::Z).exp();  // FIXME: Remove this fake expected
            }
        }
