...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_transpose_yz.yaml");
        ...
        verif_args::get_command_option_and_remaining_args(input_args, "--inputs", "q0");
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                expected.at("output").at(b) = tt_tensor(
                    {.rt = 1, .ct = 1, .z = 64, .w = 1}, DataFormat::Float32);  // FIXME: Remove this fake expected
