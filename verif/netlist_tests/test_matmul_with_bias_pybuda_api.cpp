...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    try {
        ...
        std::tie(args.netlist_path, input_args) = verif_args::get_command_option_and_remaining_args(
            input_args, "--netlist", "verif/netlist_tests/netlists/basic/netlist_matmul_with_bias.yaml");
    ...
    for (int n = 0; n < args.num_loops; ++n) {
        ...
        if (compare_to_backend) {
            ...
            for (int b = 0; b < q_desc.input_count; ++b) {
                ...
                auto matmul0_out = inputs.at("q0").at(b).matmul(inputs.at("q1").at(b));
                expected.at("output").at(b) = matmul0_out.add(inputs.at("q2").at(b).broadcast(matmul0_out.get_shape(), Dim::R));  // FIXME: Remove this fake expected
            }
        }
