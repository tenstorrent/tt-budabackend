...
echo "Generating wormhole_b0 silicon op tests for in VCS DIR=${vcs_dir}"
cd ${vcs_dir}
...
make DEVICE=${device} TARGET_FILE=test_configs/wormhole_b0/binary/binary_feeder_drainer_configs HARVESTED_ROWS=2 LOOPS=50 SEED=0 C=main.sv ARGS='+op=binary +ops_config=feeder_drainer'
make DEVICE=${device} TARGET_FILE=test_configs/wormhole_b0/matmul/matmul_single_configs HARVESTED_ROWS=2 LOOPS=400 SEED=0 C=main.sv ARGS='+op=matmul'
zip test_configs/wormhole_b0/matmul/matmul_single_configs.zip test_configs/wormhole_b0/matmul/matmul_single_configs.yaml
rm test_configs/wormhole_b0/matmul/matmul_single_configs.yaml
make DEVICE=${device} TARGET_FILE=test_configs/wormhole_b0/matmul/matmul_gradient_acc_single_configs HARVESTED_ROWS=2 LOOPS=50 SEED=0 C=main.sv ARGS='+op=matmul +gradient_acc=1'
...
make DEVICE=${device} TARGET_FILE=test_configs/wormhole_b0/matmul/matmul_feeder_drainer_configs HARVESTED_ROWS=2 LOOPS=50 SEED=0 C=main.sv ARGS='+op=matmul +ops_config=feeder_drainer'
