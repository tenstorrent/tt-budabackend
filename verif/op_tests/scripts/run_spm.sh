...
do
    ...
    LOG_NAME="${LOG_NAME%.[^.]*}"
    TEST_PARAMS=""
    TEST_CMD="./build/test/verif/op_tests/test_op --netlist $TEST_NAME --seed 0 --silicon --timeout 200 --arch $device --bin-path $DIR_NAME --reload-bin"
    echo "Running $TEST_CMD"
    bash -c "./build/test/verif/op_tests/test_op --netlist $TEST_NAME --seed 0 --silicon --timeout 200 --arch $device --bin-path $DIR_NAME --reload-bin --dump-perf-events --perf-level 1" >  $directory/logs/$LOG_NAME.log
    RETCODE=$?
