...
test_args parse_test_args(std::vector<std::string> input_args) {
    ...
    help_string += "--output-names       : List all output queue names\n";
...
void test_main(test_args args) {
    ...
    //log_assert(num_program_loops == 1, "only support outer looping of 1 right now");
    golden.reset_queues();
    VerifTestConfig test_config = verif::test_config::read_from_yaml_file(args.netlist_path);

    args.output_names.insert(args.output_names.end(), test_config.io_config["outputs"].begin(), test_config.io_config["outputs"].end());
    const int sequence_lenth = 128;

    std::map<std::string, std::string> program_parameters = {
        {"$p_microbatch_count", std::to_string(num_program_loops)},
        {"$p_loop_count", std::to_string(num_program_loops)},
    ...
    tt_runtime_config config = get_runtime_config(perf::PerfDesc{}, args.output_dir, 0, args.run_silicon);
    ...
    for (int loop=0; loop<num_loops; loop++) {
       ...
       for (const auto& it : input_queue_info_map) {
	       ...
	       } else if ((q_name.find("constant") != std::string::npos) ||
	           	  (q_name.rfind("lc.", 0) == 0) ||
	           	  (q_name.rfind("dc.", 0) == 0) ||
	           	  (q_name.rfind("ff.", 0) == 0) ||
	                  (q_name.find(".lr") != std::string::npos)) {	        
	          if (first_input) {
                      input_queues.at(q_name).at(b) = tt_tensor(md);
                      std::mt19937 &generator = verif::random::rand_gen;
                      std::uniform_real_distribution<float> distribution(0.0, 0.01);
		      //if ((q_name.find("mean") != std::string::npos)||
                      //    (q_name.find("var") != std::string::npos)) {
                      //   verif::random::set_number(input_queues.at(q_name).at(b), 1.0f/sequence_lenth);
                      //} else if (q_name.find("sum") != std::string::npos) {
                      //   verif::random::set_number(input_queues.at(q_name).at(b), 1.0f);
	              if ((q_name.rfind("lc.", 0) == 0)||(q_name.rfind("dc.",0))) {
                         verif::random::set_number(input_queues.at(q_name).at(b), 1.0f);
	              } else {  			      
                         verif::random::set_number(input_queues.at(q_name).at(b), distribution(generator));
		      }	  
                  } else {continue;}		    
	       } else if ((q_name.find("loss_ff") != std::string::npos)) {
                   input_queues.at(q_name).at(b) = tt_tensor(md);
                   verif::random::set_number(input_queues.at(q_name).at(b), 0.25f);
               } else {		    
                   input_queues.at(q_name).at(b) = tt_tensor(md);
                   generate_tensor(
      ...
      for (std::string program_name: workload.program_order) {
       ...
       auto get_golden_input = [&input_queues](tt_queue_info& queue_info, int entry_idx) {
           ...
           log_assert(
               input_queues.find(queue_info.name) != input_queues.end(), "Cannot find queue in inputs created by golden");
	...
	if (args.init_rams) {
           for (const auto& it : input_ram_info_map) {
               ...
               tt_queue_info q_info = it.second;
                ...
                input_io_desc.push_back(io_desc);
                  ...
                  cout << "Skip host write to ram "<< q_name << endl;
               }
           }
        }
        ...
        for (tt_dram_io_desc &io : input_io_desc) {
            int num_inputs = io.input_count*num_program_loops;
            bool first_loop = (loop==0);
            int ram_ptr = -1;
	    if ((io.queue_name.find("eps") != std::string::npos) ||
	         (io.queue_name.find("constant") != std::string::npos) ||
	         (io.queue_name.rfind("lc.", 0) == 0) ||
	         (io.queue_name.rfind("dc.", 0) == 0) ||
	         (io.queue_name.rfind("ff.", 0) == 0) ||
	         (io.queue_name.find(".lr") != std::string::npos) ||
                 (io.io_type != IO_TYPE::Queue)) {
		 if (first_loop) {
                    num_inputs = 1;			 
		    ram_ptr = 0;
		 } else {continue;}     	 
            }		     
            tt::io::push_host_input(io, get_golden_input, num_inputs, ram_ptr, args.force_sw_tilize);
        }
	...
	if (output_names.empty()) {
           output_queue_info_map = golden.get_device_queue_info_map(); 		   
        } else {		
           for (auto const &output_q_name : output_names) {
               output_queue_info_map.insert({workload.queues[output_q_name].my_queue_info.name, workload.queues[output_q_name].my_queue_info});
           }
	}   
