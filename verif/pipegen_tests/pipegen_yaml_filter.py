...
def filter_pipegen_yaml(original_yaml_path: str, filtered_yaml_path: str,
...
def filter_direct_pipes_l1_l1(pipe_graph: PipeGraph) -> PipeGraph:
    ...
    for pipe in pipe_graph.pipes:
        if (len(pipe.inputs) == 1 and len(pipe.outputs) == 1 and len(pipe.outputs[0]) == 1 and
            pipe.inputs[0].get_location() == BufferLocation.L1 and
            pipe.outputs[0][0].get_location() == BufferLocation.L1):
...
def filter_direct_pipes_l1_dram(pipe_graph: PipeGraph) -> PipeGraph:
    ...
    for pipe in pipe_graph.pipes:
        ...
        if (len(pipe.inputs) == 1 and len(pipe.outputs) == 1 and len(pipe.outputs[0]) == 1 and
            pipe.inputs[0].get_location() == BufferLocation.L1 and
            pipe.outputs[0][0].get_location() == BufferLocation.DRAM):
            filtered_pipe_graph.add_pipe(pipe, True)
            filtered_pipe_graph.add_buffer(pipe.inputs[0], False)
            filtered_pipe_graph.add_buffer(pipe.outputs[0][0], False)
...
def filter_direct_pipes_dram_l1(pipe_graph: PipeGraph) -> PipeGraph:
    ...
    for pipe in pipe_graph.pipes:
        ...
        if (not gathering_from_diff_buffers(pipe) and
            len(pipe.outputs) == 1 and len(pipe.outputs[0]) == 1 and
            input_buf.get_location() == BufferLocation.DRAM and
            output_buf.get_location() == BufferLocation.L1 and
            not input_buf.is_forked() and
            output_buf.is_input_operand()):
...
def filter_dram_prefetch(pipe_graph: PipeGraph, prefetch_type: PrefetchType) -> PipeGraph:
    return filter_graph_by_buffers(
        pipe_graph,
        lambda buffer: buffer.is_buf_dram_prefetch() and prefetch_type == buffer.prefetch_type)
...
def filter_dram_fork(pipe_graph: PipeGraph) -> PipeGraph:
    def buffer_condition(buffer):
        return (buffer.is_root() and
                buffer.get_location() == BufferLocation.DRAM and
                buffer.dram_buf_flag == False and
                buffer.is_forked())
...
def filter_dram_prefetch_fork(pipe_graph: PipeGraph) -> PipeGraph:
    return filter_graph_by_buffers(
        pipe_graph,
        lambda buffer: buffer.is_buf_dram_prefetch() and buffer.is_forked())

def filter_packer_fork(pipe_graph: PipeGraph) -> PipeGraph:
    return filter_graph_by_buffers(
        pipe_graph,
        lambda buffer: buffer.get_location() == BufferLocation.L1 and buffer.is_forked())
...
def filter_intermediate_buffers(pipe_graph: PipeGraph) -> PipeGraph:
    ...
    intermed_bufs = filter_graph_by_buffers(
        pipe_graph,
        lambda buffer: buffer.buffer_type == BufferType.intermediate or buffer.buffer_type == BufferType.gradient_op
    ).buffers
    ...
    for buf in intermed_bufs:
        ...
        if shared_packer_buffer is not None:
            ...
            add_buffer_connections(shared_packer_buffer, filtered_pipe_graph, visited_node_ids)
...
def filter_serial_fork_pipes_with_duplicates(pipe_graph: PipeGraph) -> PipeGraph:
    return filter_graph_by_pipes(
        pipe_graph,
        lambda pipe: does_pipe_have_consumer_duplicates(pipe)
    )
