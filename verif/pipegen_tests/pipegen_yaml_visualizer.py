...
class PipegenYamlVisualizer:
    ...
    def plot_pipe_graph(self) -> None:
        ...
        nx.draw(
            ...
            pos=layout,
            node_size=8000,
            node_color=self.node_color_map.values(),
        )
        nx.draw_networkx_edge_labels(
            self.nxgraph, pos=layout, edge_labels=self.edge_labels
        )
    ...
    def __add_pipe_input_nodes(self, pipe: PipeNode) -> None:
        ...
        for buffer in pipe.inputs:
            if buffer.is_scatter:
                group_base_id = self.__get_parent_base_id(
                    buffer.scatter_group_parent.id
                )
                node_color = self.__get_group_color(group_base_id)
                edge_label = (
                    f"{PipegenYamlVisualizer.SCATTER_EDGE_LABEL}\n{group_base_id}"
                )
    ...
    def __add_pipe_node(
        self, pipe: PipeNode, color: Tuple | str = DEFAULT_PIPE_COLOR
    ) -> None:
        """Add a node for `pipe` in self.nxgraph and color it."""
        self.__add_node(pipe, color)

    def __add_buffer_node(
        self, buffer: BufferNode, color: Tuple | str = DEFAULT_BUFF_COLOR
    ) -> None:
    ...
    def __add_edge(
        self, from_node: GraphNode, to_node: GraphNode, label: Optional[str] = None
    ) -> None:
...
def main(
    yaml_path: str, png_path: str, figsize: Tuple[int, int], expand_scatter_bufs: bool
):
    visualizer = PipegenYamlVisualizer(
        yaml_path, png_path, figsize, expand_scatter_bufs
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--yaml-path", type=str, required=True, default=None, help="Path to yaml"
    )
    parser.add_argument(
        "--png-path",
        type=str,
        required=False,
        default="PipeGraph.png",
