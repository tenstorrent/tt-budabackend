...
inline std::string get_command_option(
    ...
    std::vector<std::string>::const_iterator option_pointer =
        std::find(std::begin(verif_args), std::end(verif_args), option);
    if (option_pointer != std::end(verif_args) and option_pointer++ != std::end(verif_args)) {
...
inline std::uint32_t get_command_option_uint32(
    ...
    if (default_value.has_value()) {
        param = get_command_option(verif_args, option, std::to_string(default_value.value()));
    } else {
        param = get_command_option(verif_args, option);
    }
...
inline bool has_command_option(const std::vector<std::string> &verif_args, const std::string &option) {
    std::vector<std::string>::const_iterator option_pointer =
        std::find(std::begin(verif_args), std::end(verif_args), option);
    return option_pointer != std::end(verif_args);
}
...
inline  std::tuple<std::uint32_t, std::vector<std::string>> get_command_option_uint32_and_remaining_args(
    ...
    if (default_value.has_value()) {
        std::tie(param, remaining_args) =
            get_command_option_and_remaining_args(verif_args, option, std::to_string(default_value.value()));
    } else {
        std::tie(param, remaining_args) = get_command_option_and_remaining_args(verif_args, option);
    }
...
inline std::tuple<double, std::vector<std::string>> get_command_option_double_and_remaining_args(
    const std::vector<std::string> &verif_args, const std::string &option, const std::optional<double> &default_value = std::nullopt) {
    std::vector<std::string> remaining_args = verif_args;
    std::string param;
    if (default_value.has_value()) {
        std::tie(param, remaining_args) =
...
inline void split_string_into_vector(
    ...
    if (!input_command_modified.empty()) {
        ...
        while (current_pos != std::string::npos) {
            output_vector.push_back(verif_args::parse<T>(input_command_modified.substr(0, current_pos)));
            input_command_modified.erase(0, current_pos + 1);
            current_pos = input_command_modified.find(delimiter);
        }
        if (!input_command_modified.empty()) {
            output_vector.push_back(verif_args::parse<T>(input_command_modified));
        }
    }
}
...
T get(
    ...
    if (default_value.has_value()) {
        if constexpr (std::is_same_v<T, std::string>) {
            param = get_command_option(verif_args, option, default_value.value());
        } else {
            param = get_command_option(verif_args, option, std::to_string(default_value.value()));
        }
    } else {
    ...
    return verif_args::parse<T>(param);
}
...
inline std::vector<std::string> get_vcd_dump_cores_from_arg_string(std::string input) {
