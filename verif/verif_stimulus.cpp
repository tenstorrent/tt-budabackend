...
void generate_tensor(VerifStimulusConfig stimulus_config, tt_tensor &tensor) {
    if (stimulus_config.type == StimulusType::Normal) {
        ...
        if (tt::is_integer_format(tensor.get_data_format())) {
            ...
            } else if (tensor.get_data_format() == DataFormat::UInt8) {
                verif::random::randomize_normal<uint8_t>(
                    tensor,
                    (uint8_t)stimulus_config.normal_mean,
                    (uint8_t)stimulus_config.normal_stddev,
                    stimulus_config.set_tile_rows_range,
                    stimulus_config.set_tile_cols_range);
        ...
        if (tt::is_integer_format(tensor.get_data_format())) {
            if (tensor.get_data_format() == DataFormat::Int8) {
                ...
                verif::random::randomize_uniform<uint8_t>(
                    tensor,
                    (uint8_t)stimulus_config.uniform_lower_bound,
                    (uint8_t)stimulus_config.uniform_upper_bound,
                    stimulus_config.set_tile_rows_range,
                    stimulus_config.set_tile_cols_range);
            } else if (tensor.get_data_format() == DataFormat::Int32) {
        ...
        if (tt::is_integer_format(tensor.get_data_format())) {
            if (tensor.get_data_format() == DataFormat::Int8) {
                ...
                verif::random::set_number<uint8_t>(
                    tensor,
                    (uint8_t)stimulus_config.constant_value,
                    stimulus_config.set_tile_rows_range,
                    stimulus_config.set_tile_cols_range);
            } else if (tensor.get_data_format() == DataFormat::Int32) {
