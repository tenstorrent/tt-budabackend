...
def filter_pipegen_yamls(
    ...
    filtered_yamls_dir_name = f"{FILTERED_YAMLS_OUT_DIR_NAME_PREFIX}_{filter_type.name}"
    ...
    num_processed = execute_in_parallel(
        __filter_netlist_yamls_worker,
        worker_configs,
        __init_global_pipegen_filter_sync_vars,
        pipegen_yamls_value,
    )
...
def __filter_netlist_yamls_worker(worker_config: PipegenYamlFilterWorkerConfig) -> int:
